# Yuheng BIZ/APP 范围鉴权与 Tianshu 直连 RPC 实施计划

| Field              | Value                                                                                                                                                                                                                                                |
|--------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Document           | `2026-08-15-17-23-yuheng-biz-app-direct-rpc-implementation.md`                                                                                                                                                                                      |
| Status             | `Review`                                                                                                                                                                                                                                             |
...
| Scope              | `Yuheng HTTP/RPC data plane, Tianquan-Jianshen scope projection/adapter, RPC Starter/Tianshu direct discovery, Tianquan-Shoubing RPC credential relay`                                                                                                                            |
| Source Requirement | `2026-08-15 user request and confirmation: Yuheng checks BIZ then APP only; downstream owns operation permission; @EgonRpcDirectReference is accepted; APP scope uses effective active roles; historical Yuheng RPC Routes are explicitly retired` |
| Baseline Revision  | `main@8a64b586634d8d1fc94ffcc101e9add00c2e7730; only docs/egon is untracked at Plan creation`                                                                                                                                                        |
| Implements Spec    | [Yuheng 流量层 BIZ/APP 范围校验与 Tianshu 直连 RPC 双链路规格](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md)                                                                                                                                    |
| Spec Status        | `Review`                                                                                                                                                                                                                                             |
| Spec Revision      | `Updated 2026-08-15 16:57 CST; baseline main@8a64b586634d8d1fc94ffcc101e9add00c2e7730`                                                                                                                                                               |
| Effective Specs    | [Yuheng 流量层 BIZ/APP 范围校验与 Tianshu 直连 RPC 双链路规格](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md)                                                                                                                                    |
...
本计划实施唯一目标 Spec，把工作拆成 9 个按依赖顺序、可单独验证和提交的 Step：先建立 Tianquan-Jianshen BIZ→APP 轻量投影和一致发布，再替换
Yuheng Tianquan-Jianshen 数据面 Provider并接入 Engine；随后在中立 RPC Starter 中增加 Provider Directory/双注解，在 Tianshu Adapter 中实现
`RPC_PROVIDER` 发现；最后补齐 Tianquan-Shoubing RPC 凭证中继、Yuheng→RPC 转发和文档/实例注册回归边界。

最终证据包括：每个 Step 的 RED/GREEN 聚焦测试、RPC Components 与 Platforms 受影响模块测试、Yuheng Admin 未修改检查、
`git diff --check`、Plan/Spec 追踪检查，以及由用户启动本地栈后执行的 Yuheng/Direct 双链路运行验收。本计划不修改生产代码、不运行数据库迁移、不启动服务。
...
-
Path：[Yuheng 流量层 BIZ/APP 范围校验与 Tianshu 直连 RPC 双链路规格](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md)
...
|---------|----------------------------------------------------------------------------------------------------------|-------------------------------------------------------|--------------------|----------------------------------|
| Primary | [Yuheng BIZ/APP + Direct RPC Spec](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) | `Review`, accepted for planning; 2026-08-15 16:57 CST | §1–§20 全部          | 唯一直接约束本次行为、接口、模型、测试、兼容和发布顺序的设计基线 |

### 2.3 Superseded or excluded content

- Primary Spec 已明确修订其 `Amends` 中旧设计的接口级 Yuheng Permission 和 RPC 必经 Yuheng 表述；本 Plan 只采用 Primary
  Spec §20.4 所列仍有效边界。
- Primary Spec `Related Specs` 中 Tianquan-Jianshen IAM 文档仍为 Draft，仅作术语背景，不是 Effective Spec。
- Yuheng Admin UI、MCP Tool 权限、RPC Streaming、新数据库模型、新 Client Governance 均按 Spec §3.2 排除。
...
|-------------|------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|----------------------------------------------------------------------|---------------------------------------------------|
| `REQ-001`   | [§4](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md#4-requirements-and-acceptance-criteria) | `BUSINESS_PROTECTED` USER 先校验 Route BIZ，再校验该 BIZ 下 APP | BIZ 拒绝时 APP lookup 为 0；BIZ/APP 都允许才转发                                | Tianquan-Jianshen contract/admin/yuheng adapter、Engine tests |
| `REQ-002`   | §4                                                                                                               | Yuheng HTTP/RPC 不判断 Operation Permission              | 数据面不读取 mapping metadata/key 或 `permissions`                          | 删除旧 reader/provider；HTTP attrs 收敛                 |
| `REQ-003`   | §4                                                                                                               | 保留认证、暴露规则、范围 Fail Closed                               | 401/403/503 与 `externalAccessible` 语义保持                              | 复用 `GatewaySecurityChain`，替换 Provider             |
| `REQ-004`   | §4                                                                                                               | Yuheng Admin 本地接口权限保持                                 | Admin 路径零修改，既有 Admin 模块测试通过                                          | 无 Admin 生产文件；最终 diff gate                         |
| `REQ-005`   | §4                                                                                                               | `@EgonRpcReference` 继续走 `INTERNAL_GATEWAY`             | 旧源码兼容；Yuheng 缺失仍报 `RPC_YUHENG_UNAVAILABLE`                         | RPC Starter Yuheng demand/wiring                 |
| `REQ-006`   | §4, §9                                                                                                           | 新增 `@EgonRpcDirectReference` 精确发现 `RPC_PROVIDER`       | Tianshu key 含 BIZ/APP/env/service/group/version/grpc；Channel 不连接 Yuheng | RPC Provider Directory + Tianshu adapter              |
| `REQ-007`   | §4, §9                                                                                                           | 同应用/同契约可双字段双路径；同字段双注解失败                                | 两类 Proxy 同时注入；冲突错误含 bean/field                                       | BPP、双 Proxy Factory、context tests                 |
| `REQ-008`   | §4, §7.5                                                                                                         | 两种路径不自动 fallback                                       | Yuheng/Provider 不可用错误彼此独立                                           | 两套 Manager/Channel Strategy                       |
| `REQ-009`   | §4                                                                                                               | 所有导出 RPC Provider 注册 Tianshu，不受 Yuheng 文档影响               | 无 Yuheng Group 时 `RPC_PROVIDER` lease 仍注册                           | Tianshu registry regression                           |
| `REQ-010`   | §4                                                                                                               | Direct-only RPC 可不配置 Yuheng 文档                        | 无 `@GatewayInterfaceGroup` 就无报告/Route                                | Yuheng Starter contract test                     |
| `REQ-011`   | §4                                                                                                               | Consumer 不主动判断接口权限                                     | RPC Starter/Adapter 无 Tianquan-Jianshen permission 依赖/读取                         | 仅发现、调用、Trace、凭证中继                                 |
| `REQ-012`   | §4, §9                                                                                                           | Provider 可取得并验证受控中继 USER AT                            | 有效 token 建身份；非法/重复 token 为 `UNAUTHENTICATED`；无 token 匿名              | Tianquan-Shoubing RPC interceptors + Yuheng forwarders         |
| `REQ-013`   | §4, §10                                                                                                          | Scope 与全量快照同授权版本/Fence                                 | tenant/sub/user/version/expiry 一致；不完整发布不可见                           | 双投影、Redis key/order、reader                        |
| `REQ-014`   | §4, §16                                                                                                          | Direct-only 切换显式撤销历史 Yuheng Route                     | Active Release 不再含旧 Operation；改注解不能代替下线                              | 发布清单与人工/运行 Gate                                   |

## 4. Implementation Strategy and Dependency Order

### 4.1 Ordered strategy

1. 先发布可独立序列化的 Tianquan-Jianshen scope contract，再让 Admin 一次投影并在 Pointer 可见前双写 Redis；这为读侧提供向后兼容的生产事实。
2. 在 Yuheng Adapter 中用新 reader/provider替换旧 Permission reader/provider；所有失败仍进入现有 `GatewaySecurityChain`
   的 Fail Closed 映射。
...
4. RPC Starter 先落中立 `RpcProviderDirectory`/Manager/Channel Strategy，再落双注解和 Spring wiring，保持 Starter 不依赖
   Tianshu。
5. Tianshu Adapter 实现精确 `RPC_PROVIDER` 查询并装配端口；Provider 注册写链不改变。
6. Tianquan-Shoubing Starter 提供 RPC client/server credential adapter；Consumer 只中继 `VerifiedUserTokenCarrier` 中的可信
   token，Provider 验证并在 callback scope 建立 Spring Security Context。
7. Yuheng RPC 与 HTTP→RPC 恢复 Security Chain 已验证的 `forwardingCredential`，不复制未经验证的原始 Header。
8. 固化 Yuheng Definition opt-in 和 Tianshu lease 解耦，并把历史 Route 下线列为发布 Gate。
...
| Yuheng BIZ→APP         | 新 reader/Fail Closed tests 因 scope reader/provider 不存在失败            | scope reader按层级判断并返回稳定 reason                                 | 删除 Permission reader/provider，更新 auto-config |
...
| Tianshu Direct discovery    | directory/autoconfig tests 因 `RpcProviderDirectory` Bean/key映射不存在失败 | `DdcRpcProviderDirectory` + Bean                              | 不修改 Provider Registry key                    |
| RPC credential relay    | Tianquan-Shoubing interceptor tests 因 Authorization key/context adapter不存在失败      | trusted carrier + client/server interceptors                  | token 不写 Authentication credentials/log      |
| Yuheng→RPC relay       | Yuheng tests 因 Outcome不携带 credential或 outbound metadata缺失失败        | Outcome + `authorization` metadata                            | HTTP→HTTP现状不变                                |
...
|--------|---------------|--------------------------|------------------------------------------------|--------------------------------------|
| Step 1 | None          | Step 4                   | Tianquan-Jianshen contract/admin files                     | 先建立写侧事实；与 RPC Core 无写冲突              |
| Step 2 | Step 1        | Step 4                   | Tianquan-Jianshen yuheng adapter files                    | reader依赖新 contract/key               |
| Step 3 | Step 2        | Step 5                   | Yuheng Engine POM/config/HTTP security/script | Engine必须先能加载新 Provider               |
| Step 4 | None          | Step 1                   | RPC Starter provider/channel packages          | 中立 SPI 不依赖 Tianshu                       |
| Step 5 | Step 4        | Step 3                   | RPC Starter annotation/proxy/config files      | 双注解依赖 Provider Manager               |
| Step 6 | Step 4        | Step 3                   | RPC Tianshu Adapter files                          | Adapter只依赖已发布中立 SPI                  |
| Step 7 | Step 5        | None                     | Tianquan-Shoubing Starter + shared RPC metadata key          | AutoConfig要把 interceptor注入 RPC proxy |
| Step 8 | Steps 2, 3, 7 | None                     | Yuheng RPC/HTTP forwarding files              | Gateway使用新 credential relay contract |
| Step 9 | Steps 1–8     | None                     | Yuheng Starter/Tianshu registry tests only        | 最终锁定独立事实源与发布边界                       |
...
│   │   ├── consumer/yuheng/RpcConsumerGatewayManager.java                             MODIFY [S5]
│   │   ├── consumer/yuheng/RpcGatewayEndpoint.java                                    MODIFY [S4]
...
│       ├── consumer/yuheng/RpcConsumerGatewayManagerTest.java                         MODIFY [S5]
...
└── egon-cola-component-rpc-tianshu-adapter/
    ...
    └── src/test/java/top/egon/cola/component/rpc/tianshu/
...
│   ├── src/main/java/top/egon/cola/platform/tianquan-jianshen/contract/authorization/
│   │   ├── ApplicationAccessScope.java                                                 CREATE [S1]
│   │   ├── BusinessAccessScope.java                                                    CREATE [S1]
│   │   └── GatewayBizAppScopeSnapshot.java                                             CREATE [S1]
│   └── src/test/java/top/egon/cola/platform/tianquan-jianshen/contract/ContractSerializationTest.java MODIFY [S1]
├── egon-cola-tianquan-jianshen-core/src/main/java/top/egon/cola/platform/tianquan-jianshen/core/runtime/
...
│   └── src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/
...
└── egon-cola-tianquan-jianshen-gateway-adapter/
    ├── src/main/java/top/egon/cola/platform/tianquan-jianshen/yuheng/
    ...
    └── src/test/java/top/egon/cola/platform/tianquan-jianshen/yuheng/
...
│   └── src/test/java/top/egon/cola/component/yuheng/engine/
...
└── yuheng-starter/src/test/java/top/egon/cola/component/yuheng/starter/discovery/
...
└── src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/
...
| Runtime            | 用户自行执行 `scripts/unified-identity-local.sh` 的既有工作流                                                                                                                                                                                                                                                                                                                                                                       | 双路径观察满足§9                 | 外部Redis/PostgreSQL/Tianshu/JWK真实闭环 |
...
- 历史 Yuheng RPC Route 必须通过现有 Draft/Release 流程显式撤销；Consumer 注解不写 Yuheng Admin。
- Yuheng Admin权限链、MCP权限、HTTP/RPC Wire Descriptor、Tianshu Provider key、全量 `UserAuthorizationSnapshot` JSON、PostgreSQL
  Schema 均不可改变。
...
|-----------------|------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|---------------------------------------------------------------|----------------------------------------------------|
| `PLAN-CLAR-001` | `RpcConsumerProviderManager` 复用现有 `channelDrainTimeoutMs`，Channel建连复用 `RpcConsumerChannelFactory`；不新增 direct discovery 外部配置键 | `EgonRpcProperties.Consumer` 已有 default deadline、Yuheng discovery和channel drain；Spec 未批准新配置字段 | 只选择内部已有超时，不改变注解或DDC查询契约；`EgonRpcProperties` 仅集中校验共享正值         | 若必须独立调优 Direct discovery，需要回到 Spec 增加公共配置契约        |
| `PLAN-CLAR-002` | 保留现有 `consumer/direct/RpcDirectClientFactory` 静态 target API；新注解使用 `consumer/provider` 包，不重命名或复用其单 target 生命周期                | 已有 `RpcDirectClientFactory` 服务 Tianquan-Shoubing/DDC基础设施静态直连；Spec target tree已为发现式直连定义独立 provider manager    | 避免破坏既有基础设施调用；两者都不经过Gateway但发现来源不同                             | 若要求统一两套 API，将扩大兼容与生命周期范围，需另行设计                     |
| `PLAN-CLAR-003` | Yuheng Manager 的 demand 由双注解 BPP 在生命周期启动前登记；没有 Yuheng 字段时 Manager 不订阅 `INTERNAL_GATEWAY`                                   | 当前 BPP 在 Bean initialization 阶段扫描字段，`RpcConsumerGatewayManager` 是 `SmartLifecycle`             | 实现 Spec 的“Direct-only 不要求 Yuheng”，且保留Gateway字段的启动期 fail-fast | 若第三方在容器启动后动态创建带注解 Bean，需要补运行时 demand 生命周期，当前不支持此模式 |
...
- Dependencies:None
- Observable outcome:Tianquan-Jianshen Admin 对一次 USER projection同时产生 full snapshot和 `GatewayBizAppScopeSnapshot`，在同一
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/service/RoleEligibilityServiceTest.java`

- Purpose:固定有效 APP scope 必须同时满足本地 ACTIVE、User Business grant、Tianshu APP/Business enabled和父子一致。
...
#### File 3 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/runtime/UserAuthorizationSnapshotProjectorTest.java`
...
#### File 4 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/runtime/RedisAuthorizationRuntimeRepositoryTest.java`
...
assert ordered calls: full snapshot set, yuheng scope set,
...
#### File 5 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-contract/src/main/java/top/egon/cola/platform/tianquan-jianshen/contract/authorization/ApplicationAccessScope.java`

- Purpose:表达一个 Tianshu APP 最小访问事实。
...
#### File 6 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-contract/src/main/java/top/egon/cola/platform/tianquan-jianshen/contract/authorization/BusinessAccessScope.java`
...
#### File 7 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-contract/src/main/java/top/egon/cola/platform/tianquan-jianshen/contract/authorization/GatewayBizAppScopeSnapshot.java`

- Purpose:定义 Yuheng 唯一可读取的最小授权投影。
...
#### File 8 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-core/src/main/java/top/egon/cola/platform/tianquan-jianshen/core/runtime/Rbac3RuntimeKeyFactory.java`

- Purpose:提供 versioned Yuheng scope Redis key。
- Symbols:`gatewayScope(String tenantId, String identitySub, long authVersion)`。
- Why now:Publisher与Reader必须共享唯一 key factory。
- Contract/signature changes:输出 `tianquan-jianshen:{tenant}:yuheng-scope:{identitySub}:{authVersion}`，复用 safe
  segment/nonnegative规则。
- Implementation pseudocode:

```java
if authVersion < 0 throw;
return prefix(tenantId) + "yuheng-scope:" + segment(identitySub) + ':' + authVersion;
```

- After this file:读写端可引用同一 key契约。

#### File 9 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/service/EffectiveApplicationScope.java`

- Purpose:在 Admin内部传递已同时验证的 Tianshu BIZ/APP身份。
- Symbols:
...
#### File 10 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/service/RoleEligibilityService.java`
...
#### File 11 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/runtime/domain/vo/UserSnapshotProjectionVO.java`
...
#### File 12 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/runtime/service/UserAuthorizationSnapshotProjector.java`
...
- After this file:Projector tests GREEN；Yuheng scope无 permission数据。

#### File 13 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/runtime/repository/redis/RedisAuthorizationRuntimeRepository.java`
...
set full snapshot(versioned), then yuheng scope(versioned);
...
- Commit:`feat(tianquan-jianshen): publish yuheng biz app scope snapshots`

### Step 2 — 用 BIZ→APP Scope Provider 替换 Yuheng 接口权限 Provider

- Requirements:`REQ-001`, `REQ-002`, `REQ-003`, `REQ-013`
- Dependencies:Step 1
- Observable outcome:Tianquan-Jianshen Yuheng Adapter只读取 user pointer/version/fence和 `yuheng-scope`，先 BIZ后 APP；不再包含
  Operation Mapping/Permission reader/provider。
- Ordered files:

#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/test/java/top/egon/cola/platform/tianquan-jianshen/yuheng/runtime/Rbac3GatewayScopeSnapshotReaderTest.java`
...
- Contract/signature changes:`authorize(GatewayAuthContext)` 只消费 `tianquan-shoubing.biz-code/app-code`；严格 fake Redisson对
...
- Contract/signature changes:预期 `Rbac3GatewayScopeSnapshotReader`、`Rbac3BizAppScopeAuthorizationProvider` 单
  Bean；providerId=`tianquan-jianshen-biz-app-scope`。
- Implementation pseudocode:

```java
run enabled context with named redisson;
assert single new reader/provider;
assert provider.providerId == "tianquan-jianshen-biz-app-scope";
...
- Contract/signature changes:异常 reason=`TIANQUAN_JIANSHEN_SCOPE_RUNTIME_UNAVAILABLE`；业务/应用拒绝不被转换为 error。
- Implementation pseudocode:

```java
provider(source returns deny business/application) -> DENY same reason;
provider(source throws) -> ERROR "TIANQUAN_JIANSHEN_SCOPE_RUNTIME_UNAVAILABLE";
...
scan adapter main sources; reject EntityManager/Jdbc/WebClient/tianquan-jianshen-admin/operationMapping/permissions();
```

- After this file:旧类型/源码引用导致 RED。

#### File 5 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/main/java/top/egon/cola/platform/tianquan-jianshen/yuheng/runtime/Rbac3GatewayScopeSnapshotReader.java`
...
authorize(context):
  ...
  biz = required route["tianquan-shoubing.biz-code"]; app = required route["tianquan-shoubing.app-code"];
...
#### File 6 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/main/java/top/egon/cola/platform/tianquan-jianshen/yuheng/security/Rbac3BizAppScopeAuthorizationProvider.java`

- Purpose:把阻塞 Redis scope source适配为 Yuheng reactive SPI。
- Symbols:`PROVIDER_ID="tianquan-jianshen-biz-app-scope"`、`DecisionSource`、`authorize`。
- Why now:复用现有 Adapter模式和 boundedElastic边界。
- Contract/signature changes:source异常统一 `AuthorizationDecision.error("TIANQUAN_JIANSHEN_SCOPE_RUNTIME_UNAVAILABLE")`。
- Implementation pseudocode:

```java
authorize(context) = Mono.fromCallable(() -> source.authorize(context))
  .subscribeOn(boundedElastic())
  .onErrorReturn(error("TIANQUAN_JIANSHEN_SCOPE_RUNTIME_UNAVAILABLE"));
```
...
#### File 8 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/main/java/top/egon/cola/platform/tianquan-jianshen/yuheng/runtime/Rbac3GatewayRuntimeSnapshotReader.java`
...
OperationPermissionMapping, keyFactory.operationMapping in yuheng adapter.
```

- After this file:Adapter生产代码不再具备接口权限读取能力。

#### File 9 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/main/java/top/egon/cola/platform/tianquan-jianshen/yuheng/security/Rbac3PermissionAuthorizationProvider.java`

- Purpose:移除旧 `tianquan-jianshen-permission` Capability实现。
...
```text
delete file; no compatibility bean under "tianquan-jianshen-permission".
```

- After this file:模块 capability只有 `tianquan-jianshen-biz-app-scope`。

#### File 10 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/test/java/top/egon/cola/platform/tianquan-jianshen/yuheng/runtime/Rbac3GatewayRuntimeSnapshotReaderTest.java`
...
- Commit:`refactor(tianquan-jianshen): authorize yuheng by biz app scope`

### Step 3 — 在 Yuheng Engine 启用 Scope Capability并迁移 Policy ID

- Requirements:`REQ-001`, `REQ-002`, `REQ-003`, `REQ-004`
- Dependencies:Step 2
- Observable outcome:Engine executable classpath包含 Tianquan-Shoubing + Tianquan-Jianshen scope adapters；HTTP Route只向安全链提供服务端
  BIZ/APP/env；本地 `BUSINESS_PROTECTED` Policy引用 `tianquan-jianshen-biz-app-scope`；Admin模块无修改。
- Ordered files:

#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/test/java/top/egon/cola/component/yuheng/engine/IdpAdapterRuntimeClasspathTest.java`

- Purpose:把 Engine runtime capability预期从“只有 Tianquan-Shoubing Adapter”改为“Tianquan-Shoubing + scope Adapter”。
- Symbols:重命名测试为 `executableEngineCarriesIdentityAndBizAppScopeAdapters()`。
- Why now:先让缺失 runtime依赖产生明确 RED。
- Contract/signature changes:两个 AutoConfiguration class均 `Class.forName`成功；不再期待 Tianquan-Jianshen class缺失。
- Implementation pseudocode:
...
- Contract/signature changes:即使 Route metadata含 definitionSet/mappingVersion/applicationCode，结果也精确等于三个
  `tianquan-shoubing.*`字段。
- Implementation pseudocode:

```java
attrs = securityAttributes(routeWithUpstreamAndLegacyRbacMetadata());
assertThat(attrs).containsExactlyEntriesOf(Map.of(
  "tianquan-shoubing.biz-code", biz, "tianquan-shoubing.app-code", app, "tianquan-shoubing.env", env));
```

- After this file:当前实现仍复制三个 `tianquan-jianshen.*`字段而 RED。
...
- Contract/signature changes:返回 immutable map仅含 `tianquan-shoubing.biz-code/app-code/env`。
- Implementation pseudocode:

```java
upstream = route.route().upstream();
return Map.of("tianquan-shoubing.biz-code", upstream.bizCode(),
...
- Purpose:把 Tianquan-Jianshen Yuheng Adapter加入 executable Engine runtime。
...
- Contract/signature changes:不删除现有 `tianquan-jianshen-starter`（下游/MCP能力仍需）；不添加新版本。
- Implementation pseudocode:
...
- Purpose:显式启用 Scope Adapter并指向 Tianquan-Jianshen runtime Redis。
- Symbols:`egon.cola.platform.tianquan.jianshen.yuheng.enabled/runtime.*`。
- Why now:类在 runtime后还需满足 AutoConfiguration条件。
- Contract/signature changes:新增 Engine环境变量
  `YUHENG_TIANQUAN_JIANSHEN_SCOPE_ENABLED/REDIS_ADDRESS/REDIS_DATABASE/REDIS_PASSWORD_FILE/TIMEOUT`；不复用请求 Header或MCP
  endpoint。
- Implementation pseudocode:

```yaml
egon.cola.platform.tianquan.jianshen.yuheng:
  enabled: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_ENABLED:true}
  runtime:
    redis-enabled: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_ENABLED:true}
    redis-address: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_REDIS_ADDRESS:redis://127.0.0.1:6379}
    database: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_REDIS_DATABASE:8}
    password-file: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_REDIS_PASSWORD_FILE:}
    timeout: ${YUHENG_TIANQUAN_JIANSHEN_SCOPE_REDIS_TIMEOUT:2s}
```

- After this file:Engine配置可装配新 Provider；MCP的 `tianquan-jianshen.enabled`边界不变。
...
- Symbols:Yuheng Engine env生成；policy case中的 `authz_providers`。
- Why now:Engine capability/config已具备，最后切规则。
- Contract/signature changes:写入 scope Redis env；`authz_providers='["tianquan-jianshen-biz-app-scope"]'`；保留 PUBLIC/IDENTITY和
  `ORIGINAL_BEARER`。
- Implementation pseudocode:

```bash
write_env yuheng-biz-gateway.env YUHENG_TIANQUAN_JIANSHEN_SCOPE_ENABLED true
...
BUSINESS_PROTECTED -> authz_providers='["tianquan-jianshen-biz-app-scope"]'
before deleting old adapter in deployed environment:
  assert no desired/active policy still references tianquan-jianshen-permission
```
...
- Expected result:Maven与`bash -n` exit 0；最后一条命令无输出；Engine主源码不再出现 `tianquan-jianshen.definition-set-id`/
  `tianquan-jianshen.mapping-version`。
- Completion criteria:Engine capability、配置、HTTP attrs和策略 ID全部匹配新范围模型，Yuheng Admin零修改。
- Rollback:先把 Policy回滚到上一 Release，再回退 Engine/config/script；禁止通过清空 authorization providers回滚。
- Commit:`feat(yuheng): enable biz app scope authorization`
...
verify fake Yuheng directory is not a dependency/call;
...
- Purpose:为 Yuheng/Provider endpoint共享建链字段，不共享发现语义。
- Symbols:interface accessors `host()`, `port()`, `secure()`。
- Why now:Channel Factory不应依赖 yuheng package。
...
#### File 3 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/yuheng/RpcGatewayEndpoint.java`

- Purpose:让现有 Yuheng endpoint实现中立 transport port。
- Symbols:`implements RpcEndpoint`。
- Why now:保持现有 Yuheng Manager源码兼容，同时为 factory泛化做准备。
...
- After this file:Yuheng endpoint行为无变化。
...
- After this file:现有 Yuheng tests仍编译，Provider endpoint可建链。
...
- Purpose:表达一个 Tianshu Provider lease endpoint。
- Symbols:record同 Yuheng endpoint字段并 `implements RpcEndpoint`。
...
- Contract/signature changes:与 Yuheng subscription风格一致。
- Implementation pseudocode:
...
- After this file:Tianshu Adapter可在 Step 6实现。
...
recordFailure(query, channel): remove+drain only that provider; no yuheng call;
...
### Step 5 — 增加 Yuheng/Direct 双注解注入与 mode-specific demand
...
- Observable outcome:应用可分别或同时注入 `@EgonRpcReference`/`@EgonRpcDirectReference`；direct-only不订阅/等待
  Yuheng；同字段冲突启动失败；两者均使用已有 interceptor factory链且无隐式 fallback。
- Ordered files:
...
- Purpose:定义 Yuheng、Direct、双字段、同契约双字段、同字段双注解和缺 mode factory错误。
...
direct annotation + no direct factory -> stable startup error; yuheng equivalent likewise;
...
- Purpose:用 `ApplicationContextRunner` 固定 yuheng-only/direct-only/both和 interceptor注入 wiring。
...
- Why now:防止 Bean条件仍强制 Yuheng。
- Contract/signature changes:Manager/Proxy beans按相应 Directory条件创建；BPP总能给出稳定配置错误。
- Implementation pseudocode:

```java
with RpcProviderDirectory only + direct fixture -> context starts, no yuheng subscription;
with RpcGatewayDirectory only + yuheng fixture -> existing manager starts;
...
#### File 3 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/test/java/top/egon/cola/component/rpc/consumer/yuheng/RpcConsumerGatewayManagerTest.java`
...
- Contract/signature changes:既有 yuheng tests在 `manager.registerDemand()` 后保持；零 demand start不访问 Directory。
- Implementation pseudocode:
...
- After this file:Direct proxy构造不依赖 DDC或 Yuheng。

#### File 7 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/yuheng/RpcConsumerGatewayManager.java`
...
- After this file:Direct-only不触发 Yuheng discovery；既有 Gateway行为保持。
...
- Symbols:constructor接收 optional Yuheng proxy/manager和Direct factory；`postProcessBeforeInitialization`。
- Why now:两种 factory/manager已存在。
- Contract/signature changes:同字段双注解优先报冲突；每字段只注入一次；错误含 bean/field。
- Implementation pseudocode:

```java
for each field annotated yuheng or direct:
...
- Symbols:`Consumer.validateSharedSettings()`；现有 Yuheng-specific校验仍由 Yuheng Manager完成。
...
- Symbols:新增 `RpcConsumerChannelFactory` Bean、Provider Manager/Direct Factory Beans；调整 Yuheng beans/BPP。
- Why now:所有中立类型和注解行为已完成。
- Contract/signature changes:Yuheng beans `@ConditionalOnBean(RpcGatewayDirectory)`；Direct beans
...
- Expected result:exit 0；direct-only context无 Gateway订阅；yuheng-only和both通过；冲突信息稳定；现有
...
- Commit:`feat(rpc): add tianshu direct reference injection`

### Step 6 — 用 Tianshu Adapter 实现精确 `RPC_PROVIDER` 发现

- Requirements:`REQ-006`, `REQ-008`, `REQ-009`
- Dependencies:Step 4（实施顺序上建议在 Step 5后验证完整 context）
- Observable outcome:`RpcProviderDirectory`由 Tianshu Adapter提供，严格按 Direct query构造 `DdcServiceKind.RPC_PROVIDER`
  key并映射活动 leases；绝不查询 `INTERNAL_GATEWAY`。
- Ordered files:

#### File 1 —
`CREATE egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectoryTest.java`

- Purpose:锁定完整 Tianshu key、revision/lease映射和订阅关闭。
- Symbols:`queriesExactRpcProviderServiceKey()`、`mapsLeaseSnapshotAndPreservesRevision()`。
- Why now:Tianshu Adapter的 RED契约。
...
subscription.close -> verify Tianshu subscription close;
```

- After this file:因实现不存在 RED。

#### File 2 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/autoconfigure/DdcRpcAutoConfigurationTest.java`

- Purpose:要求 registry-enabled context同时暴露 Provider Registry、Yuheng Directory和Provider Directory。
- Symbols:扩展 `registrySwitchCreatesRegistryPortsWithOnlyRegistryCredential()`。
- Why now:锁定 Adapter Bean wiring。
- Contract/signature changes:`hasSingleBean(RpcProviderDirectory.class)`；不要求 Yuheng consumer配置。
- Implementation pseudocode:
...
#### File 3 —
`CREATE egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectory.java`

- Purpose:把 Tianshu registry snapshots适配为中立 Provider snapshots。
...
- After this file:Directory tests GREEN；没有 Yuheng slot访问。

#### File 4 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/autoconfigure/DdcRpcAutoConfiguration.java`
...
- Completion criteria:DDC读侧与现有写侧使用同一物理服务键；Direct path没有 Yuheng Directory调用。
- Rollback:回退 Adapter Bean/实现/测试；Provider registration不受影响。
- Commit:`feat(rpc-tianshu): discover direct rpc providers`

### Step 7 — 在 Tianquan-Shoubing Starter 中建立可信 RPC USER Credential Relay
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/security/rpc/IdpRpcClientCredentialInterceptorFactoryTest.java`
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/security/rpc/IdpRpcBearerServerInterceptorTest.java`
...
- Contract/signature changes:`RpcClientInterceptorFactory`中包含 Tianquan-Shoubing factory；`ServerInterceptor`中包含 Tianquan-Shoubing server
  interceptor。
- Implementation pseudocode:
...
- Purpose:为 Yuheng、Tianquan-Shoubing、RPC共享唯一 Authorization metadata key。
...
#### File 5 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/security/rpc/IdpRpcSecurityContext.java`
...
#### File 6 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/security/VerifiedUserTokenCarrier.java`
...
#### File 7 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/security/rpc/IdpRpcClientCredentialInterceptorFactory.java`
...
#### File 8 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/security/rpc/IdpRpcBearerServerInterceptor.java`
...
- Contract/signature changes:最大长度沿用 Tianquan-Shoubing 8192；缺 token匿名；非法/expired/duplicate为 `Status.UNAUTHENTICATED`。
- Implementation pseudocode:
...
- Purpose:条件注册 Tianquan-Shoubing RPC client/server安全 adapters。
...
- Contract/signature changes:不增加新 Maven dependency（Tianquan-Shoubing Starter已依赖 RPC Starter）；HTTP filter beans不变。
- Implementation pseudocode:
...
- Rollback:回退 Tianquan-Shoubing RPC beans/classes与 shared metadata key；HTTP Tianquan-Shoubing filter不受影响。
- Commit:`feat(tianquan-shoubing): relay verified user credentials over rpc`

### Step 8 — 将 Yuheng 已验证 Bearer受控转发到 RPC Provider
...
#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/test/java/top/egon/cola/component/yuheng/engine/rpc/RuleBackedRpcGatewaySecurityProcessorTest.java`
...
assert captured authContext.attributes == tianquan-shoubing biz/app/env only;
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/test/java/top/egon/cola/component/yuheng/engine/rpc/RpcGatewayCredentialForwardingTest.java`

- Purpose:固定 RPC Yuheng outbound metadata的 credential规则。
...
#### File 3 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/test/java/top/egon/cola/component/yuheng/engine/http/DefaultGatewayHttpDataPlaneHandlerCredentialForwardingTest.java`
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/test/java/top/egon/cola/component/yuheng/engine/rpc/HttpRpcUpstreamAdapterTest.java`
...
#### File 5 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/main/java/top/egon/cola/component/yuheng/engine/rpc/GatewayRpcSecurityProcessor.java`
...
#### File 6 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/main/java/top/egon/cola/component/yuheng/engine/rpc/RuleBackedRpcGatewaySecurityProcessor.java`
...
#### File 7 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/main/java/top/egon/cola/component/yuheng/engine/rpc/RpcGatewayForwarder.java`
...
- After this file:RPC Yuheng credential tests GREEN；伪造 inbound不会透传。
...
- Expected result:exit 0；两条 Yuheng→RPC路径的有效/伪造/禁止转发场景通过；test output无 token。
- Completion criteria:Provider收到原始已验证 USER AT，Gateway不做 Permission，未经验证 header无法越过 sanitizer/security
  result。
- Rollback:整体回退 Step 8会停止 Yuheng→RPC USER credential relay；不得在已下沉下游权限后单独回退，除非同步回滚
  Policy/业务发布。
- Commit:`feat(yuheng): relay verified bearer to rpc providers`

### Step 9 — 固化 DDC注册、Gateway文档与历史 Route 的独立边界

- Requirements:`REQ-004`, `REQ-009`, `REQ-010`, `REQ-014`
- Dependencies:Steps 1–8
- Observable outcome:测试证明无 Yuheng Group的 RPC不报告 Definition但 Provider lease仍注册 `RPC_PROVIDER`；发布清单要求
  Direct-only前显式删除/撤销历史 Operation并发布新 Release；Yuheng Admin权限代码零变化。
- Ordered files:

#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-yuheng/yuheng-starter/src/test/java/top/egon/cola/component/yuheng/starter/discovery/RpcGatewayDefinitionContributorTest.java`
...
#### File 2 —
`MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderRegistryTest.java`

- Purpose:证明注册请求只取 Provider registration/service identity，不依赖任何 Yuheng annotation/report bean。
...
assert metadata need not contain yuheng group/operation/definition;
```
...
- Expected result:两个 Maven命令 exit 0；Admin路径 diff无输出；无 Group Definition为空但 Tianshu captured registration存在。
- Completion criteria:文档、实例、Consumer调用三个事实源互不反向控制；`REQ-014`进入发布 Gate而非注解副作用。
- Rollback:测试文件可单独回退但不建议；没有生产行为或数据回滚。
- Commit:`test(yuheng): lock rpc reporting and registration boundaries`
...
| RED/GREEN S2                 | Step 2聚焦命令                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | RBAC Yuheng adapter                 | RED因新 reader/provider缺失；GREEN exit 0                                                                                                                                                                                                                                                         | S2                       | `REQ-001`–`REQ-003`,`REQ-013`                                                                                                                                                                                                                       |
...
| RED/GREEN S6                 | Step 6聚焦命令                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | RPC Tianshu adapter                      | key=`RPC_PROVIDER`且精确，Bean存在                                                                                                                                                                                                                                                                 | S6                       | `REQ-006`,`REQ-008`,`REQ-009`                                                                                                                                                                                                                       |
| RED/GREEN S7                 | Step 7聚焦命令                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Tianquan-Shoubing/RPC security                     | token有效/匿名/非法/清理通过                                                                                                                                                                                                                                                                           | S7                       | `REQ-011`,`REQ-012`                                                                                                                                                                                                                                 |
| RED/GREEN S8                 | Step 8聚焦命令                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Yuheng→RPC                          | 只转发verified credential                                                                                                                                                                                                                                                                       | S8                       | `REQ-002`,`REQ-003`,`REQ-012`                                                                                                                                                                                                                       |
...
| Static removal               | `rg -n "tianquan-jianshen-permission                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Rbac3PermissionAuthorizationProvider | Rbac3GatewayRuntimeSnapshotReader                                                                                                                                                                                                                                                            | tianquan-jianshen\.definition-set-id | tianquan-jianshen\.mapping-version" egon-cola-xingyuan/egon-cola-yuheng/yuheng-biz-gateway/src/main egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-gateway-adapter/src/main scripts/unified-identity-local.sh` | 数据面/脚本 | 无输出 | S2/S3 | `REQ-002` |
| Consumer permission boundary | `rg -n "tianquan-jianshen                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | permission                           | RequiresPermission" egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry` | Consumer/Directory       | 无新增 permission读取；仅可能命中文档否定语句                                                                                                                                                                                                                        | S4–S7 | `REQ-011` |
| Secret/log scan              | `rg -n "logger.*token                                                                                                                                                                                                                                                                                                                                                                                                                                                               | log.*authorization                   | metadata.*token" <changed-main-paths>` 并人工审查 diff                                                                                                                                                                                                                                            | 安全                       | 无原始 token日志/指标/Tianshu metadata                                                                                                                                                                                                                         | S7/S8 | `REQ-012` |
| Formatting/scope             | `git diff --check`; `git status --short`; `git diff --name-only -- .../yuheng-admin`                                                                                                                                                                                                                                                                                                                                                                                               | 全仓                                   | 无 whitespace错误；仅计划文件树；Admin生产路径无变化                                                                                                                                                                                                                                                           | owning Step              | All                                                                                                                                                                                                                                                 |
| Runtime E2E                  | 用户启动本地栈后按 §9.3执行                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Redis/Tianshu/JWK/Provider/Engine        | 两条路径地址、身份、授权、无fallback均符合                                                                                                                                                                                                                                                                    | 相应 Step                  | All                                                                                                                                                                                                                                                 |
...
N/A。依据 Spec §11，不新增/修改 PostgreSQL表、JPA PO、Tianshu Proto或 Flyway migration。新增的是 TTL控制的 Redis versioned key；历史
Flyway文件保持不可变。

### 9.2 Source/runtime compatibility

- `@EgonRpcReference` signature和 Gateway默认行为保持；新 `@EgonRpcDirectReference`是 additive API。
- 现有 `RpcDirectClientFactory`静态 target API不改，保证 Tianquan-Shoubing/DDC基础设施客户端兼容。
- Tianshu `RPC_PROVIDER`和`INTERNAL_GATEWAY` key schema不改；只新增读 Adapter。
- 全量 `UserAuthorizationSnapshot/AppAuthorizationContext` JSON不改；新 scope独立发布。
- RPC Starter和 RPC Tianshu Adapter必须同一平台版本发布；旧 Consumer不使用新注解时不受影响。

### 9.3 Deployment order and post-deploy gates

1. 部署 Step 1 Tianquan-Jianshen Admin写侧，保持旧 Yuheng Policy；触发活跃 USER授权重投影。
...
3. 部署 Step 2 Adapter artifact和 Step 3 Engine artifact/config，但保持旧 Engine实例/Policy受控滚动；确认新 Engine
   capability列出 `tianquan-jianshen-biz-app-scope`。
4. 发布 `BUSINESS_PROTECTED.authorizationProviderIds=["tianquan-jianshen-biz-app-scope"]`，观察 BIZ deny、APP deny、runtime
   unavailable；确认没有 active policy引用 `tianquan-jianshen-permission`后才完成旧代码清理部署。
5. 部署 Steps 4–7 RPC Starter/Tianshu Adapter/Tianquan-Shoubing Starter到一个试点 Provider+Consumer；Provider必须已有本地 Tianquan-Shoubing/RBAC3接口权限链。
...
7. 切换某契约为 direct-only前，先通过现有 Admin Draft删除其 RPC Operation/Route并发布新 Release；查询 release/diff/runtime
   consistency确认 active release不含旧 Operation。之后才移除 Provider的 `@GatewayInterfaceGroup`或 Yuheng Starter依赖。
...
| Scenario                                                   | Expected Yuheng/Tianshu path                     | Expected auth result                          |
|------------------------------------------------------------|-----------------------------------------------|-----------------------------------------------|
| Yuheng RPC + BIZ denied                                   | Consumer→`INTERNAL_GATEWAY`; no Provider call | Yuheng 403/`PERMISSION_DENIED`, APP未查        |
| Yuheng RPC + APP denied                                   | Consumer→Yuheng; no Provider call            | Yuheng 403/`PERMISSION_DENIED`               |
| Yuheng RPC + scope allowed + downstream permission denied | Consumer→Yuheng→Provider                     | Yuheng allow；Provider本地 deny                 |
| Direct RPC + downstream permission allowed                 | Consumer→exact `RPC_PROVIDER`                 | 无 Yuheng span/address；Provider验证 USER并 allow |
| Direct RPC + no Provider                                   | Provider Directory empty                      | `RPC_PROVIDER_UNAVAILABLE`，不连 Yuheng         |
| Yuheng RPC + no Yuheng                                   | Yuheng Directory empty                       | `RPC_YUHENG_UNAVAILABLE`，不直连 Provider        |
| Direct-only contract无 Group                                | Provider仍有 Tianshu lease                          | Yuheng report/active route不存在                |
...
- Direct调用点可源码改回 `@EgonRpcReference`，但仅当对应 Yuheng Definition/Active Release仍存在；没有运行时 AUTO
  fallback。
- 已显式撤销的 Yuheng Route必须走正常 Draft/Release恢复，Consumer注解回滚不会自动恢复。
- Tianquan-Shoubing/Yuheng credential relay不可在下游权限已成为唯一接口授权后单独回滚；需要与调用点/Policy整体回退。
...
| `REQ-005`   | §4, §9                 | S5           | Yuheng manager/BPP/AutoConfig                                  | Yuheng manager/context tests        | 旧注解仍查 Gateway且源兼容         |
| `REQ-006`   | §4, §9                 | S4–S6        | provider SPI/manager/direct factory/Tianshu directory               | Provider manager/Tianshu key tests       | exact `RPC_PROVIDER`直连    |
| `REQ-007`   | §4, §9                 | S4, S5       | annotation/BPP/two factories                                    | BPP/context tests                    | 双字段双路径；同字段拒绝              |
| `REQ-008`   | §4, §7.5               | S4–S6        | separate managers/providers                                     | no-candidate/no-yuheng tests        | 错误码分离且无fallback           |
| `REQ-009`   | §4                     | S6, S9       | Tianshu directory + Registry test                                   | TEST-010/S9                          | 无文档仍注册lease               |
| `REQ-010`   | §4                     | S9           | Contributor test                                                | TEST-011/S9                          | 无 Group无report/Route      |
| `REQ-011`   | §4                     | S4, S5, S7   | RPC consumer/Tianquan-Shoubing client factory                                 | consumer static boundary             | Consumer只中继不授权            |
| `REQ-012`   | §4, §9                 | S7, S8       | Tianquan-Shoubing interceptors/carrier、Yuheng forwarders                     | TEST-013/014；S7/S8                   | Provider重验token并建身份       |
...
| `DEC-003`  | 历史 Yuheng RPC Route显式下线                                           | S9/rollout             | 用户本轮确认“3 确认”                                          | User/Operator | Closed；部署前逐契约执行                                                      |
...
| `RISK-002` | Engine同时包含 tianquan-jianshen-starter和gateway-adapter，named Redis bean条件装配顺序不一致 | S3 application.yml/POM | 两者均可能声明 `rbac3RuntimeRedissonClient`且有 missing-bean条件 | Implementer   | Context/classpath test；两组配置指向同 runtime Redis；若不能保证则退回Spec，不擅自拆新Bean名 |
| `RISK-003` | Direct绕过 Gateway集中限流/熔断                                            | S4–S6                  | Spec §15已接受                                           | User          | Accepted；需治理的调用继续Gateway                                             |
| `RISK-004` | Provider未启用本地IdP/Tianquan-Jianshen                                             | S7/rollout             | Gateway不再兜底接口Permission                               | Service owner | Required rollout gate；未通过不得迁移该Provider                               |
...
Gateway文档、所有 Provider注册 Tianshu、Consumer不授权、Provider下游授权和历史 Route显式下线。没有把范围鉴权删成完全不鉴权，也没有引入隐式
fallback。

### 12.2 Spec consistency

计划未新增数据库、页面、Streaming、LoadBalance框架、MCP变更、Admin业务能力或 Yuheng/Direct AUTO模式。三个 `PLAN-CLAR-*`
...
每个行为变化均先安排可解释的 RED，再安排最小 GREEN；覆盖 contract、projection、Redis publication、Yuheng scope reader、Engine
wiring、RPC Manager/BPP/Tianshu adapter、Tianquan-Shoubing interceptors、Yuheng forwarding、文档/注册边界和 Admin零修改。数据库迁移为 N/A；真实
Redis/Tianshu/JWK/Provider/Engine闭环明确留给用户启动本地栈后验证，Plan不冒充运行态证据。
