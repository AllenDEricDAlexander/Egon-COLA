...
| Scope | `RPC Starter, RPC Tianshu Adapter, Access Guard rate-limit runtime, RPC test-contract/process tests, bilingual component documentation` |
| Source Requirement | `2026-08-19 and 2026-08-21 user requirements and confirmations: fixed Yuheng/Direct client mode, same-mode reselection, cached Tianshu discovery, Provider-owned heartbeat, five load-balancing algorithms, graceful lifecycle, Guard-based Provider rate limiting, blocking/CompletionStage/generic unary calls, shared-channel multiplexing, CGLIB proxying, business-owned idempotency, business errors remain terminal, FAIL_OPEN returns null` |
...
| Effective Specs | [RPC 运行时治理演进规格](../spec/2026-08-19-15-36-rpc-runtime-governance-evolution.md); [Tianshu 单机闭环与轻量 RPC 设计](../../superpowers/specs/2026-07-24-tianshu-standalone-rpc-framework-design.md); [Yuheng Engine RPC 数据面](../../superpowers/specs/2026-07-25-yuheng-biz-gateway-rpc-design.md); [Tianshu/RPC/Yuheng 统一服务模型](../../superpowers/specs/2026-07-26-tianshu-rpc-yuheng-unified-service-model-design.md); [Yuheng BIZ/APP 与 Tianshu 直连 RPC](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) |
...
完成证据包括 75 个 Spec 测试场景在聚焦单测、组件测试、可选 Redis Testcontainers、RPC test-contract TCP 和用户控制的进程级 Tianshu/Redis 验证中的映射；每步都定义 RED 原因、最小 GREEN、验证命令、失败回退点和独立 commit。Plan 阶段不修改生产/测试源码、不启动任何服务、不执行迁移；关系数据库和前端均不受影响。
...
| Amendment source | [Tianshu 单机闭环与轻量 RPC 设计](../../superpowers/specs/2026-07-24-tianshu-standalone-rpc-framework-design.md) | Yuheng Mock 边界修订，待确认 | §7.4、§8、§9、§10、§11.4、§13–§16、§20 | 保留 Tianshu lease、event+full-pull subscription、Proto unary、metadata/status、Provider heartbeat 基线；其“无 Consumer LB/重试/泛化/限流”和 JDK Proxy 被 Primary 修订 |
| Dependency | [Yuheng Engine RPC 数据面](../../superpowers/specs/2026-07-25-yuheng-biz-gateway-rpc-design.md) | 已实现，待验收 | §5–§13 | 保留 Yuheng raw unary、safe metadata、Provider route/retry 独立层和 trailer 重建边界；本计划不修改 Yuheng production code |
| Amendment source | [Tianshu/RPC/Yuheng 统一服务模型](../../superpowers/specs/2026-07-26-tianshu-rpc-yuheng-unified-service-model-design.md) | S1–S4 已实施，S5 勘误 | 勘误 E1/E3、§2、§3.2、§3.4、§4.3–§4.4 | 保留 `yuheng.weight` 的 1–10000 类型化事实、field-free Provider marker 和 additive RPC annotations；`retryOnlyIdempotent` 被 Primary 明确修订 |
| Predecessor/partial replacement | [Yuheng BIZ/APP 与 Tianshu 直连 RPC](../spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) | `Review`; 2026-08-15 16:57 CST | §7.1–§7.2、§9 `META-001`、§15 Security、§16 route retirement | 保留 Yuheng/Direct 明示模式、精确 Provider query、credential relay 与业务 Provider 授权；其固定 Round Robin/无 client governance 部分被 Primary supersede |

### 2.3 Superseded or excluded content

- Primary Spec 已修订 2026-07-24 的唯一 Yuheng、无 Consumer Provider Directory/LB/retry/limit/generic、JDK Proxy 与 blocking-only 约束。
- Primary Spec 已修订 2026-07-26 的 `retryOnlyIdempotent=true`：RPC Consumer 不读取 `idempotent` 来决定重试，业务以唯一单据号、upsert/覆盖等方式承担重复安全。
- Primary Spec 只 supersede 2026-08-15 Spec 的 RPC Reference/Direct/runtime governance；Yuheng BIZ/APP 安全、credential relay、Definition/Release 和 Provider lease 分离仍有效。
- Yuheng Engine production implementation、Tianshu Admin/Starter subscription implementation、Tianquan-Shoubing/Tianquan-Jianshen、Proto wire、关系数据库/Flyway、前端、streaming、AOT/native-image 均排除在写范围之外。
...
| `REQ-002` | Primary §4, §9.2.1 | `@EgonRpcReference` 固定 Yuheng primary | 只登记/读取 Yuheng demand；既有源码兼容 | Yuheng Strategy、BPP、process counter |
| `REQ-003` | Primary §4, §9.2.2 | `@EgonRpcDirectReference` 固定 Direct primary | 只登记/读取 exact `RPC_PROVIDER` query；Yuheng calls=0 | Direct Strategy、Provider Manager/Tianshu adapter |
| `REQ-004` | Primary §4, §7.3.4 | 可用性失败只在同模式换未尝试实例，禁止跨模式 | acquisition/UNAVAILABLE 才重选；候选耗尽报模式错误 | executor、LB、mode strategies |
| `REQ-005` | Primary §4, §7.3.1 | Consumer 缓存目录并复用现有 Tianshu event+full pull | 每 query 单订阅；调用热路径无 Tianshu pull | managers、existing Tianshu subscription boundary |
| `REQ-006` | Primary §4, §7.3.4 | revision 单调、lease 过期摘除、对账恢复 | 旧 revision 丢弃；Tianshu 失败后本地过期，不无限保留 | managers、Tianshu adapter regression |
| `REQ-007` | Primary §4, §10.3 | Provider 主动 heartbeat，间隔由 RPC 配置；Tianshu 不反向探活 | fixed-delay register/heartbeat/recover；interval < lease | Provider lifecycle/lease tests |
...
| `REQ-009` | Primary §4, §7.3.1, §10 | 权重统一来自 `yuheng.weight`；一致性 Hash 使用业务 affinity key | Provider/Yuheng default=100；typed resolver/generic key稳定且不泄露 | endpoint/codec、resolver、LB tests |
...
| `REQ-021` | Primary §4, §16 | 保持 Proto/Tianshu/Yuheng/Provider marker/旧 annotations 兼容 | 无 `.proto`/Yuheng/Flyway production diff；legacy APIs compile | compatibility/static gates |
...
3. 将 Yuheng/Direct 声明解析成固定模式 Strategy；Manager 继续只持有 Tianshu snapshot，Channel 所有权转交共享 pool；Consumer coordinator 统一 start/drain gate。
...
7. 真实 TCP/进程测试与 README 最后收口，且进程级 Tianshu/Redis 仍由用户显式开启；每个前置 Step 已有模块内 RED/GREEN，不依赖最终大测试定位基础错误。
...
| Weight projection | Tianshu tests因 endpoint无 weight/decoded value ignored而 fail | `ServiceInstanceMeta.weight` → both endpoint records | no Tianshu wire change |
...
| Fixed-mode cache/lifecycle | resolver/strategy/manager/coordinator tests因统一 definition/gate不存在而 fail | one mode strategy + snapshot-only managers + lifecycle state | reuse Tianshu subscription, no new polling job |
...
| Step 4 | Step 3 | Step 2 | RPC endpoint/Tianshu directory files | Adds canonical weights to LB candidates |
...
| Mode Strategy + Factory | Create；Primary §7.0, §13 | current Yuheng/Provider channel providers分离但 proxy factories重复 | 单 handler switch会允许误查另一 Directory | 2 strategies, one factory, closeable demand | Implement exact immutable mode |
| Tianshu Consumer cache/job | Reuse；Primary §7.0 | `DdcManagedRegistrySubscription` 已 listener-before-pull、event coalesce、fixed-delay reconcile、expireLocal | 新 RPC polling/cache 是重复状态 | zero new Tianshu files/jobs | Already exists; managers consume snapshots only |
...
| Unified executor | Create；Primary §7.0, §7.3 | current JDK handler owns blocking Yuheng-only loop and idempotent gate | separate blocking/async/generic loops would drift | one state machine, cancellation/terminal races | Implement |
...
| Provider heartbeat scheduler | Reuse/clarify；Primary §10.3 | `RpcProviderLifecycle` already fixed-delay schedules `heartbeatAndRecover` | Tianshu active probe rejected | add state callbacks/validation, not a second scheduler | Modify existing owner |
...
| Yuheng/Tianshu/Proto/DB/frontend changes | Not applicable/Unchanged；Primary §3, §11, §12, §16 | effective implementations already provide required wire/discovery/security | changing them expands scope without need | high cross-module compatibility cost | No production change |

No fetch-then-forward API is added：generic caller submits a complete invocation command and directly consumes bytes；typed affinity comes from an application resolver and credentials remain framework-owned. No caller-supplied Tianshu credential, arbitrary Metadata, Provider host, second cache, mapper layer, relational entity, UI state or speculative streaming abstraction enters the Plan。
...
| Endpoint weights | `REQ-008`,`REQ-009`,`REQ-021` | Tianshu directory tests | Step 3 algorithms | neutral weighted endpoints | managers/LB | Step 4 |
...
│   │           │   ├── yuheng/RpcConsumerGatewayManagerTest.java     MODIFY [S6]
...
│   │   ├── main/java/top/egon/cola/component/rpc/tianshu/registry/
│   │   │   ├── DdcRpcProviderDirectory.java                           MODIFY [S4]
│   │   │   └── DdcRpcGatewayDirectory.java                            MODIFY [S4]
│   │   └── test/java/top/egon/cola/component/rpc/tianshu/registry/
...
Inventory ownership is exactly the `[S1]`–`[S12]` marker shown above. The only semantic-preserving addition to the Primary target tree is `GuardPlanValidator.java` and its test because current repository validation lives there; no Tianshu subscription implementation file is added.
...
| Endpoint/Tianshu MODIFY | exact endpoint interface/records, two Tianshu directories and two tests | decoded weight ignored → canonical weight propagated | Step 4 | `REQ-008`,`REQ-009`,`REQ-021` | Tianshu adapter tests |
...
| RPC core/adapters | `./mvnw -B -ntp -f egon-cola-components/pom.xml -pl :egon-cola-component-rpc-starter,:egon-cola-component-rpc-tianshu-adapter -am test` | exit 0 | module tests; no external Tianshu |
...
| Process IT | `TIANSHU_TEST_REDIS_HOST=127.0.0.1 TIANSHU_TEST_REDIS_PORT=6379 ./mvnw -B -ntp -f egon-cola-components/pom.xml -pl :egon-cola-component-rpc-test-contract -am -Pddc-live-test -Dit.test=RpcProcessIT verify` | user supplies single Redis and explicitly runs; exit 0 | separate JVM/live Tianshu boundary |
...
- Tianshu event is an invalidation hint; full snapshot/revision/lease is authority. Provider heartbeat is RPC-owned and configured by RPC Provider properties; Tianshu does not call Provider health probes.
...
- Yuheng production code, Proto files, Tianquan-Shoubing credential relay and relational/frontend surfaces are unchanged.
...
- Dependencies: None
- Baseline state: Yuheng annotation already has most policy fields, Direct has only target/group/version/timeout; enum lacks weighted-random/hash; managers hardcode Round Robin.
...
- Purpose: Lock defaults and bounds for retry/LB/hash/generic cache while retaining old yuheng attempts.
...
- Repository evidence: Yuheng `ProviderLoadBalancers` tests provide factory/state precedent; RPC uses AssertJ/JUnit/Mockito.
...
- Repository evidence: Yuheng annotation already carries timeout/retries/LB/group/version/fallback/fail strategy.
...
- After this file: Yuheng declaration can express typed affinity ownership.
...
- Purpose: Add the same retries/LB/fallback/fail/hash fields as Yuheng Reference.
...
- Contract/signature changes: additive elements with exactly Yuheng defaults; no mode/fallback-to-yuheng field.
...
- After this file: Direct and Yuheng expose the same call-site policy vocabulary.
...
- Error and edge behavior: explicitly excludes Tianshu, ManagedChannel creation and mode fallback.
- Implementation pseudocode:
...
- Repository evidence: Primary requires algorithms return candidates only; Yuheng precedent has a small strategy interface.
...
- Why now: Prevents algorithms from reaching proxy/Tianshu/security internals.
...
- Repository evidence: target Spec explicitly selects nested implementations; Yuheng `ProviderLoadBalancers` is the style precedent.
...
- Expected result: exit 0; property boundary and all algorithm tests pass; no Channel/Tianshu interaction occurs.
...
### Step 4 — Project canonical Tianshu weights into neutral RPC endpoints
...
- Observable outcome: Provider and Yuheng snapshots carry validated `yuheng.weight`, with missing/invalid remote values falling back to 100 and custom endpoint implementations remaining source compatible.
- End state: no Tianshu wire/key/subscription changes; LB receives the same weight semantics in both modes.
- Test-first gate: Required — Tianshu directory tests fail because endpoint accessors/default contract do not expose decoded weight.
- Ordered files:

#### File 1 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectoryTest.java`
...
- Repository evidence: current test captures Tianshu key and snapshot mapping with fake client.
...
- Input/output and state mapping: Tianshu instance metadata → decoded meta → Provider endpoint weight.
- Error and edge behavior: missing/invalid remote value maps 100; expired lease behavior unchanged.
- Implementation pseudocode:

```java
publish(instance(metadata("yuheng.weight", "80"))); assertThat(snapshot.endpoints().getFirst().weight()).isEqualTo(80);
publish(instance(Map.of())); assertThat(endpoint.weight()).isEqualTo(100);
publish(instance(metadata("yuheng.weight", "invalid"))); assertThat(endpoint.weight()).isEqualTo(100);
```
...
#### File 2 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcGatewayDirectoryTest.java`

- Purpose: Assert identical weight projection for Yuheng snapshots.
...
- Dependencies and consumers: Yuheng endpoint, Tianshu fake client.
- Why now: Weighted Yuheng Reference requires parity, not Provider-only weight.
- Contract/signature changes: endpoint gains weight record component.
- Input/output and state mapping: INTERNAL_GATEWAY instance metadata → Yuheng endpoint weight.
...
- Verification contribution: Primary `TEST-023` Yuheng half.
...
- Why now: Weighted algorithms must not depend on Tianshu-specific types.
...
- Dependencies and consumers: Tianshu adapter, manager/LB.
...
#### File 5 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/yuheng/RpcGatewayEndpoint.java`

- Purpose: Store Yuheng instance weight with the same compatibility rules.
- Symbols: weight component and old-shape constructor.
- Repository evidence: Yuheng record mirrors Provider endpoint and is constructed throughout tests/adapters.
- Dependencies and consumers: Tianshu Yuheng directory, manager/LB.
- Why now: Yuheng annotation exposes LB, so weights cannot be Direct-only.
- Contract/signature changes: additive component; legacy constructor defaults 100.
- Input/output and state mapping: decoded weight → immutable Yuheng candidate; channel key remains host/port/secure.
...
- Verification contribution: Yuheng directory and all LB mode parity tests.
- After this file: both endpoint kinds expose identical selection metadata.

#### File 6 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectory.java`
...
- Dependencies and consumers: Tianshu codec and new Provider constructor.
...
- Input/output and state mapping: Tianshu metadata map → codec → `meta.weight()` → endpoint.
...
- Verification contribution: File 1 GREEN and no Tianshu query change.
- After this file: Provider snapshots carry canonical weights.

#### File 7 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcGatewayDirectory.java`

- Purpose: Map the same decoded weight into Yuheng endpoints.
- Symbols: snapshot mapping lambda.
- Repository evidence: identical discarded decode call exists here.
- Dependencies and consumers: Tianshu codec and Yuheng constructor.
- Why now: Completes both-mode parity.
- Contract/signature changes: subscription/query unchanged.
- Input/output and state mapping: Tianshu instance metadata → typed meta weight → Yuheng endpoint.
...
- Expected result: exit 0; 80/missing/invalid cases map identically; exact Tianshu query/revision/lease assertions remain green.
- Failure returns to: Files 3–5 for endpoint compatibility; Files 6–7 for codec projection; Tianshu wire change returns to Spec.
- Completion criteria: both modes deliver 1–10000/default100 weights to neutral LB without new Tianshu files/jobs/proto.
- Rollback: revert endpoint/adapter paths together; old consumers resume effective weight 100; no data migration.
- Commit paths: egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectoryTest.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/test/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcGatewayDirectoryTest.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/channel/RpcEndpoint.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/provider/RpcProviderEndpoint.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/yuheng/RpcGatewayEndpoint.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcProviderDirectory.java egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/registry/DdcRpcGatewayDirectory.java
...
- Dependencies: Step 4
- Baseline state: Yuheng/Provider Managers eagerly create and own separate ManagedChannels per snapshot/query; Channel factory has no stable key overload.
...
- Contract/signature changes: additive overload only; existing endpoint/Yuheng overloads remain.
...
- Baseline state: BPP/factories pass annotations directly; Managers own eager channels and partial states; Yuheng/Direct selection has no common definition or Consumer-wide invocation gate.
...
- Symbols: Yuheng/Direct creation, missing Manager, retain/close/ref-count tests.
...
#### File 3 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/test/java/top/egon/cola/component/rpc/consumer/yuheng/RpcConsumerGatewayManagerTest.java`

- Purpose: Reframe Yuheng Manager as revisioned endpoint cache/demand owner with pool-backed compatibility.
...
- Dependencies and consumers: fake directory/clock/pool, Yuheng Manager.
...
- Verification contribution: Primary `TEST-010`,`014`,`015`,`070`,`071` Yuheng portions.
...
- Input/output and state mapping: exact query/ref count + Tianshu snapshot → one cached immutable endpoint list/revision.
...
- Error and edge behavior: Yuheng startup failure closes all opened resources; Direct empty means DEGRADED; repeated start/stop/callback idempotent; new invocation during drain rejected before Tianshu/pool.
- Implementation pseudocode:

```java
coordinator.start(); assertThat(events).containsExactly("pool.start","yuheng.start","provider.start","gate.open");
 ...
 * No API in this package may switch between Yuheng and Direct after definition creation. */
```
...
- Contract/signature changes: internal record with static Yuheng/Direct factories if useful.
...
- Error and edge behavior: Direct requires exact query; Yuheng forbids Provider query; no opposite-mode target field.
- Implementation pseudocode:
...
- Purpose: Bind one Yuheng definition to the Yuheng primary demand/cache only.
- Symbols: constructor/Strategy methods/atomic close.
- Repository evidence: Manager query is process-configured; Yuheng Reference never owns Direct identity.
- Dependencies and consumers: Yuheng Manager and definition.
- Why now: Concrete mode implementation for factory.
- Contract/signature changes: internal final class.
- Input/output and state mapping: creation retains Yuheng demand; candidates/revision delegate to Yuheng snapshot; close releases demand.
...
- After this file: Yuheng strategy has no cross-mode path.
...
- Error and edge behavior: empty/expired stays Direct-unavailable; no Yuheng Manager reference/import.
- Implementation pseudocode:
...
- Verification contribution: Direct exact-query/no-Yuheng tests.
- After this file: Direct strategy is fixed and closeable.

#### File 15 — `MODIFY egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter/src/main/java/top/egon/cola/component/rpc/consumer/yuheng/RpcConsumerGatewayManager.java`

- Purpose: Own Yuheng demand/subscription/revision/expiry, not primary LB or independent raw Channels.
...
- Dependencies and consumers: Yuheng Directory, clock/properties, optional pool compatibility, Strategy.
...
- Verification contribution: File 3 GREEN and existing Yuheng compatibility tests.
- After this file: Yuheng Manager is a cache/demand owner with pool-backed legacy access.
...
- Error and edge behavior: release final ref closes subscription/removes state/LB compatibility; expired endpoints excluded; no Yuheng calls.
- Implementation pseudocode:
...
- Error and edge behavior: explicitly excludes Tianshu polling and business retry.
- Implementation pseudocode:
 ...
 * It does not select endpoints, execute RPC attempts or create an additional Tianshu reconciliation job. */
```
...
- Dependencies: Step 6
- Baseline state: JDK handler owns a blocking-only idempotent/Yuheng-stage retry loop; no shared deadline/cancel/terminal context or nullable fail-strategy implementation exists.
...
- End state: executor accepts typed or raw method plans; business errors are terminal; Direct all-rate-limited maps `RPC_RATE_LIMITED`; Yuheng or mixed failures remain mode-specific unavailable.
...
- Symbols: Direct rate-limit, Yuheng stripped trailer, provider/yuheng stage, not-found and business status tests.
...
- Error and edge behavior: rate-limit requires UNAVAILABLE+provider+error type; absent type stays provider/yuheng unavailable; messages redact descriptions.
- Implementation pseudocode:
...
- Why now: Avoids boolean confusion between Direct/Yuheng and sync/async.
- Contract/signature changes: new internal enum.
...
- Dependencies and consumers: mapper/adapter/Yuheng existing allowlist behavior.
...
// Only trusted Provider/Yuheng code may emit safe enumerated values.
```
...
- Error and edge behavior: mixed/Yuheng remains existing unavailable; never used for business rejection.
- Implementation pseudocode:
...
- Error and edge behavior: absent/unknown type ignored; rate type on non-UNAVAILABLE does not override business mapping; Yuheng stripped trailer stays unavailable.
- Implementation pseudocode:
...
- Repository evidence: raw Yuheng marshaller precedent exists; executor/status tests provide fake Strategy/pool.
 ...
 * Targets are canonical Service/Method identities and fixed Yuheng/Direct modes; framework metadata remains controlled.
...
- Repository evidence: Yuheng raw byte marshaller uses InputStream bytes; Primary rejects a new wire service.
...
- Repository evidence: current record requires contract/method/request Message; external Tianquan-Shoubing factory consumes this API.
...
- Verification contribution: File 3 and existing Tianquan-Shoubing/RPC interceptor compatibility compile.
...
- Symbols: Yuheng/Direct/same-contract/double-annotation/missing dependency/hash/fallback tests.
...
```java
postProcess(beanWithGatewayAndDirectFields); assertCglib(bean.yuheng); assertCglib(bean.direct);
...
- Repository evidence: class asserts idempotent/Yuheng-stage retry behavior explicitly superseded by Primary and Step 7.
...
- Symbols: ApplicationContextRunner scenarios yuheng-only/direct-only/both/no reference/generic/lifecycle close.
...
- Repository evidence: current BPP duplicates Yuheng/Direct branches and directly registers Yuheng demand.
...
- After this file: Direct annotation uses the same proxy/runtime as Yuheng.
...
- Purpose: Remove JDK Proxy and obsolete idempotent/Yuheng-only retry loop.
- Symbols: delete class.
...
- Observable outcome: optional class-safe AutoConfiguration maps only Guard `RATE_LIMITED` to UNAVAILABLE/provider/rate-limit before business invocation; Provider publishes explicit states, RPC-owned heartbeat recovery and graceful drain; registration emits explicit/default `yuheng.weight`.
...
- Why now: `yuheng.weight` write side and lifecycle recovery need one tested owner.
...
```java
manager.prepare(binding(serviceWeight(80)),host,port); assertMetadata("yuheng.weight","80");
properties.metadata.put("yuheng.weight","60"); prepare(); assertMetadata("yuheng.weight","60");
...
- Input/output and state mapping: configured metadata wins; else contract annotation weight → `yuheng.weight`; prepared identities vs lease map → aggregate active.
...
```java
Map metadata=registrationMetadata(binding); metadata.putIfAbsent("yuheng.weight", Integer.toString(binding.contract().contractType().getAnnotation(EgonRpcService.class).weight()));
validateWeight(metadata.get("yuheng.weight")); registrations.put(service, registration(metadata));
...
- After this file: Provider state and Tianshu weight write authority are complete.
...
- Completion criteria: Guard-less apps remain unchanged, limited calls are UNAVAILABLE with safe trailers and target count 0, Provider states/heartbeat/drain are deterministic, and Tianshu registration weight is valid.
- Rollback: disable/remove Guard annotation/rules before reverting adapter; Provider artifact rollback preserves Tianshu lease schema; no database/key deletion.
...
- End state: all 75 Primary test IDs map to executable gates or explicit user-controlled live proof; no Proto/Yuheng/Tianshu/Flyway/frontend production file changes.
...
- Repository evidence: RPC README currently documents Yuheng/Direct/blocking baseline and properties.
...
```markdown
Show Yuheng and Direct annotations with identical policy fields and state that mode never changes after injection.
...
```markdown
镜像 Yuheng/Direct 固定模式、同模式换节点、Tianshu event+全量对账、五算法与权重/Hash resolver。
...
- Input/output and state mapping: String → EchoRequest → Stage response through Yuheng mode.
...
- Repository evidence: `RpcTcpCallTest` builds real Netty Server/ManagedChannel and Mock Yuheng; no InProcess transport.
- Dependencies and consumers: final AutoConfig/runtime, fixtures, in-memory registry, Mock Yuheng.
- Why now: Cross-module acceptance after all unit contracts.
- Contract/signature changes: test only.
- Input/output and state mapping: Provider/Yuheng/Consumer contexts + one endpoint key → typed/raw requests → same responses/invocation trace; pool factory count one.
- Error and edge behavior: 100 concurrent unary calls do not serialize; async returns before completion/cancel; all resources close; no direct Provider Channel for Yuheng fixture.
- Implementation pseudocode:
...
- After this file: local real TCP proves final runtime, not live Tianshu/Redis.
...
- Why now: Process isolation counters cannot be proven with the existing Yuheng-only client.
...
- Error and edge behavior: contains no Yuheng Reference field; retries/LB can be set explicitly for same-mode reselection.
- Implementation pseudocode:
...
- Verification contribution: Direct-only Process IT and zero Yuheng demand proof.
- After this file: a direct-only Consumer fixture exists without changing the Yuheng fixture.
...
- Repository evidence: `fixture.consumer.RpcTestConsumerApplication` owns the existing Yuheng one-shot pattern.
- Dependencies and consumers: Direct client, admission test configuration, Process harness.
- Why now: Package-local component scan prevents accidentally creating the Yuheng fixture/demand.
- Contract/signature changes: test-only main class.
- Input/output and state mapping: command-line Tianshu/RPC properties → context → Direct echo → structured provider/invocation/trace output → close.
...
- After this file: process harness can launch Direct without any Yuheng bean/demand.
...
- Repository evidence: current Failsafe IT starts Tianshu Admin/Provider/Mock Yuheng/Consumer JVMs with external Redis and unique namespace.
- Dependencies and consumers: user-supplied Redis, process apps/Mock Yuheng instrumentation.
...
- Error and edge behavior: Direct failure never increments Yuheng; Yuheng exhaustion never opens Direct channel; business error one attempt; child stdout retained on failure; namespace-only cleanup.
- Implementation pseudocode:
...
- Validation working directory: `/Users/mario/SelfProject/Egon-COLA`
- Verification command: default: `./mvnw -B -ntp -f egon-cola-components/pom.xml -pl :egon-cola-component-access-guard-starter,:egon-cola-component-rpc-starter,:egon-cola-component-rpc-tianshu-adapter,:egon-cola-component-rpc-test-contract -am test`; static: `git diff --check && ! rg -n "Consumer only supports blocking|generic invocation is not supported|JDK Dynamic Proxy|retry only.*idempotent|fallbackToGateway|fallback-to-yuheng|自动.*降级.*直连|自动.*改走.*Yuheng" egon-cola-components/egon-cola-component-rpc/README.md egon-cola-components/egon-cola-component-rpc/README.zh-CN.md`; live only by user: `TIANSHU_TEST_REDIS_HOST=127.0.0.1 TIANSHU_TEST_REDIS_PORT=6379 ./mvnw -B -ntp -f egon-cola-components/pom.xml -pl :egon-cola-component-rpc-test-contract -am -Pddc-live-test -Dit.test=RpcProcessIT verify`.
...
| RED/GREEN Step 4 | repository root | Step 4 Tianshu directory/LB selector | endpoint projection | exit 0; both endpoint modes map 80/default100 and old query tests pass | Step 4 endpoint/adapter | `REQ-008`,`REQ-009`,`REQ-021`; module/mock Tianshu |
...
| RED/GREEN Step 6 | repository root | Step 6 resolver/strategy/manager/coordinator selector | discovery/fixed mode/lifecycle | exit 0; opposite Manager calls zero, revisions/expiry/ref counts/state order pass | Step 6 owning file | `REQ-001`–`REQ-006`,`REQ-011`,`REQ-023`; module/mock Tianshu |
...
| Live process IT | repository root/user environment | `TIANSHU_TEST_REDIS_HOST=127.0.0.1 TIANSHU_TEST_REDIS_PORT=6379 ./mvnw -B -ntp -f egon-cola-components/pom.xml -pl :egon-cola-component-rpc-test-contract -am -Pddc-live-test -Dit.test=RpcProcessIT verify` | separate JVM/Tianshu/Redis/topology | user-run exit 0; no skipped profile; per-mode counters/reselection/drain pass | Step 12 harness or owning runtime Step | `REQ-003`–`REQ-007`,`REQ-010`,`REQ-011`,`REQ-019`,`REQ-020`; live boundary |
...
RED runs are recorded only while executing the Plan; a compile failure for a deliberately absent new type is acceptable only when it matches the Step's stated RED reason. A lost/terminated Maven handle, a skipped opt-in integration test, a static compile, or a mock snapshot is never reported as live Tianshu/Redis/gRPC process proof.
...
- Tianshu Registry：no key/topic/API/reconcile scheduler change. Existing event+full-pull/lease behavior is reused.
...
1. Merge/publish Access Guard algorithms and RPC Starter/Tianshu Adapter in one xingyuan component version after Steps 1–12 gates pass. Guard default remains TOKEN_BUCKET, so merely upgrading does not create new Redis key types.
2. Upgrade Provider applications first with retries irrelevant and Guard disabled/Token Bucket unchanged. Verify Provider state reaches READY only after all Tianshu leases and heartbeat interval is lower than lease duration.
...
7. Enable WEIGHTED_RANDOM/SWRR only after instance `yuheng.weight` distribution is verified; enable CONSISTENT_HASH only with a deterministic named resolver/explicit generic affinity key.
...
- Provider drain failure rollback is artifact restart; Tianshu TTL removes abandoned leases if deregistration did not finish.
- Yuheng/Tianshu/Tianquan-Shoubing/DB/frontend rollback is N/A because their production code/schema is unchanged.
...
Post-deploy: Provider/Consumer state transitions, Tianshu endpoint counts/revisions, Channel count/drain count, retry attempts by fixed mode, Guard decision/storage/backend failures, Direct pure-limit versus mixed error code, null fail-open handling, business duplicate audit. Production P99, Redis contention/eviction, Pub/Sub loss, HTTP/2 stream cap and Kubernetes/systemd signal grace remain user-operated runtime evidence.
...
| `REQ-002` | Primary §4, §9.2.1 | Steps 6,10,12 | Yuheng Strategy/Manager, BPP, Process IT/docs | `TEST-001`,`008`,`009`,`067`,`068` | Yuheng demand/opposite Direct counter zero |
| `REQ-003` | Primary §4, §9.2.2 | Steps 6,7,10,12 | Direct Strategy/Manager, executor, BPP, Process IT | `TEST-004`–`008`,`066` | exact Provider mode and Yuheng counter zero |
...
| `REQ-006` | Primary §4, §7.3.4 | Steps 4,6,12 | Tianshu directories/managers/process | `TEST-013`–`015` | old revision/expired lease excluded |
...
| `REQ-009` | Primary §4, §9.2.8, §10 | Steps 3,4,7,9,11,12 | resolver/LB/Tianshu endpoints/generic/lease/docs | `TEST-017`,`019`–`023`,`074` | weight parity and stable redacted affinity |
...
| `REQ-021` | Primary §4, §16 | Steps 4,8,9,11,12 | compatibility constructors/optional POM/test contract/static gate | `TEST-023`,`024`,`035`,`057`,`062`,`064` | no Proto/Tianshu/Yuheng/DB production change |
...
| `RISK-006` | Yuheng strips Provider error-type, so application code differs by mode | Steps 7,11 | current Yuheng safe trailer reconstruction | xingyuan owner | Closed by Primary contract: Direct pure limit=`RPC_RATE_LIMITED`, Yuheng=`RPC_PROVIDER_UNAVAILABLE` |
...
| `RISK-009` | Dirty worktree/concurrent changes may overlap future implementation | every Step | current staged Yuheng deletion/untracked files | executing agent/user | Recheck before every Step; path-limited commits; stop on overlap |
...
No unresolved public-behavior decision blocks Plan review. Runtime capacity, Docker availability, live Tianshu/Redis topology and business duplicate-safety are verification/deployment responsibilities, not permission to infer different behavior.
