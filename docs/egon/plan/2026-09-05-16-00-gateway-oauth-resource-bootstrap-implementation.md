...
| Field | Value |
| --- | --- |
| Document | `2026-09-05-16-00-gateway-oauth-resource-bootstrap-implementation.md` |
| Template Version | `4` |
| Status | `Ready` |
...
| Supersedes | `None` |
| Superseded By | `None` |
| Related Plans | [主计划](2026-09-02-21-03-gateway-dual-engine-separation-implementation.md) Step 11 的身份先决 |
...
Primary Spec 明确修改父 Spec 的 local IdP 范围限制。主计划原同 app/同版本分发结论不作为本计划验收，也不可用此先决完成关闭父任务；下一分发修订继续处理。
...
一个非空 path-limited commit：fix(idp): seed separate gateway resource identities。无自动 push/PR；父任务后续修复独立提交，不 amend 历史。
...
| MCP 服务 Grant | Primary §7 Keep/扩充成员 | RBAC3_SERVICE_CLIENTS/DDC_REGISTRATION_CLIENTS/MCP_TASK_SERVICE_CLIENT | 原数据驱动协调 | 同类 grant；新 ID 避免旧 PK | Implement |
...
| --- | --- | --- | --- | --- | --- | --- |
| 三身份种子+最小授权 | REQ-001/002/003/004 | IdpDevelopmentClientBootstrapTest | 既有 IdP admin test 依赖 | 可重复引导的 MCP 身份 | 后续脚本/DDC 目标修复 | Step 1 |
...
| Resource/Grant | IdP Entity 工厂与 Repository | createResource/reconcileResourceAndGrant/reconcileMcpTaskServiceGrants | 只缺种子成员与正确 ID 前缀 | 复用 | None | Step 1 / MC-DEP-001 |
...
### Step 1 — 补齐三个独立 Gateway Resource 身份及 MCP 最小授权
...
- Input/output and state mapping: Mock empty/existing resources/clients/grants → afterSingletonsInstantiated → captures；三身份表逐字段校验；每 tenant 断言 RBAC/Task grant；DDC tenant=null
...
verify(clients).create(argThat(c -> c.clientId().equals("gateway-mcp-engine-service")
...
// MCP DDC grant: PLATFORM/null tenant/exact registration scope;
...
- Standards impact: MC-BEAN-001/MC-SCOPE-001/MC-TEST-001；仅当前 IdP admin 生效
...
- Repository evidence: MACHINE_CLIENTS 中已有 gateway-engine-service；RESOURCES 已有 Admin/API；mcpTaskServiceGrantId 仅 tenant hash
...
- Input/output and state mapping: MCP Client=gateway-mcp-engine-service；Resource=identity-gateway-mcp-engine-default-local；URI=https://api.egon.internal/local/identity/gateway-mcp-engine-default；biz=identity/app=gateway-mcp-engine-default/env=local；RBAC/mock:read沿用 Spec §7；Admin/API 全部保留
...
MACHINE_CLIENTS = List.of(existingEntries,
    new MachineClientSpec("gateway-mcp-engine-service", "Gateway MCP Engine Local Service"));
RESOURCES = List.of(existingEntries,
    new ResourceSpec("identity-gateway-mcp-engine-default-local",
        "https://api.egon.internal/local/identity/gateway-mcp-engine-default",
        "identity", "gateway-mcp-engine-default", "Gateway MCP Engine Local",
        "gateway-mcp-engine-service", "mock-backend", "mock:read", null));
RBAC3_SERVICE_CLIENTS = List.of(existingEntries, "gateway-mcp-engine-service");
DDC_REGISTRATION_CLIENTS = List.of(existingEntries, "gateway-mcp-engine-service");
MCP_TASK_SERVICE_CLIENT = "gateway-mcp-engine-service";
...
Primary §11/16：没有 Flyway/数据库迁移；已有 local-enabled bootstrap 插入缺失种子。旧 Admin/API 与旧 Task grant 不删。新 MCP ID 避免与旧 Task ID 冲突。先代码/测试，后 Gateway fan-out/脚本，最后实际 cutover。回退代码不逆向删除数据。
