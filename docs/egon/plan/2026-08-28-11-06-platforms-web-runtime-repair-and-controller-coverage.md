# platforms Web 运行修复与 Controller 覆盖补齐实施计划
...
| Scope | Portal Wujie、shared 导航、IDP/RBAC3/Gateway/DDC Admin Web、DDC Admin 查询/安全、本地 unified-platform 地址策略 |
...
| Implements Spec | [platforms Web 运行修复与 Controller 覆盖补齐规格](../spec/2026-08-28-11-06-platforms-web-runtime-repair-and-controller-coverage.md) |
| Spec Status | `Accepted` |
| Spec Revision | `2026-08-28 11:06 CST` |
| Effective Specs | [platforms Web 运行修复与 Controller 覆盖补齐规格](../spec/2026-08-28-11-06-platforms-web-runtime-repair-and-controller-coverage.md) |
| Depends On Plans | None |
| Supersedes | [前序平台 Web 实施计划](2026-08-27-11-13-platforms-web-enterprise-implementation.md) §1-§12，仅就本次运行修复和覆盖范围取代 |
| Superseded By | None |
| Related Plans | [前序平台 Web 实施计划](2026-08-27-11-13-platforms-web-enterprise-implementation.md) |

## 1. Summary

本 Plan 实施已接受的运行修复 Spec，按 8 个可回滚 Step 执行：shared/Gateway 防崩、地址策略、Wujie Core Facade、DDC 后端查询/安全、DDC 绑定页面、RBAC3 IAM 覆盖、Gateway 覆盖、最终审计。每个 Step 先写或扩展 RED，再做最小 GREEN，完成定向验证后只提交声明的路径。当前已有服务不由本 Plan 自动启动或停止。

## 2. Target Spec and Effective Design

### 2.1 Primary target

- Path: `docs/egon/spec/2026-08-28-11-06-platforms-web-runtime-repair-and-controller-coverage.md`
- Status: `Accepted`
...
| --- | --- | --- | --- | --- |
| Primary | [运行修复规格](../spec/2026-08-28-11-06-platforms-web-runtime-repair-and-controller-coverage.md) | Accepted / 2026-08-28 11:06 | §3-§20 | 本次代码、测试和回滚边界 |
| Amendment | [前序企业级规格](../spec/2026-08-27-08-09-platforms-web-enterprise-microfrontend.md) | Review / 2026-08-27 11:44 | §12 的既有逐页 UI 基线 | 保留原有菜单和页面设计，不重复发明平台壳 |
| Dependency | [DDC 分页设计](../../superpowers/specs/2026-08-10-ddc-admin-pagination-ui-modernization-design.md) | predecessor design | §1-§10 | 保留 PageResultRecord、兼容和不改数据库约束 |

### 2.3 Superseded or excluded content

前序 Plan 中“本轮只做文档、不实现 Wujie/后端/地址”的范围被本次 Accepted Spec 的 §16 取代；前序 Plan 的共享 Layout、独立子应用、同源认证和页面文字设计继续有效。Gateway active release 的数据库发布、internal/RPC API、Flyway 历史修改均排除。
...
| REQ-003 | §4, §7 | shared/Gateway 父菜单无 path 不崩溃 | Gateway `/dashboard` 首屏和 shared 深链测试成功 | sidebar guard/test |
| REQ-004 | §4, §7, §12 | 已有 external Controller 功能有 API 和页面动作/详情 | RBAC3、Gateway、DDC 覆盖路径可调用；IDP 覆盖不重复实现 | API/page/route additions |
| REQ-005 | §4, §7 | DDC page 读权限和空筛选查询可用 | DDC_READ 可访问 `/page`；PostgreSQL 不再报 lower(bytea) | Security matcher、JPQL、Maven tests |
| REQ-006 | §4, §12 | IAM 保持一个顶层树及子菜单 | 新增权限等入口位于 IAM 子树 | RBAC3 resource definitions/navigation |
...
先修共享崩溃，因为所有 Web 都依赖它；随后修启动地址，保证后续运行验证使用稳定 origin；再替换 Portal 的 Wujie wrapper 调用，以便后续 child 页面可以真实显示。DDC 后端先于 DDC 页面，RBAC3/Gateway 页面只消费已有 Controller 合同；最后运行静态覆盖审计、四 Web typecheck/build、shared build 和 DDC focused Maven 测试。数据库没有迁移步骤。
...
| DDC page authorization/query | security/controller/repository test exercises `/page` and empty params | exact GET matchers + string-safe JPQL predicate |
| binding UI | page test initially has no route/API component | binding page API/query/mutations/menu |
| RBAC3 role/permission/user coverage | API/page tests assert missing method/path/action | API methods, route descriptors, forms/actions |
| Gateway OpenAPI/application details | page/API tests assert existing controller paths are not orphaned | detail drawer and sync page |

### 4.3 Sequential and parallel boundaries

| Step | Depends on | May run in parallel with | Must not overlap with | Reason |
| --- | --- | --- | --- | --- |
| Step 1 | None | None | shared layout files | Gateway cannot render until shared guard exists |
...
| Step 4 | Step 1 | Step 5 after backend test lock | DDC Java files | page errors need backend boundary fixed |
| Step 5 | Step 4 | None | DDC Web layout/pages | binding page uses corrected page/auth behavior |
| Step 6 | Step 1 | Step 7 | RBAC3 Web feature files | IAM routes/API additions must remain coherent |
| Step 7 | Step 1 | Step 6 | Gateway Web feature files | Gateway details consume existing shared guard |
...
| DDC binding page | Spec §12.6; external binding Controller | `DdcNamespaceEnvAppBindingController` | hidden Namespace drawer only | one page query and mutations | Implement |
| RBAC3 permission route | Spec §12.4; permission Controller already external | `PermissionController`, hidden JSON route | leave hidden | one route/component | Implement |
| Gateway OpenAPI sync page | Spec §12.5; OpenAPI Controller and API already exist | `gatewayApi.openapiSyncStates` | only Application expandable rows | one route/page query | Implement |
...
| --- | --- | --- | --- | --- | --- | --- |
| Shared path safety | REQ-003 | undefined candidate test | shared deps | safe selection | Gateway layout | Step 1 |
...
| DDC page security/query | REQ-005, REQ-009 | matcher/JPA tests | PostgreSQL test profile | usable page API | DDC pages | Step 4 |
| DDC binding route | REQ-004, REQ-007 | page query/mutation test | Step 4 API | `/bindings` page | DDC menu | Step 5 |
| RBAC3 IAM additions | REQ-004, REQ-006, REQ-007 | API/path/page tests | existing resource registry | permission/role/user actions | IAM tree | Step 6 |
| Gateway detail/sync | REQ-004, REQ-007 | API/page tests | existing gateway client | OpenAPI/application views | Gateway menu | Step 7 |
...
| --- | --- | --- | --- | --- |
| Architecture profile | DDC `controller/service/repository`; RBAC3 existing feature-first tree | Spec §6.1 selects traditional three-layer semantics for touched DDC files | no new layer/package; only existing Security config and Repository query edits | Step 4; MC-ARCH-001, MC-BLOCKER-001 |
...
| Naming/model/validation/conversion | DDC query only; no new Java model | no new POJO/DTO/Converter; preserve existing Controller validation | no model or hand-copy changes | Step 4; MC-NAME-001, MC-MODEL-001, MC-VALID-001, MC-CONVERT-001 |
...
| Page auth | DDC SecurityFilterChain | DDC SecurityConfiguration | matcher gap only | modify existing config | none | Step 4; MC-DEP-001 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Rule 1 | Spec §6.2/§8 | no Java type created | inspect existing DDC repositories/config only | no new carrier | `rg` type inventory + compile | Step 4 | PASS |
| Rule 2 | Spec §6.2/§7 | DDC page path/query handoff | Security config -> existing Service -> Repository | preserve existing validation; query string normalization semantics | focused controller/JPA tests | Step 4 | PASS |
...
| Rule 11 | Spec §6.1/§6.2/§8 | DDC current three-layer tree | Security config and Repository stay in place | no third/hybrid tree | architecture tree + Maven | Every Step | PASS |
...
MODIFY RBAC3 role/directory/application/navigation/resource definition files
...
Wujie Core version remains 2.1.0; existing public API paths, permission checks, JSON envelope, child lifecycle names and standalone entry paths remain compatible. No new database migration, no BFF, no internal API exposure, no automatic active Gateway release publication, and no automatic process start/stop.

### 6.4 Plan Clarifications

| ID | Small implementation inference | Repository evidence | Why semantics are unchanged | Impact if wrong |
| --- | --- | --- | --- | --- |
| PLAN-CLAR-001 | DDC repository predicates may use `coalesce(:param, '') = ''` to retain null-as-unfiltered semantics | Ddc*Repository JPQL and PostgreSQL failure | only parameter typing changes; result predicate remains unfiltered for null/blank | focused JPA test blocks Step if provider rejects syntax |
...
| PLAN-CLAR-003 | IDP needs no duplicate management page because its external admin Controller coverage is already present | IDP App/router/features and Controller inventory | final audit records IDP as covered | future uncovered IDP endpoint is reported, not silently invented |

## 7. Ordered File-by-file Implementation Steps

### Step 1 — Make shared path selection safe and restore Gateway first render
...
- Repository evidence: `EnterpriseNavigationItem.path?` and Gateway `filterNavigation` return parent entries without `path`.
- Dependencies and consumers: `EnterpriseLayout`, all four Admin Web sidebars, React Router pathname.
- Why now: the Gateway error occurs before useful page content is rendered.
...
- Contract/signature changes: `UNIFIED_PLATFORM_ADVERTISED_HOST` remains the override; default changes to `127.0.0.1`.
...
resolve_local_advertised_host() {
  ...
  [[ "${candidate}" != *[[:space:]/\\]* ]] || unified_platform_fail "invalid advertised host"
  printf '%s' "${candidate}"
}
```
...
- Dependencies and consumers: generated DDC/IDP/RBAC3/Gateway env files and Gateway allowed CIDR.
...
- Dependencies: `Step 1`
- Baseline state: DDC `/page` paths fall through `anyRequest().denyAll`; empty optional filters produce PostgreSQL `lower(bytea)`.
- Observable outcome: existing DDC page Controller paths are readable with DDC_READ and empty filters retain unfiltered query semantics.
- End state: one DDC backend commit with no schema/migration change.
- Validation working directory: repository root for the Maven reactor and the DDC Admin module for focused tests.
...
- Dependencies and consumers: DDC capability authorities and existing bearer filters.
...
- Input/output and state mapping: HTTP method/path + DDC_READ/ALL -> allow or existing deny handler.
...
.requestMatchers(HttpMethod.GET,
    ...
    .hasAnyAuthority(DDC_READ, DDC_ALL)
```
...
- Repository evidence: binding page has four nullable `is null` filters and will be the new DDC page consumer.
...
- Repository evidence: existing security integration test already uses MockMvc, DDC capability authorities and fake read-only handlers.
- Dependencies and consumers: DdcAdminSecurityConfiguration and the existing DDC controllers.
...
void protectsPagedManagementReadsWithReadCapability() throws Exception {
    mockMvc.perform(get("/api/v1/ddc/configs/page").with(authority("CAP_DDC_READ")))
            .andExpect(status().isOk());
    mockMvc.perform(get("/api/v1/ddc/apps/page").with(authority("CAP_DDC_WRITE")))
...
- Repository evidence: current DDC layout already groups biz/env/app/namespace under `metadata`.
...
- Literal rule enforcement: `Rule 5, Rule 11` — use existing shared menu and preserve DDC tree.
- Implementation pseudocode:
...
- Repository evidence: all current DDC pages are nested under one authenticated route.
...
- Repository evidence: DDC pages use `usePageState`, `ddcPageApi`, `ddcApi`, `PageState`, and Ant Design Table/Modal.
...
- Contract/signature changes: UI only; paths are `/api/v1/ddc/namespace-env-app-bindings` and `/page`.
...
```tsx
const query = useQuery({ queryKey: ['ddc', 'bindings', submitted, page.page],
  queryFn: ({ signal }) => ddcPageApi<DdcNamespaceEnvAppBinding>(`/api/v1/ddc/namespace-env-app-bindings/page?${buildQuery(submitted, page)}`, { signal }) })
const save = useMutation({ mutationFn: ({ id, values }) => id
  ? ddcApi(`/api/v1/ddc/namespace-env-app-bindings/${id}`, { method: 'PUT', body: values })
  : ddcApi('/api/v1/ddc/namespace-env-app-bindings', { method: 'POST', body: values }) })
```
...
- Repository evidence: existing DDC page tests use Testing Library and mocked `fetch`/client responses.
- Dependencies and consumers: BindingsPage and DDC page test setup.
...
- Repository evidence: RoleController exposes POST/PUT/POST inheritance/DELETE inheritance under `/api/rbac3/v1/iam/roles`.
...
```ts
create: (command: CreateRoleCommand) => client.request<RoleMutationResult>('/api/rbac3/v1/iam/roles', { method: 'POST', body: command })
update: (roleId: string, command: UpdateRoleCommand) => client.request<RoleMutationResult>(`/api/rbac3/v1/iam/roles/${roleId}`, { method: 'PUT', body: command })
addInheritance: (roleId: string, command: InheritanceCommand) => client.request<RoleImpactView>(`/api/rbac3/v1/iam/roles/${roleId}/inheritances`, { method: 'POST', body: command })
removeInheritance: (roleId: string, juniorRoleId: string, query: object) => client.request<RoleImpactView>(`/api/rbac3/v1/iam/roles/${roleId}/inheritances/${juniorRoleId}`, { method: 'DELETE', query })
```
...
```ts
users: (query, status, page, size) => client.request<DirectoryPage<UserDirectoryView>>('/api/rbac3/v1/iam/users', { query: { query, status, page, size } })
createUser: (command) => client.request<UserDirectoryView>('/api/rbac3/v1/iam/users', { method: 'POST', body: command })
changeUserStatus: (userId, command) => client.request<UserDirectoryView>(`/api/rbac3/v1/iam/users/${userId}/status`, { method: 'PUT', body: command })
```
...
- Contract/signature changes: page query uses `/api/rbac3/v1/iam/users`; current detail path remains available.
...
```tsx
const users = useQuery({ queryKey: ['rbac3', 'users', filters, page], queryFn: () => api.users(filters, page) })
...
{ "kind": "ROUTE", "code": "iam.permissions", "name": "权限", "parentCode": "iam.catalog",
  "path": "/iam/permissions", "componentKey": "rbac3-permissions", "hidden": false,
...
{ key: 'permissions', path: '/iam/permissions', title: '权限', permission: 'system:permission:read',
  componentKey: 'rbac3-permissions', component: PermissionCatalogPage, navigationOrder: 43 }
```
...
```tsx
const query = useQuery({ queryKey: ['rbac3', 'permissions', tenant, applicationId], queryFn: () => api.permissions(applicationId) })
...
- Repository evidence: RBAC3 feature tests use mocked Rbac3Provider/FeatureApiProvider and Testing Library.
...
it('renders permissions and calls the IAM permission controller paths', async () => {
  ...
  expect(request).toHaveBeenCalledWith('/api/rbac3/v1/iam/permissions', expect.anything())
})
```
...
- Contract/signature changes: GET `/api/v1/gateway/admin/applications/{id}` only; no backend change.
...
- Literal rule enforcement: `Rule 1, Rule 6, Rule 11` — semantic method/type use, exact JSON, existing gateway API tree.
- Implementation pseudocode:
...
- Literal rule enforcement: `Rule 5, Rule 6, Rule 11` — reuse existing query/UI/client and gateway tree.
- Implementation pseudocode:
...
- Repository evidence: Gateway governance already groups groups/applications/interface catalog/providers.
- Dependencies and consumers: shared sidebar and App route.
- Why now: dedicated page needs a discoverable route.
- Contract/signature changes: add leaf `/openapi-sync` under `gateway-governance`.
- Input/output and state mapping: menu item -> route.
- Error and edge behavior: capability remains `gateway:read`; no new permission bypass.
- Standards impact: `MC-REUSE-001, MC-SCOPE-001`; no Java impact.
- Literal rule enforcement: `Rule 5, Rule 11` — preserve Gateway navigation structure and shared boundary.
- Implementation pseudocode:

```tsx
{ key: '/openapi-sync', path: '/openapi-sync', icon: <ApiOutlined />, label: 'OpenAPI 同步', capability: 'gateway:read' }
// The capability matches the existing Gateway governance read permission.
...
- Repository evidence: App uses lazy page imports and `RequireCapability` around Gateway routes.
- Dependencies and consumers: OpenApiSyncPage, Gateway AdminLayout.
- Why now: menu link requires a route component.
- Contract/signature changes: route `/openapi-sync` only.
- Input/output and state mapping: route -> page; `gateway:read` guard.
...
{ path: 'openapi-sync', element: <RequireCapability capability="gateway:read"><OpenApiSyncPage /></RequireCapability> }
// Lazy loading preserves the existing Gateway bundle boundary and fallback.
...
- Repository evidence: Gateway feature tests use mocked `gatewayApi` and Testing Library.
...
- Baseline state: route coverage is distributed across four Web trees and active Gateway release can lag behind source draft.
...
#### File 2 — `MODIFY docs/egon/spec/2026-08-28-11-06-platforms-web-runtime-repair-and-controller-coverage.md`
...
| DDC Java | focused Maven compile/test | 0; no migration diff | Step 4 stays open |
| DDC Web | typecheck/test/build | 0; binding route green | Step 5 stays open |
| RBAC3 | typecheck/test/build | 0; IAM route/API tests green | Step 6 stays open |
| Gateway | typecheck/test/build | 0; OpenAPI/detail tests green | Step 7 stays open |
...
| REQ-004 | Steps 5-7 | DDC binding/RBAC3 permission/role/Gateway OpenAPI | Web focused tests |
| REQ-005 | Step 4 | DDC Security + repositories | Maven security/JPA |
| REQ-006 | Step 5-7 | IAM resource definitions, DDC/Gateway nested navigation | navigation tests |
...
| active Gateway release drift | local stack uses skip-release mode and active revision can be older | do not publish automatically; report separately | Controlled |
| PostgreSQL coalesce inference | current `lower(bytea)` log | focused JPA test is blocking | Controlled |
| IDP external management coverage | current IDP pages consume main admin Controller routes | no duplicate page; final audit verifies | Resolved |
...
Accept only when applicable gates pass, Portal Wujie and direct children are separately verified, DDC page/query defects are fixed, and any active release/runtime limitation is clearly labeled.
...
Typecheck/build/unit tests are static/component evidence. Existing process/browser checks, if run after code changes, are runtime evidence; a green static gate never substitutes for live Wujie mount or Gateway release publication.
...
| MC-JSON-001 | Applicable | PASS | DDC/Gateway/RBAC3 pages keep existing JSON envelopes | no invented response wrapper | None |
