# Tianquan-Shoubing OAuth2 Client、租户权威与 Tianshu SERVICE Token 迁移实施计划

| Field              | Value                                                                                                                                                                                                                                                                                 |
|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Document           | `2026-08-21-14-14-tianquan-shoubing-oauth-client-tenant-migration.md`                                                                                                                                                                                                                               |
| Template Version   | `2`                                                                                                                                                                                                                                                                                   |
| Status             | `Review`                                                                                                                                                                                                                                                                              |
| Created            | `2026-08-21 14:14 CST`                                                                                                                                                                                                                                                                |
| Updated            | `2026-08-22 12:53 CST`                                                                                                                                                                                                                                                                |
| Owner              | `Mario / Egon-COLA xingyuan maintainers`                                                                                                                                                                                                                                              |
| Repository         | `Egon-COLA`                                                                                                                                                                                                                                                                           |
| Scope              | `egon-cola-tianquan-shoubing、egon-cola-tianquan-jianshen、egon-cola-tianshu、Tianquan-Shoubing/Tianquan-Jianshen Admin Web 与统一身份维护脚本`                                                                                                                                                              |
| Source Requirement | `业务服务改用 Spring Security OAuth2 Client + AppID/App Key/Secret；凭证由 Tianquan-Shoubing Web 预配并入库；tenant 权威迁入 Tianquan-Shoubing；Tianshu 直接验证定向 PLATFORM SERVICE Token；用户确认 1A/2A/3A 以及权限、membership port、bootstrap 三项修订`                                                                                                   |
...
| Implements Spec    | [Tianquan-Shoubing OAuth2 Client、租户权威与 Tianshu SERVICE Token 准入迁移规格](../spec/2026-08-21-07-51-tianquan-shoubing-oauth-client-tenant-ownership.md)                                                                                                                                                                     |
| Spec Status        | `Review`                                                                                                                                                                                                                                                                              |
| Spec Revision      | `Updated 2026-08-21 22:04 CST；baseline main@2bb9262400fae3ed60c88ac5032e4bbc6af65542`                                                                                                                                                                                                 |
| Effective Specs    | [主规格](../spec/2026-08-21-07-51-tianquan-shoubing-oauth-client-tenant-ownership.md)；[统一身份平台设计](../../superpowers/specs/2026-08-01-unified-identity-xingyuan-design.md)；[无 Session JWT 与 Yuheng 自动刷新规格](../../superpowers/specs/2026-08-13-unified-identity-stateless-jwt-session-removal-design.md) |
...
本 Plan 实现唯一主规格，并保留两份规范依赖中未被主规格取代的 USER JWT、Yuheng、Resource Server 与 PEP 行为。实施分为 18
个语义步骤：先建立 Tianquan-Shoubing V5 与 RBAC V8 的可重复迁移证据，再按编译依赖完成 SERVICE principal/context、Client Secret、Token
Endpoint、Spring OAuth2 Client facade、Tianquan-Shoubing tenant/membership、Identity Directory RPC、RBAC authorization
state/主体校验/bootstrap、Tianshu registration token、两套 Admin Web 和维护窗口工具。
...
tenant 符号归零，以及需由用户控制环境执行的 PostgreSQL/Redis/Tianshu/Yuheng 联调与恢复演练。Plan 本身不执行代码、migration、服务或浏览器。

## 2. Target Spec and Effective Design

### 2.1 Primary target

- Path: [docs/egon/spec/2026-08-21-07-51-tianquan-shoubing-oauth-client-tenant-ownership.md](../spec/2026-08-21-07-51-tianquan-shoubing-oauth-client-tenant-ownership.md)
- Status: `Review`
...
|----------------------|-------------------------------------------------------------------------------------------------------------------------------|-------------------------------|-----------------------------------------------------------------------------------------------------------|------------------------------------------------|
| Primary              | [Tianquan-Shoubing OAuth2 Client、租户权威与 Tianshu SERVICE Token 准入迁移规格](../spec/2026-08-21-07-51-tianquan-shoubing-oauth-client-tenant-ownership.md)             | `Review`；2026-08-21 22:04 CST | 全文，尤其 `REQ-001`–`REQ-021`、`DEC-001`–`DEC-008`、§7–§16                                                      | 本次实现的唯一主目标和最新机器身份/tenant/Tianshu 权威                |
| Normative dependency | [统一身份平台设计](../../superpowers/specs/2026-08-01-unified-identity-xingyuan-design.md)                                            | 已批准，2026-08-02 进入实施           | §8.1、§9.1、§10.1、§11、§12–§13 中未被主规格/2026-08-13 取代的 Identity User、USER Token、Resource Server、Yuheng/PEP 边界 | 保留 USER/Resource/PEP 基础模型，不恢复其旧 tenant 权威规则    |
| Normative dependency | [无 Session JWT 与 Yuheng 自动刷新规格](../../superpowers/specs/2026-08-13-unified-identity-stateless-jwt-session-removal-design.md) | 已确认且核心实现完成；2026-08-14         | §6–§11、§13–§15 中未被主规格取代的 USER JWT、Cookie、Yuheng、无 Session、Signing Key 行为                                 | 保护 `REQ-017` 的 USER/Auth Code/Signing JWK 回归边界 |
...
- 2026-08-01 设计中“Tianquan-Shoubing 不保存 tenant membership、Tianquan-Jianshen 是 tenant/membership 权威”的规则被主规格
  `DEC-002/DEC-007/DEC-008` 替换。
- 2026-08-13 中仅为兼容旧 SERVICE 凭证/Admission/RBAC tenant 权威而保留的段落被主规格取代；USER JWT、Yuheng refresh、无
  Session 与 Tianquan-Shoubing Signing Key 继续有效。
...
|-------------|---------------------|------------------------------------------------------------------------------|--------------------------------------------------------------------------|-----------------------------------------|
| `REQ-001`   | 主规格 §4              | Confidential Client 仅由 Tianquan-Shoubing 管理员预配，应用不得自注册                                     | 无动态注册；SERVICE Client 不能管理自身；审计含 operator                                 | Tianquan-Shoubing backend/Web/tests                   |
| `REQ-002`   | 主规格 §4              | appId=业务应用身份、App Key=client_id、Secret=client_secret                          | UI/配置/claim/数据库映射一致，appId active client 唯一                               | Tianquan-Shoubing V5、Client/Token/Web                 |
...
| `REQ-005`   | 主规格 §4              | tianquan-shoubing-starter 使用 Spring Security OAuth2 Client                                 | `ClientRegistration`/`OAuth2AuthorizedClientManager` 获取/续期 Token，无签名私钥代码 | starter POM/autoconfig/facade           |
...
| `REQ-007`   | 主规格 §4              | SERVICE Token 含 app/source/credential/context claims                         | 签发/解码字段完整；缺失或矛盾 fail closed                                              | core claim/signer/verifier/Tianshu          |
| `REQ-008`   | 主规格 §4              | TENANT 必有 tid；PLATFORM 无 tid 且由 Grant 派生                                     | caller 不能自报 context；`tid=*`/混合状态拒绝                                       | Grant/policy/token/principal            |
| `REQ-009`   | 主规格 §4              | Tianshu 直接验证 Tianshu audience PLATFORM SERVICE Token                                 | 无 Ticket/RPC；错误 audience/context/scope/source/credential 拒绝              | Tianshu client/admin                        |
| `REQ-010`   | 主规格 §4              | Tianshu 将 app/client 与 biz/app/env/instance/lease 绑定，lease≤Token exp             | cross-binding replay 零写；heartbeat 精确匹配；expiry 上界成立                       | Tianshu verifier/Redis/service              |
| `REQ-011`   | 主规格 §4              | Tianquan-Shoubing 是 tenant catalog/membership 唯一权威                                         | login/resolve 只读 Tianquan-Shoubing DB；无 Tianquan-Shoubing→RBAC membership HTTP                       | Tianquan-Shoubing V5/tenant/local port                |
...
| `REQ-013`   | 主规格 §4              | RBAC 创建/启用主体前验证 Tianquan-Shoubing ACTIVE membership                                        | missing/disabled/timeout 零写；RPC fail closed                              | Directory RPC/RBAC adapter              |
...
| `REQ-015`   | 主规格 §4              | 历史 Flyway immutable；Tianquan-Shoubing 仅 V5、RBAC 仅 V8、Tianshu 无 migration                       | checksum 不变；每 DB 恰一新版本                                                   | two migration Steps/static gate         |
| `REQ-016`   | 主规格 §4              | Tianquan-Shoubing Web 管理 Secret/tenant/member，RBAC Web 移除 tenant page                      | 权限、表单、确认和完整 UI states；Secret 不进存储/URL                                    | frontend Steps                          |
| `REQ-017`   | 主规格 §4              | USER OAuth/session/Signing Key 行为不变                                          | Authorization Code+PKCE、USER JWT/refresh、SigningKeyPage regression       | core/admin/yuheng/frontend regression  |
...
| `REQ-019`   | 主规格 §4/`DEC-006`    | 保留 `tianquan-shoubing:oauth-client:read/create/update`                                     | backend/Web/permission seed 完全一致，无 `tianquan-shoubing:client:*`                        | Client API/Web/RBAC seed                |
| `REQ-020`   | 主规格 §4/`DEC-007`    | membership port 不含 rbac3UserId，删除旧未接线 USER Resource policy/port              | reduced record 编译；旧 symbol/test 归零                                       | Tianquan-Shoubing core/admin tests                    |
...
1. 先固化 Tianquan-Shoubing V5 schema contract，因为 Client Secret、Grant context 与 Tianquan-Shoubing tenant entity 都依赖它；V5 只在 rehearsal
   中执行，执行中间提交不得部署。
2. 先扩展 Tianquan-Shoubing core claim/principal 与删除过期 USER Resource policy，再让 admin Token Endpoint、starter、Yuheng/Tianshu 消费同一
   `ServiceTokenContext`。
3. Client Secret 管理先于 Token Endpoint；Token Endpoint 先于 starter；starter 先于 Tianshu registration runtime，形成单向
   `Tianshu registration starter -> tianquan-shoubing-starter`，并移除旧反向 admission 依赖。
4. Tianquan-Shoubing tenant backend 先于 Identity Directory RPC；RPC contract/provider 先于 RBAC membership gate/bootstrap。
5. RBAC V8 schema/state 先于 policyVersion consumer refactor；所有 tenant master code/HTTP/Web 删除在 V8 gate 可证明且
   RPC gate存在后进行。
6. Tianshu transport/client 改为 registrationToken 后，admin verifier/lease 才能接收新可信 identity；两者作为同一发布单元部署但保持两个可验证提交。
7. Backend contracts 完成后再改 Tianquan-Shoubing/RBAC Web；最后增加维护脚本、runbook 与全局删除/兼容门禁。

### 4.2 Test-first strategy

| Behavior               | RED test                                     | Expected RED reason                               | Minimum GREEN                           |
|------------------------|----------------------------------------------|---------------------------------------------------|-----------------------------------------|
| Tianquan-Shoubing V5                 | `IdpClientTenantV5MigrationTest`             | V5 resource/table/constraint/old-table-drop 缺失    | 单个 V5 SQL                               |
...
| tenant/membership      | service/controller/local-port tests          | package/table/read model不存在                       | Tianquan-Shoubing feature package                     |
...
| Tianshu Token registration | runtime/verifier/lease tests                 | Ticket supplier/claims仍是输入                        | registrationToken + verified identity   |
...
|------|----------------------------|----------------------------------|-------------------------------------|----------------------------------|
| 1    | None                       | None                             | Tianquan-Shoubing historical migrations           | 单一 V5 owner                      |
| 2    | Step 1 contract understood | None                             | Tianquan-Shoubing core claim files                | shared public model              |
| 3    | Step 1                     | None                             | Tianquan-Shoubing oauth Client aggregate          | Secret transaction owner         |
| 4    | Steps 2–3                  | None                             | Token/OAuth/JWK files               | authentication cutover atomicity |
| 5    | Step 4                     | Step 6 only after path isolation | tianquan-shoubing-starter POM/autoconfig          | starter public beans             |
| 6    | Step 1                     | Step 5                           | Tianquan-Shoubing tenant persistence/service      | local DB owner                   |
...
| 13   | Step 5                     | None                             | Tianshu starter/http runtime            | dependency inversion             |
| 14   | Steps 2,13                 | None                             | Tianshu admin security/lease            | trusted identity rename          |
| 15   | Steps 3–4                  | Step 17                          | Tianquan-Shoubing Client Web files                | backend contract fixed           |
| 16   | Step 7                     | Step 15                          | Tianquan-Shoubing tenant Web files                | backend contract fixed           |
...
少量共享文件按不重叠symbol分步修改：`OAuthConfig` 在Step 4只处理Token/credential beans、Step 7只处理membership bean；Tianquan-Shoubing
...
| Tianquan-Shoubing tenant/member tables/API                  | Add；§7/§9/§11                  | Tianquan-Shoubing 反向调用 RBAC membership                                        | 保留 RBAC owner违反 2A                                     | two tables, API/page/RPC           | Implement                                      |
...
| Directory RPC membership method               | Expand；§9 RPC-001              | existing Tianquan-Shoubing→RBAC directory contract/provider/consumer          | 新 HTTP 或 RBAC copy都会增加协议/双权威                           | additive RPC method/deadline       | Implement on existing service/version          |
...
| Tianshu Admission Ticket                          | Remove；§7.0/`DEC-003`          | Ticket repeats app/source/scope/expiry and no workload identity | Tianshu audience PLATFORM Token derives same facts         | fewer RPC/cache states             | Delete；no compatibility adapter                |
| Tianshu registration Token verifier               | Modify/rename；§9 INTERNAL-002  | existing JWT/source/lease checks reusable                       | new second verifier duplicates logic                   | Java semantic rename + claim rules | Implement by adapting existing verifier        |
| Tianquan-Shoubing tenant Web                                | Add；§12                        | no current tenant page                                          | CLI-only does not meet Web requirement                 | route/page/types/tests             | Implement direct page；no global store          |
| RBAC tenant Web                               | Remove；§12                     | `/iam/tenants` is local target-context/catalog surface          | forwarding user to Tianquan-Shoubing would create cross-app coupling | delete route/page/api              | Delete                                         |
...
|--------------------------|-----------------------------------------------------------------------|------------------------------|-------------------------------|------------------------------------------------|--------------------|-------------|
| Tianquan-Shoubing V5                   | `REQ-002`,`REQ-003`,`REQ-008`,`REQ-011`,`REQ-014`,`REQ-015`,`REQ-018` | migration test               | V1–V4 fixtures                | Client Secret/context/tenant schema            | Steps 3,4,6        | Step 1      |
...
| Tianquan-Shoubing tenant service/API   | `REQ-011`,`REQ-016`,`REQ-020`                                         | service/controller tests     | V5                            | local authority/read model                     | Steps 7,8,16       | Steps 6–7   |
| membership RPC           | `REQ-013`,`REQ-021`                                                   | contract/provider tests      | Step 6                        | ACTIVE profile method                          | Steps 9,11         | Step 8      |
| RBAC V8/state            | `REQ-012`,`REQ-014`,`REQ-015`                                         | migration/state tests        | imported Tianquan-Shoubing gate artifact    | auth-state/FK target                           | Steps 10–12        | Step 9      |
...
| Tianshu client/server        | `REQ-009`,`REQ-010`,`REQ-018`                                         | runtime/verifier/lease tests | Steps 2,5                     | Token-bound registration/lease                 | Step 18            | Steps 13–14 |
| Admin Web                | `REQ-003`,`REQ-016`,`REQ-019`                                         | component tests              | backend contracts             | one-time Secret/Tianquan-Shoubing tenant/no RBAC tenant page | Step 18            | Steps 15–17 |
...
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------|---------------------------------------------------------------------|-----------------------------|------|-----------------------------------------------------------------------|----------------------------------|
| CREATE               | `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/resources/db/migration/V5__adopt_client_secrets_and_tenant_authority.sql`                                                                                                      | Tianquan-Shoubing V1–V4                                                      | V5 schema/data contract                                             | Tianquan-Shoubing schema cutover          | 1    | `REQ-002`,`REQ-003`,`REQ-008`,`REQ-011`,`REQ-014`,`REQ-015`,`REQ-018` | `IdpClientTenantV5MigrationTest` |
| MODIFY/CREATE        | `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/{contract,core}`                                                                                                                                | existing SERVICE claims/principal and old USER/Admission ports | explicit context/app claims; reduced membership; stale paths absent | stable security model       | 2    | `REQ-007`,`REQ-008`,`REQ-017`,`REQ-020`                               | core/starter tests               |
...
| CREATE               | `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant`                                                                                                                                  | absent                                                         | tenant/member persistence + service                                 | Tianquan-Shoubing authority               | 6    | `REQ-011`,`REQ-020`                                                   | tenant service tests             |
| MODIFY/CREATE/DELETE | `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/{tenant,support,oauth/config}`                                                                                                           | RBAC HTTP membership adapter                                   | Tianquan-Shoubing tenant HTTP + local port                                        | Tianquan-Shoubing authority exposure      | 7    | `REQ-011`,`REQ-016`,`REQ-020`                                         | controller/local-port tests      |
| MODIFY/DELETE        | `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract`；`egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/identity/support/rpc/IdentityDirectoryRpcProvider.java` | profile RPC + Admission RPC                                    | additive membership RPC; Admission contract absent                  | Tianquan-Shoubing→RBAC identity fact      | 8    | `REQ-013`,`REQ-018`,`REQ-021`                                         | RPC contract/provider tests      |
...
| MODIFY               | `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/{role,policy}/repository`；`.../simulation/repository`                                                                          | TenantPO policy version reads/locks                            | authorization-state reads/locks                                     | policyVersion compatibility | 10   | `REQ-012`                                                             | repository tests                 |
| MODIFY/CREATE        | `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/{bootstrap,iam/user,iam/organization}`                                                                                             | tenantCode/TenantPO bootstrap and unchecked subject writes     | RPC ACTIVE gate + external tenantId                                 | orphan prevention/bootstrap | 11   | `REQ-013`,`REQ-019`,`REQ-021`                                         | user/bootstrap tests             |
| DELETE/MODIFY        | `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/{tenant,user,organization}`                                                                                                    | tenant/member CRUD/resolve                                     | tenant-scoped authorization only                                    | old owner removal           | 12   | `REQ-012`,`REQ-018`,`REQ-020`                                         | compile/404/static gates         |
| MODIFY/DELETE        | `egon-cola-xingyuan/egon-cola-tianshu/{egon-cola-tianshu-starter,egon-cola-tianshu-http-registration-starter}`                                                                             | Ticket supplier/input                                          | registrationToken from IdpServiceOAuth2Client                       | registration caller         | 13   | `REQ-005`,`REQ-009`,`REQ-010`,`REQ-018`                               | Tianshu runtime tests                |
| RENAME/MODIFY        | `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin`                                                                                                    | Admission claims/verifier/lease naming                         | verified SERVICE identity + Token-bound lease                       | registration server         | 14   | `REQ-007`,`REQ-009`,`REQ-010`,`REQ-018`                               | Tianshu admin tests                  |
...
| MODIFY/DELETE        | `egon-cola-xingyuan/egon-cola-tianquan-jianshen/{egon-cola-tianquan-jianshen-admin-web,egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/domain/Rbac3DevelopmentTopology.java}`                                               | RBAC tenant page/permissions                                   | no tenant page; Tianquan-Shoubing tenant permissions seeded                       | frontend/permission cleanup | 17   | `REQ-012`,`REQ-016`,`REQ-019`,`REQ-021`                               | RBAC Web/topology tests          |
...
- 相对旧基线 `0b7b9b3a`，目标模块唯一代码差异是 RBAC `IdentityProfileDirectory` 显式采用
  `RpcReferenceMode.GATEWAY`；它保持既有 Tianquan-Shoubing→RBAC RPC 依赖方向，不改变本 Plan 的合同、Step 顺序或写范围。
...
- Protobuf generated sources 位于 Maven `target/generated-sources`，只修改 `.proto`/Java contract source，不提交
  `target/`。
- Flyway V1–V4/V1–V7 历史文件 immutable；只允许创建主规格命名的 Tianquan-Shoubing V5 和 RBAC V8；Tianshu migration 文件不变。
...
| Tianquan-Shoubing Web              | `npm test -- --runInBand` 不适用；实际脚本为 `npm test -- <selector>`、`npm run typecheck`、`npm run build`                             | 在 Tianquan-Shoubing admin-web 执行 `npm ci` 后                                                     | component/static bundle；不是浏览器手工              |
...
| PostgreSQL rehearsal | Tianquan-Shoubing/RBAC Testcontainers/Flyway integration tests                                                                             | Docker 可用；与目标 PostgreSQL/Flyway major 一致                                          | rehearsal；不证明生产数据分布/锁时长                      |
...
| Live release         | §9 runbook steps                                                                                                             | Tianquan-Shoubing/RBAC backups、maintenance freeze、Secret manager、Redis/Tianshu/Yuheng environments | 用户控制；本 Plan 不执行                              |
...
- PLATFORM/TENANT 由 Grant 派生，caller 不提交 context；Tianshu 只接受 Tianshu audience PLATFORM + 最小 scope。
- tenant catalog/membership 只在 Tianquan-Shoubing；RBAC 只保留外部 tenantId 分区的 authorization state/facts。
- Client 权限保留 `tianquan-shoubing:oauth-client:read/create/update`；membership port 无 `rbac3UserId`；bootstrap 仅外部
...
| `PLAN-CLAR-002` | 新 Tianquan-Shoubing tenant Java 文件放在 feature-first `admin.tenant.{controller,service,service.impl,repo,domain}`  | Tianquan-Shoubing oauth/identity/resource 现有结构；主规格 §6.1 要求保留            | 只决定内部路径/命名，不改变 API/schema                             | 路径不同只影响 imports/Plan inventory                                                  |
| `PLAN-CLAR-003` | Spring OAuth2 Client 使用 Boot dependency management，不写显式版本；facade 放 `tianquan-shoubing.starter.client`            | starter POM 统一依赖管理 + 主规格目标树                               | 标准依赖/包 placement，不改变 token contract                   | 若父 POM未管理依赖，需在 xingyuan parent补受控版本而非局部硬编码                                      |
| `PLAN-CLAR-004` | Tianshu Java security package从 `admission` rename为 `registration`，Redis/SQL已有 `admission_*` 物理字段本期保持   | 主规格 `ASM-004`；Tianshu schema无 migration                       | 只消除 Java语义歧义，持久数据/键兼容不变                               | 若 serialization 直接依赖 Java property name，Step 14要加 explicit legacy field mapping |
| `PLAN-CLAR-005` | 一次性 tenant cutover 工具为单脚本多 subcommand，放 `scripts/unified-xingyuan`，runbook 放 `docs/runbooks`       | 现有 unified-xingyuan scripts/common.sh 与 runbook locations | one-shot operations placement，不增加 runtime service/job | 若运维要求独立 artifact signing binary，可后续 amendment替换脚本实现                             |
| `PLAN-CLAR-006` | Step 11 的 Tianquan-Shoubing membership RPC consumer 沿用 `@EgonRpcReference(mode = RpcReferenceMode.GATEWAY, ...)` | 当前 `IdentityProfileDirectory` 在 `2bb92624` 已显式采用该模式        | 仅同步当前 RPC reference 注解惯例，不改变 RPC identity、deadline 或 fail-closed 语义 | 若目标部署要求其他 mode，必须回到 Spec 重新确认 topology，而不能在执行时静默切换                         |
...
### Step 1 — 固化 Tianquan-Shoubing V5 Client Secret 与 tenant authority schema
...
- Baseline state: Tianquan-Shoubing migration 仅 V1–V4；`identity_client` 无 app_id/secret，Grant tenant 无
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/support/migration/IdpClientTenantV5MigrationTest.java`
...
- Dependencies and consumers: 读取全部 Tianquan-Shoubing migrations；不启动 Spring application；为 Steps 3/4/6 的 Entity/schema 映射提供
  proof。
...
- Purpose: 在一个 immutable Tianquan-Shoubing migration中完成主规格 §11 的 schema/data transition。
...
- Verification command:
  `./mvnw -B -ntp -pl egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin -am -Dtest=IdpClientTenantV5MigrationTest -Dsurefire.failIfNoSpecifiedTests=false test`
- Expected result: 已实现证据为 exit 0、3 个 migration source-contract tests、追加 7 个 Tianquan-Shoubing migration
...
- Commit: `feat(tianquan-shoubing): add client secret and tenant authority migration`
...
- Observable outcome: Tianquan-Shoubing core可构造严格 TENANT/PLATFORM SERVICE identity，membership port不暴露RBAC ID，旧未接线 USER
...
#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientCredentialsAccessPolicyTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ResourceServerPolicyTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/token/TokenFacadeTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthResourceSecurityMatrixIT.java`
...
construct PLATFORM access from an explicit xingyuan grant and assert context PLATFORM with null tenant and no magic wildcard
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/ServiceTokenContext.java`
...
- Dependencies and consumers: `Rs256TokenService`、`IdpJwtVerifier`、Yuheng、Tianshu、starter；这些消费者在Steps 4/5/14适配。
...
#### File 3 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/IdpClaimNames.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/IdpPrincipal.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/ServiceIdentityPrincipal.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/token/ServiceAccessTokenClaims.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientResourceGrant.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientCredentialsAccessPolicy.java`
...
- Dependencies and consumers: File 2 enum；`Rs256TokenService`、`IdpJwtVerifier`、Yuheng、Tianshu、starter 在后续 Steps 适配；admin persistence mapping 留在后续声明的 resource paths。
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/TenantMembershipPort.java`

- Purpose: 从既有 Tianquan-Shoubing membership boundary 移除 RBAC 内部 ID，同时保留 list/resolve 行为。
...
- Input/output and state mapping: Tianquan-Shoubing tenant/member read model→port record；无RBAC lookup或authorization decision
  mapping。
...
#### File 5 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/UserResourceAccessAuthorizationPort.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/UserResourceAccessPolicy.java`
...
- Dependencies and consumers: 仅已移除的测试引用；不替代 Yuheng/PEP 或 USER TokenFacade 行为。
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientCredentialsAccessPolicyTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ResourceServerPolicyTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/test/java/top/egon/cola/platform/tianquan-shoubing/core/token/TokenFacadeTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthResourceSecurityMatrixIT.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/ServiceTokenContext.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/IdpClaimNames.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/IdpPrincipal.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/contract/ServiceIdentityPrincipal.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/token/ServiceAccessTokenClaims.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientResourceGrant.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientCredentialsAccessPolicy.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/TenantMembershipPort.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/UserResourceAccessAuthorizationPort.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/UserResourceAccessPolicy.java`
- Commit: `refactor(tianquan-shoubing): make service context explicit and remove rbac identity leakage`
...
### Step 3 — 实现 Tianquan-Shoubing 管理员预配与轮换 hash-only Client Secret
...
- Observable outcome: `POST /api/v1/tianquan-shoubing/clients` 对Confidential Client一次返回AppID/App Key/Secret；
...
- Contract/signature changes: create return改为one-time result；list/detail仍safe `OAuthClientVO`
  ；rotate用expectedVersion并要求 `tianquan-shoubing:oauth-client:update`。
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthClientControllerTest.java`
...
assert exact tianquan-shoubing:oauth-client:create or update permissions, no-store headers and one-time secret response fields
...
- Repository evidence: Tianquan-Shoubing domain/dto/vo records与validation annotations；主规格complete JSON field mapping。
...
controller requires tianquan-shoubing:oauth-client:create for create and tianquan-shoubing:oauth-client:update for rotate, sets Cache-Control no-store and never accepts a SERVICE client self-management shortcut
...
- Commit: `feat(tianquan-shoubing): manage hash-only client secrets`
...
- End state: Tianquan-Shoubing core/admin/rpc-contract reactor GREEN；USER refresh/login/signing JWK保持；starter consumer待Step5。
- Test-first gate: Required — Basic/claim/metadata/old-path negative tests当前因旧authenticator和JWK endpoints存在而
  RED。
- Ordered files:

#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/token/service/impl/ClientCredentialsTokenServiceTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/config/OAuthConfigTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/security/IdpJwtVerifierTest.java`
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthTokenClientCredentialsTest.java`
...
#### File 3 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/oauth/ClientSecretAuthentication.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/service/impl/ClientSecretBasicAuthenticator.java`
...
- Dependencies and consumers: Tianquan-Shoubing Client/Secret repositories与PasswordHashPort；File 4 Token controller/service。
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthTokenController.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/token/service/impl/ClientCredentialsTokenService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/controller/OAuthMetadataController.java`
...
#### File 5 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/oauth/OAuthClient.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/repo/JpaOAuthClientStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/token/service/impl/Rs256TokenService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/security/IdpJwtVerifier.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-gateway-adapter/src/main/java/top/egon/cola/platform/tianquan-shoubing/yuheng/security/IdpTrustedIdentityMapper.java`
...
- Dependencies and consumers: core records、Resource state projection、Yuheng headers、Tianshu Step14。
...
yuheng mapper writes X-Egon-Tenant-Id only for TENANT service/user principals and never converts PLATFORM null into an empty or wildcard header
```

- Verification contribution: IdpJwtVerifier/Token service/Yuheng unit regression。
- After this file: new claims roundtrip GREEN；legacy files/endpoints仍待删除。

#### File 6 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/oauth/ClientAssertionAuthentication.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/ClientAssertionReplayStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/port/ClientCredentialStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ClientJwkCredential.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/AdmissionRequest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/AdmissionTicketClaims.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-core/src/main/java/top/egon/cola/platform/tianquan-shoubing/core/resource/ResourceServerAdmissionPolicy.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/service/impl/PrivateKeyJwtAuthenticator.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/repo/RedisClientAssertionReplayStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/repo/JpaClientCredentialStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/service/impl/ResourceServerAdmissionServiceImpl.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/support/rpc/ResourceServerAdmissionRpcProvider.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/support/oauth/LocalServiceAccessTokenSupplier.java`
...
- Input/output and state mapping: None；删除所有JWK/assertion/Ticket/JTI replay state；Tianquan-Shoubing signing key types不在集合中。
...
delete Admission policy, issuer service, RPC provider and local service-token supplier after Tianshu/token replacements are named
...
preserve Tianquan-Shoubing signing-key entities, JWKS endpoint and USER token signer because they are outside this deletion set
```
...
#### File 7 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/config/OAuthConfig.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/config/ResourceServerConfig.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/controller/ResourceServerController.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/service/ResourceServerService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/service/impl/ResourceServerServiceImpl.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/dto/CreateResourceServerDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/pojo/IdentityResourceServerEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/vo/ResourceServerVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/repo/JpaResourceServerStore.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/service/ResourceServerProjectionService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/support/bootstrap/IdpDevelopmentClientBootstrap.java`
...
- Dependencies and consumers: OAuth Token controller、Resource Admin Web Step15、Resource projection/Tianshu state保持。
...
- Verification contribution: full Tianquan-Shoubing admin/rpc/core compile，Resource controller tests更新后的404/unchanged projection
  proof。
- After this file: retained Tianquan-Shoubing backend contract is Basic-only；dedicated JWK types remain for File 8 deletion。

#### File 8 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/dto/CreateClientJwkDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/pojo/IdentityClientJwkEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/domain/vo/ClientJwkVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/resource/repo/IdentityClientJwkRepository.java`

- Purpose: 物理删除 dedicated Client JWK DTO/entity/VO/repository types。
- Symbols: four Client JWK admin types absent；Tianquan-Shoubing Signing Key types remain。
...
search the Tianquan-Shoubing admin module for these exact type names and old key-management URLs and require zero production matches
...
search all Tianquan-Shoubing tests for deleted production symbols and require zero hits, while keeping wire-level negative coverage in the new controller test
```

- Verification contribution: `TEST-028/029` obsolete test and symbol deletion completeness。
- After this file: Tianquan-Shoubing tests compile solely against new Token/Resource contracts。
...
- Execution evidence: corrected RED reached the planned `ClientSecretAuthentication` absence, then the Step 4 reactor command passed
  ...
  already completed by the prior corrective Step 2 commit). No live DB、service、Tianshu、Yuheng or browser was started.

### Step 5 — 用 Spring Security OAuth2 Client 实现 tianquan-shoubing-starter SERVICE Token facade
...
- Baseline state: tianquan-shoubing-starter只有Resource Server/Jose与private-key Admission Ticket beans，POM反向依赖DDC
...
- End state: starter POM/autoconfig/facade/tests GREEN；Tianshu caller尚未切换，留给Step13。
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/client/IdpServiceOAuth2ClientTest.java`
...
- Repository evidence: current auto-config ApplicationContextRunner tests、admission supplier tests、Spring Security test
  stack。
- Dependencies and consumers: new facade/autoconfig/POM；Step13 Tianshu registration runtime。
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/autoconfigure/IdpStarterAutoConfigurationTest.java`
...
- Symbols: `spring-security-oauth2-client` dependency；移除 Tianshu starter/Admission RPC contract dependencies。
- Repository evidence: parent管理Spring Security版本；主规格INTERNAL-001；starter现有composition style。
- Dependencies and consumers: File 4 client classes、Spring Boot dependency management；biz/Tianshu consumers。
- Why now: tests确定contract后添加最少依赖与types，不新增cache server或自签名组件。
- Contract/signature changes: module dependency graph 改为 starter→Spring OAuth2 Client；不新增显式版本或 Tianshu 反向依赖。
...
remove Tianshu starter and Admission RPC contract dependencies only after File 6 deletes every corresponding main-source import
inspect the resulting dependency tree and require no reverse tianquan-shoubing-starter to Tianshu/Admission edge while retaining Jose/Resource Server dependencies
```
...
- Dependencies and consumers: File 3 OAuth2 Client dependency、File 5 autoconfiguration、biz/Tianshu Step 13。
...
- Dependencies and consumers: Files 3–4 Spring OAuth2 Client/facade；Tianshu Step 13；Resource Server beans 保持。
...
#### File 6 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/CachingDdcAdmissionTicketSupplier.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/OwnerOnlyPrivateKeyLoader.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/PrivateKeyJwtAssertionFactory.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/RpcResourceServerAdmissionClient.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/package-info.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/CachingDdcAdmissionTicketSupplierTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/OwnerOnlyPrivateKeyLoaderTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/PrivateKeyJwtAdmissionAssertionFactoryTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/RpcResourceServerAdmissionClientTest.java`
...
- Dependencies and consumers: Step 13 Tianshu 使用新 facade；Resource Server verification beans 不在 deletion set。
...
- After this file: tianquan-shoubing-starter contains no Admission/private-key code and uses only the standard OAuth2 Client facade。
...
- Commit paths: `CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/client/IdpServiceOAuth2ClientTest.java`,   `MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/autoconfigure/IdpStarterAutoConfigurationTest.java`,   `MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/pom.xml`,   `CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/client/IdpServiceTokenRequest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/client/ServiceAuthorizationKey.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/client/IdpServiceOAuth2Client.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/client/IdpClientCredentialsRequestEntityConverter.java`,   `MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/autoconfigure/IdpStarterProperties.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/autoconfigure/IdpStarterAutoConfiguration.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/package-info.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/autoconfigure/package-info.java`,   `DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/CachingDdcAdmissionTicketSupplier.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/OwnerOnlyPrivateKeyLoader.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/PrivateKeyJwtAssertionFactory.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/RpcResourceServerAdmissionClient.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/main/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/package-info.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/CachingDdcAdmissionTicketSupplierTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/OwnerOnlyPrivateKeyLoaderTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/PrivateKeyJwtAdmissionAssertionFactoryTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-starter/src/test/java/top/egon/cola/platform/tianquan-shoubing/starter/admission/RpcResourceServerAdmissionClientTest.java`
- Commit: `feat(tianquan-shoubing-starter): add spring oauth2 service client facade`
...
- Execution evidence: Step 5 RED stopped at the absent Spring OAuth2 Client dependency/facade and the expected old Admission
  ...
  Tianshu starter/Admission RPC edge；`git diff --check` and declared-path gate passed. Commit `35d1879d` contains the 20 declared
  paths. No live Tianquan-Shoubing/Token endpoint, service, Tianshu, database or browser was started.

### Step 6 — 实现 Tianquan-Shoubing tenant 与 membership 本地权威服务
...
- Observable outcome: Tianquan-Shoubing service在本地事务内创建/更新tenant、upsert membership、按identity/tenant解析effective
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantServiceImplTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantMembershipServiceImplTest.java`
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/pojo/IdentityTenantEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/pojo/IdentityTenantMembershipEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/repo/IdentityTenantRepository.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/repo/IdentityTenantMembershipRepository.java`
...
- Symbols: tenant/member states/factories/transitions；find page/by code/by id；find membership/list by subject/tenant
  with locks。
- Repository evidence: Tianquan-Shoubing feature JPA naming/audit/version style与V5 exact schema。
...
#### File 3 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/TenantService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/TenantMembershipService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantServiceImpl.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantMembershipServiceImpl.java`
...
- After this file: Tianquan-Shoubing local authority service ready；no external endpoints yet。
...
- Completion criteria: Tianquan-Shoubing DB/service成为tenant/member唯一writer/read model，模型无RBAC ID。
- Rollback: revert三组paths；若V5已有导入数据，应用rollback不得删除数据，按snapshot/forward-fix处理。
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantServiceImplTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantMembershipServiceImplTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/pojo/IdentityTenantEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/pojo/IdentityTenantMembershipEntity.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/repo/IdentityTenantRepository.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/repo/IdentityTenantMembershipRepository.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/TenantService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/TenantMembershipService.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantServiceImpl.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/impl/TenantMembershipServiceImpl.java`
- Commit: `feat(tianquan-shoubing): own tenant and membership state`
...
- Execution evidence: Step 6 RED 在 admin `testCompile` 因 tenant entity/repository/service types 不存在而失败；补齐 V5 对应
  ...
  删除范围；未启动 live PostgreSQL、Tianquan-Shoubing/RBAC 服务、RPC、Tianshu 或浏览器。

### Step 7 — 暴露 Tianquan-Shoubing tenant API 并把登录 membership 切到本地 port
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/controller/TenantControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/LocalTenantMembershipPortTest.java`
...
- Contract/signature changes: routes/inputs/outputs完全按主规格；permissions `tianquan-shoubing:tenant:read/manage`；local port reduced
  record。
...
verify read operations require tianquan-shoubing:tenant:read while every mutation requires tianquan-shoubing:tenant:manage and records the current operator subject
...
#### File 2 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/CreateTenantDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/UpdateTenantDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/UpsertTenantMembershipDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/vo/TenantVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/vo/TenantMembershipVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/controller/TenantController.java`

- Purpose: 实现API-005–009 transport/authorization/mapping。
- Symbols: `/api/v1/tianquan-shoubing/tenants` list/create/update；`/{tenantId}/memberships` list/upsert。
- Repository evidence: feature-first Tianquan-Shoubing controllers、validation annotations、AdminAuthorization、GatewayOperation
  metadata。
...
```java
controller list validates page/size/filter and requires tianquan-shoubing:tenant:read before delegating to TenantService deterministic query
controller create/update requires tianquan-shoubing:tenant:manage, derives operator from principal and maps validated DTO plus expectedVersion to service
...
#### File 3 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/LocalTenantMembershipPort.java`

- Purpose: 新建 local port adapter，让 TokenFacade/login 从 Step 6 Tianquan-Shoubing service 读取 membership。
...
- Input/output and state mapping: Tianquan-Shoubing repository membership→local port→TokenFacade；不再构造 RBAC HTTP request。
...
#### File 5 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/support/tianquan-jianshen/HttpTenantMembershipAdapter.java`

- Purpose: 物理删除不再装配的 Tianquan-Shoubing→RBAC membership HTTP adapter。
...
- After this file: Tianquan-Shoubing tenant/member backend/API/login is fully local；RPC remains for Step 8。
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/controller/TenantControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/LocalTenantMembershipPortTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/CreateTenantDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/UpdateTenantDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/dto/UpsertTenantMembershipDTO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/vo/TenantVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/domain/vo/TenantMembershipVO.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/controller/TenantController.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/tenant/service/LocalTenantMembershipPort.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/oauth/config/OAuthConfig.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/resources/application.yml; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/resources/application-local.yml; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/support/tianquan-jianshen/HttpTenantMembershipAdapter.java`
- Commit: `feat(tianquan-shoubing): expose local tenant authority`
...
- Execution evidence: Step 7 RED 在 admin `testCompile` 因 `LocalTenantMembershipPort` 与 `TenantController` 不存在而失败；新增
  ...
  keys，故保持内容不变；提交 `fd31f5ba` 含 12 个实际变更路径。未启动 live PostgreSQL、Tianquan-Shoubing/RBAC 服务、RPC、Tianshu 或浏览器。
...
#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/test/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/IdentityDirectoryRpcContractTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/identity/support/rpc/IdentityDirectoryRpcProviderTest.java`
...
#### File 2 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/test/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/ResourceServerAdmissionRpcContractTest.java`
...
#### File 5 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/identity/support/rpc/IdentityDirectoryRpcProvider.java`
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/test/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/IdentityDirectoryRpcContractTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/test/java/top/egon/cola/platform/tianquan-shoubing/admin/identity/support/rpc/IdentityDirectoryRpcProviderTest.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/test/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/ResourceServerAdmissionRpcContractTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/proto/identity_directory.proto; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/IdentityDirectoryRpc.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/proto/resource_server_admission.proto; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/ResourceServerAdmissionRpc.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/package-info.java; egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-rpc-contract/src/main/java/top/egon/cola/platform/tianquan-shoubing/rpc/contract/proto/v1/package-info.java`,   `egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin/src/main/java/top/egon/cola/platform/tianquan-shoubing/admin/identity/support/rpc/IdentityDirectoryRpcProvider.java`
- Commit: `feat(tianquan-shoubing-rpc): expose tenant membership and remove admission contract`

- Execution evidence: Step 8 RED 首先在 contract descriptor 中因新增 RPC method 尚不存在而失败；随后补齐 append-only
  `GetTenantMembership` protobuf/messages/enums、Tianquan-Shoubing tenant service mapping、NOT_FOUND fail-closed provider validation，删除
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3ExternalTenantV8IT.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/JpaTenantAuthorizationStateRepositoryTest.java`
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3MigrationContractTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3FlywayPostgresqlIT.java`
...
#### File 3 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/domain/po/TenantAuthorizationStatePO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/repository/TenantAuthorizationStateRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/repository/jpa/JpaTenantAuthorizationStateRepository.java`
...
ensureVerifiedTenant inserts version zero only after the caller supplies a successful Tianquan-Shoubing membership verification result and handles duplicate races deterministically
...
- Rollback: pre-V8 revert paths；post-V8 only coordinated RBAC+Tianquan-Shoubing snapshot restore or forward-fix。
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3ExternalTenantV8IT.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/JpaTenantAuthorizationStateRepositoryTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3MigrationContractTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/repository/Rbac3FlywayPostgresqlIT.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/resources/db/migration/V8__externalize_tenant_authority.sql`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/domain/po/TenantAuthorizationStatePO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/repository/TenantAuthorizationStateRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/authorizationstate/repository/jpa/JpaTenantAuthorizationStateRepository.java`
- Commit: `feat(tianquan-jianshen): externalize tenant authorization state`
...
#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/RoleControlFacadeTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/simulation/PostgresqlRoleImpactSourceTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/activation/RoleActivationCandidateServiceTest.java`
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/repository/jpa/JpaRoleRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/policy/repository/jpa/JpaConstraintRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/simulation/repository/jdbc/PostgresqlRoleImpactRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/activation/repository/jpa/JpaRoleActivationFactRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/service/RoleEligibilityService.java`
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/RoleControlFacadeTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/simulation/PostgresqlRoleImpactSourceTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/activation/RoleActivationCandidateServiceTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/repository/jpa/JpaRoleRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/policy/repository/jpa/JpaConstraintRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/simulation/repository/jdbc/PostgresqlRoleImpactRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/activation/repository/jpa/JpaRoleActivationFactRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/role/service/RoleEligibilityService.java`
- Commit: `refactor(tianquan-jianshen): read policy versions from authorization state`
...
### Step 11 — 对 RBAC 主体写入与 bootstrap 强制 Tianquan-Shoubing membership gate
...
- Observable outcome: 一个RPC
  adapter将missing/inactive/timeout统一为fail-closed；用户create/update-to-active与CLI/development/xingyuan
...
#### File 1 —
`CREATE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityTenantMembershipDirectory.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityTenantMembershipDirectoryTest.java`
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/UserControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/Rbac3PlatformAdminBootstrapCliIT.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentBootstrapTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentTopologyTest.java`
...
#### File 3 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/service/UserCrudFacade.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/service/DefaultDirectoryCommandService.java`
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/controller/cli/Rbac3PlatformAdminBootstrapCli.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/DevelopmentBootstrapPort.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/PlatformAdminBootstrapRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/PlatformAdminBootstrapService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentBootstrap.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/internal/DefaultPlatformAdminBootstrapService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaDevelopmentTopologyBootstrapRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaPlatformAdminBootstrapRepository.java`
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityTenantMembershipDirectory.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityTenantMembershipDirectoryTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/UserControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/Rbac3PlatformAdminBootstrapCliIT.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentBootstrapTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentTopologyTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/service/UserCrudFacade.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/service/DefaultDirectoryCommandService.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/controller/cli/Rbac3PlatformAdminBootstrapCli.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/DevelopmentBootstrapPort.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/PlatformAdminBootstrapRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/PlatformAdminBootstrapService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentBootstrap.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/internal/DefaultPlatformAdminBootstrapService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaDevelopmentTopologyBootstrapRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaPlatformAdminBootstrapRepository.java`
- Commit: `feat(tianquan-jianshen): gate subjects and bootstrap through tianquan-shoubing membership`
...
- Execution state: `Committed` at `c52fe991`; no empty or corrective Step 11 commit was created. RED was observed
  ...
  `git diff --check` passed; no live Tianquan-Shoubing/RBAC database, service or browser was started.
...
#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/controller/IamTenantControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/TenantContextFilterTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/IdentityMembershipResolveRequestDTOTest.java`

- Purpose: 将旧tenant controller测试改成route absence，并保留trusted tenant context/target authorization regression。
- Symbols: old `/api/tianquan-jianshen/v1/xingyuan/tenants` and internal resolve 404；TenantContextFilter still requires signed
...
#### File 2 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/controller/TenantController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/dto/CreateTenantCommandDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/dto/TenantStatusCommandDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/enums/TenantStatusEnum.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/po/TenantPO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/vo/TenantVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/repository/TenantLookupRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/repository/jpa/JpaTenantLookupRepository.java`
...
#### File 3 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/controller/DirectoryController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/service/DirectoryQueryService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/service/DefaultDirectoryQueryService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/DirectoryCommandRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/DirectoryQueryRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/jpa/JpaDirectoryCommandRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/jpa/JpaDirectoryQueryRepository.java`
...
- Error and edge behavior: invalid tenantId formatting仍由context/DTO validation；missing local authorization
  rows保持not-found；no remote Tianquan-Shoubing fetch added。
- Implementation pseudocode:
...
#### File 4 —
`DELETE egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/InternalIdentityController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/InternalUserController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/dto/IdentityMembershipResolveRequestDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/exception/IdentityMembershipNotFoundException.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/vo/TenantMembershipResponseVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/vo/TenantMembershipVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityMembershipRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/jpa/JpaIdentityMembershipRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/jpa/MembershipRow.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/service/IdentityMembershipFacade.java`
...
- Repository evidence: Step7 Tianquan-Shoubing local authority替换all Tianquan-Shoubing consumers；Step8/11新RPC方向为RBAC→Tianquan-Shoubing。
- Dependencies and consumers: old external/internal callers必须由Step18 inventory归零；no replacement HTTP。
- Why now: Tianquan-Shoubing/RBAC new dependency direction已完成，物理删除可防止回退双权威。
...
search Tianquan-Shoubing and RBAC main sources for old URI strings, DTOs, repository and facade symbols and require zero current references
verify the only membership authority call direction is RBAC IdentityTenantMembershipDirectory to Tianquan-Shoubing IdentityDirectoryRpc
```
...
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/controller/IamTenantControllerTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/TenantContextFilterTest.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/IdentityMembershipResolveRequestDTOTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/controller/TenantController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/dto/CreateTenantCommandDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/dto/TenantStatusCommandDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/enums/TenantStatusEnum.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/po/TenantPO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/domain/vo/TenantVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/repository/TenantLookupRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/tenant/repository/jpa/JpaTenantLookupRepository.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/controller/DirectoryController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/service/DirectoryQueryService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/service/DefaultDirectoryQueryService.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/DirectoryCommandRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/DirectoryQueryRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/jpa/JpaDirectoryCommandRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/organization/snapshot/repository/jpa/JpaDirectoryQueryRepository.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/InternalIdentityController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/controller/InternalUserController.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/dto/IdentityMembershipResolveRequestDTO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/exception/IdentityMembershipNotFoundException.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/vo/TenantMembershipResponseVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/domain/vo/TenantMembershipVO.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/IdentityMembershipRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/jpa/JpaIdentityMembershipRepository.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/repository/jpa/MembershipRow.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/iam/user/service/IdentityMembershipFacade.java`
- Commit: `refactor(tianquan-jianshen): remove tenant and membership authority`
...
- Execution state: `Committed` at `551d834e`; no empty or corrective Step 12 commit was created. RED first proved
  ...
  No live PostgreSQL, Tianquan-Shoubing, service or browser was started. The four additional compile-consumer paths were necessary
...
### Step 13 — 让 Tianshu registration runtime 直接获取并提交 SERVICE Token

- Requirements: `REQ-005`, `REQ-009`, `REQ-010`, `REQ-018`
- Dependencies: Step 5
- Baseline state: Tianshu starter暴露DdcAdmissionTicketSupplier/Admission models；HTTP registration
  runtime每次register/heartbeat向supplier取Ticket。
- Observable outcome: Tianshu register/heartbeat transport字段为registrationToken；HTTP runtime用IdpServiceOAuth2Client请求DDC
  PLATFORM scope Token，无Ticket supplier/model/cache。
- End state: Tianshu starter/http-registration-starter tests GREEN；Tianshu admin尚在Step14适配新field。
- Execution state: `Committed` at `b045c318`; the required test-first RED was observed before the
  ...
  failures/errors/skips (5 model, 6 HTTP runtime, 8 auto-configuration); RPC Tianshu adapter compile/test
  consumers also compiled after the protobuf field rename. Yuheng RPC slot/MCP token callers and Tianshu
  ...
  `admission_ticket` transport symbols; Tianshu Admin security symbols intentionally remain for Step 14.
  `git diff --check` passed. No live PostgreSQL, Redis, Tianquan-Shoubing, Tianshu, Yuheng service, or browser was started.
- Test-first gate: Required — runtime/model/autoconfig tests因仍依赖Ticket types与无Idp facade dependency而RED。
- Ordered files:

#### File 1 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/test/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationRuntimeTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/test/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationAutoConfigurationTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/test/java/top/egon/cola/component/tianshu/model/registry/DdcServiceRegistrationTest.java`

- Purpose: 定义Token request、register/heartbeat mapping、renewal/re-register与no Ticket symbol行为。
- Symbols: Tianshu audience/context/scope request capture；`registrationToken` accessors；runtime retry transitions。
...
- Input/output and state mapping: serviceKey biz/app/env + fixed Tianshu resource/scopes→IdpServiceTokenRequest PLATFORM→raw
...
```java
capture each IdpServiceTokenRequest and assert Tianshu audience, PLATFORM context, null tenant and the exact minimum registration scope
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/config/DdcInstanceRegisterRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/config/DdcHeartbeatRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/registry/DdcServiceRegistration.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/registry/DdcServiceLeaseRequest.java`
...
- Repository evidence: current runtime/admin mappings引用这四个request types；Spec Tianshu physical schema unchanged但Java
  transport rename。
- Dependencies and consumers: HTTP runtime、Tianshu admin mappers/services；Step14同步server。
...
#### File 3 —
`DELETE egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/api/extension/DdcAdmissionTicketSupplier.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/admission/DdcAdmissionRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/admission/DdcAdmissionTicket.java`
...
- After this file: Tianshu shared starter has no Ticket extension/model；HTTP runtime remains for File 4。

#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/pom.xml; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationRuntime.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationAutoConfiguration.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationProperties.java`

- Purpose: 依赖idp-starter facade并实现DDC定向PLATFORM token acquisition。
- Symbols: IdpServiceOAuth2Client injection；Tianshu resource/scopes/registration id properties；runtime `registrationToken`
  mapping。
- Repository evidence: current runtime state machine可复用；Step5已移除idp-starter→Tianshu dependency，因此新增DDC→IdP不成环。
...
build one Tianshu PLATFORM token request from registrationId, appId, configured Tianshu resource URI and fixed registration scope for each credential refresh point
...
- After this file: Tianshu caller noTicket；server needsStep14。
...
- Expected result: exit 0；Token request/register/heartbeat/recovery/model tests通过；Tianshu starter无Ticket types。
- Failure returns to: File1 behavior；File2 model mapping；File3 Ticket deletion；File4 dependency/autoconfig/runtime。
- Completion criteria: all Tianshu callers提交SERVICE Token且无Admission Ticket acquisition/cache。
- Rollback: revert四组paths仅在admin未切换/未部署时；发布中caller/admin必须同一兼容单元。
- Commit paths: `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/test/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationRuntimeTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/test/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationAutoConfigurationTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/test/java/top/egon/cola/component/tianshu/model/registry/DdcServiceRegistrationTest.java`,   `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/config/DdcInstanceRegisterRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/config/DdcHeartbeatRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/registry/DdcServiceRegistration.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/registry/DdcServiceLeaseRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/api/extension/DdcAdmissionTicketSupplier.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/admission/DdcAdmissionRequest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu/model/admission/DdcAdmissionTicket.java`,   `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/pom.xml; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationRuntime.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationAutoConfiguration.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-http-registration-starter/src/main/java/top/egon/cola/component/tianshu/http/registration/DdcHttpRegistrationProperties.java`
- Commit: `feat(tianshu): register with tianquan-shoubing service tokens`

- Additional compile-consumer paths included in the same Step 13 commit: RPC Tianshu adapter protobuf
  contracts/mappers/registry bridge and tests; Tianshu starter `DdcInstanceService`/auto-configuration and
  lifecycle tests; Yuheng `RpcGatewaySlotRuntime`, `GatewayEngineConfiguration`, MCP service-token
  adapter and tests; RPC process test fixtures; Tianshu integration-test fixtures; and Yuheng Admin Tianshu
  configuration test. These were necessary fallout of the declared Ticket type deletion, not new
  authorization surfaces.

### Step 14 — 让 Tianshu 验证 PLATFORM SERVICE Token 并绑定 lease identity

- Requirements: `REQ-007`, `REQ-009`, `REQ-010`, `REQ-018`
- Dependencies: Steps 2, 4 and 13
- Baseline state: Tianshu admin verifier校验Admission Ticket claims/package
...
- End state: Tianshu admin/component tests GREEN，Java无Ticket/Admission security symbols；Redis/SQL旧物理字段通过explicit
  mapping保留。
- Execution state: `Committed` at `233eb901`; the initial rename/consumer compile exposed stale Tianshu Admin
  ...
  `git diff --check` and the Tianshu old-security-symbol search passed, and no Tianshu migration file was added. Additional
  compile-consumer fallout included `DdcAuthBootstrapController`, Tianshu integration/RPC fixtures, and the registration
  JSON explicit mapping; no live PostgreSQL, Redis, Tianquan-Shoubing, Tianshu, Yuheng service, or browser was started.
...
```java
verify valid Tianshu-audience SERVICE token with PLATFORM context, exact scope, app/client/source/credential claims and matching biz/app/env/instance request
...
#### File 2 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/service/lease/DdcConfigLeaseServiceTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcConfigLeaseRedisRepositoryTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcServiceRegistryRedisRepositoryTest.java`
...
```java
verify registrationToken through ServiceAccessTokenVerifier and require principal context PLATFORM, null tenant, Tianshu audience and exact registration scope
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminProperties.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminSecurityPropertiesValidator.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminRedisConfig.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcConfigLeaseService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcLeaseValidator.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/registry/DdcServiceRegistryService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/repository/DdcConfigLeaseRedisRepository.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/repository/DdcServiceRegistryRedisRepository.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/resources/application.yml`
...
- Verification contribution: makes Files1–2 verifier/repo/lease tests GREEN；Tianshu no migration invariant。
- After this file: Tianshu caller/server Token path完整，无Admission Java security symbols。

#### File 5 —
`MODIFY egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcInstanceAdminService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/config/DdcAdminSecurityPropertiesTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/config/DdcAdminRedisConfigTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/service/registry/DdcServiceRegistryServiceTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcV8MigrationTest.java`
...
- Repository evidence: repository-wide `DdcAdmission*` consumer search列出这些路径；Tianshu
  `V8__add_resource_admission_audit.sql` immutable。
- Dependencies and consumers: Files1–4 renamed types/config/services；Tianshu admin context/module compile。
...
keep DdcV8MigrationTest assertions on existing physical admission columns and checksums, documenting that Java mapping changed but no Tianshu migration was added
...
- Verification contribution: Tianshu admin reactor compile、config/context regression、no-migration proof。
- After this file: allknown Tianshu admin consumers use registration semantics，old Java package absent。
...
- Expected result: exit 0；claim/source/replay/concurrency/expiry与legacy mapping通过；Tianshu migration目录无新文件。
...
- Completion criteria: direct SERVICE Token保持全部防伪/lease约束，Ticket链删除，Tianshu schema unchanged。
- Rollback: caller/admin必须同版本rollback；Redis旧physical mapping允许整组旧binary恢复但需同步IdP credentials/snapshots。
- Commit paths: `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/security/admission/IdpJwtDdcAdmissionVerifierTest.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/security/registration/IdpJwtDdcRegistrationCredentialVerifierTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/security/admission/DdcAdmissionTestFixture.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/security/registration/DdcRegistrationTestFixture.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/service/lease/DdcConfigLeaseServiceTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcConfigLeaseRedisRepositoryTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcServiceRegistryRedisRepositoryTest.java`,   `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/admission/DdcAdmissionClaims.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/registration/VerifiedDdcRegistrationIdentity.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/admission/DdcAdmissionVerifier.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/registration/DdcRegistrationCredentialVerifier.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/admission/DdcAdmissionException.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/registration/DdcRegistrationAuthenticationException.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/admission/IdpJwtDdcAdmissionVerifier.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/registration/IdpJwtDdcRegistrationCredentialVerifier.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/admission/package-info.java -> egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/security/registration/package-info.java`,   `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminProperties.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminSecurityPropertiesValidator.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/config/DdcAdminRedisConfig.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcConfigLeaseService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcLeaseValidator.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/registry/DdcServiceRegistryService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/repository/DdcConfigLeaseRedisRepository.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/repository/DdcServiceRegistryRedisRepository.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/resources/application.yml`,   `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/service/lease/DdcInstanceAdminService.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/config/DdcAdminSecurityPropertiesTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/config/DdcAdminRedisConfigTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/service/registry/DdcServiceRegistryServiceTest.java; egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/test/java/top/egon/cola/component/tianshu/admin/repository/DdcV8MigrationTest.java`
- Commit: `feat(tianshu): verify service tokens for registration leases`

### Step 15 — 完成 Tianquan-Shoubing Client/Secret Web 并移除 Client JWK UI
...
- End state: Tianquan-Shoubing Client/Resource Web tests/typecheck/build GREEN；SigningKeyPage无diff。
...
- Contract note: 当前 Tianquan-Shoubing admin Resource Grant Java DTO/service 尚未在前序 Step 将 V5 `grant_context`
...
add rotate confirmation under tianquan-shoubing:oauth-client:update, send expectedVersion and invalidate the client query only after success
...
retain source Resource identity, status/version and management client mappings required by service token issuance and Tianshu projection
...
- After this file: Tianquan-Shoubing Client/Resource Web符合new backend，no Client JWK UI。
...
- Commit: `feat(tianquan-shoubing-web): manage app secrets and remove client jwks`

### Step 16 — 增加 Tianquan-Shoubing tenant/member 管理页面

- Requirements: `REQ-011`, `REQ-016`
- Dependencies: Step 7
- Baseline state: Tianquan-Shoubing Web无`/tenants` route/nav/types/page。
- Observable outcome: `tianquan-shoubing:tenant:read/manage`驱动tenant list/detail drawer、create/update、membership list/upsert和完整UI
  states。
- End state: Tianquan-Shoubing tenant Web tests/typecheck/build GREEN。
...
```typescript
query tenant list under tianquan-shoubing:tenant:read, render table and detail drawer, and expose create or update only under tianquan-shoubing:tenant:manage
...
add one navigation and overview capability entry guarded by tianquan-shoubing:tenant:read plus a deterministic breadcrumb label
...
- Verification contribution: full Tianquan-Shoubing Web route/build regression。
- After this file: Tianquan-Shoubing Web tenant/member management complete。
...
- Completion criteria: Tianquan-Shoubing Web成为tenant/member human management入口。
...
- Commit: `feat(tianquan-shoubing-web): manage tenants and memberships`
...
- Baseline state: RBAC Web仍注册`/iam/tenants`、tenant.api和target-context page；bootstrap seed含system:tenant:
  read/manage且无tianquan-shoubing:tenant permissions。
...
- Execution evidence: File 1 将 landing regression 从退役 `/iam/tenants` 改为稳定
  ...
  `tianquan-shoubing:tenant:read/manage` 与既有 `tianquan-shoubing:oauth-client:read/create/update`。
...
- Symbols: route resolver assertions；`tianquan-shoubing:tenant:read/manage`，existing `tianquan-shoubing:oauth-client:*`，remove system tenant
...
- Input/output and state mapping: bootstrap permissions→Tianquan-Shoubing Admin nav/API authorization；RBAC target permission remains
  fortrusted xingyuan operations。
- Error and edge behavior: no available route falls backexisting authorized landing/overview；no permission alias or
  silent migration。
- Implementation pseudocode:

```typescript
assert resolveApplicationLanding never returns /iam/tenants and routes/resources contain no tianquan-jianshen-tenants component key
assert the xingyuan topology retains tianquan-shoubing:oauth-client:read/create/update exactly and adds tianquan-shoubing:tenant:read plus tianquan-shoubing:tenant:manage
...
```typescript
delete TenantListPage, TenantPages.test and tenant.api after Step 16 makes the Tianquan-Shoubing page the only catalog management UI
...
#### File 4 —
`MODIFY egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/domain/Rbac3DevelopmentTopology.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaPlatformAdminBootstrapRepository.java`

- Purpose: 同步development/xingyuan admin permission definitions到新owner/既有Client codes。
- Symbols: permission sets remove read/manage，keep target，add Tianquan-Shoubing tenant read/manage，retain OAuth client
  read/create/update。
- Repository evidence: both files hardcodeplatform permission sets；controller/Web exact codes由Spec DEC-006。
- Dependencies and consumers: bootstrap Step11、Tianquan-Shoubing Admin authorization through RBAC snapshots。
...
retain system:tenant:target because TenantContextResolver still guards explicit xingyuan target operations
add tianquan-shoubing:tenant:read and tianquan-shoubing:tenant:manage to the same xingyuan administrator role that already owns Tianquan-Shoubing administration permissions
preserve tianquan-shoubing:oauth-client:read, create and update exactly and rely on idempotent bootstrap fact reconciliation
```

- Verification contribution: topology test GREEN；Tianquan-Shoubing Web permissions available。
...
- Completion criteria: RBAC Web无tenant owner，Tianquan-Shoubing tenant与existingClient permissions可由平台管理员获得。
- Rollback: revert四组paths only beforebackend/V8 cutover；after cutover old page不可用且不得单独恢复。
- Commit paths: `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/app/App.integration.test.tsx; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/service/Rbac3DevelopmentTopologyTest.java`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/tenant/TenantListPage.tsx; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/tenant/TenantPages.test.tsx; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/tenant/tenant.api.ts; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/governance.routes.tsx; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/app/resourceDefinitions.json; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/shared/FeatureApi.tsx`,   `egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/domain/Rbac3DevelopmentTopology.java; egon-cola-xingyuan/egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/main/java/top/egon/cola/platform/tianquan-jianshen/admin/bootstrap/repository/jpa/JpaPlatformAdminBootstrapRepository.java`
- Commit: `refactor(tianquan-jianshen-web): remove tenant catalog administration`
...
- Symbols: subcommands `export-tianquan-jianshen`, `import-tianquan-shoubing`, `verify-tianquan-shoubing`, `verify-rbac`, `report`；versioned JSON/TSV
  artifact+SHA-256。
- Repository evidence: existing `scripts/unified-xingyuan/lib/common.sh`、fixture/test shell conventions；Spec §16.3 exact
  sequence。
- Dependencies and consumers: `psql`、explicit Tianquan-Shoubing/RBAC DSNs、maintenance operator；V8 gate report。
...
- Contract/signature changes: newoperator CLI only；credentials从env/secret source且不输出；no DB cross-connection
  inFlyway。
- Input/output and state mapping: ordered RBAC tenants/users→artifact schema/counts/status counts/checksum；artifact→Tianquan-Shoubing
...
```bash
export-tianquan-jianshen reads ordered tenant catalog and identitySub membership rows, writes versioned restrictive-permission artifacts and computes counts plus SHA-256
import-tianquan-shoubing verifies schema version and checksum, then upserts tenants/memberships transactionally without changing ids and records deterministic progress
verify-tianquan-shoubing and verify-rbac query duplicate, orphan, placeholder, grant and FK invariants and refuse to emit a PASS report on any mismatch
...
- Repository evidence: existing unified identity/Tianshu/RBAC READMEs/runbooks；Spec §16。
...
document V5, artifact import/gates, administrator Secret creation/distribution, Token/RPC smoke, Tianshu caller/admin cutover and V8/bootstrap order
list stable metrics/log dimensions and post-deploy checks for auth failures, token latency, Tianshu leases, membership RPC and policyVersion
...
- Purpose: 修改三个既有 module READMEs，记录 AppID/AppKey/Secret/Spring Client、tenant/Tianshu/RBAC ownership boundaries。
- Symbols: standard biz configuration、Tianquan-Shoubing tenant authority、Tianshu direct Token、RBAC authorization-state summaries。
...
```text
update the Tianquan-Shoubing README with administrator-provisioned appId, client_id and one-time Secret plus Spring OAuth2 Client boundaries
update the Tianshu README for direct Tianshu-audience PLATFORM SERVICE tokens and retained anti-replay or lease binding without Ticket terminology
...
#### File 4 —
`MODIFY scripts/unified-xingyuan/verify-local-stack.sh; scripts/unified-xingyuan/prepare-local-stack.sh; scripts/unified-xingyuan/fixtures/unified-xingyuan-release.json`

- Purpose: 更新本地fixture/verification为Secret/Spring Client、Tianquan-Shoubing tenant/RPC、Tianshu Token和RBAC external ID语义，并加入global
  deletion assertions。
...
- Input/output and state mapping: env→standard Spring registration/service-client；smoke→Token claims/RPC/Tianshu
...
static verification rejects private_key_jwt, Client JWK, Admission RPC/Ticket, RBAC tenant CRUD/member resolve, rbac3UserId and tianquan-shoubing:client permission symbols
when the user has started the stack, live verification decodes TENANT/PLATFORM tokens, checks Tianquan-Shoubing membership RPC and asserts Tianshu lease identity/expiry behavior
release fixture records Tianquan-Shoubing/RBAC/Tianshu compatible versions and exact tianquan-shoubing:oauth-client plus tianquan-shoubing:tenant permission contracts
```
...
- Execution state: `Committed` at `772df2b2`; File 1 shell contract first failed because the executable tool was absent,
  ...
  operator cutover/restore runbook; File 3 updated Tianquan-Shoubing, Tianshu, and RBAC ownership/configuration boundaries; File 4 added
  `--static-only`, moved the live tenant probe to Tianquan-Shoubing, and versioned the release fixture with Tianquan-Shoubing tenant and PLATFORM
...
- Validation correction: Maven reactor selection uses the three executable Admin module paths (Tianquan-Shoubing Admin, Tianquan-Jianshen Admin,
  and Tianshu Admin) with `-am`; the reliable selector is `-pl :egon-cola-tianquan-shoubing-admin,:egon-cola-tianquan-jianshen-admin,:egon-cola-tianshu-admin`.
- Validation evidence: shell contract, affected-reactor `-DskipTests` compilation, Tianquan-Shoubing focused OAuth tests (26), Tianshu
...
|--------------------------|---------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|-------------------------------------------------------------------|------------------------|---------------------------------------------------|
| RED Step 1               | repo root                       | Step 1 selector before V5                                                                                                                                                           | Tianquan-Shoubing migration                    | fails only for absent V5/schema                                   | Step 1 File 1          | `REQ-002/003/008/011/014/015/018`; Testcontainers |
| GREEN Step 1             | repo root                       | `./mvnw -B -ntp -pl egon-cola-xingyuan/egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin -am -Dtest=IdpClientTenantV5MigrationTest -Dsurefire.failIfNoSpecifiedTests=false test` | Tianquan-Shoubing V5                           | exit 0, constraint/checksum assertions pass                       | Step 1 File 2          | migration rehearsal, not production               |
| GREEN Steps 2–4          | repo root                       | each Step exact Maven selector                                                                                                                                                      | Tianquan-Shoubing core/admin/yuheng           | exit 0, Basic/context/Secret/USER regression pass                 | owning file            | module/static                                     |
| GREEN Step 5             | repo root                       | Step 5 exact Maven selector                                                                                                                                                         | tianquan-shoubing-starter                      | exit 0, manager/key/autoconfig/legacy failure pass                | Step 5                 | module/static                                     |
| GREEN Steps 6–8          | repo root                       | each Step exact Maven selector                                                                                                                                                      | Tianquan-Shoubing tenant/RPC                   | exit 0, local authority/API/RPC pass                              | owning Step            | module/Testcontainers where selected              |
...
| GREEN Steps 13–14        | repo root                       | each Step exact Maven selector                                                                                                                                                      | Tianshu caller/admin                 | exit 0, Token/lease/replay/source mapping pass                    | owning Step            | module/Redis fakes or component                   |
| Tianquan-Shoubing frontend             | Tianquan-Shoubing admin-web                   | `npm test && npm run lint && npm run typecheck && npm run build`                                                                                                                    | Client/Secret/tenant/Resource UI | all scripts exit 0; no persisted Secret/JWK UI                    | Steps 15–16            | component/static bundle                           |
...
| USER compatibility       | repo root + Tianquan-Shoubing/Yuheng modules | selected Authorization Code/TokenFacade/JWKS/Yuheng tests from existing suites                                                                                                     | `REQ-017`                        | exit 0; USER claim/cookie/refresh/signing behavior identical      | Steps 2/4/18           | module/static                                     |
...
| Live topology smoke      | user-started stack              | runbook Token/RPC/Tianshu/Yuheng/browser steps                                                                                                                                         | Tianquan-Shoubing/RBAC/Tianshu/Redis/Yuheng/Web   | decoded claims, membership gate, lease binding, UI flows observed | owning Step            | user-controlled runtime/manual                    |
...
1. Complete Steps 1–8 and publish compatible Tianquan-Shoubing core/admin/starter/RPC artifacts; no V5 environment deployment occurs
...
2. Complete Steps 9–12 and prove RBAC V8/state/gate/removal against fixtures; no V8 deployment occurs before the Step 18
   external report.
3. Complete Steps 13–17 so Tianshu caller/admin and both Web bundles form compatible cutover artifacts.
...
1. Freeze Client/tenant/member/role writes; verify every private_key/JWK/Admission/RBAC membership consumer is
   inventoried; take and test readable Tianquan-Shoubing/RBAC backups.
2. Stop old Tianquan-Shoubing, Tianshu registration callers and SERVICE consumers; apply only Tianquan-Shoubing V5.
3. Execute `migrate-tenant-authority.sh export-tianquan-jianshen`, verify artifact checksum/version, import into Tianquan-Shoubing, then require
   zero placeholder/orphan/duplicate and exact ID/status/settings/membership counts.
4. Start new Tianquan-Shoubing in maintenance network; provision appId/App Key/one-time Secret, TENANT grants and Tianshu PLATFORM grant;
   deploy Secrets through the approved Secret Manager; incomplete clients remain DISABLED.
5. Deploy PLATFORM-aware starter/Yuheng/resource consumers; verify Basic Token Endpoint, metadata, TENANT/PLATFORM
   claims and RPC-001 without opening business traffic.
6. Deploy Tianshu caller/admin together; register/heartbeat with registrationToken and verify no Admission traffic plus
...
8. Restore traffic/writes; monitor authentication failure/latency, token cache/renewal, Tianshu lease/source mismatch,
...
|-------------------------------------------------|------------------------------------|-----------------------------------------------------------------------------------------|
| Old Tianquan-Shoubing + new Secret consumer                   | No                                 | new consumer needs Basic Token Endpoint; inventory/maintenance order prevents this pair |
| New Tianquan-Shoubing + old private_key consumer              | No                                 | deterministic `invalid_client`; no dual track                                           |
| New Tianquan-Shoubing provider + old IdentityDirectory client | Yes                                | existing batch method unchanged; additive membership method ignored                     |
| Old Tianquan-Shoubing provider + new RBAC membership client   | Only before RBAC writer activation | method unavailable causes fail closed; provider deploys first                           |
| Old Tianshu caller + new Tianshu admin or reverse       | No                                 | Ticket and registrationToken contracts are intentionally incompatible; deploy one unit  |
| New Tianquan-Shoubing V5 + old Tianquan-Shoubing binary                     | No                                 | V5 drops JWK/TTL; old binary remains stopped                                            |
| New RBAC V8 + old RBAC binary/Web               | No                                 | tenant table/routes removed; deploy one unit                                            |
| Old USER OAuth/Web + new identity backend       | Yes within `REQ-017`               | USER JWT/Cookie/JWKS/Yuheng contract unchanged                                         |

### 9.4 Rollback and forward-fix boundaries

- Before V5: cancel window; no data change.
- After V5 but before valid import/Secret distribution: restore Tianquan-Shoubing snapshot and all old binaries/configs. Never edit V5
  or repair Flyway history.
- After valid import but before V8: either restore Tianquan-Shoubing snapshot and old unit or retain the additive authority data and
...
- After V8: rollback is coordinated restoration of both Tianquan-Shoubing/RBAC snapshots plus all old Tianquan-Shoubing/RBAC/Tianshu/Yuheng
...
- Tianshu retains legacy physical Redis/SQL field names, enabling compatible data reading during whole-unit rollback; this
...
| `REQ-005`   | §4/INTERNAL-001        | 5,13                  | starter facade/Tianshu runtime                   | facade/autoconfig/runtime              | standard manager path                        |
| `REQ-006`   | §4/INTERNAL-001        | 5                     | request/key/single-flight                    | concurrency/key matrix                 | no cross-context Token reuse                 |
| `REQ-007`   | §4/SERVICE claims      | 2,4,14                | core/signer/verifier/Tianshu identity            | claim/verifier tests                   | all required claims enforced                 |
| `REQ-008`   | §4/context rules       | 1,2,4                 | V5 Grant/core policy/Token                   | context/tid matrix                     | server-derived TENANT/PLATFORM               |
| `REQ-009`   | §4/INTERNAL-002        | 13,14                 | Tianshu caller/verifier                          | runtime/component                      | direct Tianshu SERVICE Token, no Ticket          |
...
| `REQ-011`   | §4/tenant authority    | 1,6,7,8,16            | V5/service/API/local port/RPC/Web            | tenant/backend/frontend                | Tianquan-Shoubing sole authority                           |
...
| `REQ-015`   | §4/§11                 | 1,9,18                | only V5/V8/static checks                     | checksum/migration gates               | immutable history, no Tianshu migration          |
...
| `REQ-017`   | §4/unchanged boundary  | 2,4,10,18             | USER core/signer/Yuheng regression          | existing USER/JWKS/Yuheng suites      | USER/session/signing behavior unchanged      |
...
| `RISK-006` | Tianshu physical admission字段形成短期命名债               | 14                   | Spec `ASM-004`                     | Tianshu maintainer      | Accepted; explicit Java mapping, no schema migration in this scope                                               |
...
Plan覆盖AppID/App Key/Secret管理员预配、hash-only/one-time、Spring Security OAuth2 Client、无应用自注册、Tianquan-Shoubing tenant
authority、RBAC authorization-only、Tianshu direct SERVICE Token和用户确认的权限/port/bootstrap `1A/2A/3A`。没有恢复Admission
...
计划包含focused RED/GREEN、Tianquan-Shoubing/RBAC/Tianshu affected reactors、protobuf generation、V5/V8 Testcontainers rehearsal、两套frontend
test/lint/typecheck/build、shell artifact contract、legacy deletion/config gate、USER/JWKS/Yuheng regression、maintenance
rollout与phase-specific rollback。静态/模块证据不等于真实PostgreSQL数据、Redis/Tianshu/Yuheng topology、Secret Manager、browser
