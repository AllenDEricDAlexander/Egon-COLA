...
| Plan | [双 Engine 实施计划](../plan/2026-09-02-21-03-gateway-dual-engine-separation-implementation.md) |
| Primary Spec | [单 Admin、双 Engine 规格](../spec/2026-09-02-19-52-gateway-dual-engine-separation.md) |

本文件保留执行预检发现。用户已明确同意本修正案；修正后的顺序和版本权威已回写主 Spec/Plan，生产实现按修正后的文件执行。

## 已确认的执行授权

用户于 2026-09-05 确认按 `egon-coding-executing-plan` 执行本计划，并明确要求拆分完成后启动全量 platforms、使用浏览器测试及修复测试问题，重点包含 RBAC3 角色分配权限接口、Gateway OpenAPI 3 接口清单页面。

因此后续启动本地 platforms、浏览器验证和复现问题后的范围内修复已经获得授权，不需要再次确认运行权限。每 Step 单独验证、路径限定提交的要求继续生效。推送、远程部署和发布制品未包含在本次授权内。
...
### MODEL-001：Spec 将 DDC Version 错误归为 Snapshot 字段

当前 `gateway-contract/.../rule/GatewayRuleSnapshot.java:12` 的全部字段为：
...
它不存在数字 `ruleVersion`。`ruleSchemaVersion` 是字符串 Schema 版本，不能作为 DDC 发布版本。

当前 `GatewayRuleActivationApplier.apply(String key, String value, long version)` 接收真实 DDC Version；`compiler.compile(snapshot)` 只接收 Snapshot；`successStatus(prepared, version)` 将 DDC Version 写入 `GatewayRuleRuntimeStatus.activeDdcVersion`。`restoreLkg()` 当前明确使用 `successStatus(prepared, 0)`，表示脱离 DDC 的恢复状态。`GatewayEngineConfiguration.gatewayRuntimeMetadata()` 从 status 读取版本，且 `activeRuleChecksum` 使用 `artifactSha256`。
...
**建议修正：保留现有版本权威和 compiler 签名，数字 DDC Version 只属于 Activation Runtime Status。**
...
- Admin/DDC 仍校验各在线角色的 releaseId、`status.activeDdcVersion()`、Artifact SHA 和 ACK。
- Snapshot/Artifact JSON、DDC active key、LKG 文件格式、公开 Admin/MCP/API/RPC 契约不变。
- LKG 恢复继续保留 `version=0`、degraded 语义，不伪造正数版本；DDC 正常应用后才更新真实版本与 ACK。
...
必须增加/保留的验证：同一 Snapshot 编译确定性、DDC Apply 版本传递、LKG 恢复为 0/degraded、相同 release 不同 DDC version 的一致性、Artifact SHA 映射、无 Snapshot wire 变化。

### SCOPE-001：Step 2 提交范围漏掉实际生产引用方

Step 2 File 3 要求更新所有 API Engine imports，但 Commit paths 只列 common 生产目录、Engine 测试和 POM，未列实际引用 common 的其他生产文件。例如 `bootstrap/config/GatewayEngineConfiguration.java`、`bootstrap/lifecycle/GatewayEngineRuntime.java`、HTTP/RPC/Operation/Rule 的消费者均需要同步改 import。`gateway-test-suite/.../mcp/McpHaRecoveryIT.java:22` 也直接导入旧 `ProviderDirectory`。
...
| platforms 启动/浏览器测试/问题修复 | 未开始；按用户要求在拆分完成后执行 |
...
同一技能 Entry conditions 要求遇到 effective Spec conflict 停在实施之前。本次不是缺少启动服务或浏览器权限；需要确认的是交换两个 Step，以及将错误的 compiled DTO version 要求修正为已有 DDC Runtime Status 权威。
