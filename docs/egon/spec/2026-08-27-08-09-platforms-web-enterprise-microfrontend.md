# xingyuan Admin Web 企业级平台与微前端需求架构规格

| Field | Value |
| --- | --- |
| Document | docs/egon/spec/2026-08-27-08-09-xingyuan-web-enterprise-microfrontend.md |
| Template Version | 6 |
| Status | Review |
| Type | Architecture |
| Complexity | Complex |
...
| Scope | egon-cola-xingyuan 下的 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu Admin Web、admin-web-shared 与 Wujie Portal；后端接口缺口继续登记为后续契约输入，未来 Java 实现采用传统三层，本轮不进入 Java 实现 |
...
| Source Requirement | 用户确认继续梳理 xingyuan Web 企业级方案，要求左侧若依风格菜单、逐页文字布局/UI 设计、后端缺失接口清单，并提出考虑 wijie 微前端框架 |
| Baseline Revision | main@95039c0b；工作区包含用户既有文档修改、未跟踪 Spec/Plan 和 egon-cola-archetype-web-open，均不属于本 Spec 变更 |
| Amends | None |
| Supersedes | None |
| Depends On | [Tianshu Admin 全量分页查询与前端现代化设计](../../superpowers/specs/2026-08-10-tianshu-admin-pagination-ui-modernization-design.md) §1-§10；本 Spec 不改变其已确认的分页、兼容和不改数据库边界 |
| Related Specs | [Tianquan-Shoubing & Tianquan-Jianshen 前端企业级优化设计](../../../egon-cola-xingyuan/docs/superpowers/specs/2026-08-04-tianquan-shoubing-tianquan-jianshen-frontend-optimization-design.md)、[Yuheng Admin Web 设计](../../superpowers/specs/2026-07-25-yuheng-admin-web-design.md)、[Yuheng Admin Web 企业级前端重构设计](../../../egon-cola-xingyuan/egon-cola-yuheng/docs/superpowers/specs/2026-07-31-admin-web-enterprise-redesign.md)、[Tianshu Admin Web 设计](../../superpowers/specs/2026-07-28-tianshu-admin-web-design.md) |
| Related Plans | [xingyuan Web/Wujie 混合宿主实施计划](../plan/2026-08-27-11-13-xingyuan-web-enterprise-implementation.md) |

## 1. Summary

当前 xingyuan 已经不是从零开始的后台系统：四个 Admin Web 都使用 React 19、TypeScript、Vite、Ant Design、TanStack Query，并依赖共享包提供企业级 Layout、主题、鉴权客户端、页面状态和 i18n。共享 Layout 已经提供左侧树菜单、折叠、移动端抽屉和最长路径高亮，因此首要工作是补齐业务页面闭环、接口契约闭环和跨平台一致性，而不是重新实现一套若依菜单。
...
- 平台管理员：在 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 之间切换，确认平台健康、授权和配置状态。
- 身份管理员：维护用户、租户、OAuth Client、Resource Server、签名密钥和安全审计。
- 权限管理员：维护用户目录、组织、岗位、应用资源、角色、角色分配、授权策略和模拟。
- 网关运维人员：维护 Yuheng Group、路由策略、发布、Provider、MCP 和调用观测。
- 配置运维人员：维护 Tianshu 元数据、配置版本、发布任务、缓存和服务注册。
...
| EVD-005 | Static repository | egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin-web/src/app/AdminLayout.tsx:24 | Tianquan-Shoubing 通过权限过滤后将身份目录、OAuth 与资源、安全治理导航交给 shared | Tianquan-Shoubing 的业务导航应保留自治，宿主只编排入口 | 当前页面权限多为读权限，写操作细化仍需补强 |
| EVD-006 | Static repository | egon-cola-tianquan-shoubing/egon-cola-tianquan-shoubing-admin-web/src/app/router.tsx:43 | Tianquan-Shoubing 已有 overview、users、clients、tenants、resource-grants、resource-servers、keys、audits 路由 | 页面设计可以从真实路由开始 | 未启动运行时验证深链部署 |
| EVD-007 | Static repository | egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/app/resourceDefinitions.json:2 | Tianquan-Jianshen 资源定义包含 MENU、ROUTE、ACTION、FIELD，并有隐藏权限、角色资源和角色任职路由 | Tianquan-Jianshen 当前是资源注册表驱动，不宜直接复制若依数据库菜单模型 | 当前资源注册表消费和后端运行时闭环需联调确认 |
...
| EVD-009 | Static repository | egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/role/role.api.ts:70 | 角色列表和影响分析前端调用路径缺少 /iam | 与后端 RoleController 的 /api/tianquan-jianshen/v1/iam/roles 不一致 | 静态路径不一致，实际代理重写仍需运行时确认 |
| EVD-010 | Static repository | egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin-web/src/features/constraint/constraint.api.ts:45 | 约束前端调用 /sod-sets、/data-rules 等路径 | 与后端 ConstraintController 的 /api/tianquan-jianshen/v1/iam/policies 不一致 | 同上 |
| EVD-011 | Static repository | egon-cola-yuheng/yuheng-admin-web/src/layouts/AdminLayout.tsx:36 | Yuheng 已有总览、网关治理、MCP、观测与审计左侧导航 | Yuheng 信息架构已有基础，应补齐页面状态和操作闭环 | 不代表所有操作按钮已连接后端 |
...
| EVD-013 | Static repository | egon-cola-tianshu/egon-cola-tianshu-admin-web/src/App.tsx:14 | Tianshu 已有 registry、configs、bizs、envs、apps、namespaces、publish-tasks、cache 路由 | Tianshu 页面范围真实存在，但缺少总览和审计入口 | 不代表后端所有集合已经分页或有真实总数 |
| EVD-014 | Static repository | egon-cola-tianshu/egon-cola-tianshu-admin-web/src/auth/AuthContext.tsx:23 | Tianshu 前端只用 TIANSHU_READ 判断 authorized，未发现 TIANSHU_WRITE、TIANSHU_PUBLISH、TIANSHU_CACHE 的按钮级判断 | 后端权限边界已有，前端体验需要细化 | 不等同于后端权限绕过 |
| EVD-015 | Static repository | egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/model/entity/DdcOperationLogEntity.java:15 | Tianshu 已持久化 ddc_operation_log，包含作用域、资源、操作类型、操作者、IP、内容和时间 | 可优先复用现有日志数据补审计查询，不先新增表 | 未检查现场数据量、索引和执行计划 |
| EVD-016 | Static repository | egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin/repository/DdcOperationLogRepository.java:8 | 当前 Repository 只有按 bizCode、env、appCode 的 List 查询 | Tianshu 审计缺少面向管理端的分页/过滤读取边界 | 源码未证明无其他隐藏查询实现，已搜索当前 admin 模块 |
| EVD-017 | Static repository | egon-cola-xingyuan/pom.xml:19 | xingyuan Maven 父工程包含 Tianshu、Yuheng、Tianquan-Jianshen、Tianquan-Shoubing 四个后端平台模块 | 四个平台是同一仓库下的协同控制面 | 不证明部署时一定是同一进程 |
...
| EVD-020 | Static predecessor | docs/superpowers/specs/2026-08-10-tianshu-admin-pagination-ui-modernization-design.md:20 | Tianshu 既有设计确认新增 /page、保留 List/RPC、复用 PageResultRecord、不改数据库和不自动启动服务 | 本 Spec 不得重新改写 Tianshu 分页基础设计 | 该文档状态为等待书面规格复核，需以用户最终批准为准 |
| EVD-021 | Static predecessor | docs/superpowers/specs/2026-07-25-yuheng-admin-web-design.md:14 | Yuheng 既有设计规定前端不直接访问 Tianshu、Redis、Kafka、Engine，并要求发布、ACK、重试、回滚可审计 | 宿主和页面不能绕过 Yuheng Admin API | 设计证据不等于生产运行闭环 |
| EVD-022 | Static architecture | egon-cola-tianquan-jianshen/egon-cola-tianquan-jianshen-admin/src/test/java/top/egon/cola/platform/tianquan-jianshen/admin/architecture/AdminLayerBoundaryTest.java | Tianquan-Jianshen Admin 当前按功能域组织 controller/domain/service/repository，并显式禁止旧技术根目录 | 当前 Web Plan 不改 Java；后续新增 Java API 统一采用已确认的传统 biz.controller/service/service.impl/dao/config/utils/domain 结构，不在既有 feature-first 模块内混入新层 | 现有 Java 历史结构不因本 Spec 自动迁移，后续若要迁移必须另立 Spec |
| EVD-023 | Static validation | four Admin Web npm typecheck commands on 2026-08-26 | Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu typecheck 通过；shared 因本地缺少 tsc 未执行 | 当前 TypeScript 类型基线部分可用 | 未运行本次 Spec 相关的前端实现测试或浏览器验收 |
...
2. 业务页面层：Tianquan-Jianshen 的用户/组织/岗位/角色/策略页面存在职责错配或只读；Tianquan-Shoubing Grant 缺少查询和删除体验；Yuheng 缺少 Trace 详情与 Operation 生命周期操作；Tianshu 缺少审计和配置校验/Diff。
...
| --- | --- | --- | --- | --- | --- |
| 用户进入 Admin Web | 独立 Vite main.tsx -> I18nProvider -> AdminThemeProvider -> 平台 App/Router -> 平台 AdminLayout -> shared EnterpriseLayout/Sidebar | 当前路由、登录 bootstrap、前端 Query Cache | Yuheng Auth、对应 Admin API | 四个平台 Web | EVD-001、EVD-005、EVD-011、EVD-013、EVD-018 |
...
| Tianquan-Shoubing Grant 管理 | Grant Page -> Resource Server List -> PUT Grant；后端 Grant Controller 同时存在 DELETE 和 Batch | Client Resource Grant | Tianquan-Shoubing PostgreSQL、授权边界 | Tianquan-Shoubing 管理员 | Tianquan-Shoubing Grant Page、ClientResourceGrantController |
| Tianquan-Jianshen 角色/策略管理 | 角色/策略页面 -> FeatureApiClient -> 路径调用；后端 RoleController/ConstraintController 在 /iam 下 | 角色图、约束策略、版本/审计 | Tianquan-Jianshen Admin、PostgreSQL、运行时快照 | 权限管理员 | EVD-007、EVD-009、EVD-010 |
| Tianshu 配置管理 | ConfigsPage -> /configs/page、/versions/page、publish、rollback -> DdcConfigController -> Service/Repository/Redis | 配置、版本、发布任务、缓存 | PostgreSQL、Redis、发布链路 | 配置运维人员 | EVD-013、DdcConfigController、Tianshu predecessor |
| Tianshu 审计查询候选 | 操作写入 -> DdcOperationLogEntity/Repository；当前无独立 Admin Audit Controller -> 未来分页查询 | ddc_operation_log | PostgreSQL | 审计/支持人员 | EVD-015、EVD-016；缺少读取入口 |
| Yuheng Trace 观测 | TracesPage -> Yuheng API -> /observability/traces -> GatewayObservabilityController -> QueryService/存储 | Trace 摘要、状态、Provider/Engine 标识 | Yuheng observability store | Yuheng 运维人员 | EVD-012、EVD-021 |
| 发布/配置失败恢复 | 页面 Mutation -> 409/422/5xx 或异步状态 -> 页面保留输入/刷新/重试/回滚 | Draft Revision、Release、PublishTask 状态 | Yuheng Engine、Tianshu、Redis、事件边界 | 运维/支持人员 | EVD-020、EVD-021；运行态未在本次启动验证 |

当前链路的证据边界：上述“当前调用链”来自源码和既有设计；本 Spec 没有启动 Java 服务、Redis、PostgreSQL、Yuheng、浏览器或微前端宿主，因此不能把源码路径一致性表述为生产路由已验证。
...
- G-008：保持 Tianshu 既有 /page 分页设计的兼容边界，前端不把分页扩散为 RPC/Starter 契约。
...
- 本 Spec 不修改 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 已有业务 API、RPC、JWT、Redis、Kafka、租约或发布语义。
- 本 Spec 不直接修改数据库和 Flyway 文件；Tianshu 审计优先复用现有 ddc_operation_log，数据索引/迁移另行审批。
...
- 本 Spec 不让浏览器直接访问 PostgreSQL、Redis、Kafka、Engine、Tianshu RPC 或内部机器凭据。
...
| 四个平台导航和逐页页面设计 | Affected | Tianquan-Shoubing AdminLayout/router、Tianquan-Jianshen resourceDefinitions/governance routes、Yuheng AdminLayout/App、Tianshu AdminLayout/App | 目标是页面职责清晰、权限可见、独立深链可用 | 详细设计菜单、页面布局、状态和验收 | §7, §8, §12, §14, §15, §16 |
...
| 后端现有 Controller、Service、Repository、RPC、数据库 | Context-only | Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Admin Controller 和既有 Specs | 业务所有权、权限、租户、RPC、数据存储保持不变；候选缺口只登记不落实现 | 只记录边界、缺口、契约决策点和验证要求 | §7, §9, §11, §15, §16 |
| Backend API 缺口候选表 | Context-only | Tianshu OperationLog Entity/Repository、Tianquan-Shoubing Grant Controller、Yuheng Observability/Catalog Controller、Tianquan-Jianshen Controller | 区分后端已存在、前端未接入和真正新增/扩展候选 | 保留后续契约清单；本轮不生成 API-* 详细合同，避免把未审查的 Java 变更混入 Web Plan | §7, §9, §15, §16, §18 |
| 现有分页、List、RPC 兼容策略 | Unchanged | Tianshu 分页 Spec §1-§10、DdcConfigController | 保留旧 List/Catalog/Snapshot/RPC，新增分页不污染机器契约 | 简述 preserved invariant 和回归边界 | §9, §11, §16 |
...
| 既有后端业务功能和机器协议 | Unchanged | Tianshu/Yuheng/Tianquan-Jianshen/Tianquan-Shoubing 既有服务、RPC 和 Starter | 不因 UI 重构改变业务语义 | 只记录 unchanged boundary | §7, §9, §11, §15, §16 |
...
| REQ-003 | 每个当前页面必须有可审查的文字布局/UI 设计 | Must | Spec 的 §12 为 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 每个路由给出页面结构、主要操作、数据/权限、空态和错误态 | 用户明确要求逐页说明 |
| REQ-004 | 页面必须使用 URL、Query Cache 和服务端数据作为状态来源 | Must | 刷新可恢复深链和筛选；列表使用服务端分页；Mutation 后精确失效；旧请求不能覆盖新筛选 | 既有 shared/React Query；Tianshu 分页 predecessor |
...
| REQ-007 | 宿主与子应用不得共享或转发明文 Token、Secret、Cookie 内容 | Must | 子应用通过同源认证/bootstrap 获取授权上下文；宿主桥接只传路由/作用域/能力摘要，不传敏感凭据 | 既有 Yuheng Auth 和安全边界 |
| REQ-008 | 后端缺口必须区分“新增/扩展接口”和“后端已有但前端未接入” | Must | §9 表格逐项给出平台、路径、现状、候选动作、所有者和是否需要新契约；不把已有接口标成缺失 | 用户要求后端缺失清单 |
| REQ-009 | Tianquan-Shoubing 管理页面必须支持可规模化的列表和 Grant 闭环 | Should | 用户、Client、Resource Server 支持服务端查询/分页；Grant 可查询当前状态、保存和删除；审计支持筛选 | Tianquan-Shoubing 当前全量 List、Grant 页面证据 |
| REQ-010 | Tianquan-Jianshen 目录、角色、权限、策略页面必须与后端路径和业务职责一致 | Must | 组织/岗位不再展示用户 ID 查询页；角色/策略请求路径与 Controller 一致；用户、权限、角色分配和策略有明确读写入口 | EVD-008/EVD-009/EVD-010 |
| REQ-011 | Yuheng 必须支持可审计的目录、发布和观测体验 | Should | Operation 详情显示来源/定义/生命周期；Release 可看 Diff；Trace 可进入安全脱敏详情；不直连 Engine/Tianshu/Redis/Kafka | EVD-011/EVD-012/EVD-021 |
| REQ-012 | Tianshu 必须支持配置、发布、缓存、服务注册和审计的管理闭环 | Should | 页面展示真实分页总数、发布状态、缓存检查和审计；配置编辑有校验/Diff 入口；沿用既有 /page 兼容策略 | EVD-013/EVD-015/EVD-016/EVD-020 |
| REQ-013 | 关键异常必须按认证、权限、校验、冲突、超时、未知结果和服务错误区分 | Must | 页面分别显示登录恢复、403、字段错误、409 冲突、可控重试、未知发布结果和 Trace ID | 既有 Yuheng/Tianquan-Shoubing/Tianquan-Jianshen 错误约束 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| S-001 宿主进入普通管理页 | 平台管理员打开宿主并点击 Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu 菜单 | 宿主 Manifest 可用、Cookie 有效、子应用版本受支持 | 宿主解析路由 -> 挂载子应用 -> 子应用 bootstrap -> 权限剪枝 -> 页面查询 -> 渲染 | Manifest 超时、子应用加载失败或版本不兼容时显示错误页并允许独立入口/重试 | 只产生前端挂载状态和 Query Cache，不写业务事实 | 目标页面显示正确菜单、标题、作用域和数据 | REQ-001, REQ-002, REQ-005 |
...
| S-004 Tianquan-Shoubing Grant 查询和变更 | 身份管理员在 Client 详情打开 Grant | Client 和 Resource Server 可见，拥有 Grant manage 权限 | 加载当前 Grant -> 选择资源 -> 编辑 -> 保存/删除 -> 精确刷新 | Grant 已被并发修改时 409；删除后刷新；列表为空显示授权 CTA | Grant 状态在 Tianquan-Shoubing 服务端变化 | 当前状态、版本、结果和审计可见 | REQ-009, REQ-014 |
| S-005 Tianquan-Jianshen 组织/角色/策略管理 | 权限管理员进入组织、岗位、角色或策略 | 正确 tenant，具备对应 read/manage 权限 | 页面加载对应列表/树 -> 新增/编辑 -> 服务端校验 -> 刷新 | 当前代码路径错误或后端拒绝时显示错误；版本冲突不覆盖本地输入 | Tianquan-Jianshen 管理事实改变，运行时快照按既有机制更新 | 页面职责和 Controller 路径一致 | REQ-010, REQ-013, REQ-014 |
| S-006 Yuheng Draft 发布 | Yuheng 运维人员编辑 Route/Policy 并发布 | 当前 Draft Revision、操作权限、目标 Group 可用 | 编辑 -> 校验 -> Diff -> 发布 -> 轮询 Target ACK | 422 字段错误、409 Revision、超时/UNKNOWN/部分失败 -> 保留证据并提供 Retry/Rollback | Draft/Release/Target 状态按既有服务改变 | 不把 UNKNOWN/部分成功显示为绿色成功 | REQ-011, REQ-013, REQ-014 |
| S-007 Tianshu 配置发布和回滚 | 配置运维人员编辑 YAML | 作用域完整、TIANSHU_WRITE/PUBLISH 权限、配置格式可校验 | 保存 -> 校验 -> 版本 -> 发布任务 -> 查询状态 -> 必要时回滚 | YAML 无效、发布失败、任务超时；Retry 只对允许状态显示 | 配置版本、发布任务、缓存/目标状态改变 | 显示版本、changeId、状态和 Trace | REQ-012, REQ-013, REQ-014 |
...
| S-009 子应用局部失败 | 宿主中 Yuheng 子应用 API 5xx 或前端运行异常 | 其他子应用仍健康 | 子应用 PageState 显示局部错误、Retry；宿主菜单和其他子应用保持可用 | 子应用挂载异常由 Error Boundary 隔离；宿主提供重新挂载/独立打开 | 仅前端 mounted/error 状态变化 | 不白屏、不影响其他子应用 | REQ-005, REQ-013, REQ-015 |
...
| ACTOR-002 | 身份管理员 | 管理用户、租户、OAuth Client、Resource Server、Grant、密钥和 Tianquan-Shoubing 审计 | Tianquan-Shoubing 页面 | Tianquan-Shoubing capability/tenant context | Tianquan-Shoubing AdminLayout/router |
| ACTOR-003 | 权限管理员 | 管理 Tianquan-Jianshen 用户目录、组织、岗位、角色、资源、策略和模拟 | Tianquan-Jianshen 页面 | Tianquan-Jianshen permission/tenant context | Tianquan-Jianshen resourceDefinitions/governance routes |
| ACTOR-004 | 网关运维人员 | 管理 Yuheng Group、目录、Draft、Release、Provider、MCP、Trace | Yuheng 页面 | Yuheng capability、biz/app/env/namespace scope | Yuheng AdminLayout/App |
| ACTOR-005 | 配置运维人员 | 管理 Tianshu 配置、元数据、发布任务、注册、缓存和审计 | Tianshu 页面 | TIANSHU_READ/WRITE/PUBLISH/CACHE 和 scope | Tianshu AuthContext、安全配置和页面 |
| ACTOR-006 | 审计/支持人员 | 检索变更和失败证据并触发受控恢复 | Audit/详情/Retry/Rollback 页面 | 只读审计或显式恢复权限 | Yuheng/Tianshu/Tianquan-Shoubing/Tianquan-Jianshen audit boundaries |
...
flowchart LR
    ...
    Auth["Yuheng Auth / Tianquan-Shoubing"]
    Tianquan-Shoubing["Tianquan-Shoubing Admin"]
    RBAC["Tianquan-Jianshen Admin"]
    GW["Yuheng Admin"]
    Tianshu["Tianshu Admin"]

    subgraph Scope["xingyuan Admin Web 控制面"]
        UC001(["UC-001 统一导航与平台切换"])
        UC002(["UC-002 管理身份与租户"])
        UC003(["UC-003 管理 Tianquan-Jianshen 权限"])
    ...
    A6 -->|"证据检索/恢复"| UC006
    UC001 --> Host
    UC002 --> Tianquan-Shoubing
    UC003 --> RBAC
    UC004 --> GW
    UC005 --> Tianshu
    UC006 --> Tianquan-Shoubing
    UC006 --> RBAC
    UC006 --> GW
    UC006 --> Tianshu
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UC-001 | 统一导航与平台切换 | ACTOR-001 | ACTOR-007、Yuheng Auth | 点击菜单或平台切换 | 认证有效；宿主/独立入口可用 | 进入目标平台页面并保持作用域/权限一致 | 子应用加载失败、版本不兼容、401/403 | 失败不影响其他平台，独立入口仍可用 | REQ-001, REQ-002, REQ-005 | Portal/各 AdminLayout | TEST-001, TEST-002, TEST-003 |
| UC-002 | 管理身份与租户 | ACTOR-002 | Tianquan-Shoubing Admin API、审计 | 新增/编辑/授权/撤销 | Tianquan-Shoubing tenant 与 capability 正确 | 事实保存、状态刷新、敏感值按规则一次展示 | 422、409、网络失败、Grant 不存在 | Tianquan-Shoubing 数据和审计保持服务端权威 | REQ-009, REQ-013, REQ-014 | Tianquan-Shoubing users/tenants/clients/grants/audits | TEST-004, TEST-005 |
| UC-003 | 管理 Tianquan-Jianshen 权限 | ACTOR-003 | Tianquan-Jianshen Admin、运行时快照 | 维护用户/组织/岗位/角色/策略 | tenant、读写权限、版本有效 | 页面职责正确，修改通过既有 Tianquan-Jianshen 运行时链路 | 路径 404、403、约束失败、版本冲突 | 管理事实提交，运行时按既有快照机制异步/原子更新 | REQ-010, REQ-013, REQ-014 | Tianquan-Jianshen directory/roles/policies | TEST-006, TEST-007 |
| UC-004 | 管理网关与发布 | ACTOR-004 | Yuheng Admin、Engine/Provider 投影 | 编辑 Draft 或发布 | scope、revision、权限、目标可用 | 校验、Diff、Release、ACK 状态完整可见 | 409、422、TIMEOUT、UNKNOWN、部分失败 | Release 证据可重试/回滚，历史不被覆盖 | REQ-011, REQ-013, REQ-014 | Yuheng groups/draft/releases/traces | TEST-008, TEST-009 |
| UC-005 | 管理配置与注册 | ACTOR-005 | Tianshu Admin、PostgreSQL、Redis | 编辑/发布/检查/重建 | scope 完整、capability 正确 | 配置版本和发布状态可追踪 | YAML 错误、发布任务失败、缓存不一致 | Tianshu 既有版本/发布/缓存语义不变 | REQ-012, REQ-013, REQ-014 | Tianshu configs/tasks/cache/registry | TEST-010, TEST-011 |
...
- 四个平台的后端领域所有权不合并：Tianquan-Shoubing 负责身份/租户，Tianquan-Jianshen 负责授权管理与运行时授权状态，Yuheng 负责网关控制面，Tianshu 负责配置和注册控制面。
- 浏览器不直接访问 Tianshu、Redis、Kafka、Engine 或内部 RPC；平台页面继续调用对应 Admin API。
- Tianshu 已确认的分页策略是新增 /page、保留 List/Catalog/Snapshot/RPC，不新增数据库表或迁移；本 Spec 不修改该边界。
- 既有工作区修改和未跟踪文件必须保持不动。
...
| ASM-003 | 菜单权威仍由各平台的静态资源定义/权限过滤提供 | shared 类型明确平台注入 navigation；Tianquan-Jianshen 有 MENU/ROUTE/ACTION/FIELD；DEC-103 | 不新建菜单数据库、不改权限所有权 | 若未来要求运行时动态菜单，需新增后端资源模型、版本和缓存 |
...
| ASM-005 | Tianshu 审计第一候选读取既有 ddc_operation_log，不先添加表 | DdcOperationLogEntity 和 Flyway 表已存在 | 查询层可独立回退，无 schema 变更 | 现场数据量/索引不足时需单独 DB Spec |
...
| --- | --- | --- | --- | --- |
| DEC-001 | 继续使用 shared EnterpriseLayout 作为四个平台共同壳的基线 | User / xingyuan owner | 用户确认此前总体方向；EVD-001/EVD-002 | REQ-001, REQ-002 |
| DEC-002 | 页面设计必须覆盖当前路由和目标补齐页面，而非只设计菜单树 | User / xingyuan owner | 用户明确要求每个页面通过文字说明布局和 UI | REQ-003 |
| DEC-003 | 后端清单区分新增/扩展与已有未接入，不重复制造已有 Tianquan-Jianshen/Yuheng/Tianshu 能力 | Spec author based on repository evidence | EVD-008 至 EVD-016；最小设计原则 | REQ-004, REQ-008 |
...
| --- | --- | --- | --- | --- | --- | --- |
| DEC-101 | “wijie”按 Wujie/无界处理 | User / xingyuan owner | 用户回答“1 是”；EVD-004、EVD-025 | Portal 依赖与生命周期按 Wujie React wrapper 设计，版本先锁定 2.1.0 并经过兼容性 spike | REQ-005/006 | Closed |
| DEC-102 | 采用混合宿主：Wujie 统一入口，四个子应用仍可独立访问、构建和回滚 | User / xingyuan owner | 用户回答“2 是”；保留独立入口能降低宿主失败的回滚成本 | 新增 Portal、Manifest、mount/unmount/ErrorBoundary；不删除原入口 | REQ-005/015 | Closed |
| DEC-103 | 菜单采用静态/配置驱动的业务路由定义，结合 bootstrap capability 裁剪；不先做数据库动态菜单 | User / xingyuan owner | 用户回答“3 是”；EVD-007、EVD-014；当前资源注册表已表达 MENU/ROUTE/ACTION/FIELD | Portal 只做平台级切换，各子应用继续拥有菜单和按钮权限；不新增菜单表/API/缓存 | REQ-002/005 | Closed |
| DEC-104 | 宿主与子应用采用同源/同站点 Cookie + CSRF；宿主不读取或转发明文凭据 | User / xingyuan owner and security owner | 用户回答“4 是”；EVD-004、各平台 auth client；避免跨域 token relay | 子应用各自 bootstrap；bridge 仅传 route/scope/capability summary；部署需保证 API Base 与 Cookie/CSRF 同站点 | REQ-006/007/015 | Closed |
| DEC-105 | 统一首页提供只读跨平台摘要 Facade；不扩展为全页面通用 BFF | User / xingyuan owner | 用户回答“5 是”；首页需要统一 loading、partial、timeout 和按平台重试 | Portal 只消费摘要边界；业务页面仍直连所属 Admin API；summary 具体后端接口仍留在 §9 后续契约清单 | REQ-001/004/008 | Closed |
...
| Frontend state | React Query + Context | 各页面 useQuery/useMutation，Tianshu QueryClientProvider | 不新增 Redux/Zustand；客户端状态仅保存 UI 临时状态 |
| Backend | Java 21、Spring Boot 3.5.16、Maven | egon-cola-xingyuan/pom.xml | 后端候选接口保持各 Admin 模块所有权 |
| Persistence | PostgreSQL/JPA/Flyway，Tianshu 另有 Redis 和 RPC 聚合 | 各 Admin POM、Tianshu OperationLog、既有 Specs | 本 Spec 不改表；查询缺口先复用现有数据 |
| Security | Yuheng Auth、HttpOnly Cookie/CSRF/bootstrap、平台 capability | shared gatewayAuthClient、各 AuthContext、Tianshu security | 宿主不读取 Token/Secret；后端继续最终鉴权 |
| API error | 各平台现有错误包装和权限边界 | Yuheng/Tianquan-Shoubing/Tianquan-Jianshen/Tianshu Controller 与既有 Specs | UI 需区分 401/403/409/422/5xx；不重写全局错误体系 |
...
- Tianquan-Shoubing 当前使用 feature-first 包，例如 admin/identity/controller、service、repo、domain。
- Tianquan-Jianshen 当前使用 admin/iam、authorization、audit 等功能域，并由 AdminLayerBoundaryTest 约束。
- Yuheng 当前使用 component/yuheng/admin/{application,catalog,credential,group,...} 功能域。
- Tianshu 当前使用 component/tianshu/admin/controller、model、repository、service 等功能域。
...
| HTTP/Auth | browser fetch、Context | None | shared gatewayAuthClient/httpClient | 无 gap | Keep shared/xingyuan clients |
| Server state/cache | React Query | None | existing package dependency | 无 gap | Keep React Query |
| Tianshu pagination | None | Spring Data Page | existing PageResultRecord/PageQuery and Tianshu Admin support | 无 gap for existing pagination | Keep predecessor design |
| Tianshu audit read | Spring Data JPA Pageable | spring-boot-starter-data-jpa | existing DdcOperationLogEntity/Repository | 缺少 Admin query/controller，不是缺少 ORM | Add only a bounded read path after approval |
| Unified home aggregation | direct browser calls | None | existing Admin APIs | direct calls cause inconsistent loading/partial failures for a single cross-xingyuan summary | Add the approved read-only summary Facade; business pages remain direct |
...
| DB changes for Tianshu audit | Candidate | REQ-008/012 | 复用 ddc_operation_log 查询 | 当前只是缺少读取 API，尚无数据/索引证据证明必须改表 | 迁移、锁、兼容和回滚成本 | Remove from this Spec |
...
- shared：负责公共视觉与 Layout 组件，不拥有 Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu 业务菜单。
- Tianquan-Shoubing 子应用：身份、租户、OAuth、Resource Server、密钥和 Tianquan-Shoubing 审计。
- Tianquan-Jianshen 子应用：用户目录、组织/岗位、应用资源、角色、权限、策略、运行时和 Tianquan-Jianshen 审计。
- Yuheng 子应用：Yuheng Group、目录、Draft、Release、Provider、MCP、Trace 和 Yuheng 审计。
- Tianshu 子应用：配置、版本、发布任务、注册、缓存、元数据、作用域绑定和 Tianshu 审计。
...
flowchart LR
    ...
    subgraph Browser["Browser / same-site deployment candidate"]
        ...
        IDPWeb["Tianquan-Shoubing Admin Web"]
        RBACWeb["Tianquan-Jianshen Admin Web"]
        GWWeb["Yuheng Admin Web"]
        DDCWeb["Tianshu Admin Web"]
    end

    subgraph ControlPlane["Existing backend control planes"]
        IDPAPI["Tianquan-Shoubing Admin API"]
        RBACAPI["Tianquan-Jianshen Admin API"]
        GWAPI["Yuheng Admin API"]
        DDCAPI["Tianshu Admin API"]
        Auth["Yuheng Auth / Tianquan-Shoubing bootstrap"]
...
Boundary rule: Host does not call xingyuan business APIs on behalf of children. The approved `PlatformSummaryFacade` is a host-owned, read-only orchestration boundary for homepage aggregate cards only; it may call only approved read-only Admin API adapters and must expose per-xingyuan partial/error state. A child does not call another child’s API directly. Navigation may cross xingyuan boundaries, but business data and permission decisions remain in the owning Admin service.

#### 7.1.2 Boundary and responsibility table

| Module/component | Capability and data owned | Inputs/outputs | Allowed dependencies | Forbidden responsibility | Requirements |
| --- | --- | --- | --- | --- | --- |
| Portal | Host route, child manifest, mount state, xingyuan switch | route intent, manifest, non-sensitive context; emits navigation/mount status | shared shell, wujie-react@2.1.0 after spike | business CRUD, Token/Secret, backend authority | REQ-005/006 |
| admin-web-shared | Layout, theme, page state, HTTP/auth primitives | navigation config, user/actions, children | React, AntD, Query/Router peer dependencies | xingyuan menu, tenant policy, business API | REQ-001/002/003 |
| Tianquan-Shoubing Admin Web | identity/tenant/OAuth UI and calls | Tianquan-Shoubing routes and Admin API | shared, Tianquan-Shoubing API | Tianquan-Jianshen role policy decisions, direct DB | REQ-009 |
| Tianquan-Jianshen Admin Web | authorization management UI and calls | Tianquan-Jianshen route/resource definitions and Admin API | shared, Tianquan-Jianshen SDK/API | Tianquan-Shoubing tenant master data, direct runtime snapshot writes | REQ-010 |
| Yuheng Admin Web | yuheng control-plane UI and calls | scope, Draft, Release, observability API | shared, Yuheng Admin API | direct Tianshu/Redis/Kafka/Engine access | REQ-011 |
| Tianshu Admin Web | configuration/register/cache UI and calls | Tianshu scope and Admin API | shared, Tianshu Admin API | direct Redis/PostgreSQL/RPC access | REQ-012 |
| Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Admin API | owning business fact and authorization | authenticated principal, tenant/scope, request data; returns existing wrappers | own services/repositories and approved adapters | cross-domain menu ownership or browser orchestration | REQ-008/010/011/012 |
| Summary Facade | unified home read-only aggregate of xingyuan health | authenticated xingyuan scope; returns per-xingyuan result/partial/error/trace | four Admin APIs or xingyuan read adapters | mutating domain facts, replacing xingyuan APIs | REQ-001/004 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 导航一致性 | 四平台有共同尺寸/交互，业务树不互相污染 | shared Layout + xingyuan-provided navigation | 菜单资源缺失时隐藏无效节点；页面仍可独立打开 | shared 版本需要兼容管理 | shared component tests + visual/manual review | REQ-001/002 |
...
| 审计/隐私 | 操作可追踪，不泄露 Secret/Body/Cookie | xingyuan audit + redacted details + trace | 详情不可用时仍展示摘要和 Trace | 详情接口/脱敏需后端配合 | sensitive-field tests + runtime review | REQ-008/013/014 |
...
| 4 | Child -> Auth bootstrap | existing xingyuan auth client | Cookie/CSRF/session -> identity/permissions/scope | Query cache stores bootstrap, no Token relay | 401 login; 403 denied | REQ-002/007 |
| 5 | Child -> owning Admin API | existing route or separately approved additive API candidate | URL/query/body -> xingyuan wrapper/page/state | server authoritative data and audit | 422 field; 409 conflict; 5xx retry rule | REQ-008/009/010/011/012 |
...
| 7 | Host -> summary Facade | DEC-105 closed | tenant/scope -> aggregate cards with per-xingyuan partial results | read-only aggregate cache | per-xingyuan timeout -> partial card | REQ-001/004 |
| 8 | User/support -> recovery command | existing Retry/Rollback or approved contract | stable resource/version/idempotency -> operation result | xingyuan-owned state transition + audit | duplicate/conflict/unknown -> reconcile | REQ-013/014/015 |
...
sequenceDiagram
    ...
    participant Auth as Yuheng Auth/Bootstrap
    participant API as Owning Admin API
    participant Store as Platform Store/Projection

    Admin->>Host: Open xingyuan route
    alt Wujie host mode
        Host->>Manifest: Load xingyuan manifest and version
    ...
    alt Session invalid
        ...
        alt Query success
            ...
            Child-->>Admin: Retry/reconcile/rollback according to xingyuan
        end
    end
```
...
| --- | --- | --- | --- | --- | --- | --- |
| Tianquan-Shoubing user/Client/Grant | Tianquan-Shoubing Admin Service and existing database/service logic | Preserve existing version/authorization contract | UI disables duplicate; backend remains authoritative | Existing service commit | Existing Tianquan-Shoubing error wrapper | REQ-009/014 |
| Tianquan-Jianshen role/assignment/policy | Tianquan-Jianshen Admin and existing role/policy transaction/runtime projection | Preserve existing version, graph validation and snapshot path | stale version -> 409/conflict; no client-side merge | Existing Tianquan-Jianshen commit/projection | UI retains local edits; runtime follows existing recovery | REQ-010/013 |
| Yuheng Draft/Release | Yuheng Admin service | Existing Draft Revision, Release/Target ACK/idempotency rules | duplicate publish uses existing identity; UNKNOWN requires reconcile | Release/Target state from Yuheng | never render UNKNOWN/partial as success | REQ-011/014 |
| Tianshu config/publish | Tianshu Admin service and existing Page/Publish/Cache paths | Preserve config version/changeId/publish task semantics | retry only according to existing task state; no blind duplicate | Tianshu config/version/task commit | show task failure/unknown and support retry/rollback | REQ-012/014 |
| Host child mount | Host runtime, not business database | lifecycle state machine in frontend only | repeated mount/unmount must clean listeners and cache references | mounted only after child ready signal | local MOUNT_FAILED; no xingyuan data write | REQ-005/015 |
| Optional summary | Summary Facade, read-only | request timeout/deadline per child; no cross-service transaction | partial child results represented as partial, not cached as global success | aggregate response assembled | partial cards and retry per xingyuan | REQ-001/004 |

No new database transaction is selected in this Spec. New Admin API implementations must not claim atomicity across Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu; cross-xingyuan summary is read-only and partial by design.
...
| 403 | bootstrap/API response | stop request or hide action | no business change | no retry until permission changes | denied page/action hidden | owning xingyuan security | permission test |
...
| 409 version/idempotency | API conflict code | preserve local form and fetch current version if allowed | other writer may be committed | user-controlled reload/merge/retry with identity | conflict Drawer; no overwrite | owning xingyuan service | concurrent test |
| timeout/5xx before known commit | transport error/trace | preserve input; only retry if contract permits | committed state unknown | same operation identity or read-after-write reconcile | retry/reconcile state | xingyuan service/support | retry/unknown test |
| asynchronous partial failure | Release/PublishTask status | poll current resource, stop false success | partial/unknown remains evidence | xingyuan-defined retry/rollback only | explicit PARTIAL/UNKNOWN status | Yuheng/Tianshu owner | state matrix test |
...
| page query trace | child HTTP client/API | xingyuan, routeKey, operation, status, traceId, latency | no Cookie/Authorization/body | error ratio/latency per xingyuan | open Trace/Support link | existing client/static; runtime needed |
...
| stale/partial state | Yuheng/Tianshu service + child UI | release/changeId, target, status, observedAt, stale | no provider credential | UNKNOWN/TIMEOUT/PARTIAL never green | retry/rollback/reconcile | existing xingyuan contract |
...
| optional summary partial | summary Facade | xingyuan result, duration, partial flag, traceId | only aggregate counts/status | any child timeout marks partial | card-level retry | Facade integration |
...
| 后端缺口先复用已有资源路径并单独补最小读取能力 | Tianshu OperationLog 已持久化但无 Admin query；Tianquan-Shoubing/Yuheng/Tianquan-Jianshen 多个写/读接口已有 | REQ-008/009/011/012；避免把已有接口重复建模 | Extend existing list contracts where only filtering/paging is missing; Add only independent audit/detail/validation goals | 减少 API 数量；新增接口仍有权限、分页、错误和运维成本 | contract inventory review + per-xingyuan integration |
...
宿主不复制业务页面；子应用仍需保留 standalone entry，并通过 Wujie 生命周期暴露 embedded mount/unmount。统一首页的 `PlatformSummaryFacade` 只编排各 Admin API 的只读摘要，后端 `GET /api/v1/xingyuan/console/summary` 仍是后续可选服务端优化，不是本轮必须新增的公共接口。
...
| Modify | each Admin Web/src/app or layouts | xingyuan AdminLayout/router | 平台业务导航、页面标题、permission pruning、embedded/standalone mode | shared + xingyuan API | REQ-001/002/003/005 |
...
| Create | xingyuan-admin-portal/src/manifest | Manifest loader/types | 加载子应用版本/入口/compatibility，不加载业务数据 | wujie-react@2.1.0 | REQ-005/006 |
| Create | xingyuan-admin-portal/src/lifecycle | Mount boundary/child error | 子应用 mount/unmount/error isolation | Wujie React wrapper | REQ-005/015 |
| Create | xingyuan-admin-portal/src/bridge | non-sensitive host-child context | 路由、scope display、xingyuan switch event；不传 Token/Secret | Wujie props/bus; browser events only for non-sensitive context | REQ-007 |
| Create | xingyuan-admin-portal/src/summary | PlatformSummaryFacade | 统一首页只读卡片、超时、partial/error 和按平台重试 | existing Admin API read adapters | REQ-001/004 |
...
Scope disposition: Context-only for backend implementation. 本 Spec 的后端接口表仍是候选缺口登记，不改变既有接口；但它是本轮 Web Plan 的正式消费边界和后续后端契约输入。DEC-105 已确认统一首页需要 Facade，本轮由 Portal 内的 `PlatformSummaryFacade` 编排已有只读 Admin API 摘要并表达 partial/error；服务端 `GET /api/v1/xingyuan/console/summary` 保留为后续性能/权限证据成立后的独立 API，不在本轮生成 API-* 公共合同或 Java 文件。真正进入后端实现 Plan 前，应对被批准的接口逐个创建 Interface ID，并按 interface-contract-design 展开完整请求、响应、错误、权限、租户、分页、幂等和测试契约。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P1 | Tianshu | GET /api/v1/tianshu/audits/page | New | ddc_operation_log 已存在，但当前没有 Audit Controller/分页读取入口 | 审计人员独立检索配置变更 | Tianshu Admin | Candidate; reuse existing table, no migration in this Spec |
| P1 | Tianshu | GET /api/v1/tianshu/audits/{id} | New | Entity 有 id、content、operator、createdAt，但无详情 Controller | 查看单条脱敏操作证据 | Tianshu Admin | Candidate; detail must redact operationContent |
| P1 | Tianshu | POST /api/v1/tianshu/configs/{id}/validate | New | DdcConfigController 当前有 list/page/create/update/publish/versions/rollback，无独立 validate | 保存/发布前显示结构和业务校验结果 | Tianshu Admin | Candidate; verify service can derive config identity |
| P1 | Tianshu | GET /api/v1/tianshu/configs/{id}/diff?fromVersion=&toVersion= | New | 当前有版本列表和回滚，但未发现版本 Diff Admin route | 运维独立比较版本，避免直接回滚试错 | Tianshu Admin | Candidate; large diff must be bounded |
| P1 | Tianshu | GET /api/v1/tianshu/dashboard | New | Tianshu App 当前首页跳转 registry，无聚合 dashboard endpoint | 一屏观察配置、注册、发布、缓存 | Tianshu Admin or optional Portal Facade | Candidate; depends on DEC-105 |
| P1 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/clients/{clientId}/resources | New | Grant Controller 当前有 PUT、DELETE、batch，无 list/get Grant | Client 详情独立展示当前授权状态 | Tianquan-Shoubing Admin | Candidate; command must revalidate Grant state |
| P1 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/users with page/size/query/status | Extend | IdentityUserController 当前返回 List<IdentityUserVO>；前端全量加载 | 企业用户规模下服务端查询 | Tianquan-Shoubing Admin | Candidate extension; prefer existing path |
| P1 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/clients with page/size/query/status | Extend | OAuthClientController 当前返回 List<OAuthClientVO>；前端全量加载 | Client 规模化管理 | Tianquan-Shoubing Admin | Candidate extension |
| P1 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/resource-servers with page/size/query/status | Extend | ResourceServerController 当前返回 List，另有 detail/batch | Resource Server 规模化管理 | Tianquan-Shoubing Admin | Candidate extension |
| P1 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/audits with actor/event/result/from/to/traceId | Extend | 当前 Audit Controller 只接 page/size，前端也只有分页 | 安全审计按时间/操作者/结果检索 | Tianquan-Shoubing Admin | Candidate extension |
| P2 | Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/users/{subject}/sessions；DELETE .../sessions/{sessionId} | New | 当前仅有 revoke-all；会话存储和单会话 identity 未在 Admin API 暴露 | 单会话踢出和支持排障 | Tianquan-Shoubing Admin | Open domain decision; do not add without session ownership |
| P1 | Yuheng | GET /api/v1/yuheng/admin/observability/traces/{traceId} | New | Controller 当前提供 dashboard、trace list、audit list；GatewayTraceVO 是摘要 | 追踪一次调用的时间线/尝试/脱敏错误 | Yuheng Admin | Candidate; traceId/eventId granularity must be confirmed |
| P1 | Yuheng | GET /api/v1/yuheng/admin/observability/traces with from/to/group/operation/node/provider | Extend | 当前 trace query 主要 env、namespace、traceId、protocol、statusCategory、page、size | 运维独立按时间和目标组件检索 | Yuheng Admin | Candidate extension |
| P2 | Yuheng | GET /api/v1/yuheng/admin/audit/{eventId} | New | 当前 audit list 有摘要，未发现独立 detail | 查看变更前后值和资源版本 | Yuheng Admin | Candidate; redact secrets/body |
| P2 | Platform Portal | GET /api/v1/xingyuan/console/summary | New, deferred server-side option | 当前没有统一宿主或聚合接口；本轮 Portal 可通过 `PlatformSummaryFacade` 编排已有只读 Admin API | 统一首页需要一次可部分成功的 summary | Portal/Facade owner | Deferred; host-side Facade is the current Web plan |
| P2 | Tianquan-Jianshen | GET/POST/PUT/DELETE /api/tianquan-jianshen/v1/iam/frontend-resources | New | 当前资源定义由 static JSON/registry/CI registration 组成 | 运行时动态菜单/按钮管理 | Tianquan-Jianshen or Platform owner | Remove from first phase; DEC-103 required |
...
| --- | --- | --- | --- |
| Tianquan-Jianshen | GET/POST/PUT/DELETE /api/tianquan-jianshen/v1/iam/users、PUT .../status | UserController | 当前 UserDirectoryPage 只按 userId lookup |
| Tianquan-Jianshen | GET/POST/PUT/DELETE /api/tianquan-jianshen/v1/iam/organizations | OrganizationController | 没有组织树 CRUD 页面，路由错误复用用户页 |
| Tianquan-Jianshen | GET/POST/PUT/DELETE /api/tianquan-jianshen/v1/iam/positions | PositionController | 没有岗位 CRUD 页面，路由错误复用用户页 |
| Tianquan-Jianshen | /api/tianquan-jianshen/v1/iam/users/{userId}/organizations 与 /positions | assignment Controllers | 没有完整成员关系维护页面 |
| Tianquan-Jianshen | /api/tianquan-jianshen/v1/iam/roles 的 create/update/inheritance | RoleController | RoleGraphPage 主要展示列表/影响分析 |
| Tianquan-Jianshen | /api/tianquan-jianshen/v1/iam/permissions | PermissionController | resourceDefinitions 中权限路由隐藏且没有实际组件绑定 |
| Tianquan-Jianshen | /api/tianquan-jianshen/v1/iam/policies 下写接口 | ConstraintController | ConstraintPage 当前主要展示说明/表格，无完整维护动作 |
| Tianquan-Jianshen | POST /api/tianquan-jianshen/v1/internal/directory-snapshots、GET .../directory-snapshots/{snapshotId} | DirectoryController | 前端调用路径不同，快照提交页不在当前主导航闭环 |
| Tianquan-Shoubing | DELETE /api/v1/tianquan-shoubing/clients/{clientId}/resources/{resourceServerId}；POST .../resource-grants/actions/batch | ClientResourceGrantController | Grant 页面只实现 PUT 保存 |
| Tianquan-Shoubing | GET /api/v1/tianquan-shoubing/resource-servers/{resourceServerId}、POST .../actions/batch | ResourceServerController | 当前页面主要是列表和单条启停，批量/详情体验不完整 |
| Yuheng | GET /api/v1/yuheng/admin/releases/{releaseId}/diff | GatewayReleaseController | ReleaseDetailPage 未完整消费 Diff |
| Yuheng | PUT /api/v1/yuheng/admin/operations/{operationId}/metadata | GatewayCatalogController | OperationPage 没有元数据编辑 |
| Yuheng | PUT /api/v1/yuheng/admin/operations/{operationId}/manual-definition | GatewayCatalogController | OperationPage 没有手工定义维护 |
| Yuheng | POST /api/v1/yuheng/admin/operations/{operationId}/deprecate | GatewayCatalogController | OperationPage 没有废弃操作 |
| Tianshu | GET /api/v1/tianshu/instances/page | DdcInstanceController | 后端已有配置客户端实例分页，前端没有独立页面 |
| Tianshu | GET /api/v1/tianshu/namespace-env-app-bindings/page | DdcNamespaceEnvAppBindingController | 当前绑定能力嵌在 Namespace 抽屉，不是独立运营页 |

Tianquan-Jianshen 的角色、策略和目录路径错误属于前端契约修复项，不应新增后端别名来掩盖；后端新增接口则需在用户确认后进入正式接口 Spec。
...
- Tianquan-Shoubing 的 IdentityUserVO、OAuthClientVO、ResourceServerVO、ClientResourceGrantVO 等仍由 Tianquan-Shoubing Admin API 负责。
- Tianquan-Jianshen 的资源、角色、审计、运行时返回类型仍由 Tianquan-Jianshen Admin/SDK 负责。
- Yuheng 的 Operation/Release/Trace VO 仍由 Yuheng Admin API 负责。
- Tianshu 的 DdcConfigVO、DdcConfigVersionVO、DdcPublishResultVO、DdcOperationLogEntity 等仍由 Tianshu Admin 负责。
...
- 基础背景和主色继续使用 shared tokens：primary #2447b8、background #f4f7fb、border #e7eaf0；Yuheng 现有独立 #3157d5 定义应后续统一，不在本 Spec 直接修改。
...
#### 12.2.1 Tianquan-Shoubing Admin Web
...
| 用户 /users | FilterBar：subject/username/displayName/status；SummaryCard 总数；主表：Subject、用户名、显示名、状态、更新时间、操作；右侧 User Drawer | tianquan-shoubing:identity-user:read；创建、编辑、重置密码、撤销全部会话按 action 权限控制 | 当前前端调用全量 List，应迁移服务端分页；一次性密码只在 Modal 展示并提供复制，关闭后不再读取 |
...
| Client Grant /clients/:clientId/resource-grants | 左侧 Client 摘要；主体为 Resource Server/Grant 矩阵表：资源名、Grant Type、Scope、Tenant、状态、版本、更新时间；右侧 Grant Drawer | 保存、删除、批量变更需 tianquan-shoubing:resource-server:grant | 先查询当前 Grant 状态；空授权显示 CTA；删除使用确认；若当前后端缺少查询则按 §9 后续契约接入 |
...
#### 12.2.2 Tianquan-Jianshen Admin Web
...
| 运行状态 /iam/diagnostics/runtime | 运行时健康卡、Mutation/Outbox/Snapshot/Tianshu/Yuheng 一致性；失败列表 | 只读、受控 retry | 状态以 evidence/observedAt 展示；健康不替代实际路由证据 |
...
| Yuheng Group /yuheng-groups | Group 表：code/name/scope/Ready/target release/consistency；FilterBar；创建/启停 | group read/write | 空态 CTA；状态卡显示 target/actual；批量启停需明确权限 |
| Group 概览 /yuheng-groups/:groupId/overview | 顶部 Group 摘要；Engine 节点表；一致性卡；Tabs 跳转 Draft/Releases | 只读 + 受控诊断 | 保留节点 instance/lease/observedAt/stale/ACK；10 秒轮询只在页面可见 |
| Draft Routes /yuheng-groups/:groupId/draft/routes | Draft revision 顶部条；Route 表；右侧 720px Drawer/Steps：基本、Transport、高级与变更原因 | draft write；新增/编辑/删除/批量删除 | 409 展示本地/服务端 Revision；Transport 不可编辑时跳过；未保存离开提示 |
| Draft Policies /yuheng-groups/:groupId/draft/policies | Policy 表按类型筛选；详情 Drawer；风险组合提示 | policy write | 必须先校验；单位和边界明确；不可用组合前端提示、后端最终裁决 |
| 发布历史 /yuheng-groups/:groupId/releases | Release 表：releaseId/revision/status/actor/time/target summary；发布 Drawer | validate、Diff、publish、retry/rollback 入口 | 发布按钮 pending 禁用；UNKNOWN/partial 不显示成功；新 Release 详情保留 |
| Release 详情 /yuheng-groups/:groupId/releases/:releaseId | Release 摘要；Diff Tab；Target ACK/Attempt 时间线；错误脱敏；操作区 | retry、rollback 按 releases write | 后端已有 Diff 需接入；Retry 新 Attempt；Rollback 说明会生成新 Release |
...
#### 12.2.4 Tianshu Admin Web

| 页面/路由 | 文字布局与组件 | 主要动作和权限 | 数据/状态设计 |
| --- | --- | --- | --- |
| Tianshu 总览 /dashboard | ScopeBar；配置总数、发布中/失败、服务注册、缓存不一致摘要；最近变更；快速入口 | TIANSHU_READ | Tianshu 子应用提供本域摘要；跨平台摘要由 Portal Facade 聚合；Tianshu 专属 dashboard API 仍列在 §9 后续接口清单 |
| 服务注册 /registry | Service Catalog 分页表；Service Key、protocol、kind、group、version、serviceId；实例 Drawer | TIANSHU_READ | 服务目录和实例按需加载；不把完整 Snapshot 误称为 DB cursor 分页 |
| 配置资源 /configs | ScopeBar（biz/namespace/env/app）；配置表、版本/状态；编辑 YAML Drawer；右侧版本 Drawer | TIANSHU_READ/WRITE/PUBLISH | 使用既有 /configs/page 和 /versions/page；保存前 validate/Diff 候选；删除/发布确认 |
| 配置版本子视图 /configs/:id/versions | 版本表、创建时间、操作者、checksum、发布状态；版本 Diff 面板 | 只读；回滚按 PUBLISH/WRITE | 不加载无界内容；回滚生成新的结果/任务提示 |
| 业务域 /bizs | FilterBar + 分页表：code/name/enabled/更新时间；编辑 Drawer | TIANSHU_READ/WRITE | 真实总数；删除/禁用确认；影响下游 scope 时提示 |
| 环境 /envs | biz/namespace 级联筛选；环境分页表；状态 Tag | TIANSHU_READ/WRITE | Scope 变化清空下级选择；当前页空态与全局无数据区分 |
| 应用 /apps | biz/env/namespace 筛选；应用表；详情 Drawer | TIANSHU_READ/WRITE | 复用级联选项 List；分页表与选项查询分离 |
| 命名空间/绑定 /namespaces | Namespace 表；点击行打开 Binding Drawer；绑定矩阵可切换为独立运营页 | TIANSHU_READ/WRITE | Binding 用 Select multiple/搜索/虚拟滚动，不使用无界 Checkbox；保存后精确失效 |
| 发布任务 /publish-tasks | Scope/status/changeId FilterBar；任务分页表；详情 Drawer：状态、版本、ACK、错误 | TIANSHU_READ；retry 需 PUBLISH | 保留当前 15 秒轮询，后台错误不重复 toast；只对允许状态显示 Retry |
| 缓存 /cache | 物理 scope 必填；匹配/不匹配摘要；检查结果表；重建按钮 | TIANSHU_CACHE | 当前页检查；重建 Popconfirm；完成后可重新检查；长值 Drawer/ellipsis |
| 配置客户端实例 /instances | scope 筛选；实例表：instanceId/status/lease/lastSeen/config version；详情 Drawer | TIANSHU_READ | 后端已有 /instances/page；前端补独立入口，区分配置客户端和服务注册实例 |
| Tianshu 审计 /audits | 时间/scope/operator/operationType/resourceName/result FilterBar；审计分页表；详情 Drawer | TIANSHU_READ | 后端读取 ddc_operation_log 的分页候选；operationContent 默认脱敏；无数据和无权限分开 |
...
| 创建/编辑 | manage 权限 | 点击主操作 -> Drawer/Modal -> 校验 -> 确认 -> mutation | required/length/enum/cross-field | 201/200、422、409 | 关闭/刷新/定位新记录 | 422 映射字段；409 保留表单 | xingyuan existing commands |
...
| Draft/Policy 编辑 | Draft revision | Steps -> validate -> save | 规则组合、单位、JSON/schema | success/422/409 | 更新 revision、未保存标记清除 | 409 Diff/本地保留 | Yuheng existing |
| 发布/回滚 | publish 权限、目标状态 | validate -> Diff -> reason -> confirm -> publish -> poll | reason、revision、target | Release/Task async | 进入详情、轮询终态 | UNKNOWN/PARTIAL reconcile/retry/rollback | Yuheng/Tianshu existing |
...
| ScopeBar | Tianquan-Shoubing tenant/Yuheng/Tianshu/Tianquan-Jianshen scope pages | choose scope | existing option APIs; URL query | option loading/empty/stale; mutation revalidates | scope read | REQ-004 |
...
| Configuration-driven navigation | 四个平台菜单由各自资源定义和权限决定 | xingyuan AdminLayout/resource registry | 宿主硬编码所有业务菜单会复制权限和业务所有权 | shared 类型要求平台注入 navigation；Tianquan-Jianshen 有 resourceDefinitions |
...
| State | 子应用 mount、Release、PublishTask、PageState 有显式状态 | host lifecycle + existing xingyuan state models | 复杂状态不能用布尔值和硬编码颜色表达 | 既有 Yuheng/Tianshu 状态和 shared PageState |
...
- 信息隐藏：宿主不知道 Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu 业务模型；shared 不知道平台菜单。
...
- 复杂业务状态使用既有平台状态机/State 语义；不在 UI 中复制 Yuheng Release 或 Tianshu PublishTask 的业务状态转换。
...
| TEST-003 | permission navigation | bootstrap with read/no-read/action permissions | menu/button visibility follows capability, not business data | xingyuan Vitest | REQ-002 |
| TEST-004 | Tianquan-Shoubing list/Grant UI | empty/paged/current grant/delete | total, current state, delete and refresh behavior | Tianquan-Shoubing RTL/Vitest | REQ-009 |
| TEST-005 | sensitive Tianquan-Shoubing values | create/rotate/reset response | one-time value displayed once, not stored in host/global state | Tianquan-Shoubing RTL | REQ-007/009 |
| TEST-006 | Tianquan-Jianshen page ownership | organizations/positions/users routes | correct components render and no user lookup on org/position | Tianquan-Jianshen RTL | REQ-010 |
| TEST-007 | Tianquan-Jianshen API paths | role/policy/directory snapshot client calls | exact /iam and /internal paths are asserted | Tianquan-Jianshen API test | REQ-010 |
| TEST-008 | Yuheng release flow | validate/diff/publish/409/UNKNOWN | state labels, conflict retention, no false success | Yuheng RTL/Vitest | REQ-011/013 |
| TEST-009 | Yuheng trace | summary/detail/timeout/redaction | detail lazy loads, no body/credential renders | Yuheng RTL/Vitest | REQ-011/014 |
| TEST-010 | Tianshu page query | page/filter/pageSize/empty/error | query key, URL, total and retry are correct | Tianshu Vitest/RTL | REQ-004/012 |
| TEST-011 | Tianshu mutation/polling | publish/retry/background error | no duplicate toast, current page invalidates, polling bounded | Tianshu Vitest | REQ-012/014 |
| TEST-012 | audit filter | time/operator/resource/result | filter serialization, page reset, empty/403 behavior | xingyuan RTL/API tests | REQ-008/013 |
| TEST-013 | recovery actions | 409/timeout/unknown | retry identity/reconcile/rollback UI does not duplicate unsafe mutation | xingyuan tests | REQ-013/014/015 |
...
| TEST-017 | bridge safety | host sends scope/xingyuan event | no Token/Secret/Cookie/body in event payload | host security test | REQ-007 |
...
- Tianquan-Jianshen integration：验证 users/organizations/positions/roles/policies/directory snapshot 的真实路径和权限；不以 TypeScript typecheck 代替后端路由验证。
- Tianquan-Shoubing integration：验证 Grant list/save/delete 的授权、并发版本和一次性 Secret；审计过滤按 tenant/actor 约束。
- Yuheng integration：验证 Release Diff、Trace detail、Operation metadata/definition/deprecate 的权限、审计和脱敏。
- Tianshu integration：验证 /page 响应、既有 List 兼容、审计 query 读取、配置 validate/diff 候选和 Tianshu capability。
...
- Runtime boundary：真实服务、Yuheng、Tianshu、Redis、PostgreSQL、浏览器启动和生产路由不属于本 Spec 已完成证据，需用户自行启动后按 Runbook 验证。

### 14.3 Test cases and data

| ID | Level | Target | Scenario/input | Expected assertion | Test double/data | Tool/path | Requirements |
| --- | --- | --- | --- | --- | --- | --- | --- |
| TEST-021 | Contract | Tianquan-Shoubing list extension | page=1,size=20,empty/filter | stable page wrapper and empty array | API fixture | Tianquan-Shoubing Admin contract test | REQ-009 |
| TEST-022 | Contract | Tianshu audit candidate | scope/operator/time/page | redacted result and permission error | Tianshu operation log fixture | Tianshu MVC test after approval | REQ-008/012 |
| TEST-023 | Contract | Yuheng trace detail candidate | traceId/eventId not found/redaction | detail/404/no sensitive body | trace fixture | Yuheng MVC test after approval | REQ-011 |
...
| TEST-026 | Regression | Tianshu existing List/RPC | old callers and page callers | old wrappers/RPC unchanged | existing fixtures | Tianshu backend tests | REQ-008/012/015 |
| TEST-027 | Regression | Yuheng direct API boundary | page attempts no direct Redis/Tianshu/Kafka | only Admin API client calls exist | static scan | repository script | REQ-011 |
...
- Tianquan-Shoubing tenant context 来自认证/平台约束；Tianquan-Jianshen 使用有效 tenant；Yuheng/Tianshu 使用各自业务作用域。
...
- Yuheng Release、Tianshu PublishTask 的状态以服务端权威；UNKNOWN/TIMEOUT/PARTIAL 保留证据。
...
- Yuheng 当前独立 ConfigProvider 与 shared token 的差异应在实施中统一；不能长期维护两套主色/背景/圆角定义。
...
- Tianshu 原 List/Catalog/Snapshot/RPC 和既有 /page 设计保持不变。
...
5. 再逐步接入 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu；每个子应用有独立制品和 manifest。
...
| RISK-005 | Tianquan-Jianshen 动态菜单与当前 static resource registry/CI registration 权威冲突 | Medium | 权限显示和发布状态不一致 | DEC-103 已确定静态菜单；不新增 DB menu | Closed |
...
| RISK-008 | Tianshu 审计表数据量和索引无法支撑分页 | Unknown | 审计页面性能或数据库压力 | 先读取现场统计/EXPLAIN；另写 DB Spec | Open |
...
| --- | --- | --- | --- | --- | --- | --- |
| REQ-001 | UC-001 | shared shell/host §7/§12/§13 | existing xingyuan APIs unchanged | EnterpriseLayout/PageHeader; Portal `PlatformSummaryFacade` | TEST-001/002/018/019 | four pages share shell rules and homepage partial cards |
| REQ-002 | UC-001/002/003 | navigation/guards §7/§12/§15 | backend final authorization unchanged | xingyuan navigation/resource registry | TEST-003/007/025 | no unauthorized menu/action |
...
| REQ-004 | UC-001/005/006 | URL/Query state §7/§12/§15 | Tianshu existing pagination preserved | Query keys, PageState, URL scope | TEST-010/011/020/026 | refresh/filter/page behavior |
...
| REQ-009 | UC-002 | Tianquan-Shoubing pages §9/§12/§14 | existing Tianquan-Shoubing auth/service ownership | list/Grant/audit candidates | TEST-004/005/021 | scalable list and Grant closure |
| REQ-010 | UC-003 | Tianquan-Jianshen pages/path correction §7/§9/§12 | Tianquan-Jianshen runtime snapshot unchanged | route/page bindings | TEST-006/007 | organizations/positions/role/policy route alignment |
| REQ-011 | UC-004 | Yuheng pages/API gap §9/§12/§15 | no direct Tianshu/Redis/Kafka/Engine | Trace detail/Operation UI candidates | TEST-008/009/023/027 | publish/trace/catalog auditable |
| REQ-012 | UC-005 | Tianshu pages/audit/API gap §9/§11/§12 | Tianshu List/RPC/Flyway unchanged | audit/validate/diff/dashboard candidates | TEST-010/011/022/026 | config/publish/cache/audit closure |
| REQ-013 | UC-002-006 | failure matrix §7/§12/§15 | xingyuan error wrappers preserved | PageState/error mappings | TEST-002/006/008/009/012/013 | distinct 401/403/409/422/5xx/unknown |
...
- 已覆盖用户要求的 xingyuan Web 范围，按 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 和 shared 进行分析。
...
- shared Layout、四个平台路由、Tianquan-Jianshen 路径问题、Tianquan-Shoubing Grant、Yuheng Trace/Operation、Tianshu OperationLog 均有具体证据。
- 当前只验证源码和 TypeScript 基线，不宣称后端路由、浏览器、数据库、Redis、Yuheng 或生产部署已验证。
...
- Tianshu 分页 predecessor 作为 Depends On，保留其新增 /page、旧 List/RPC 兼容、不改数据库和不自动启动服务的边界。
- Yuheng、Tianquan-Shoubing/Tianquan-Jianshen 和 Tianshu 既有设计作为 Related Specs，不在本 Spec 静默改写其规范性内容。
...
| MC-REUSE-001 | Applicable | PASS | §6 reuse/capability ledger、EVD-001/EVD-003/EVD-018/EVD-020 | 已检查 shared、React Query、Spring/JPA、Tianshu PageResultRecord 和平台模块；无理由重复造能力 | None |
