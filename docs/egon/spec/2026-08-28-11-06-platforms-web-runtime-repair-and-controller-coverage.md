# xingyuan Web 运行修复与 Controller 覆盖补齐规格
...
| Type | `Bugfix / Feature` |
| Complexity | `Complex` |
| Complexity Drivers | Wujie 沙箱生命周期、Portal 运行时清单、四个管理端的 Controller 覆盖、Yuheng 发布版本漂移、Tianshu PostgreSQL 参数类型、跨平台认证与回滚 |
| Created | 2026-08-28 11:06 CST |
| Updated | 2026-08-28 13:00 CST |
| Owner | User / Egon-COLA xingyuan owner |
| Repository | Egon-COLA |
| Scope | `egon-cola-xingyuan` 下的 Portal、admin-web-shared、Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Admin Web、Tianshu Admin 查询与安全配置、本地统一平台启动脚本 |
| Change Surface | Wujie 挂载生命周期、共享侧栏路径保护、四个平台前端路由/API/页面覆盖、Tianshu 分页安全与查询、Portal 运行时地址清单、本地默认地址策略 |
...
| Amends | [平台 Web 企业级与微前端前序规格](2026-08-27-08-09-xingyuan-web-enterprise-microfrontend.md) §2.3、§3.2、§7、§12、§16、§18 |
| Supersedes | None |
| Depends On | [Tianshu Admin 分页设计](../../superpowers/specs/2026-08-10-tianshu-admin-pagination-ui-modernization-design.md) §1-§10 |
| Related Specs | [平台 Web 企业级与微前端前序规格](2026-08-27-08-09-xingyuan-web-enterprise-microfrontend.md) |
| Related Plans | [前序平台 Web 实施计划](../plan/2026-08-27-11-13-xingyuan-web-enterprise-implementation.md)、[本次运行修复实施计划](../plan/2026-08-28-11-06-xingyuan-web-runtime-repair-and-controller-coverage.md) |

## 1. Summary

本次修复针对当前已经运行的 xingyuan 本地栈。源码盘点与运行日志显示，Portal 的子应用宿主节点没有形成可见的 Wujie 子实例，Yuheng 共享侧栏会因无 `path` 的父菜单项在运行时崩溃，Tianshu 的分页请求既有安全匹配缺口又有 PostgreSQL 可空查询参数类型问题；另外 Tianquan-Jianshen、Yuheng、Tianshu 前端仍有后端外部 Controller 已实现但没有路由、API 或页面消费的能力。
...
成功标准：Portal 路由能真实挂载任一子应用，失败时能显示可诊断错误并重试或独立打开；四个独立 Web 仍可直接访问；Yuheng 不因父级菜单没有路径而白屏；Tianshu 所有已有 `/page` Controller 可被授权并能处理空筛选；新增的前端页面/API 与真实 Controller 路径一致；现有动态发布、认证、租户和数据库兼容边界不被改变。
...
| --- | --- | --- | --- | --- | --- |
| EVD-001 | Runtime evidence | `target/local-unified-xingyuan/logs/yuheng-admin-web.log`，2026-08-28 | Yuheng 首屏报 `Cannot read properties of undefined (reading 'endsWith')` | shared 路径匹配必须拒绝非字符串候选值 | 当前现场日志，重启后需复核 |
| EVD-002 | Runtime evidence | Portal `http://127.0.0.1:18125/xingyuan/tianquan-shoubing/overview` DOM 检查，2026-08-28 | Portal Shell 存在，但没有 iframe、Wujie 元素或 `data-wujie-id` | 挂载必须有真实 host、Promise 错误回调和清理逻辑 | DOM 检查不等于所有沙箱内部脚本执行证据 |
...
| EVD-005 | Static repository | `egon-cola-xingyuan-admin-web-shared/src/layout/EnterpriseSidebar.tsx`、Yuheng `src/layouts/AdminLayout.tsx` | `EnterpriseNavigationItem.path` 可选，Yuheng 父项确实无 path；`matchesPath` 却按 string 调用 `endsWith` | 共享路径判断要以运行时类型为边界 | 仅静态证明根因，修复后需重建 shared |
...
| EVD-008 | Static repository | `egon-cola-tianquan-jianshen/.../features/role/role.api.ts` | 后端 RoleController 有创建、更新、继承增删，前端只有列表、影响分析和资源树 | Tianquan-Jianshen 角色管理存在真实未消费能力 | 当前 active Yuheng release 可能仍旧，需区分发布漂移 |
| EVD-009 | Static repository | Tianquan-Jianshen `directory.api.ts`、`constraint.api.ts`、`managementPolicy.api.ts`、`simulation.api.ts` | 用户变更、约束写操作、可管理对象查询、角色变更影响模拟未完整接入 | IAM 二级菜单需要补 API 与操作状态 | Internal Controller 不纳入 Admin Web |
| EVD-010 | Static repository | Yuheng Admin Controller 与 `gatewayApi.ts` | Yuheng 多数 API 已有，但应用详情、OpenAPI 同步可视化、Trace 详情或 Controller 报告存在页面覆盖不足 | 采用已有 API 补齐页面消费，不新增 BFF | 需按真实方法映射验证 |
| EVD-011 | Static repository | Tianshu `DdcAdminSecurityConfiguration.java` | GET matcher 未包含 `/page`，`/configs/page` 会被拒绝 | 安全配置必须覆盖实际公开 Controller 路径 | 不放宽写权限 |
| EVD-012 | Runtime evidence | Tianshu 日志、`Ddc*Repository.search` | 空关键词被绑定成 PostgreSQL `bytea`，触发 `lower(bytea) does not exist` | JPQL 可选字符串参数需用字符串类型安全的 `coalesce` | 需执行 Tianshu 定向测试与运行复核 |
| EVD-013 | Static repository | Tianshu `DdcNamespaceEnvAppBindingController.java`、Tianshu Web `NamespacesPage.tsx` | 后端已有独立分页/增删改接口，前端只有命名空间内嵌绑定编辑，没有独立列表路由 | 增加绑定管理页面并保留原内嵌入口 | 不重复引入数据库表 |
| EVD-014 | Static repository | 四个平台 `src/app`/`src/features`/`src/pages` 与对应 Java Controller | Tianquan-Shoubing 管理端已覆盖主要身份 Controller；Tianquan-Jianshen 与 Tianshu 存在多处外部 Controller 未消费；Yuheng 的 API 层比页面层完整 | 修复必须按“已存在但未组织/未接入”与“真正缺失”分类 | 全量 Controller 计数仍以静态脚本和构建为边界 |
| EVD-015 | Validation evidence | 2026-08-27 四个 Web typecheck、Portal/Tianquan-Jianshen/Yuheng 测试 | Portal 18、Tianquan-Jianshen 31、Yuheng 74 个基线测试通过；Tianquan-Shoubing/Tianshu 并行测试有超时，需单独重跑 | 不能把并行资源竞争当成修复回归 | 本次执行会以定向串行结果为准 |
...
问题不是单一的“路由少”：一部分是路由/页面没有组织到已有能力，一部分是前端请求路径与 Controller 不一致，一部分是后端安全/查询实现阻断了已有页面，还有一部分是本地启动地址策略错误。此次只补充已有外部管理能力和确定性运行缺陷；没有证据的内部 RPC、跨域代理、数据库迁移或 Yuheng active release 发布动作不在默认代码变更内。
...
| Yuheng 首屏 | Yuheng `filterNavigation` -> shared `EnterpriseSidebar.resolveNavigationSelection` -> `matchesPath` | 当前 pathname、导航树 | React Router、Ant Design Menu | Yuheng 管理员 | EVD-001、EVD-005 |
| Tianshu 分页 | Web `ddcPageApi` -> Yuheng/Controller -> SecurityFilterChain -> Service -> JPA Repository | 分页参数、元数据表 | PostgreSQL、Yuheng Cookie | Tianshu 管理员 | EVD-011、EVD-012 |
| Tianquan-Jianshen 角色治理 | RoleGraphPage -> `roleApi` -> RoleController -> role service/repository | 角色、继承、版本、资源授权 | Tianquan-Jianshen Admin、Yuheng release | 权限管理员 | EVD-008 |
| Tianshu 绑定治理 | BindingPage -> binding API -> `DdcNamespaceEnvAppBindingController` -> service/repository | namespace/env/app binding | PostgreSQL、Tianshu scope gate | 配置管理员 | EVD-013 |
...
- G-002：默认本地使用 127.0.0.1；外部访问只由 `UNIFIED_XINGYUAN_ADVERTISED_HOST` 或同名 identity 配置显式决定。
- G-003：修复 shared 侧栏运行时类型边界，保证父级无 path 的 IAM/Yuheng/ Tianshu 树只负责展开。
- G-004：把已有外部 Controller 能力接入对应前端 API、路由、页面或现有详情面板；内部服务 Controller 继续排除。
- G-005：修复 Tianshu `/page` 的读权限和空字符串 JPQL 参数问题，不修改数据库结构及既有 Flyway 文件。
- G-006：保持 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 的业务自治、租户权限和异常隔离；IAM 所有功能仍归于 IAM 顶层菜单。
...
- 不因为 active Yuheng release 落后于当前 draft 就修改数据库发布状态；需要发布时由用户显式执行已有发布命令。
...
| Tianshu Admin security/query | Affected | `DdcAdminSecurityConfiguration.java`、`Ddc*Repository.java` | 放行真实 GET page 路径，字符串空筛选不触发 bytea | 设计安全边界、查询和 Java 规则 | §7, §8, §14, §15, §16 |
...
| Existing public Controller contracts | Context-only | Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Controller annotations | 保留路径、HTTP 方法、权限和 envelope；前端只消费已有合同 | 给出覆盖清单和未接入边界，不新增合同 | §9, §12, §16 |
...
| --- | --- | --- | --- | --- |
| REQ-001 | Portal 子应用路由必须在 host 节点内创建真实 Wujie 实例 | Must | 打开四个 `/xingyuan/{key}/...` 深链时可见子页面；挂载异常有错误码、重试和独立打开入口 | 用户要求修复 Wujie 聚合 |
...
| REQ-003 | Yuheng 和共享侧栏不能因父菜单 path 缺失而崩溃 | Must | `filterNavigation` 生成父级无 path 的树时，路由高亮函数不调用 undefined 方法；Yuheng 首屏正常 | 运行日志 EVD-001 |
| REQ-004 | 前端必须覆盖 Controller 已有的外部管理能力 | Must | 覆盖矩阵中的每项都有 API 方法和页面动作/详情入口，或有证据说明已由现有组件消费 | 用户要求补四个平台功能 |
| REQ-005 | Tianshu 分页读接口必须受正确权限保护并能处理空筛选 | Must | `/configs/page`、元数据/page、任务/page 在 TIANSHU_READ 下可访问；无关键词时 PostgreSQL 查询成功 | Tianshu 运行日志与 Controller |
...
| REQ-009 | 前端不得绕过领域边界访问内部组件 | Must | 浏览器只调用现有 Admin API/Yuheng 代理；不新增 `internal/v1` 页面请求、数据库、Redis 或 RPC 直连 | 前序规格与 Controller 分类 |
| REQ-010 | 运行时发布漂移必须与代码缺失分开报告 | Should | 验证输出同时区分源码无路由、前端未消费、active release 未发布三类结论 | 当前 Yuheng draft/active 证据 |
...
| Yuheng 父级导航 | ACTOR-002 网关管理员 | parent item 无 path、child 有 path | resolve selection 跳过 parent path 并选 child | 非字符串 active prefix 被忽略 | 无 | 首屏不崩溃，菜单可展开 | REQ-003, REQ-006 |
| Tianshu 空筛选分页 | ACTOR-003 配置管理员 | TIANSHU_READ、数据库可用 | `/page` -> security matcher -> repository | 403 显示权限错误；数据库错误显示 trace | 只读 | 页面显示分页或可诊断失败 | REQ-005, REQ-007 |
| 角色变更 | ACTOR-004 权限管理员 | Tianquan-Jianshen manage 权限、版本有效 | 页面提交 PUT/继承/资源授权 | 409 保留输入并刷新版本 | 角色版本/授权变化 | 成功消息与列表刷新 | REQ-004, REQ-006, REQ-008 |
...
| --- | --- | --- | --- | --- | --- |
| ACTOR-001 | 平台管理员 | 跨平台进入、重试和独立打开子应用 | Portal Web | Yuheng USER cookie、租户 bootstrap | Portal Router/Wujie |
| ACTOR-002 | 网关管理员 | 管理 Yuheng Group、Application、路由、发布、Provider、MCP、观测 | Yuheng Admin Web | `yuheng:read` 与领域 capability | Yuheng AdminLayout |
| ACTOR-003 | 配置管理员 | 管理 Tianshu 元数据、配置、发布任务、缓存、绑定 | Tianshu Admin Web | TIANSHU_READ/WRITE/PUBLISH/CACHE | Tianshu AuthContext/SecurityConfiguration |
| ACTOR-004 | 权限管理员 | 管理 IAM 目录、角色、资源、策略和模拟 | Tianquan-Jianshen Admin Web | Tianquan-Jianshen bootstrap permissions | Tianquan-Jianshen resource registry |
| ACTOR-005 | 本地开发者 | 在回环或显式局域网 host 下启动/访问平台 | shell scripts | OS process/config scope | unified-xingyuan scripts |

#### 4.2.2 Use-case artifact

| ID | Use case/goal | Primary actor | Supporting actors/systems | Trigger | Preconditions | Main success outcome | Alternatives/failures | Postconditions | Requirements | Interfaces/pages | Tests |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UC-001 | Portal 挂载平台子应用 | ACTOR-001 | Wujie Core、child Web | 点击左侧平台或深链 | manifest 通过校验 | child 页面 mount | mount reject 可重试或独立打开 | 生命周期可清理 | REQ-001 | Portal `/xingyuan/tianquan-shoubing/overview` 等 | TEST-001 |
| UC-002 | 修复管理端首屏 | ACTOR-002 | shared Layout、React Router | 访问 Yuheng `/dashboard` | 导航树含无 path 父项 | 首屏和高亮正常 | 非法 candidate 被忽略 | 无异常日志 | REQ-003 | Yuheng Dashboard/Layout | TEST-002 |
| UC-003 | 维护 IAM 资源 | ACTOR-004 | Tianquan-Jianshen Controller、Yuheng release | 进入 IAM 二级菜单 | 权限和 active route 可用 | 页面能读写角色/资源/策略 | 版本冲突可重试 | 审计与版本保持 | REQ-004, REQ-006 | Tianquan-Jianshen IAM pages | TEST-003 |
| UC-004 | 处理 Tianshu 元数据与发布 | ACTOR-003 | Tianshu Controller、PostgreSQL、Redis | 进入分页页面 | TIANSHU_READ 或写权限 | 分页、编辑、绑定、发布可用 | 403/56999 有明确状态 | 不绕过权限 | REQ-004, REQ-005 | Tianshu metadata/binding pages | TEST-004 |
...
flowchart LR
    ...
    A3[ACTOR-003 配置管理员] --> U4((UC-004 Tianshu 管理))
    A4[ACTOR-004 权限管理员] --> U3((UC-003 IAM 资源治理))
    A5[ACTOR-005 本地开发者] --> U5((UC-005 稳定本地地址))
    subgraph PlatformScope[xingyuan 管理控制面]
      U1 --> Portal[Portal + Wujie]
      U2 --> Shared[admin-web-shared]
      U3 --> RBAC[Tianquan-Jianshen Admin Web]
      U4 --> Tianshu[Tianshu Admin Web/Admin API]
      U5 --> Scripts[统一启动脚本]
    end
```
...
| ASM-003 | Tianshu JPQL 的 `coalesce(:param, '')` 可由当前 Hibernate/PostgreSQL 组合推断为字符串 | `Ddc*Repository.search` 字段均为文本列 | 可由定向 JPA 测试快速回退到参数规范化方案 | 若 provider 仍不能推断，Step 阻断并改用已验证绑定方式 |
...
| DEC-002 | 地址优先级为显式 `UNIFIED_XINGYUAN_ADVERTISED_HOST`，否则 127.0.0.1；Portal Manifest 是 child 地址唯一运行时来源 | User / implementation owner | 回环地址不受 Wi-Fi 变化影响，显式覆盖支持跨设备 | REQ-002, REQ-010 |
| DEC-003 | IAM 只保留一个顶层树节点，新增能力归并到治理/授权/资源目录/诊断子树 | User / implementation owner | Tianquan-Jianshen `resourceDefinitions.json` 已有 parentCode 机制 | REQ-004, REQ-006 |
| DEC-004 | Controller 覆盖优先补前端；仅 Tianshu 已证实的安全/查询缺陷修改后端，排除 migration | User / implementation owner | 区分“没写 UI”和“后端坏了”，控制变更面 | REQ-004, REQ-005, REQ-009 |
...
| Backend | Spring Boot、Spring Security、Spring Data JPA | Tianshu Admin Controller/Security/Repository | 只修已有三层边界，不增加新层 |
| Data migration | Flyway `classpath:db` | xingyuan/各模块 migration 目录 | 本次不新增或修改 migration |
| Validation | Vitest、Testing Library、TypeScript、Maven/JUnit | 各 Web scripts 与 Tianshu tests | 每步先定向 RED 再 GREEN，运行现场单独标注 |

### 6.1 Java architecture profile and capability baseline

本次后端改动只触及 Tianshu 现有 Controller 安全配置和 Repository 查询，选择传统三层语义作为新增/修改边界：Controller -> Service -> Repository/DAO；不在既有模块旁边创建第三种包树。历史 Tianshu 包名为 `controller/service/repository`，属于既有 feature-first 例外，本次不迁移。

| Architecture profile | Archetype/template or base package | Exact evidence and verifier | Existing deviations | Design action |
| --- | --- | --- | --- | --- |
| Traditional Three-Layer | `top.egon.cola.component.tianshu.admin.controller/service/repository` | Tianshu Admin Controller、Service、Repository tree；Maven compile and focused tests | 历史包名没有 `biz.*` 前缀，已有构造器/注解风格 | 保留现有边界，只改 matcher 与查询字符串；不创建新 Java type |
...
| String query typing | JPQL `coalesce` | Spring Data JPA | Existing Tianshu repositories | nullable bind 的 PostgreSQL 类型推断 | 修改查询，不改 schema |
| Page auth | Spring Security matcher | Existing Security starter | Tianshu capability enum | page path 未列出 | 补 GET matcher，不放宽写权限 |
| UI state | React Query/PageState | Existing shared package | `@egon-cola/xingyuan-admin-web-shared` | no proven gap | reuse |

### 6.2 User-mandated Java rule compliance

以下保留用户强制规则的逐项矩阵。此次没有创建 Java POJO、DTO、Converter、Bean 或配置 profile；Tianshu Repository 与 Security 配置只做局部修改，执行阶段仍必须按 Plan 每一步复核。
...
| Rule 2 | Yes | 查询边界接收可空字符串筛选，Controller/Service 合同不变 | 不新增验证对象；保持现有 Controller -> Service -> Repository handoff，先做 query 定向测试 | Tianshu page Controller/Service/Repository | Tianshu focused test + API envelope probe | PASS |
| Rule 3 | No | 不修改 Entity/DTO/VO/Converter | 不引入 POJO 或手工转换 | Existing Tianshu entities/VOs unchanged | diff/type inventory | PASS |
| Rule 4 | No | 不修改 Service/Component/Bean | 不新增 Bean 或注入关系 | Existing Tianshu services unchanged | diff + context compile | PASS |
...
| Rule 7 | No | 不增加配置 key | 不修改 profile 结构 | Existing Tianshu configuration | profile diff shows none | PASS |
...
| Rule 11 | Yes | Tianshu 当前 Controller/Service/Repository 三层目录已确认 | 不创建混合/第三层结构；修改仍留在现有层 | exact Tianshu paths above | architecture tree + Maven test | PASS |
...
| shared `matchesPath` type guard | Modify | REQ-003 | 假设 TS 类型总是可信 | Yuheng 运行时对象来自过滤后的动态树，undefined 已复现 | 无网络/状态成本 | Keep |
| Tianshu binding page | New | REQ-004, REQ-007 | 继续把绑定藏在 Namespace Drawer | 后端独立 Controller 的分页/审计能力无法被独立使用 | 一条 page query、增删改 mutations | Add |
...
| Tianshu metadata page | 1 GET page | loading/empty/denied/error/ready | existing Controller/JPA | permission matcher、null bind、DB unavailable | 修复现有页面而非新契约 |

### 7.1 System Architecture Design

Portal 只拥有宿主路由、清单校验、Wujie 生命周期和跨应用 presentation context；Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu 继续拥有各自 API、权限、租户和业务状态。浏览器访问路径如下：

`Portal Router -> runtime Manifest -> Wujie Core -> child Vite entry/lifecycle -> child Router/Layout -> existing Yuheng Admin API`。

四个平台独立访问路径保持不变。共享包只渲染导航，不定义平台菜单；Tianquan-Jianshen 的导航资源仍由 `resourceDefinitions.json` 和 bootstrap 权限决定，IAM 作为一个 parent 节点承载所有子项。

Tianshu 后端仍是 `Controller -> Service -> Repository -> PostgreSQL/Redis`。安全 matcher 只增加真实 GET page path；JPQL 只把可选文本参数转换为字符串空值语义。Yuheng active release 的状态不是源码路由事实，覆盖脚本需分别报告。
...
flowchart TB
    ...
    subgraph Children[Independent Admin Webs]
      Tianquan-Shoubing[Tianquan-Shoubing Admin Web]
      RBAC[Tianquan-Jianshen Admin Web / IAM tree]
      GW[Yuheng Admin Web]
      Tianshu[Tianshu Admin Web]
    end
    subgraph APIs[Admin API boundaries]
      IDPA[Tianquan-Shoubing Controller]
      RBACA[Tianquan-Jianshen Controller]
      GWA[Yuheng Controller]
      DDCA[Tianshu Controller + Security]
    end
    Browser --> Router --> Manifest --> Facade
    Facade --> Tianquan-Shoubing & RBAC & GW & Tianshu
    Shared --> Router
    Tianquan-Shoubing --> IDPA
    RBAC --> RBACA
    GW --> GWA
    Tianshu --> DDCA
```
...
sequenceDiagram
    ...
    U->>P: 打开 /xingyuan/tianquan-shoubing/overview
    P->>M: loadManifest(tianquan-shoubing, environment)
...
4. 前端覆盖按平台分步：Tianquan-Jianshen 先补 IAM 写操作/查询，Yuheng 补已有详情与同步/报告入口，Tianshu 补 binding page 与可用状态；Tianquan-Shoubing 只补确认有缺口的 profile/详情消费，不重复已有页面。
...
| --- | --- | --- | --- |
| Portal Router | host URL 与 child intent | child 业务路由 | unknown xingyuan -> 404 |
...
| Shared Sidebar | tree render/path selection | xingyuan menu data | invalid path -> ignore |
| Platform page | query/mutation/detail/UI state | other xingyuan DB | API error -> PageState |
| Tianshu Security | request method/path/capability | 前端按钮状态 | 401/403 envelope |
...
| 认证边界 | 沿用 same-origin/Yuheng cookie | 子应用不接收 host token，不新增跨域授权 |
| Controller 覆盖 | 只消费 `EXTERNAL` Admin API | 静态 route/method 对照，排除 internal |
| 数据安全 | Tianshu matcher 只补读 page | 写 matcher 和 capability 不放宽 |
...
```text
explicit UNIFIED_XINGYUAN_ADVERTISED_HOST
...
| --- | --- | --- | --- |
| Tianquan-Shoubing | users/clients/tenants/resource servers/keys/audits/grants | 主要能力已有页面 | 校正 profile/详情消费并做覆盖验证；不复制现有 CRUD |
| Tianquan-Jianshen | roles、inheritance、users、permissions、constraints、management selectors、role-impact simulation、runtime yuheng status | 多项只读或无 UI | 补 API、页面动作、详情抽屉和 IAM 子菜单 |
| Yuheng | applications detail、OpenAPI sync/document、projection/provider、MCP artifacts/tasks/approval/report | API 层较完整，页面覆盖不齐 | 将已有方法接入相应详情/工作台/同步视图；不新增 BFF |
| Tianshu | metadata page、bindings page、config version、publish retry、cache check | binding 独立入口缺失，page 请求被后端问题阻断 | 补 binding route/menu；修 security/query；保持 Config Drawer |
...
| Yuheng 白屏是共享路径边界问题 | parent `path?` + `endsWith` runtime error + shared function | 先补类型保护再扩大页面覆盖，避免把首屏错误误判成后端缺接口 |
| Tianshu 页面既有前端入口但后端未闭环 | page Controller + missing matcher + `lower(bytea)` log | 修 matcher/query，不新增 API 或 migration |
| Tianquan-Jianshen/Yuheng 的部分 404 可能是发布漂移 | draft route count > active release and skip release env | 代码覆盖与 active release 验证分开报告，不自动发布 |
...
scripts/{unified-xingyuan,start-local-stack.sh,unified-identity-local.sh}
```
...
| --- | --- | --- | --- |
| Tianquan-Jianshen IAM | `/api/tianquan-jianshen/v1/iam/**`、`/api/tianquan-jianshen/v1/users/**` | Tianquan-Jianshen Admin Controller | Feature API + IAM pages |
| Yuheng Admin | `/api/v1/yuheng/admin/**` | Yuheng Admin Controller | `gatewayApi` + Yuheng pages |
| Tianshu Admin | `/api/v1/tianshu/**` | Tianshu Admin Controller | `ddcApi/ddcPageApi` + Tianshu pages |
| Tianquan-Shoubing Admin | `/api/v1/tianquan-shoubing/**` | Tianquan-Shoubing Admin Controller | `httpClient` + Tianquan-Shoubing pages |

## 10. POJO and Data Model Design

不新增 Java 或 TypeScript 领域数据模型；前端新增字段只按 Controller 已返回 JSON 建立带语义后缀的 view type，复杂交互复用现有 `*View`/`*Mutation` 类型。Tianshu page 响应继续使用 `PageResultRecord<T>`，不改变实体列、版本、时间和 envelope。

## 11. Database Design

数据库为 `Unchanged`：不新增表、列、索引或 Flyway 文件；Tianshu 查询只改变 JPQL 参数的字符串空值表达，不改变结果语义和排序。当前 `ddc_operation_log` 等历史表不在本次读取能力扩展范围内。
...
```text
平台工作台
├── 身份与安全 -> /xingyuan/tianquan-shoubing/overview
├── 权限治理 -> /xingyuan/tianquan-jianshen/roles
├── API 网关 -> /xingyuan/yuheng/dashboard
└── 配置中心 -> /xingyuan/tianshu/registry
```

Tianquan-Jianshen 菜单保持一个 IAM 顶层：
...
| Child route `/xingyuan/{key}/*` | Header 保留 Portal 标题；下方 MountFailure 条带或全宽 Wujie host；失败区显示错误类型、retry、standalone | Manifest query + Wujie lifecycle；loading/mounted/crashed/unmounting |

### 12.3 Tianquan-Shoubing pages
...
Tianquan-Shoubing 目前主要管理 Controller 已有前端消费；`/api/v1/tianquan-shoubing/me`、OAuth metadata、token 和 internal refresh 是身份协议/运行接口，不伪装成管理菜单。若后续需要个人资料卡，沿用 Header，不新建平台顶层菜单。

### 12.4 Tianquan-Jianshen IAM pages
...
| 审计/运行 `/iam/diagnostics/*` | 审计查询、运行状态、失败 mutation 重试、Yuheng-Tianshu 状态 | Audit/Runtime Controller |
...
| Yuheng Group | 列表、创建/编辑/启停；详情使用 tabs：概览、Engine 节点、draft routes/policies、releases | group/projection/draft/release |
...
| TEST-003 | Tianquan-Jianshen API/pages | role CRUD/inheritance、user mutation、permissions/constraints selectors | Tianquan-Jianshen targeted Vitest/typecheck |
| TEST-004 | Tianshu Java/Web | page matcher、coalesce query、binding page CRUD | Tianshu Maven controller/repository tests + Web Vitest |
...
| TEST-006 | Yuheng coverage | app detail/OpenAPI/trace detail/provider/MCP page API requests | Yuheng Vitest/typecheck/build |
...
| TEST-008 | final build | 四 Web typecheck/build、shared build、Tianshu Maven module test | command exit 0；runtime stack只做复核不宣称构建等价 |

## 15. Non-functional and Cross-cutting Design

- 安全：Portal 不接收 access token；child 沿用 cookie/CSRF；Manifest 只允许同源或显式允许的 loopback child origin；Tianshu page 只增加 READ matcher。
...
- 性能：Portal 每个平台 route 只加载对应清单和 child；表格使用分页；Yuheng traces 不扩大刷新范围。
...
实施顺序为 shared/Yuheng 防崩 -> 地址策略 -> Wujie -> Tianshu 后端 -> 各平台前端覆盖 -> 全量验证。每一步一个可回滚提交，运行栈不自动重启。现有已启动进程可以在用户确认后通过 HMR/重新构建复核；active Yuheng release 不在代码提交中自动变化。

回滚方式：按提交反向回退对应 Step；Portal 可临时使用独立 URL；Tianshu query/security 变更只影响匹配/查询逻辑，不影响历史数据。若 Wujie Core 构建兼容性不成立，保留原 wrapper 作为明确阻断，不降级为未经批准的裸 iframe。
...
| Yuheng/Tianquan-Jianshen active release 落后 | current active revision 与 draft route count 不同 | 静态 coverage 与 release status 分开输出，不自动 publish | 用户决定何时发布 |
| Tianshu JPQL provider 对 null coalesce 推断差异 | PostgreSQL `lower(bytea)` runtime log | focused JPA test；失败返回 Plan 阻断并改服务端字符串规范化 | 依赖当前 Hibernate 版本 |
...
用户审查重点为：Portal 是否真实挂载；IAM 是否仍是一个顶层；各页面是否按文字布局实现；四个平台 API 是否只调用已有外部 Controller；127.0.0.1 默认和显式 host 覆盖是否符合预期；Tianshu 403/bytea 是否消失。
...
当前运行日志和 DOM 是修复前证据；构建/单测只能证明静态和组件行为；Portal/Wujie、四个运行进程和 active Yuheng release 的闭环必须在用户启动或保留现场后单独复核。
...
| MC-VALID-001 | Applicable | PASS | §14 列出 Controller/Service/Repository 与 Web Query 各边界验证 | Tianshu 查询改动有 focused gate | None |
...
| 4 | `3ec52e907` | Tianshu page matcher/JPQL；安全测试 13、分页仓储测试 2 通过 |
| 5 | `0035439bc` | Tianshu 作用域绑定管理页；Tianshu Web 全量测试 21 files/53 tests、typecheck/build 通过 |
| 6 | `e2fe5f9cb` | Tianquan-Jianshen IAM 角色/用户/权限覆盖；全量测试 15 files/32 tests、typecheck/build 通过 |
| 7 | `00842c0da` | Yuheng Application 详情/OpenAPI 同步；全量测试 24 files/77 tests、typecheck/build 通过 |

最终静态审计命令 `scripts/unified-xingyuan/check-admin-web-controller-coverage.sh` 通过，输出 Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu 的 external Controller 与浏览器 Admin API 静态计数，并明确排除协议、internal/RPC、直接数据访问和 active release 发布。Tianquan-Shoubing 的主要管理 Controller 已有现有页面消费，因此没有复制 CRUD；Tianquan-Jianshen 新增权限入口已挂到 `IAM -> 资源目录`。

四个独立 Web 和 shared 的最终验证均通过：Tianquan-Shoubing 9 files/30 tests、Tianshu 21 files/53 tests、Portal 7 files/21 tests、shared 2 files/9 tests；所有受影响 Web 的 typecheck/build 通过。shared 构建脚本会清理本地 `node_modules`，验证前按现有 lockfile 重新安装，未产生源码或锁文件变更。

本次执行没有自动启动、停止或重启服务，也没有发布 Yuheng active release；因此运行现场的 Wujie 实际 DOM、子应用 HMR/静态资源加载和 active release 内容仍属于用户后续重载后的 runtime evidence，不能由上述静态测试替代。用户原有未跟踪文档 `docs/egon/spec/2026-08-27-20-31-archetype-two-stage-source-generation.md` 未被修改或暂存。
