...
| Type | `Architecture` |
| Complexity | `Complex` |
| Complexity Drivers | `RPC 公共 Java API 与兼容性、Tianshu/Redis 最终一致发现、模式内多实例重试未知结果、五种负载均衡、一致性 Hash 键、Access Guard Provider 并发限流、同步/异步双调用、泛化调用、HTTP/2 多路复用、CGLIB 代理、跨生命周期部分失败` |
| Created | `2026-08-19 15:36 CST` |
| Updated | `2026-08-21 10:19 CST` |
| Owner | `Egon-COLA xingyuan owner / User` |
| Repository | `Egon-COLA` |
| Scope | `egon-cola-component-rpc-starter、egon-cola-component-rpc-tianshu-adapter、egon-cola-component-access-guard-starter 的限流算法扩展、RPC/Guard README 与聚焦测试；Tianshu Registry/Redis 和 Yuheng RPC 数据面仅作边界上下文` |
| Change Surface | `两种 Reference 注解的公共字段归一与模式策略/工厂、Consumer 调用执行器/发现快照/负载均衡/模式内换节点/异步/泛化/CGLIB/共享 Channel、Provider 生命周期与 Access Guard 限流接线/算法工厂/UNAVAILABLE 映射、配置/异常/文档/测试；不改业务 Protobuf wire、Tianshu Admin 存储模型、Yuheng 路由实现、关系库或前端` |
...
| Baseline Revision | `main@4ae5419c4504250436b98886c03271889a8f2879；2026-08-21 10:19 CST dirty-worktree snapshot，现有未提交 Yuheng/其他 Spec/Plan 改动不属于本规格修改` |
| Amends | `[Tianshu 单机闭环与轻量 RPC §4.2、§9.3、§11.4、§15.2/§15.3、§17.2、§20.2、§21.2](../../superpowers/specs/2026-07-24-tianshu-standalone-rpc-framework-design.md#42-明确不实现)`；`[统一服务模型 §3.4、§4.3、§4.4](../../superpowers/specs/2026-07-26-tianshu-rpc-yuheng-unified-service-model-design.md#34-策略层servicecallpolicy解决-g3)` |
| Supersedes | `[Yuheng BIZ/APP 与 Tianshu 直连 RPC Spec](./2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) §1、§3.2、§4 REQ-005–REQ-011、§5 DEC-003–DEC-005、§7.3–§7.5、§8 RPC Consumer 子树、§9 ANN-001/ANN-002、§13.2、§15 Direct 治理、§16–§18 中的 RPC Reference/Direct 行为；其 Yuheng BIZ/APP 安全与凭证中继设计不被替换` |
| Depends On | `[Tianshu 单机闭环与轻量 RPC §8 服务注册中心](../../superpowers/specs/2026-07-24-tianshu-standalone-rpc-framework-design.md#8-tianshu-服务注册中心)`；`[GWS-04 §5–§13 Yuheng RPC 数据面](../../superpowers/specs/2026-07-25-yuheng-biz-gateway-rpc-design.md#5-动态-grpc-handler)`；`[Yuheng BIZ/APP 与 Tianshu 直连 RPC Spec §7.1–§7.2、§15 Security](./2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md#71-architecture-overview)` |
| Related Specs | `[统一服务模型与管理端优化](../../superpowers/specs/2026-07-26-tianshu-rpc-yuheng-unified-service-model-design.md)`；`[Yuheng Provider 发现与负载均衡](../../superpowers/specs/2026-07-25-yuheng-provider-discovery-load-balancing-design.md)` |
| Related Plans | [RPC 运行时治理演进实施计划](../plan/2026-08-21-12-59-rpc-runtime-governance-implementation.md) |

## 1. Summary

本规格把当前“两个注解、两个 Proxy Factory、两个 Channel Manager、部分未接线策略字段”的 RPC Consumer，演进为一条统一调用内核：两种 Reference 仍保持源码兼容，但先解析为共同的 `RpcReferencePolicy` 和带模式差异的 `RpcReferenceDefinition`；`RpcReferenceStrategyFactory` 在代理创建时固定选择 Yuheng 或 Direct 策略，单次逻辑调用绝不跨模式。`RpcInvocationExecutor` 统一执行阻塞、异步、模式内换节点、Deadline、可用性故障重试、终态失败和本地 fallback。Consumer 复用当前 Tianshu `Redis Topic + 定期 RPC 全量拉取` 订阅，不创建第二套远程缓存；进程内保留按精确服务键的不可变 Provider/Yuheng 快照，并以五种指定算法选择尚未尝试的同模式实例。

Provider 继续主动注册和 heartbeat，心跳间隔仍归 `egon.cola.component.rpc.provider` 配置；Tianshu 只验证租约身份、续期 TTL、被动过期和发布变更事件，不主动探测 Provider。`@EgonRpcProvider` 继续保持无字段的纯发现标记；业务实现方法使用现有 `@RateLimitGuard(ruleId, key)`，限流参数和算法归 Access Guard rule 配置。Guard 内以 Strategy + Factory 补齐漏桶、令牌桶、滑动窗口，RPC 通过异常适配器把 `RATE_LIMITED` 映射为 Provider-stage `UNAVAILABLE`。Typed Contract 同时允许 Protobuf 响应和 `CompletionStage<Protobuf响应>`，泛化调用通过原始 Protobuf bytes/Descriptor 调用既有 unary `fullMethodName`；其唯一格式是 gRPC canonical `fully.qualified.Service/Method`，不增加第二套 wire 服务。
...
RPC 组件已经有 Provider 租约、Yuheng/Direct 两条引用、Tianshu Redis 订阅和 unary gRPC 基础链，但能力不完整：Direct 只轮询且单次调用；Yuheng 重试只允许幂等且只识别 Yuheng-stage `UNAVAILABLE`；注解的 `retries/loadBalance/failStrategy` 大部分未进入执行链；代理是 JDK Proxy；没有异步 Java 方法、泛化客户端、共享 Channel 池或 Provider 自身限流。

用户要求把这些能力作为 RPC 组件的一致运行时能力，而不是继续依赖 Yuheng 代偿。业务代码明确承担重试幂等，例如使用唯一单据编号让重复写覆盖；框架因此不能再把 `@EgonRpcMethod.idempotent` 作为是否重试的硬门槛，但仍必须限制可重试故障、总次数和总 Deadline，避免对业务拒绝与参数错误盲目重放。

### 2.2 Repository evidence

以下证据均为当前 dirty-worktree 的静态源码/文档证据；未启动 Tianshu、Redis、Yuheng、Provider 或 Consumer，未证明真实网络吞吐、Redis 丢消息恢复、连接并发或停机排空时间。为保持表格可读，下列别名是规范化的 repository-relative 路径前缀，表内 `别名/剩余路径` 可直接还原为实际文件：
...
| `RPC_TIANSHU_JAVA` | `egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu` |
...
| `TIANSHU_STARTER_JAVA` | `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-starter/src/main/java/top/egon/cola/component/tianshu` |
| `TIANSHU_ADMIN_JAVA` | `egon-cola-xingyuan/egon-cola-tianshu/egon-cola-tianshu-admin/src/main/java/top/egon/cola/component/tianshu/admin` |

| Evidence ID | Classification | Exact path/symbol/decision/command | Observed fact | Design significance | Verification limit/freshness |
| --- | --- | --- | --- | --- | --- |
| `EVD-001` | User decision | 需求 1 + 2026-08-21 决定 1 | 两个 Reference 要抽取公共字段并采用策略；Direct/Yuheng 模式由客户端配置，失败只按 LB 换同模式实例，全部失败才报错，绝不跨模式 | 必须统一声明解析、模式策略工厂和候选排除；不得增加 Direct→Yuheng 状态 | 用户已关闭原 `DEC-008` |
| `EVD-002` | Static repository | `RPC_JAVA/annotation/EgonRpcReference.java:18-74` | Yuheng Reference 已声明 timeout、retries、loadBalance、group、version、fallbackBean、failStrategy | 这些字段是公共策略基线，但当前执行只使用 timeout | 静态声明不证明运行语义 |
| `EVD-003` | Static repository | `RPC_JAVA/annotation/EgonRpcDirectReference.java:16-28` | Direct 只有 bizCode、appCode、env、group、version、timeout | Direct 缺少与 Yuheng 一致的 retries/loadBalance/终态策略 | 不证明外部项目的注解使用量 |
...
| `EVD-006` | Static repository | `RPC_JAVA/consumer/proxy/RpcConsumerInvocationHandler.java:73-143` | 每次调用线性查 Method；只阻塞 `blockingUnaryCall`；只有幂等 Yuheng-stage UNAVAILABLE 重试 | 需要预编译 Method Plan、同步/异步执行器和新的重试判定 | 未证明当前热路径耗时占比 |
...
| `EVD-008` | Static repository | `TIANSHU_STARTER_JAVA/listener/registry/DdcManagedRegistrySubscription.java:77-98,107-143`；`TIANSHU_STARTER_JAVA/listener/registry/DdcInstanceSubscription.java:69-102` | 先监听 Redis Topic，首次全量拉取，事件合并刷新，固定周期全量对账，失败时按租约过期本地摘除 | 已满足“事件监听 + 定期 Tianshu 拉取”的核心机制 | Redis 实际投递/断线未验证 |
| `EVD-009` | Static repository | `RPC_TIANSHU_JAVA/autoconfigure/DdcRpcAutoConfiguration.java:134-171`；`TIANSHU_STARTER_JAVA/autoconfigure/properties/DdcProperties.java:624-669` | RPC Tianshu Adapter 复用 `DdcRegistrySubscriptionCoordinator`，默认每 10 秒对账 | 不应复制轮询线程；保持 Tianshu Adapter 边界 | 当前间隔属于 Tianshu Registry 配置而非 RPC Consumer 配置 |
| `EVD-010` | Static repository | `TIANSHU_ADMIN_JAVA/repository/DdcServiceRegistryRedisRepository.java:51-75,78-124,255-312,484-492` | 注册/注销/过期发布事件；heartbeat 只续 TTL；查询惰性清理过期租约 | Tianshu 是被动租约账本，不是主动健康探针 | 未验证生产 Redis 原子性/延迟 |
| `EVD-011` | Static repository | `RPC_JAVA/config/EgonRpcProperties.java:76-86`；`RPC_JAVA/provider/lifecycle/RpcProviderLifecycle.java:220-231` | heartbeat 间隔/lease/优雅关闭均已由 RPC Provider 配置，Provider 定时主动上报 | REQ-007 主要是固化与完善，而不是迁移到 Tianshu 配置 | 当前使用秒粒度固定延迟 |
...
| `EVD-014` | Static repository | `RPC_JAVA/annotation/EgonRpcService.java:33-39`；`TIANSHU_STARTER_JAVA/format/ServiceInstanceMetaCodec.java:50-54,205-227`；`RPC_TIANSHU_JAVA/registry/{DdcRpcProviderDirectory,DdcRpcGatewayDirectory}.java` | Contract/metadata 有 weight；Tianshu metadata 可解码 `yuheng.weight`，但两个 RPC Directory 都丢弃结果 | Direct Provider 和 Yuheng endpoint 的 weighted 算法可复用现有 metadata wire，不改 Tianshu Proto | 当前 Provider Contract 默认 weight 尚未稳定写入 metadata |
...
| `EVD-017` | Static repository | `RPC_JAVA/consumer/channel/RpcConsumerChannelFactory.java:29-49`；`RPC_JAVA/consumer/{yuheng/RpcConsumerGatewayManager,provider/RpcConsumerProviderManager}.java` 的 `connect/reconcile/drain` | 每个 Manager/Query 会创建 ManagedChannel；没有跨 Query/模式共享池 | gRPC 本身能在一个 HTTP/2 Channel 多路复用，但当前所有权阻止充分复用 | 未测远端 SETTINGS_MAX_CONCURRENT_STREAMS |
...
| `EVD-019` | Static repository | `README.zh-CN.md:240-243,427-434,529-539` | 文档明确直连不重试、Yuheng 幂等重试，且把完整策略列为 Roadmap | README 必须与新合同同步更新 | 外部文档不在仓库内不可见 |
| `EVD-020` | Static repository + User decision | `docs/egon/spec/2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md §3.2/§4/§16`；2026-08-21 决定 1 | 前一 Review Spec 禁止两种路径自动 fallback；用户本次再次确认模式不切换 | 不需要 Yuheng fallback 字段、Route 前提或 fallback-only demand | Yuheng Reference 仍独立走现有 Yuheng 数据面 |
...
| `EVD-025` | Static repository + User decision | `RPC_JAVA/contract/validation/RpcContractValidator.java`；`RPC_JAVA/contract/descriptor/RpcMethodDescriptor.java`；Yuheng `RuntimeRpcRoute.java`；2026-08-21 决定 5 | 当前 authority 是 `grpcMethod.getFullMethodName()`；Yuheng 强制值包含 `/`，gRPC fixture 用 `generateFullMethodName` 生成 `fully.qualified.Service/Method` | Generic 唯一定位必须沿用斜杠格式；`fullservice.method` 仅表示“完整服务+方法”概念，不是可接受 wire key | 具体业务 service/method 仍来自各自 descriptor |
...
2. Tianshu 发现缓存已存在，但负载均衡仍硬编码轮询，实例 weight 被解码后丢弃，Channel 又按 Query 重复创建；
3. Retry 只服务 Yuheng 幂等故障，Direct 不重试；两种模式都缺少“同一逻辑调用按 LB 排除失败实例、改选同模式节点”的统一语义；
...
| --- | --- | --- | --- | --- | --- |
| Yuheng 字段注入与调用 | `BeanPostProcessor -> RpcConsumerProxyFactory -> JDK Proxy -> RpcConsumerInvocationHandler -> GatewayRpcInvocationChannelProvider -> RpcConsumerGatewayManager` | Yuheng snapshot、ManagedChannel；无持久写 | Tianshu subscription、INTERNAL_GATEWAY、Yuheng | Spring 业务 Bean | `EVD-002`,`EVD-004`–`EVD-006` |
| Direct 字段注入与调用 | `BeanPostProcessor -> RpcDirectReferenceProxyFactory -> RpcConsumerProviderManager.register -> ProviderRpcInvocationChannelProvider -> fixed RR` | `Registration.activeProviders/revision/sequence` | Tianshu RPC_PROVIDER、Redis Topic、Provider | Spring 业务 Bean | `EVD-003`,`EVD-007`–`EVD-009`,`EVD-013` |
| Provider 启动与健康 | `RpcProviderLifecycle.start -> Server.start -> leaseManager.prepare/registerAll -> heartbeatAndRecover` | Tianshu 临时租约、availability set | Tianshu Admin/Redis | Direct Consumer、Yuheng Engine | `EVD-010`–`EVD-012` |
...
| Tianshu 变更通知 | `register/deregister/expire -> Redis Topic -> DdcManagedRegistrySubscription -> RPC full snapshot pull -> Consumer reconcile` | Redis TTL、revision、进程内 current snapshot | Redis + Tianshu Registry RPC | Yuheng/Provider Directory listener | `EVD-008`–`EVD-010` |
...
- 明确复用 Tianshu Redis 事件订阅、首次/定期全量拉取和 Consumer 本地不可变快照。
- 在 Consumer 侧实现随机、加权随机、轮询、平滑加权轮询、一致性 Hash；保留既有 `LEAST_IN_FLIGHT` 兼容值。
- 让 Provider 主动 heartbeat，Tianshu 只做租约接收、TTL/身份验证、过期与事件发布。
...
- 不把 Tianshu 改成主动探活系统，不让 Tianshu 反向调用 Provider Health RPC。
- 不新增 Tianshu Proto 方法/Tianshu Redis Key、数据库表、Flyway migration 或关系型 ER 模型；Guard 仅为新算法增加既有 rate-limit namespace 下的 transient suffix keys，详见 §11。
- 不在 RPC Starter 内实现任何限流算法/存储；Local/Redisson 作用域、rule key、配额状态都由 Access Guard 既有配置与后端负责。
- 不把 Provider Guard 限流复用成 Yuheng 流量治理，也不修改 Yuheng 现有规则/限流实现。
- 不允许 Direct 与 Yuheng 在一次逻辑调用中互相切换，不新增 `fallbackToGateway`、Yuheng fallback demand 或跨模式错误码。
...
- 不改 Yuheng BIZ/APP 授权、Provider 下游鉴权、凭证中继和接口定义上报职责。
- 不启动项目、Tianshu、Redis、Yuheng 或业务进程；Spec 验证是文档/源码边界。
...
| Generic invocation | `Affected` | 当前无生产类型；`RpcContractValidator` 与 Yuheng raw forwarding 提供 descriptor/bytes 证据 | 新增无接口 unary blocking/async API，复用同一执行链 | 公共方法、模型、错误/安全/测试完整设计 | `§7, §8, §9, §10, §13, §14, §15, §16, §17, §18` |
...
| Tianshu Registry subscription implementation | `Context-only` | `DdcRegistrySubscriptionCoordinator`、`DdcManagedRegistrySubscription`、`DdcInstanceSubscription` | 保留 Redis Topic + first pull + periodic reconcile；RPC 只消费 | 记录复用边界，不设计第二套实现；聚焦契约测试 | `§7, §14, §15` |
| Tianshu Admin/Redis registry storage | `Context-only` | `DdcServiceRegistryRedisRepository` | 保留租约/TTL/revision/topic；不主动探活、不改 key | 记录权威/一致性/运行验证边界 | `§7, §11, §15, §16` |
| Yuheng RPC data plane | `Context-only` | GWS-04；`RpcGatewayForwarder` | 仅供 Yuheng Reference 使用；Direct 失败不得进入 Yuheng，既有 fullMethodName Route/治理/错误 Trailer 不变 | 只定义模式隔离和兼容回归，不改 Yuheng | `§7, §9, §14, §16, §18` |
...
| `REQ-002` | 既有 `@EgonRpcReference` 源码写法继续编译并默认以 Yuheng 为主路径 | Must | 不改既有属性名/类型/默认；无 Direct 选择时仍只查询 INTERNAL_GATEWAY | 需求 1 + compatibility |
| `REQ-003` | Yuheng/Direct 模式必须由客户端 Reference 配置并在代理/调用期间固定，失败不得切换另一模式 | Must | Yuheng Reference 只查询/调用 Yuheng；Direct Reference 只查询/调用 Provider；源码与进程测试均证明无跨模式 demand/call | 需求 1 + 2026-08-21 决定 1 |
...
| `REQ-005` | Consumer 必须按精确服务键缓存一份不可变 Provider 列表，并由 Redis 事件和周期 Tianshu 拉取更新 | Must | 同 query 只订阅一次；首次快照、事件刷新、周期对账均替换同一缓存；调用不每次访问 Tianshu | 需求 2 |
| `REQ-006` | Consumer 缓存必须拒绝旧 revision、过滤过期租约并在 Tianshu/Redis 暂失时仅使用未过期本地实例 | Must | out-of-order snapshot 不回退；过期实例在本地时钟到期后不可选；恢复全量快照可收敛 | 需求 2/3 |
| `REQ-007` | Provider heartbeat 必须由 RPC 侧主动调度，间隔和 lease 在 RPC Provider 配置，Tianshu 不主动探测 | Must | `provider.heartbeat-interval-seconds < lease-seconds` 启动校验；每周期 Provider 调 heartbeat；Tianshu 无 Provider probe 调用 | 需求 3 |
...
| `REQ-020` | Retry/换节点必须受总 Deadline、最大次数、同模式候选集合、故障白名单和取消控制；业务错误必须原样报错 | Must | 只有 acquisition failure 与 Provider/Yuheng-stage `UNAVAILABLE` 可换节点；`UNAVAILABLE` 保留给 availability/Guard rate-limit，业务 exception mapper 不得用它表达业务条件；INVALID_ARGUMENT、PERMISSION_DENIED、NOT_FOUND、FAILED_PRECONDITION、ALREADY_EXISTS、ABORTED、DEADLINE_EXCEEDED 等业务/语义错误不重试、不 fail-open、不 fallback；取消立即终止；无无限循环 | 2026-08-21 决定 2 + 正确性必要条件 |
| `REQ-021` | 本改造不得改变业务 Protobuf wire、Tianshu Registry Proto、关系库或 Yuheng 安全合同 | Must | Descriptor SHA/字段不因 runtime 改造变化；无 proto/db migration/Yuheng production diff | 最小变更与前置 Spec |
| `REQ-022` | 双语 README、配置元数据、单元/组件/进程测试必须与新行为一致 | Must | 旧“JDK Proxy/Direct 不重试/限流不在模块”文案消失；测试矩阵和配置绑定通过 | 可交付性 |
| `REQ-023` | Consumer 优雅启动必须先安装全部已声明模式的 Directory subscriptions/共享 Channel runtime，再允许新 invocation；模式缓存为空不得伪装 READY | Must | STARTING 时执行器拒绝调用；Yuheng primary 保持现有 fail-fast；Direct 空集合进入 DEGRADED 并在调用时返回 Provider unavailable；启动失败清理所有 subscription/pool entry | 需求 5 + 2026-08-21 决定 1 |

### 4.1 Scenario matrix

| Scenario | Actor/trigger | Preconditions | Main path | Alternative/failure path | Data/state change | Observable result | Requirements |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Typed blocking Yuheng | Caller 调接口 | Yuheng snapshot READY | CGLIB plan→Yuheng LB→blocking unary | 可重试 UNAVAILABLE 选未尝试 Yuheng | in-flight 计数；无持久写 | Message 或稳定异常 | `REQ-001`,`REQ-002`,`REQ-014`,`REQ-018`–`REQ-020` |
| Direct 成功 | Direct Caller | Provider cache有有效租约 | 按 LB 选 Provider，共享 Channel 调用 | 首个 Provider UNAVAILABLE 可按 retries 选另一个 | cache只读；attempt 状态 | 不触发 Yuheng | `REQ-003`,`REQ-005`–`REQ-009`,`REQ-017` |
| Direct 节点耗尽 | Direct Caller | Provider candidates 依次 acquisition/UNAVAILABLE | 每次按 LB 在未尝试 Provider 中选下一个 | 候选或 `1+retries` 预算耗尽；不查询 Yuheng | 本地 attempted-instance 状态 | `RPC_PROVIDER_UNAVAILABLE` 或最后限流时 `RPC_RATE_LIMITED`；无 Yuheng call | `REQ-003`,`REQ-004`,`REQ-019`,`REQ-020` |
| Yuheng 节点耗尽 | Yuheng Caller | Yuheng candidates 依次 acquisition/UNAVAILABLE | 每次按 LB 在未尝试 Yuheng 中选下一个 | 候选或预算耗尽；不查询 Provider Directory | 本地 attempted-instance 状态 | `RPC_YUHENG_UNAVAILABLE`；无 Direct Provider call | `REQ-003`,`REQ-004`,`REQ-020` |
| Consumer graceful startup | Spring lifecycle | Reference demand 已在 BeanPostProcessor 阶段登记 | Coordinator 先启动各自模式 subscriptions/pool，再 READY/DEGRADED | primary Yuheng fail-fast；Direct 空集合 DEGRADED；异常清理；无 fallback-only demand | Consumer runtime state | Application 不在未初始化 discovery 上接受调用 | `REQ-005`,`REQ-011`,`REQ-023` |
| Redis 事件丢失 | Scheduler | 旧缓存未过期，Redis event 未达 | 周期 RPC 全量拉取较新 revision | Tianshu 暂不可用→只本地过期，不延长旧 lease | immutable snapshot替换或过期收缩 | 最迟一个 reconcile interval 收敛 | `REQ-005`,`REQ-006` |
...
| Provider 心跳失败/恢复 | Heartbeat scheduler | 曾有 lease | 先 unavailable、丢旧 lease、重新 register | Tianshu 仍不可用保持 unavailable | Tianshu TTL最终过期；新 lease替换 | Consumer/Gateway摘除后恢复 | `REQ-006`,`REQ-007`,`REQ-010` |
...
| `ACTOR-004` | Tianshu Registry/Redis | 保存临时服务租约并通知/提供权威快照 | Tianshu RPC + Redis Topic | Registry HMAC/mTLS 现状不变 | `EVD-008`–`EVD-010` |
| `ACTOR-005` | Yuheng RPC Runtime | 仅为 Yuheng Reference 按已有 Route 转发；不参与 Direct Reference 失败处理 | INTERNAL_GATEWAY unary | Yuheng BIZ/APP/Provider 安全沿用前置 Spec | 2026-08-21 决定 1；GWS-04 |
...
flowchart LR
    ...
    Tianshu["ACTOR-004 Tianshu Registry and Redis"]
    Yuheng["ACTOR-005 Yuheng RPC Runtime"]
    Operator["ACTOR-006 xingyuan developer/operator"]
    ...
    UC001 -->|"Yuheng Reference only"| Yuheng
    UC002 -->|"pull and subscribe"| Tianshu
    UC001 --> UC003
    UC004 --> UC003
    Provider -->|"lifecycle"| UC005
    UC005 -->|"register and heartbeat"| Tianshu
...
| `UC-002` | Reference 注册 exact query并启动 Consumer runtime | Tianshu Registry + Redis 配置存在或按 mode 允许 DEGRADED | subscriptions/pool 安装；本地不可变列表通过事件快速、周期兜底收敛 | event 丢失、Tianshu timeout、旧 revision、lease expiry、startup cleanup | STARTING 不接受调用；不使用过期实例；恢复后替换 | `REQ-005`–`REQ-007`,`REQ-011`,`REQ-023`; `TEST-010`–`TEST-015`,`070`–`072` |
...
| `UC-005` | Spring start/stop 或 heartbeat tick | Provider 配置/Contract/Registry 合法 | READY 只在 lease 后；停止排空并注销 | 初始注册失败、丢 lease、Tianshu outage、排空超时 | 不发布假 READY；失败/停止无恢复线程或残留 lease（TTL 兜底） | `REQ-007`,`REQ-010`,`REQ-011`; `TEST-031`–`TEST-038` |
...
- Provider heartbeat/lease interval 归 RPC Provider 配置，Tianshu 只被动接收和过期。
- 用户明确决定重试不检查框架幂等标记；业务代码保证重复安全。
- 用户明确决定 Reference mode 由客户端配置且调用期间不可切换；Direct 不依赖 Yuheng Route，Yuheng 也不得退化为 Direct。
- 只有连接/Channel 获取失败和 Provider/Yuheng-stage `UNAVAILABLE` 属于自动换节点白名单；业务错误是正常可见结果，必须报错且不进入 fail strategy。
- Provider/Yuheng `UNAVAILABLE` 是 availability protocol contract（包含 Guard rate-limit）；业务 `RpcProviderExceptionMapper` 必须使用非-UNAVAILABLE status 表达业务校验、前置条件、冲突、拒绝或未找到。
...
- Registry、Yuheng、Provider、业务授权是独立边界；本 Spec 不把服务发现成功当成业务权限允许。
...
| --- | --- | --- | --- | --- |
| `ASM-001` | `SMOOTH_WEIGHTED_ROUND_ROBIN` 即用户所说“加权轮询”，保留现有名称避免枚举破坏 | `LoadBalance` 与 Yuheng 已有相同常量 | 算法实现/文档可局部调整 | 若必须使用 `WEIGHTED_ROUND_ROBIN` 名称，需别名/弃用周期 |
| `ASM-002` | 旧 `LEAST_IN_FLIGHT` 继续保留，但不计入本次必须新增的五种算法 | 已发布 enum 与 Yuheng 使用 | 保留值无新默认行为 | 若要求严格只保留五种，删除会破坏源码/配置 |
| `ASM-003` | Tianshu reconcile interval 继续使用 `egon.cola.component.tianshu.registry.reconcile-interval-seconds`，不复制 RPC 属性 | Adapter 当前直接复用 coordinator | 配置归属可在后续调整 | 若必须全部置于 rpc namespace，需要 Tianshu Adapter 属性优先级迁移 |
...
| `ASM-005` | Programmatic `RpcDirectClientFactory` 的显式基础设施 target 继续独占 handle，不加入业务发现共享池 | README 区分该能力与 Tianshu Direct Reference | 独立生命周期可局部保留 | 若也要池化，需定义不同 TLS/HMAC profile 的 pool key/ownership |
...
| `DEC-002` | 复用现有 Tianshu subscription 两层本地状态，不新建远程 cache/job/topic | Repository fact | 已有首次 pull、Redis event、periodic reconcile、expireLocal，新增一套只会增加一致性状态 | `REQ-005`,`REQ-006` |
| `DEC-003` | gRPC Client transport retry 继续 `disableRetry()`；RPC Consumer 到其所配 endpoint 的重试/同模式换节点由一个 `RpcInvocationExecutor` 控制 | Existing factory + observability need | 防止 Consumer 层与 gRPC service config 双重重试，确保 Consumer transmissions/Deadline 可计算；Yuheng 数据面内部 Provider retry保持 Context-only 现状 | `REQ-019`,`REQ-020` |
| `DEC-004` | 实例 weight 沿用现有 `yuheng.weight` metadata wire，RPC model 增 typed weight，不改 Tianshu Proto | `EVD-014` | 既有单一 metadata authority 已存在，新增字段会造成双事实源 | `REQ-008`,`REQ-009`,`REQ-021` |
...
| `DEC-006` | Provider 实现方法复用 `@RateLimitGuard`；Guard advice 在 business method 前执行，RPC exception adapter 将 `RATE_LIMITED` 映射为 Provider-stage `UNAVAILABLE` | User + `EVD-016`,`EVD-022`–`EVD-024` | 保持 Provider marker 单一职责，不复制 Guard rule/key/storage；限流可在同模式其他实例继续尝试；Yuheng 保持现有安全 trailer 重建 | `REQ-012`,`REQ-013` |
| `DEC-007` | `idempotent` 字段保留作文档/Yuheng/业务提示，但从 Consumer retry gate 移除 | User | 保持 contract compatibility，同时落实业务自保责任 | `REQ-019` |
| `DEC-008` | Yuheng/Direct 模式在客户端 Reference 上固定；失败不跨模式，只在同模式候选中改选节点，全部失败报错 | User（2026-08-21 决定 1） | 避免隐藏拓扑切换和 Yuheng Route 前提；与前置 Direct Spec 的禁止 fallback 规则一致 | `REQ-003`,`REQ-004`,`REQ-023` |
| `DEC-009` | 只有 channel acquisition failure 与 Provider/Yuheng-stage `UNAVAILABLE` 可自动重试/换节点；业务错误和其他语义 status 立即报错 | User（2026-08-21 决定 2） | “不满足业务条件需要报错”；避免业务拒绝被另一实例或 FAIL_OPEN 隐藏 | `REQ-004`,`REQ-020` |
...
| `DEC-012` | Generic 使用 canonical `fullMethodName + Protobuf bytes`，可选 Descriptor/DynamicMessage convenience；唯一格式为 `fully.qualified.Service/Method` | User（2026-08-21 决定 5）+ `EVD-025` | 复用 gRPC/Yuheng 既有 identity，不增加 Map/Object[] 或 Generic Proto 服务；点号格式不是 alias | `REQ-016`,`REQ-021` |
...
| Registry/event | Tianshu Registry RPC + Redisson Topic | `DdcRpcAutoConfiguration`、Tianshu subscription classes | Redis event 是提示，full snapshot/revision/lease 是权威 |
| Consumer architecture | neutral Directory SPI + Tianshu adapter + SmartLifecycle managers | `RpcProviderDirectory`、`RpcGatewayDirectory`、两个 Manager | RPC Starter 不依赖 Tianshu 实现；Adapter 转换 metadata/快照 |
...
| Persistence | Tianshu Redis transient registry + optional Guard transient rate keys；无 relation table | Tianshu repository、Guard Redisson backend/key factory | §11 设计新算法 suffix keys；无 Flyway/ER；不改 Tianshu keys/旧 migration |
| Repository state | Dirty worktree with unrelated Yuheng/spec/plan work | 2026-08-21 `git status` | 后续实施需刷新基线、逐文件避让并 path-limited commit；本 Spec 只改本文档 |
...
| --- | --- | --- | --- | --- |
| Other — feature-oriented Core SPI + Adapter + AutoConfiguration | `top.egon.cola.component.rpc` | `annotation/config/contract/consumer/provider/context/exception` 当前树；Tianshu adapter 独立模块 | 不使用 `biz.controller/service/dao`，这是基础设施组件而非三层业务模块 | Preserve current structure；不迁移到传统三层、DDD/COLA |
...
| `RpcReferenceStrategyFactory` + Yuheng/Direct strategies | New/Merge | `REQ-001`–`REQ-004` | BeanPostProcessor if/else + 两 Proxy Factory | 两模式缺共同创建合同，且调用循环会把模式分支散落在 executor | 一个按 mode 枚举建策略的 Factory；两个策略各只注册/读取自身 Directory | Add；Factory 只在代理创建时选择一次，禁止跨模式 |
...
| Existing Tianshu subscription/cache | Keep | `REQ-005`,`REQ-006` | 新建 RPC polling cache | 当前机制已完整，复制会双写/双线程 | 无新增调用；继续每 interval 拉取 | Keep |
| `weight` on neutral/Yuheng/Provider endpoint models | Expand | `REQ-008`,`REQ-009` | 所有 weight=100 | 两种 Reference 均无法执行加权算法 | metadata decode 字段；wire不变 | Add default/fields |
...
| New Tianshu Proto/Redis key | Candidate | none | 复用 metadata/snapshot/topic | 无需求证明 | wire/部署/兼容成本高 | Remove |
| New generic envelope RPC service | Candidate | `REQ-016` | raw existing canonical fullMethodName | 增第二 endpoint/权限/路由且非必要 | Proto/Provider/Yuheng 全链变更 | Remove（`DEC-012`） |
...
flowchart LR
    ...
    subgraph ConsumerJVM["RPC Consumer JVM"]
        ...
        GatewayStrategy["Yuheng Reference Strategy"]
        DirectStrategy["Direct Reference Strategy"]
        GatewayCache["Yuheng exact-key snapshots"]
    ...
    subgraph ControlPlane["Existing control plane boundaries"]
        Tianshu["Tianshu Registry RPC"]
        Redis[("Redis leases and registry topics")]
    end

    Yuheng["Existing Yuheng RPC data plane"]
    ...
    Pool -->|"Yuheng-mode HTTP/2 unary"| Yuheng
    Pool -->|"Direct-mode HTTP/2 unary"| Server
    Yuheng -->|"existing routed unary"| Server
    Lifecycle -->|"register and heartbeat"| Tianshu
    Tianshu --> Redis
    Redis -->|"topic event"| GatewayCache
    Redis -->|"topic event"| ProviderCache
    GatewayCache -->|"periodic full pull"| Tianshu
    ProviderCache -->|"periodic full pull"| Tianshu
    Server --> Rate --> Bean
...
| CGLIB Proxy/plan cache | Java method到预编译执行计划 | Method+request→blocking value/future | validator、executor | Tianshu/Channel ownership | `REQ-014`,`REQ-015`,`REQ-018` |
| Reference Strategy Factory | 依据 Reference mode 一次性创建 Yuheng 或 Direct strategy | normalized definition→one strategy | two Directory managers | 运行时切换 mode、跨模式注册 demand | `REQ-001`–`REQ-004` |
...
| Provider/Yuheng managers | exact query快照、候选过滤、subscription refs | Directory snapshots→candidates | neutral Directory、LB、Channel pool | Tianshu 实现类型、业务 Route 发布 | `REQ-005`–`REQ-009`,`REQ-017` |
| Tianshu adapter | Tianshu key/snapshot/metadata 与 neutral RPC model转换 | Tianshu models↔RPC endpoints | Tianshu SDK、RPC SPI | 调用策略/限流/业务授权 | `REQ-005`–`REQ-009`,`REQ-021` |
| Tianshu Registry | 临时租约、revision、catalog、topic | register/heartbeat/query/event | Redis | 主动调用 Provider 健康检查 | `REQ-005`–`REQ-007` |
...
| Consumer lifecycle coordinator | subscriptions/managers/pool/invocation gate state | Spring start/stop + registered demands | managers、generic cache、pool、executor gate | Tianshu implementation、LB algorithm、business retry | `REQ-005`,`REQ-011`,`REQ-023` |
| Provider lifecycle | Server/lease/heartbeat/runtime state | Spring start/stop | Registry SPI、Server、availability | Tianshu 主动探活、业务授权 | `REQ-007`,`REQ-010`,`REQ-011` |
...
| Yuheng | existing Route/security/governance/forwarding | canonical fullMethodName unary | existing Yuheng subsystems | Direct Reference fallback、Consumer 注解解析、Provider heartbeat | `REQ-003`,`REQ-021` |
...
| --- | --- | --- | --- | --- | --- | --- |
| Discovery convergence | 事件快速、轮询兜底、无每调用 RPC | keep Tianshu coordinator + immutable Manager cache | Tianshu outage只本地过期，不延长 | 最多 reconcile interval 陈旧 | fake clock/event/revision tests + live gap | `REQ-005`,`REQ-006` |
| Mode isolation/availability | 多实例失败只在所配模式内换节点 | Factory 固定 Strategy；executor 维护 attempted identities | 同模式候选/预算耗尽即报错；无跨模式 demand/call | 可能重复业务执行，但拓扑行为可预测 | Direct/Yuheng no-cross-mode + failure-stage matrix | `REQ-003`,`REQ-004`,`REQ-019`,`REQ-020` |
...
| 9 | Provider Lifecycle→Tianshu | register/heartbeat/deregister | ProviderRegistration→lease/result | Redis TTL/revision/event | lost lease unavailability then recovery | `REQ-007`,`REQ-010`,`REQ-011` |
...
sequenceDiagram
    ...
    participant Tianshu as Tianshu Registry
    participant Redis as Redis Topic/Lease

    par Background discovery
        Redis-->>Cache: DdcRegistryEvent hint
        Cache->>Tianshu: GetServiceInstances full snapshot
        Tianshu-->>Cache: revision + leased endpoints
    and Provider liveness
        Provider->>Tianshu: heartbeat(active lease)
        Tianshu->>Redis: renew TTL only
    end
...
| Provider lease | Tianshu Registry + Provider lease manager | exact service+instance+lease identity, Redis lock/TTL | stale heartbeat/deregister rejected | Tianshu returns renewed/registered lease | Provider unavailable/re-register | `REQ-007`,`REQ-010`,`REQ-011` |
...
| Tianshu initial pull fails | subscription start exception | Yuheng primary保持 fail-fast；Direct 进入 DEGRADED/按当前配置失败 | no cache | no business retry；不注册另一模式 demand | mode-specific discovery unavailable | Tianshu/operator | auto-config tests + live gap |
...
| Tianshu reconcile fails | exception/log | expire locally, keep unexpired | no lease extension | next scheduled reconcile | empty after expiry→unavailable | Tianshu/operator | subscription tests |
...
| --- | --- | --- | --- | --- | --- | --- |
| invocation structured log | Executor on terminal/reselection | service, method, immutable mode, algorithm, attemptCount, distinctEndpointCount, errorCode, latency, invocationId | no request/response/Authorization/hash key | warn on exhaustion；normal reselection debug/trace | investigate same-mode endpoints/Tianshu | source/unit; production aggregation unverified |
| discovery log | Manager on snapshot/reconcile failure | query service identity, old/new revision, endpoint count | no Tianshu credential/metadata payload | warn on reconcile failure or revision anomaly | check Tianshu RPC/Redis | source/component; live unverified |
| lease lifecycle log | LeaseManager | service identity, instanceId, leaseId hash/suffix, state/result | no admission ticket/HMAC secret | warn on heartbeat/recovery/deregister failure | check Tianshu/admission | existing + tests; live unverified |
...
| --- | --- | --- | --- | --- | --- |
| 复用 Tianshu 订阅而非新建 cache | `EVD-007`–`EVD-010` | `REQ-005`,`REQ-006` | 保留 coordinator/current snapshot，扩展 RPC endpoint model/LB | 最少状态；更新延迟受 reconcile interval 约束 | event/poll/revision/expiry tests + live boundary |
...
| Mode 由客户端固定且不跨模式 | 用户 2026-08-21 决定 1、`EVD-020` | `REQ-003`,`REQ-004` | Strategy Factory 创建一个 immutable-mode strategy；executor 只排除/重选其候选 | 去掉 Yuheng fallback 容灾，但拓扑、安全和错误可预测 | Direct/Yuheng 双向 no-call tests + process call counters |
...
│   │   ├── yuheng/{RpcConsumerGatewayManager,GatewayRpcInvocationChannelProvider,RpcGatewayDirectory,RpcGatewayEndpoint,RpcGatewaySnapshot}.java
...
└── egon-cola-component-rpc-tianshu-adapter/src/main/java/top/egon/cola/component/rpc/tianshu/
...
| Modify | `consumer.yuheng/provider Managers` | cache selection | 保留订阅/快照，改用 LB+pool；只提供各自模式 primary demand | neutral Directory | `REQ-003`–`REQ-009`,`REQ-017` |
...
| Modify | Tianshu Provider/Yuheng Directories | metadata projection | `ServiceInstanceMetaCodec.decode(...).weight`→neutral endpoint | Tianshu SDK | `REQ-008`,`REQ-009`,`REQ-021` |
| Modify | README/tests/properties/errors | docs/config/error/tests | 公开合同同步、兼容/验收 | existing build | `REQ-022` |

## 9. Interface Definitions

本章只设计 RPC Component 的受影响 Java API/SPI。业务 `.proto` unary operation、Tianshu Registry RPC 和 Yuheng RPC 入站/转发 operation 均保持 wire 不变，不为它们分配新的受影响接口 ID。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `INTERNAL-001` | Existing/Keep+runtime expansion | Yuheng Reference declaration | Java annotation API | Spring business bean field | RPC Starter | `@EgonRpcReference` | shared call-site fields | Yuheng-primary typed proxy | existing RPC metadata/auth | startup contract error；runtime stable RPC errors | existing annotation binary/source shape; additive resolver field only | `REQ-001`,`REQ-002`,`REQ-008`,`REQ-018`–`REQ-020` |
...
| Independent consumer goal | 业务字段以 Yuheng 为主路径获得 typed proxy |
...
| Round trips and failure points | 成功一条 Yuheng unary；重试使用同总 Deadline；不访问 Provider Directory |
| Verdict | Keep/Expand runtime，满足 `REQ-001`,`REQ-002` |

##### Identity and purpose

- Target: `FIELD`，runtime retention；字段类型继续必须是 `@EgonRpcService` interface。
- Mode: `GATEWAY` primary；注册 primary Yuheng demand，启动行为保持现有 fail-fast。
...
| primary Yuheng Directory 缺失/启动未 READY | `RPC_YUHENG_UNAVAILABLE` | 仅按 `DEC-009` availability policy |
...
3. Yuheng Strategy 注册 primary demand；CGLIB Factory 预生成 Method Plan。
4. Invocation 通过 Yuheng endpoint cache/LB/shared Channel 发出。
5. Retry 只按 §7.3.4 availability 白名单在其他 Yuheng 实例上执行；总 Deadline 不重置，不读取 idempotent。
6. Yuheng availability exhaustion 后才执行 failStrategy；业务/语义错误直接抛出；Direct path 永不作为 Yuheng primary 的 fallback。
7. RPC Component 无 frontend；Caller 处理返回值/异常/future。

##### Compatibility and verification

- 旧注解源码与二进制属性保持；运行行为变化是原来未接线的 retries/loadBalance/failStrategy 开始生效。
- 测试：旧 fixture 编译、Yuheng-only Directory 查询、字段覆盖、CGLIB 类型、Method Plan、每种终态策略。
...
| Concern | Decision |
| --- | --- |
| Change classification | Existing annotation，补与 Yuheng Reference 同义的共同 call-site 字段；不增加跨模式字段 |
...
| Round trips and failure points | 成功 1 RTT；失败最多 `min(1+retries, distinct Direct candidates)`；不产生 Yuheng RTT |
...
- Mode immutable: Proxy 创建后只持有 `DirectRpcReferenceStrategy`；没有 `fallbackToGateway` 属性、Yuheng demand 或 runtime switch API。
...
与 INTERNAL-004/INTERNAL-005 相同。Direct 成功只来自选中的 RPC_PROVIDER；任何 Yuheng 调用都视为违反 `DEC-008`。

##### Error responses

| Condition | Stable outcome | Notes |
| --- | --- | --- |
| exact Provider cache empty/expired | `RPC_PROVIDER_UNAVAILABLE` | 无每调用 Tianshu pull；不查询 Yuheng |
...
2. Strategy Factory 只注册 Provider subscription；不得解析/注入 Yuheng Directory。
...
- 新共同属性全部 additive 且有默认值；Direct 成功/失败都不依赖 Yuheng，与前置 Spec 保持兼容。
- 测试覆盖 Direct-only、Direct success、multi-provider reselection、all-failed、Yuheng manager 调用计数恒为零、业务错误不重试和总 Deadline。
...
| Independent consumer goal | Provider owner在具体导出方法边界保护 Direct/Yuheng 两类入站调用 |
...
| Direct/no-new-interface alternative | 往 `@EgonRpcProvider` 加字段会复制 Guard 并破坏 marker 单一职责；业务手写会复制算法；Yuheng limit 对 Direct 无效 |
...
`@EgonRpcProvider` 的 `@Component`、TYPE target、runtime retention、无字段和“至少实现一个 `@EgonRpcService`”规则全部保持。`@RateLimitGuard` 放在 Provider 实现方法上，METHOD target、runtime retention，`value` 是必填 Guard rule ID，`key` 是可选显式 key binding；该 rule 只能启用 rate-limit policy。Rate policy 不是服务身份/Proto metadata，不上报 Tianshu。
...
| `AccessGuardRejectedException` 且 `outcome.decision=RATE_LIMITED` | `UNAVAILABLE` + `x-egon-rpc-failure-stage=provider` + `x-egon-rpc-error-type=rate-limit` | Direct exhaustion=`RPC_RATE_LIMITED`；Yuheng exhaustion=`RPC_PROVIDER_UNAVAILABLE` | Yes，按同模式未尝试实例和总预算 |
...
No Provider/Yuheng/Proto changes. Tests compare typed and raw payload bytes for same Echo method, invalid service/method, malformed request, auth interceptor presence and no arbitrary metadata injection.
...
Context exposes service identity、fullMethodName、read-only request Message；不暴露 Authorization/Tianshu secret。
...
| Existing `RpcProviderSnapshot` | reuse/expand only through endpoint | Directory→Manager | revision/observedAt/list语义已匹配，不新建 cache DTO | Tianshu adapter | `REQ-005`,`REQ-006` |
| Existing `RpcProviderEndpoint` | neutral endpoint record, add weight | Directory→LB/pool | exact lease/transport already owned；weight是选择必需同生命周期字段 | Tianshu adapter | `REQ-008`,`REQ-009` |
| Existing `RpcGatewayEndpoint` | neutral endpoint record, add weight | Yuheng Directory→LB/pool | Yuheng Reference 也公开 loadBalance；必须与 Provider 使用同一 weight 语义 | Tianshu adapter | `REQ-008`,`REQ-009` |

这些对象均非 PO/ORM Entity/DTO/VO。RPC 模块没有数据库对象；`Command` 只用于说明 `RpcGenericInvocation` 是一次调用意图，不引入传统三层/DDD 类型体系。

### 10.2 Persistence objects, ORM entities, and business data objects

N/A — §3.3 已证明本范围无关系持久化。Tianshu Redis lease 是 Context-only 的外部临时事实；Consumer cache/LB/Channel 是进程内对象。Access Guard 可按现有配置把 rate-limit state 放在 Local 内存或 Redisson key-value backend，但本规格不改变关系模型、不创建 PO/Entity/表，也不新增 Redis key namespace。
...
| `.bizCode/.appCode/.env` | String | Direct required；Yuheng absent | exact target Provider scope | Direct annotation/process | `REQ-003`–`REQ-006` |
...
| `RpcProviderEndpoint.weight` / `RpcGatewayEndpoint.weight` | int | default 100 | 1–10000; remote invalid→codec default | Tianshu metadata `yuheng.weight` | `REQ-008`,`REQ-009` |
...
| existing `consumer.yuheng-max-attempts` | `2` | `1..10` | when retries are fully INHERIT, Yuheng mode resolves retries to `yuheng-max-attempts - 1`；also caps distinct Yuheng endpoint transmissions | preserves current two-Yuheng attempt count while removing only the idempotent gate |
...
| existing `provider.lease-seconds` | `30` | `> heartbeat interval` | Tianshu passive lease TTL request | unchanged |
...
RPC Consumer retry accounting is single-layered：`RpcInvocationExecutor` owns at most `1 + resolvedRetries` transmissions to its configured-mode endpoints，且实际 transmissions=`min(1+resolvedRetries, distinct eligible same-mode endpoints)`；`yuheng-max-attempts` 是 Yuheng endpoint transmission 的兼容上限，不形成 Consumer 内嵌循环。When method/reference/service retries are all INHERIT，Yuheng mode resolves to `yuheng-max-attempts - 1`，Direct mode resolves to `0`。每次失败只在 acquisition/UNAVAILABLE 时把 endpoint 加入 excluded set 并重选；mode 永远不变。Yuheng data plane 对下游 Provider 的现有 Route retry 是 Context-only 的独立层，仍可能让一次 Yuheng endpoint transmission 产生多个 Provider attempts；RPC Consumer metrics必须分开报告 endpoint attempts，不能谎称全链业务执行总次数。
...
No mapper chain is added. Tianshu adapter remains the single owner of `DdcServiceInstance.metadata -> ServiceInstanceMeta.weight -> RpcProviderEndpoint/RpcGatewayEndpoint.weight` conversion。

Provider registration uses `@EgonRpcService.weight` as the default `yuheng.weight` only when `egon.cola.component.rpc.provider.metadata.yuheng.weight` is absent；an explicit deployment metadata value therefore overrides the Contract default for that instance。Both values are validated 1–10000 before registration。`DdcRpcProviderDirectory` and `DdcRpcGatewayDirectory` decode the same canonical metadata into their endpoint weight，so Yuheng and Direct Reference weighted algorithms have identical semantics without a new wire field。
...
- Consumer concrete LB algorithms remain private/package-private in `RpcLoadBalancers`/Factory following existing Yuheng precedent；Guard concrete algorithms remain internal to Guard rate-limit packages/backends，RPC 不引用其实现类型。
...
- Existing Yuheng primary demand retains fail-fast discovery timeout。Direct empty snapshots do not claim READY；they enter DEGRADED and invocation returns Provider unavailable until the Provider snapshot recovers；no fallback-only demand exists。
- DRAINING atomically closes the new-invocation gate，then closes generic cache/demands/subscriptions，then drains Channel pool；new calls fail fast without Tianshu/Channel work。
...
Scope disposition: relational database/migrations are `Not applicable`；Tianshu Redis registry remains `Context-only`；Access Guard Redisson rate-limit transient keys are `Affected` because the two added algorithms need storage-specific state and atomic scripts。
...
| --- | --- | --- | --- | --- |
| Strategy | Yuheng vs Direct mode-specific demand/query/candidate behavior | `consumer.reference.RpcReferenceStrategy` | if/else across injection/executor would scatter mode rules and risk accidental cross-mode calls | existing `RpcInvocationChannelProvider` already expresses mode variation |
| Factory Method / Simple Factory | Select exactly one Reference Strategy and one LB implementation from validated enums | `RpcReferenceStrategyFactory`、`RpcLoadBalancerFactory/RpcLoadBalancers` | proxy/executor direct constructors would duplicate switches and allow mid-call replacement | existing Yuheng `ProviderLoadBalancers` is factory-style registry；Factory output is immutable per plan |
| Strategy | 5 required LB algorithms + compatibility algorithm | `consumer.loadbalance.RpcLoadBalancer` | Manager hardcoded RR cannot vary/test independently | Yuheng uses `ProviderLoadBalancer/ProviderLoadBalancers` |
...
| Observer | Redis event hint updates subscription cache | existing Tianshu subscription | polling-only increases staleness；event-only loses recovery | current `DdcManagedRegistrySubscription` kept |
| Adapter | Tianshu snapshot/metadata to neutral RPC endpoint | `rpc-tianshu-adapter` | Starter must remain registry-neutral | current Tianshu directories |
...
| State model | Provider and Channel lifecycle states | lifecycle/pool | booleans cannot prove READY/DRAINING/terminal transitions | current Yuheng/Tianshu runtimes use explicit states |
...
- Reject a second RPC-side Tianshu cache or polling job. Existing Observer + reconciliation already satisfies the requirement.
...
- Dependency direction remains `rpc-starter` neutral SPI/runtime ← `rpc-tianshu-adapter` implementation；Yuheng/Tianshu production modules do not become RPC Starter dependencies。
- Information hiding: business caller sees annotations/typed proxy/generic API, not Redisson、Tianshu keys、ManagedChannel pool entries or limiter buckets。
...
- Tianshu Adapter contract test uses fake `DdcServiceRegistryClient`/Redisson coordinator fixtures to prove event + periodic full snapshot and weight mapping；does not start live Redis。
...
- Process IT extends current Tianshu Admin/Mock Yuheng/Provider/Consumer harness for two Providers/two Gateways、lease removal、mode isolation counters、same-mode reselection、graceful drain；this is later implementation validation and no service is started during Spec writing。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `TEST-001` | Unit | Reference resolver | existing Yuheng annotation defaults | mode Yuheng；old fields preserved | reflection fixture | starter annotation/reference test | `REQ-001`,`REQ-002` |
...
| `TEST-004` | Unit | Direct Strategy | Direct success | Yuheng never asked | fake managers | reference test | `REQ-003` |
| `TEST-005` | Unit | Direct Strategy | first Provider UNAVAILABLE；second healthy | second selected by effective LB；Yuheng manager calls 0 | fake endpoints/status | reference/executor test | `REQ-003`,`REQ-004` |
| `TEST-006` | Unit | Direct Strategy | all Providers UNAVAILABLE/exhausted | `RPC_PROVIDER_UNAVAILABLE`；Yuheng manager calls 0 | fake managers | executor test | `REQ-003`,`REQ-004` |
| `TEST-007` | Unit | Reference Strategy Factory | Direct/Yuheng definitions | exactly one matching Strategy；no runtime switch method/other Directory dependency | fake factories/managers | reference test | `REQ-003`,`REQ-004` |
| `TEST-008` | Unit | Yuheng Strategy | Yuheng primary failure | never implicit Direct | fake Provider manager | reference test | `REQ-002`,`REQ-003` |
...
| `TEST-011` | Unit | Tianshu subscription | initial start | listener registered before full pull；snapshot published | fake Redisson/loader | Tianshu starter test | `REQ-005` |
| `TEST-012` | Unit | Tianshu subscription | relevant event burst | refresh coalesced and full pull | scheduler fake | Tianshu test | `REQ-005` |
| `TEST-013` | Unit | Tianshu subscription | event lost | periodic pull converges | fake clock | Tianshu test | `REQ-005`,`REQ-006` |
| `TEST-014` | Unit | Provider Manager | revision N then N-1 | N-1 ignored | snapshots | manager test | `REQ-006` |
| `TEST-015` | Unit | cache expiry | Tianshu down past lease expiry | endpoint no longer selectable | fake clock/channel | manager test | `REQ-006` |
...
| `TEST-023` | Adapter | weight projection | Provider/Yuheng metadata 80/invalid/missing | both endpoint kinds 80/default100/default100 | Tianshu snapshots | both Tianshu Directory tests | `REQ-009`,`REQ-021` |
...
| `TEST-035` | Unit | Tianshu boundary | heartbeat | no reverse Provider call/probe type | module boundary/static | Tianshu/RPC boundary test | `REQ-007`,`REQ-021` |
...
| `TEST-045` | Unit | RPC Guard Adapter/status mapper | RATE_LIMITED vs other Guard rejection；Direct vs Yuheng trailers | only RATE_LIMITED→UNAVAILABLE；Direct error-type maps RPC_RATE_LIMITED；Yuheng stage-only maps RPC_PROVIDER_UNAVAILABLE；other Guard rejection delegates | outcome/trailer fixtures | mapper test | `REQ-013`,`REQ-020` |
...
| `TEST-051` | Unit | Executor | rate-limit UNAVAILABLE then healthy；all-rate-limited；mixed unavailable+limited | second endpoint selected/mode unchanged；all-rate Direct→RPC_RATE_LIMITED；mixed Direct→RPC_PROVIDER_UNAVAILABLE；Yuheng→RPC_PROVIDER_UNAVAILABLE | status/trailer fixtures | executor test | `REQ-013`,`REQ-020` |
...
| `TEST-066` | Process | Direct mode isolation | two Providers；first unavailable/limited，second healthy；Yuheng counter enabled | Direct reselects Provider and Yuheng counter remains 0 | existing harness | RpcProcessIT | `REQ-003`,`REQ-004`,`REQ-019` |
| `TEST-067` | Process | Yuheng mode isolation | two Gateways fail/exhaust；Direct Providers healthy | Yuheng reselect/exhaust follows policy and Direct Provider-direct counter remains 0 | harness | RpcProcessIT | `REQ-003`,`REQ-004` |
...
| `TEST-070` | Unit | Consumer lifecycle | registered Yuheng primary + Direct primary demands | only configured-mode subscriptions/pool start before gate；READY/DEGRADED exact；no fallback-only demand | fake managers/pool | lifecycle coordinator test | `REQ-005`,`REQ-023` |
| `TEST-071` | Unit | Consumer startup failure | primary Yuheng discovery throws/times out | FAILED then all opened subscriptions/pool entries close；no invocation accepted | fake failure/latches | lifecycle coordinator test | `REQ-011`,`REQ-023` |
...
| Availability | Redis event loss recovered by poll；Tianshu outage never extends expired lease；mode never changes | existing coordinator + user decision | event hint + full snapshot + local expiry + same-mode candidate exclusion | empty/exhausted cache produces mode-specific unavailable；no cross-mode rescue | event/poll/outage/no-cross-mode tests |
...
- Direct/Yuheng discovery target仍来自 Reference/Contract/server configuration，不从未经验证的 inbound request Header 选择 Tianshu scope。
...
- Behavior: Yuheng Reference 永远只走 Yuheng；Direct Reference 永远只走 Provider。可用性失败开始按 LB 换同模式节点；业务错误仍报错且不触发 fail strategy。
...
- Wire: no business/Tianshu Proto modification；service/method/request/response descriptor unchanged。Generic call只接受 existing canonical `fully.qualified.Service/Method`。
- Status: add Direct-observable `RPC_RATE_LIMITED` application code；Guard limit transport status is always `UNAVAILABLE` + Provider stage，Provider/Direct 还带 rate-limit type；Yuheng 当前重建 trailer 后映射 `RPC_PROVIDER_UNAVAILABLE`。不增加 `RPC_FAILOVER_EXHAUSTED`。
- Configuration: existing RPC properties remain；fully inherited Yuheng calls preserve `yuheng-max-attempts=2` but no longer check idempotent，fully inherited Direct calls preserve zero retries。Guard `algorithm=TOKEN_BUCKET` default不变，新增 enum values只在显式配置时生效。
...
2. Publish Access Guard algorithm extension and RPC Starter/Tianshu Adapter at one compatible component version；Tianshu Admin/Yuheng need no production code deployment。
...
5. Upgrade Consumers with retries=0 first；verify discovery cache/channel pool/CGLIB and Direct/Yuheng no-cross-mode counters。
...
- Consumer can roll back to previous Starter because wire/Proto/Tianshu storage unchanged；new async Java Contract binaries cannot run on an old runtime that rejects return shape, so roll back Contract/Provider/Consumer together for those methods。
- `@EgonRpcProvider`/`@RateLimitGuard` 无新字段，不需要 annotation binary rollback。Async Contract仍需 Contract/Provider/Consumer成组回退。
- Same-mode retries 可把 Reference `retries=0` 后 redeploy；不存在跨模式 fallback 开关或 Yuheng Route cleanup。
...
| D — use Yuheng for all LB/retry/limit | no Direct governance | centralized | contradicts Consumer/Provider-side requirements and Direct mode | conflicts user | Rejected |
| E — new GenericInvoke Proto service | envelope method/Provider adapter/Yuheng route | Map-like UX possible | second protocol、auth/routing/version/size attack surface | conflicts Proto single authority/minimal design | Rejected by `DEC-012` |
...
| --- | --- | --- | --- |
| Mode failure | same-mode endpoint reselection | Direct/Yuheng cross-mode fallback | user fixed mode on client；前置 Direct Spec也禁止 fallback；select same-mode only |
...
| Generic | canonical `Service/Method` + bytes/descriptor | dot alias、Map/Object[] or envelope | existing gRPC/Yuheng identity and no second serializer |
...
| `RISK-010` | Yuheng 当前不保留 Provider rate-limit error-type trailer | Confirmed static | Yuheng-mode exhaustion maps `RPC_PROVIDER_UNAVAILABLE` instead of `RPC_RATE_LIMITED` | 两者 transport status均为 UNAVAILABLE；Spec明确 mode-specific application-code差异；若未来要求统一应用码，另行修改 Yuheng safe trailer allowlist | Closed by explicit scope/contract |
...
| `RISK-012` | Tianshu reconcile interval 10s and lease 30s may be unsuitable for some deployments | Medium | slow change convergence or load | existing configurable Tianshu property；capacity test before production | Open runtime tuning, not code blocker |
...
| --- | --- | --- | --- | --- | --- | --- |
| `REQ-001` | `UC-001` | reference resolver/proxy `§7–§10,§13–§18` | Tianshu/Yuheng contracts unchanged | `INTERNAL-001`,`INTERNAL-002`; `RpcReferencePolicy` | `TEST-001`–`003`,`056` | both annotations resolve identical common semantics |
| `REQ-002` | `UC-001` | Yuheng strategy/proxy | Provider Directory unchanged for Yuheng | `INTERNAL-001`,`INTERNAL-004/005` | `TEST-001`,`008`,`009`,`057`,`065` | old source compiles and queries only Yuheng |
| `REQ-003` | `UC-001` | Strategy Factory/mode isolation | Yuheng/Provider Directory implementations context-only | `INTERNAL-001`,`INTERNAL-002`; immutable mode | `TEST-004`–`008`,`066`,`067` | Direct/Yuheng opposite-mode call count is zero |
...
| `REQ-005` | `UC-002` | Manager cache/LB integration | existing Tianshu coordinator kept | existing snapshot models | `TEST-010`–`013`,`069` | one cache/query, event+poll updates；generic demands bounded |
| `REQ-006` | `UC-002` | revision/expiry | Tianshu Redis storage context | endpoint/snapshot | `TEST-013`–`015` | no old revision/expired endpoint |
| `REQ-007` | `UC-005` | Provider lifecycle/config | Tianshu no probe | Provider registration/state; DB N/A | `TEST-031`–`035` | RPC-owned heartbeat and validated intervals |
| `REQ-008` | `UC-003` | LB strategies | Yuheng LB unchanged | LoadBalance enum/SPI | `TEST-016`–`025` | all five algorithms + compatible least-in-flight |
| `REQ-009` | `UC-003` | weight/hash resolver | Tianshu metadata wire unchanged | endpoint weight; `INTERNAL-008` | `TEST-017`,`019`–`023` | weighted distribution and stable key |
| `REQ-010` | `UC-005` | Provider state/start order | Tianshu lease API unchanged | runtime state | `TEST-031`–`033` | READY only after lease, clean failure |
...
| `REQ-013` | `UC-006` | Guard AOP/RPC Adapter/error mapping | Yuheng safe trailer reconstruction context-only | `INTERNAL-003`; Direct `RPC_RATE_LIMITED` / Yuheng `RPC_PROVIDER_UNAVAILABLE` | `TEST-044`,`045`,`047`,`051`,`066`,`073`,`075` | both UNAVAILABLE/provider，bean not invoked，same-mode retry，application-code差异显式 |
...
| `REQ-016` | `UC-004` | generic API/raw marshaller/cache | no new Generic Proto/Yuheng API | `INTERNAL-006`,`INTERNAL-007`; generic invocation/cache | `TEST-062`–`064`,`069` | canonical Service/Method, same unary result/error/security，bounded target state |
...
| `REQ-021` | all | compatibility/rollout | Proto/Tianshu/Yuheng/DB unchanged | Interface/model only; DB/frontend N/A | `TEST-023`,`024`,`035`,`057`,`062`,`064` | zero proto/migration/Yuheng production change |
...
| `REQ-023` | `UC-002` | Consumer lifecycle/gate `§7,§8,§10,§14–§16` | Tianshu/Yuheng primary semantics preserved | `RpcConsumerLifecycleCoordinator/RuntimeState` | `TEST-070`–`072` | subscriptions/pool precede invocation；failure/drain cleanup complete |
...
- Paths/symbols、Java/gRPC/Protobuf/Spring versions、现有 Tianshu subscription、Redis lease、Provider/Consumer lifecycle、tests 和 dirty-worktree 均来自本轮静态检查。
- 本设计保留 current custom feature architecture，不套用三层业务包；无 DAO/DB/frontend。
- 未把静态/Mock 证据表述为 live Tianshu/Redis/gRPC proof。
- 2026-08-21 基线存在无关 Yuheng 删除/未跟踪 Spec/Plan/文件；本次只修改当前 RPC Spec。后续 Plan 必须刷新 RPC/Guard path 与 status，并继续 path-limited 操作。

### 20.3 Cross-section consistency

- Architecture、file tree、API、models、failure matrix、tests、rollout 和 traceability 使用同一“Strategy Factory 固定 Yuheng/Direct mode→same-mode cache/LB→shared pool→endpoint”链，任何章节都不再允许跨模式 fallback。
- Tianshu Event 只触发 full pull；revision/lease full snapshot 是 authority；没有第二 cache/job/topic。
