# Yuheng Admin 单控制面与 API/RPC、MCP 双 Engine 拆分架构

| Field | Value |
| --- | --- |
| Document | [2026-09-02-19-52-yuheng-dual-engine-separation.md](2026-09-02-19-52-yuheng-dual-engine-separation.md) |
| Template Version | `6` |
| Status | `Accepted` |
| Type | `Architecture / Refactor` |
| Complexity | `Complex` |
| Complexity Drivers | 一个 Admin 控制面、两个可独立部署的数据面角色、统一 Release 在多进程内的局部原子激活、Tianshu 发现与 ACK、HTTP/gRPC/MCP 兼容、MCP Redis/PostgreSQL 状态、独立安全身份、HA/灰度/回滚及跨模块依赖重排 |
| Created | `2026-09-02 19:52 CST` |
| Updated | `2026-09-05 07:01 CST` |
| Owner | `User / Egon-COLA Yuheng maintainers` |
| Repository | `Egon-COLA` |
| Scope | `egon-cola-xingyuan/egon-cola-yuheng`；Yuheng Admin、共享 Runtime、现有 API/RPC Engine、新 MCP Engine、Admin Web、Yuheng 测试与部署/本地运行契约 |
| Change Surface | 新增 `yuheng-runtime-core` 与可执行 `yuheng-mcp-gateway`；现有 `yuheng-biz-gateway` 移除 MCP 入口/Bean/依赖并保留 API+RPC；统一 Rule Artifact 由两个 Plane Compiler 分别编译和局部原子激活；Tianshu 元数据增加固定 Engine Role；Admin Runtime Consistency 要求 `API_RPC` 与 `MCP` 两个角色；更新 Admin Web、测试、Compose、脚本、CI 和运行文档；不改数据库 Schema、HTTP/gRPC/MCP Wire Contract、Provider 上报或 Admin 管理写 API |
| Affected Chapters | `§7, §8, §9, §10, §12, §13, §14, §15, §16, §17, §18` |
| Source Requirement | 用户请求“把 mcp yuheng 和 api 和 rpc yuheng 拆分开，现在混在一起的”；用户于 2026-09-02 决定“一个 admin server 两个 yuheng engine” |
...
| Amends | [Yuheng Engine 与 MCP Core 功能域分包设计](2026-08-19-13-51-yuheng-biz-yuheng-mcp-package-refactor.md) 的 §3.2、§5.1、§5.3 DEC-004、§7–§9、§12、§14–§18、§20；[Egon Yuheng 全能力 MCP 网关需求与技术设计](../../superpowers/specs/2026-08-02-yuheng-complete-mcp-design.md) 的 §5.1–§5.2、§6–§7、§13.1–§13.2、§21.3 中“同一进程”部署结论 |
| Supersedes | `None` |
| Depends On | [Egon Yuheng 全能力 MCP 网关需求与技术设计](../../superpowers/specs/2026-08-02-yuheng-complete-mcp-design.md) 的 §3–§4、§8–§12、§14–§20、§21.1–§21.2；[GWS-09 Yuheng Admin 后端 Spec](../../superpowers/specs/2026-07-25-yuheng-admin-backend-design.md) 的 §3.5–§3.6、§6.4、§8、§14–§16 |
| Related Specs | [Yuheng HTTP OpenAPI 3.1 单一事实源改造](2026-08-25-19-01-yuheng-openapi31-source-refactor.md)；[Yuheng 多 OpenAPI Group 增补](2026-08-25-19-43-yuheng-openapi-group-aggregation-amendment.md)；[Yuheng BIZ/APP 与直连 RPC](2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) |
| Related Plans | [Yuheng Dual-Engine Separation Implementation Plan](../plan/2026-09-02-21-03-yuheng-dual-engine-separation-implementation.md) |

## 1. Summary

当前 Yuheng 已有 `yuheng-mcp-core` 类库和按 HTTP、RPC、MCP 分组的源码包，但运行时仍只有一个 `GatewayEngineApplication`、一个
...
选定方向是保留一个逻辑 Yuheng Admin 控制面和两个数据面 Engine 角色：现有 `yuheng-biz-gateway` 保留
...
API/RPC Yuheng，从而保持当前调用跳数、鉴权和重试语义。

Admin 继续生成一个 Canonical `GatewayRuleSnapshot`、一个 Artifact SHA 和同一个 `yuheng.rules.active`。两个 Engine
在各自进程中使用 Plane Compiler Strategy 编译同一 Snapshot、维护独立 LKG 和 `AtomicReference`，并通过 Tianshu
...
平台维护者希望将 MCP 的协议、会话、任务、远端联邦、审批和存储故障域从 API/RPC 数据面隔离，同时继续使用统一的 Yuheng
Admin、Catalog、Release、Tianshu、Provider Directory、安全、流量治理和可观测性。API 客户端、RPC Consumer 与 MCP Client
...
| --- | --- | --- | --- | --- | --- |
| `EVD-001` | Static repository | `yuheng-biz-gateway/pom.xml`、`GatewayEngineApplication` | 只有一个可执行 Engine Artifact/MainClass；POM 同时依赖 MCP Core、gRPC、JDBC、PostgreSQL、Redisson、Tianquan-Shoubing、Tianquan-Jianshen、Tianshu、Kafka | 现状是依赖和进程混合，而不是仅包名混合 | 静态 POM/源码，不证明生产进程 |
...
| `EVD-005` | Static repository | `CompiledGatewayRules`、`EngineGatewayRuleCompiler` | 单一记录同时含 HTTP Route、RPC Method Index、Provider/Traffic/Security/CORS 与 `CompiledMcpRules`；单一 Compiler 同时编译所有 Plane | 共享 Snapshot 可以保留，但编译结果必须按 Plane 分离 | 不证明 Tianshu 同步时序 |
| `EVD-006` | Static repository | `McpToolsCallHandler`、`McpTaskOperationExecutor`、`EngineGatewayOperationInvoker` | MCP 通过 `GatewayOperationInvoker` 直接选择 Provider 并调用 HTTP/RPC 出站 Adapter；没有必要请求 Yuheng 的公开 Route | 独立 MCP 进程仍可复用直接出站链路，避免 Yuheng-to-Yuheng 双跳 | 仅源码调用链 |
| `EVD-007` | Static repository | `GatewayRuleActivationApplier.ACTIVE_CONFIG_KEY`、`GatewayRuleApplierRegistrar` | Active Key 固定为 `yuheng.rules.active`，Chunk 使用 `yuheng.rules.chunk.*`；Applier 校验、编译、准备 Provider、持久化 LKG 后切换内存引用 | 两个 Engine 可消费同一配置事实并各自局部原子激活 | 不构成跨进程原子性 |
...
| `EVD-009` | Static repository | `DdcManagementConfigClientInstance.metadata`、`DdcInstanceMetadataContributor.metadata()` | Tianshu Config Client 已携带开放的 `Map<String,String>` 元数据；当前 Engine 已贡献 Release/ACK 字段 | 可用一个受限字符串键新增固定角色，不修改 Tianshu 协议/表 | 元数据上线效果未验证 |
| `EVD-010` | Static repository | `yuheng-admin-web/src/api/types.ts`、`GatewayGroupDetailPage`、`McpRuntimeStatus` | Admin Web 已查询 `engine-nodes` 和 `runtime-consistency`，但 `EngineNode` TS 类型遗漏后端已返回的 `metadata`，页面只显示租约角色 | 前端可用现有 API 展示双 Engine Role，无需新页面/接口 | 浏览器未运行 |
| `EVD-011` | Static repository | `deployment/compose*.yml`、`yuheng-biz-gateway/Dockerfile`、`scripts/unified-xingyuan/*` | Compose、Docker、启动/状态/验证脚本、Runbook 和 CI 都只认识一个 Engine Jar；当前第二 Engine 是同角色副本 | 拆分必须覆盖制品、端口、健康、证书、进程名和验证脚本 | 未执行 Compose/脚本 |
| `EVD-012` | Static repository | Engine `src/test`、`mcp-core/src/test`、`yuheng-test-suite` | MCP 单元/集成测试目前位于 Engine，Live Suite 只依赖 `yuheng-biz-gateway`，并把跨节点 MCP 场景解释为两个同构 Engine | 测试所有权必须随可执行角色迁移并增加依赖边界验证 | 仅测试清单 |
...
| `EVD-014` | Authoritative predecessor | `2026-08-02-yuheng-complete-mcp-design.md` §5、§12–§13 | 旧设计拒绝“单独 MCP Yuheng”的原因是 MCP 再 HTTP 调 Yuheng；同时要求单 Snapshot、无自调用、直接 Operation Invoker | 新方案只替换部署拓扑，保留无自调用和单 Snapshot 约束 | 旧文档部分内容已被后续 Spec 更新 |
| `EVD-015` | User decision | “确定 一个 admin server 两个 yuheng engine” | 一个逻辑 Admin、两个 Engine Role 的目标已由决策人确认 | 关闭最主要部署拓扑歧义 | 不指定副本数；见 `ASM-001` |
...
Yuheng。目标是让两个可执行数据面共享同一控制事实和同一核心能力，同时拥有真实的进程、依赖、资源、失败和发布观察边界。

### 2.4 Evidence and current-chain map

| Entry/trigger | Current call chain | Data read/written | External dependency | Consumers | Evidence |
| --- | --- | --- | --- | --- | --- |
| API/HTTP 请求 | `GatewayHttpListener -> GatewayCompositeHttpDataPlaneHandler -> DefaultGatewayHttpDataPlaneHandler -> ProviderSelector -> HTTP/RPC upstream` | Active Rule、Provider/Traffic/Health、Trace | Tianshu、Redis、HTTP/gRPC Provider、Tianquan-Shoubing/Tianquan-Jianshen | API Client | `EVD-003`,`EVD-006` |
| RPC 请求 | `RpcGatewayServer -> RpcGatewayHandlerRegistry -> RpcGatewayForwarder -> RpcProviderChannelCache` | RPC Method Index、Provider/Traffic/Health、Trace | Tianshu、gRPC Provider、Tianquan-Shoubing/Tianquan-Jianshen | RPC Consumer | `EVD-001`,`EVD-002` |
| MCP 请求 | 相同 `GatewayHttpListener -> Composite -> McpEngineHttpHandler -> McpMethodDispatcher -> Handler/Driver -> GatewayOperationInvoker` | MCP Rule、Session、Task、Approval、Provider/Traffic、Trace/Audit | Redis、PostgreSQL、Remote MCP、HTTP/gRPC Provider、Tianquan-Shoubing/Tianquan-Jianshen | MCP Client/Host | `EVD-003`,`EVD-006` |
| Release 激活 | `Admin -> Tianshu yuheng.rules.active/chunk -> one GatewayRuleActivationApplier -> EngineGatewayRuleCompiler -> one AtomicReference` | Admin 发布表、Tianshu Change/ACK、LKG、内存 Snapshot | PostgreSQL、Tianshu、filesystem | 同一 Engine 内 HTTP/RPC/MCP | `EVD-005`,`EVD-007` |
| 运行一致性查询 | `Admin Web/script -> GatewayProjectionController -> GatewayProjectionService -> Tianshu Config Clients` | 读 Release History、Release Target、Config Client metadata/cache | Tianshu Management RPC、Yuheng Admin DB | Operator、Tianquan-Jianshen 状态客户端、验证脚本 | `EVD-008`,`EVD-010` |
| 本地/Compose 启动 | `prepare/start script -> yuheng-biz-gateway-exec.jar -> readiness -> MCP probes` | env/properties、PID/log、LKG、MCP artifact | OS、Docker/Compose、TLS files | Developer/operator | `EVD-011`,`EVD-012` |

## 3. Goals and Non-goals

### 3.1 Goals

1. 保留一个逻辑 Yuheng Admin Server，继续拥有 Catalog、Draft、Release、Tianshu 发布、运行投影和 Admin Web。
...
3. 新增独立可执行 `yuheng-mcp-gateway`，独占 MCP HTTP/Legacy SSE、Session、Task、App、Remote
   Federation、MCP Tianquan-Jianshen/Approval 与 MCP 存储。
4. 新增非可执行 `yuheng-runtime-core`，供两个 Engine 复用 Rule Activation 基础设施、Provider Directory、Traffic、Operation
   Invoker、HTTP/RPC 出站和 Observability；两个可执行 Engine 不互相依赖。
5. 两个 Engine 继续消费一个 Canonical Snapshot、一个 Artifact SHA、一个 Tianshu Active Key；分别编译、LKG、激活、ACK，并由 Admin
   检查角色与版本一致性。
...
7. 保持 MCP Operation 直接调用 Provider；不新增 MCP -> API/RPC Yuheng 内部网络跳转。
8. 为两个 Engine 建立独立应用名、进程、端口、健康、Tianshu 实例、服务身份、TLS、数据目录、日志和可独立扩缩/回滚的部署单元。
...
- 不为两个 Engine 创建两个 Draft、两个 Release、两个 `yuheng.rules.active` 或跨进程两阶段提交协议。
- 不让 API/RPC Engine 代理 MCP，也不让 MCP Engine 通过公开/内部 Yuheng Route 再调用 API/RPC Engine。
- 不改变 HTTP、WebSocket、gRPC、MCP JSON-RPC、Kafka Call Event、Provider Service Key、Operation ID、Tool ID、Session/Task
  状态机和业务错误语义。
- 不修改 Yuheng Starter、OpenAPI/RPC Descriptor 上报的 Source of Truth。
...
| --- | --- | --- | --- | --- | --- |
| Yuheng Maven/module/dependency topology | `Affected` | `yuheng/pom.xml`、Engine/MCP Core POM | 新增 Runtime Core/MCP Engine；现有 Engine 去除 MCP/JDBC 依赖 | 完整模块依赖、制品与边界设计 | `§7, §8, §13, §16, §17, §18` |
...
| Tianshu Engine metadata and Admin consistency | `Affected` | `DdcInstanceMetadataContributor`、`GatewayProjectionService` | 新增固定 Role metadata；一致性必须覆盖两个角色 | 完整内部契约、算法、错误、测试设计 | `§7, §8, §9, §10, §13, §14, §15, §16, §18` |
...
| Yuheng Admin write APIs/Release persistence | `Context-only` | Release Controller/Coordinator/Repository | 继续一个 Release 与现有 Target ACK；不改请求/表/事务 | 只证明统一 Artifact 与停止边界 | `§7, §9, §11, §16` |
...
| Provider/Starter/OpenAPI/RPC Descriptor | `Unchanged` | Yuheng Starter、OpenAPI modules、RPC Contributor | Provider 注册和 Definition Report 不变 | 一条不变记录与回归扫描 | `§9, §14` |
...
| Yuheng Admin business pages outside runtime status | `Unchanged` | Catalog/Draft/Release/MCP configuration pages | 路由、表单、权限和写 API 不变 | 一条不变记录 | `§12, §14` |

## 4. Requirements and Acceptance Criteria

| ID | Atomic requirement | Priority | Observable acceptance criteria | Source |
| --- | --- | --- | --- | --- |
| `REQ-001` | 系统必须保持一个逻辑 Yuheng Admin 和两个可独立部署的 Engine Role：`API_RPC`、`MCP` | Must | Reactor 中恰有两个可执行 Engine Jar；Admin 仍为唯一 Yuheng 控制面 | 用户“一个 admin server 两个 yuheng engine” |
| `REQ-002` | 现有 `yuheng-biz-gateway` 必须保留制品名并只装配 API/HTTP/WebSocket 与 RPC/gRPC 数据面 | Must | 其 POM/源码/Bean/Classpath 不含 `yuheng-mcp-core`、`engine.mcp`、MCP JDBC Store 或 MCP Handler；现有 API/RPC 回归通过 | 兼容优先与当前制品消费者 |
...
| `REQ-004` | 两个 Engine 必须复用共享 Runtime Core，且可执行模块不得相互依赖 | Must | Maven/包边界测试证明 `yuheng-biz-gateway -/-> mcp-engine/mcp-core`，`mcp-engine -/-> yuheng-biz-gateway`，两者均依赖 runtime-core | 避免代码复制和伪拆分 |
| `REQ-005` | MCP 本地 Operation 必须继续直接调用 Provider，不得经 API/RPC Engine 二次转发 | Must | MCP 调用链只有 MCP Engine -> selected HTTP/RPC Provider；无新增内部 Yuheng API、Service Discovery 或第二次鉴权/限流 | 旧 MCP 设计“无自调用”约束 |
| `REQ-006` | Admin 必须继续发布一个 Snapshot/Artifact/Active Key，两个 Engine 分别局部原子激活并 ACK | Must | 相同 Release ID、Tianshu Version、Artifact SHA 到达两个角色；每个进程只有旧/新完整 Snapshot；不出现单进程半编译状态 | 单 Snapshot 与 LKG 约束 |
| `REQ-007` | Admin Runtime Consistency 必须要求 `API_RPC` 与 `MCP` 两个在线角色齐全且所有在线节点 ACK 同一 Release/Version/Checksum | Must | 缺一角色、未知 Role、任一在线节点未 ACK/版本或哈希不符时 `consistent=false`；两个角色的全部在线节点一致时为 true；期望副本数仍由部署系统而非 Yuheng Admin 管理 | 双 Engine 可运维性 |
...
| `REQ-011` | 双 Engine 的新增/迁移 Spring 配置必须保持 Base/Operations Key 结构一致并校验 | Must | 新 MCP Bootstrap keys 在 `application.yml`/`application-operations.yml` 对齐；原 `YUHENG_MCP_*` 环境变量和 MCP Runtime prefix 保持兼容；绑定负例失败启动 | 用户 Java Rule 7 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HTTP/WebSocket API 调用 | API Client | API_RPC Engine Ready、Route Active | API Listener -> Security/Traffic -> Provider | Provider/Tianquan-Shoubing/Tianquan-Jianshen/Traffic 失败沿现有错误映射 | API_RPC 本地 health/circuit/trace | Wire 结果与拆分前一致 | `REQ-002`,`REQ-008`,`REQ-009` |
| gRPC Yuheng 调用 | RPC Consumer | API_RPC Engine RPC Slot Online | gRPC Listener -> Method Index -> Provider | Deadline/cancel/Provider 不可用保持现有 gRPC Status | API_RPC Channel/health/trace | fullMethodName/Metadata 兼容 | `REQ-002`,`REQ-008` |
...
| Release 两角色成功 | Operator/Admin | 两角色在线并订阅同一 Tianshu App | Admin 发布一次；两角色分别 verify/compile/LKG/activate/ACK | N/A | 两份本地 LKG、同一 Release/Version/SHA | Runtime Consistency true | `REQ-006`,`REQ-007` |
| 一角色编译/持久化失败 | Tianshu Apply | 另一角色可能已成功 | 失败角色保留旧 Active/LKG 并 ACK failure；成功角色不自动回滚 | Operator retry/fix/显式 rollback | 跨进程暂时版本分歧 | `consistent=false`，失败角色仍服务旧完整快照 | `REQ-006`,`REQ-007`,`REQ-009` |
| Tianshu 不可用后重启 | Engine restart | 各角色存在有效 LKG | 各自恢复 LKG 并按现有规则决定 Ready | LKG 缺失/损坏则对应角色 Not Ready | 不写 Admin DB | 角色独立健康；不伪造全局一致 | `REQ-006`,`REQ-009` |
...
| 角色缺失或未知 | Admin consistency poll | Tianshu 只见一个 Role 或 metadata 非法 | Role Strategy 计算必需集合差集 | Stale cache 明示 stale；无 cache 的 Tianshu 失败返回现有 503 | 仅读投影 | `consistent=false`，UI 显示缺失/未知 | `REQ-007`,`REQ-012` |
...
| --- | --- | --- | --- | --- | --- |
| `ACTOR-001` | API Client | 通过 HTTP/WebSocket Yuheng 调用业务接口 | 现有 API Yuheng URL | 现有 Tianquan-Shoubing/Tianquan-Jianshen/Route Context | HTTP 数据面源码 |
| `ACTOR-002` | RPC Consumer | 通过 INTERNAL_GATEWAY 调用 gRPC Provider | Tianshu `INTERNAL_GATEWAY` + gRPC | PLATFORM SERVICE/USER Metadata 现有规则 | RPC Slot/Forwarder 源码 |
| `ACTOR-003` | MCP Client/Host | 发现并调用 MCP 能力、会话、任务和远端能力 | `/mcp/**`、Legacy/Metadata paths | MCP Server Resource、USER/SERVICE、Tianquan-Jianshen/Approval | MCP Handler/Core 源码 |
| `ACTOR-004` | Yuheng Operator | 发布统一 Release，观察两个 Engine Role 并处理分歧 | Yuheng Admin/API/Web/Runbook | `CAP_yuheng:read`/管理权限 | Admin Runtime API/Web |
| `ACTOR-005` | Deployment operator | 独立部署、扩缩、切流、回滚两个 Engine | Compose/scripts/CI/runtime | 主机、TLS、Secret 管理权限 | deployment/scripts |
| `ACTOR-006` | Tianshu | 向两个 Engine 分发同一配置并收集租约/ACK/Metadata | Config Client/Management RPC | Tianshu runtime/registry credentials | Tianshu Starter/Client |
...
flowchart LR
    ...
    Operator["ACTOR-004 Yuheng Operator"]
    Deploy["ACTOR-005 Deployment operator"]
    Tianshu["ACTOR-006 Tianshu"]

    subgraph Scope["Yuheng Admin + dual Engine boundary"]
...
- 现有 `yuheng-biz-gateway` Artifact 名、API/RPC 默认端口和 MainClass Consumer 兼容优先。
- MCP 继续直接使用 `GatewayOperationInvoker` 调 Provider；禁止 Yuheng-to-Yuheng 自调用。
- 一个 Yuheng Group 仍只有一个 Draft、Release、Canonical Snapshot、Artifact SHA 和 `yuheng.rules.active`。
...
| `ASM-002` | 两个 Engine 继续使用相同 Tianshu `biz/env/appCode` 配置订阅身份，但 `instanceId` 与 Role metadata 不同 | Admin `engineNodes` 按 target appCode 查询；单 Active Key 依赖同一配置空间 | 可在后续 Spec 改为 Bundle 多目标发布 | 若必须独立 appCode，需要 Admin 多目标发布、一致性和迁移新设计 |
| `ASM-003` | 新 MCP 数据面默认端口为 `18084`，Management 为 `18085`；外部稳定 Host/Path 不变 | 现有 Engine 使用 18081/18082/18083/19090；18084/18085 在 Yuheng Compose 未占用 | 端口是部署配置，可局部覆盖 | 端口冲突只改配置/脚本，不改协议模型 |
| `ASM-004` | MCP Runtime 配置 prefix `egon.cola.component.yuheng.engine.mcp` 与 `YUHENG_MCP_*` 环境变量先保持兼容；仅新 Bootstrap keys 使用 `...yuheng.mcp-engine` | 当前 `McpRuntimeProperties` 和脚本依赖旧 prefix/env | 后续可有弃用期迁移 | 立即改 prefix 会扩大 Tianshu/env/secret 迁移和兼容范围 |
| `ASM-005` | 过渡部署期间不发布新 Yuheng Release | 旧 Combined 与新双 Role 会同时订阅同一 Active Key；发布会扩大目标/ACK 不确定性 | 仅 Runbook 时序约束 | 若必须边切流边发布，需要 mixed-version publication 兼容 Spec |

### 5.3 Resolved decisions

| ID | Decision | Decision owner | Evidence and rationale | Requirements |
| --- | --- | --- | --- | --- |
| `DEC-001` | 一个 Yuheng Admin + `API_RPC`/`MCP` 两个 Engine Role | User | 用户明确确认；API/RPC 共享当前 Yuheng 数据面而 MCP 有独立状态/故障域 | `REQ-001`–`REQ-003` |
| `DEC-002` | 保留现有 `yuheng-biz-gateway` 名称作为 API_RPC Engine，新增 `yuheng-mcp-gateway` | Spec | 比重命名两个 Artifact 少一个制品迁移和调用方变化 | `REQ-002`,`REQ-003`,`REQ-008` |
| `DEC-003` | 新增 `yuheng-runtime-core` 共享非可执行能力；禁止两个 executable 互相依赖 | Spec | 直接让 MCP 依赖现有 Engine 会把 API/RPC Classpath 重新带回 MCP，属于伪拆分 | `REQ-004` |
...
| `DEC-008` | MCP Engine 使用独立 Tianquan-Shoubing Resource/Service Client/TLS Secret/数据目录；共享业务 MCP Server Resource 与现有存储 | Spec | 进程身份必须隔离，业务 Token/Server Resource Contract 不应变化 | `REQ-008`–`REQ-010` |

### 5.4 Open major decisions

`None`。用户已关闭部署拓扑决策；`ASM-*` 是保持现有协议和发布事实的局部、可逆实现推断。若未来要求两个独立 Tianshu
...
| RPC | gRPC 1.75.0、Protobuf 4.32.0、Unary Yuheng | Platform POM、RPC Server/Forwarder | API_RPC 拥有 Listener/Slot；Runtime Core 拥有共享出站 Channel Adapter |
| MCP | `yuheng-mcp-core` + Engine adapters；Stable/RC/Legacy | MCP Core/Handler/legacy Spec | MCP Core 仍为类库；新 MCP Engine 负责装配和 HTTP Ingress |
| Config/discovery | Tianshu Config Client + Service Registry + Redis reconciliation | Tianshu Starter/Adapter | 相同 appCode/Active Key；固定 Role metadata；不新增 Tianshu API |
| Security | Tianquan-Shoubing Yuheng Adapter/Starter、Tianquan-Jianshen Adapter/Starter、MCP Approval | Engine POM/config/classes | 两个进程独立 Resource/Service identity；业务授权和 Token 转发不变 |
...
| --- | --- | --- | --- | --- |
| Traditional Layered 的仓库批准 feature-first infrastructure variant | `top.egon.cola.component.yuheng.{admin,engine,mcp,runtime}`；feature 下 `controller/service/repository/domain/adapter/bootstrap` | 当前 Yuheng tree、`GatewayEnginePackageBoundaryTest`、Accepted `2026-08-19` §6.1、Accepted OpenAPI Spec §6.1 | Runtime/Data Plane 不使用 `biz.*`/`service.impl`；这是已批准的既有非业务基础设施结构 | 保持 feature-first + role package；只新增部署/module boundary，不引入 COLA/DDD/第三种业务层 |

当前和目标都不是任一 `egon-cola-archetype-*` 生成工程。本 Amendment 继承已批准的 Yuheng 基础设施变体，不把 Runtime
...
| Tianshu Role | Enum + Map | Tianshu Starter | `DdcInstanceMetadataContributor` | 现有 metadata 无 Engine Role | 复用 contributor；不改 Tianshu contract |
...
| Provider 调用 | Reactor Netty/gRPC | None | `GatewayOperationInvoker`、HTTP/RPC adapters | 无 gap | 下沉 runtime-core；无内部 Yuheng API |
| Runtime consistency | Set/Enum | Existing MVC | `GatewayProjectionService`/existing APIs | 缺角色完整性判断 | Strategy 复用现有 Tianshu metadata，不加接口/表 |
...
| Rule 7 | Yes | Engine base/operations、Admin base/local、脚本 env；新 MCP base/operations | 每个新增/迁移 key 在 MCP Base/Operations 同层；API 现有 key 不重命名；`YUHENG_MCP_*` 保留；配置 parity test | `application*.yml`、Properties records | `TEST-019` key parity/binding | PASS |
...
| Rule 11 | Yes | 当前 Yuheng feature-first infrastructure 及已批准 predecessor；目标模块/包树见 §8 | 保持同一 Traditional Layered repository variant；不混入 Archetype/COLA/DDD/biz hybrid；Plan 必须再次确认 | 全目标 tree | Maven/package boundary tests | PASS |
...
| `yuheng-runtime-core` | New module | `REQ-004`–`REQ-006` | MCP Engine 依赖 API_RPC Engine 或复制 common/rule/operation | 前者把全部 API/RPC Classpath 带入，后者复制校验/LKG/治理 | 1 Artifact；共享 API 兼容成本 | Add |
| 现有 `yuheng-biz-gateway` 作为 API_RPC | Modify/Keep | `REQ-002`,`REQ-008` | 重命名为新 api-rpc artifact | 重命名增加 Jar/Compose/CI/外部消费者迁移，无额外价值 | 删除 MCP 依赖/Bean；保留兼容名 | Keep |
| `yuheng-mcp-gateway` | New executable | `REQ-001`,`REQ-003`,`REQ-009`,`REQ-010` | 单 Binary `mode=MCP` | 万能 Binary 仍携带/可误装所有 Plane，依赖与 Role 不固定 | 新进程、端口、Context、凭证、健康 | Add |
...
| Yuheng-to-Yuheng internal invoke API | Candidate | None | MCP 复用 Runtime Operation Invoker | 增加 RTT、重复 auth/traffic/retry/trace 和新故障 | 1 network hop + discovery/versioning | Remove |
...
| 当前 Release | Admin -> Tianshu -> N identical Combined replicas | one Admin state | one compiled record type per replica | replicas may diverge；Admin observes ACK | existing HA |
| 选定 Release | Admin -> Tianshu -> N API_RPC + M MCP replicas | unchanged Admin write contract | two compiled DTO types/local atomics, same ID/SHA | role may temporarily diverge；Admin explicitly detects | 两角色独立演进且共享控制事实 |
...
flowchart LR
    ...
    subgraph Control["One logical Yuheng Admin control plane"]
        Admin["yuheng-admin"]
        AdminWeb["yuheng-admin-web"]
        AdminDb[("Yuheng PostgreSQL")]
        AdminWeb -->|"existing Admin APIs"| Admin
        Admin --> AdminDb
    end

    Tianshu["Tianshu\none yuheng.rules.active\none Artifact SHA"]

    subgraph ApiRpcBoundary["API_RPC Engine deployment boundary"]
        ApiRpcApp["yuheng-biz-gateway\nGatewayEngineApplication"]
    ...
    Shared["yuheng-runtime-core library\nRule activation / Provider / Traffic / Operation / outbound HTTP+RPC / telemetry"]
    Providers["HTTP/RPC Providers"]
    Remote["Remote MCP Servers"]
    IdpRbac["Tianquan-Shoubing + Tianquan-Jianshen"]

    Operator --> AdminWeb
    Admin -->|"publish once"| Tianshu
    Tianshu -->|"same snapshot; role API_RPC"| ApiRpcApp
    Tianshu -->|"same snapshot; role MCP"| McpApp
    Api --> ApiIngress
    Rpc --> RpcIngress
    Mcp --> McpIngress
    ...
    ApiRpcApp --> IdpRbac
    McpApp --> IdpRbac
    Admin -->|"read role/release ACK"| Tianshu
```
...
| --- | --- | --- | --- | --- | --- |
| `yuheng-admin` | Single Catalog/Draft/Release/publication/runtime projection | existing Admin APIs -> one Snapshot/Active Key; Tianshu metadata -> consistency | Admin DB、Tianshu、Tianquan-Shoubing/Tianquan-Jianshen | request hot path、per-plane duplicate Release | `REQ-001`,`REQ-006`,`REQ-007` |
| `yuheng-runtime-core` | shared immutable activation pipeline、Provider/Traffic/Operation/outbound transport/telemetry | Snapshot + Strategy -> active DTO；Invocation -> Provider result | contract/core/Tianshu/RPC/Reactor/Jackson/Micrometer | MainClass、plane ingress、MCP capability、Admin persistence | `REQ-004`–`REQ-006` |
| `yuheng-biz-gateway` (`API_RPC`) | HTTP/WebSocket/gRPC ingress、RPC Slot、API/RPC security/readiness | API/gRPC -> Provider；Role/ACK -> Tianshu | runtime-core、contract/core、Tianquan-Shoubing/Tianquan-Jianshen/Tianshu | MCP paths/beans/core/JDBC/Remote MCP | `REQ-002`,`REQ-008`,`REQ-009` |
| `yuheng-mcp-core` | protocol/capability behavior independent of Spring/process | MCP internal request -> capability result | contract/core/Jackson/Reactor | Spring Boot entry、JDBC/Redis concrete adapter、API/RPC ingress | `REQ-003`,`REQ-004` |
| `yuheng-mcp-gateway` (`MCP`) | MCP ingress、Spring assembly、Redis/JDBC/filesystem/remote adapters、MCP health | MCP wire -> direct Provider/Remote result；Role/ACK -> Tianshu | runtime-core、mcp-core、Tianquan-Shoubing/Tianquan-Jianshen/Tianshu、existing stores | general API Route、gRPC Listener、INTERNAL_GATEWAY Slot | `REQ-003`,`REQ-005`,`REQ-009`,`REQ-010` |
| Tianshu | shared config delivery、client lease、version/ACK/metadata | one Active Key -> two roles；metadata -> Admin | existing Tianshu contracts | Release semantics、business authorization | `REQ-006`,`REQ-007` |
...
两个 Engine 在 Maven、Spring Context、端口、身份、资源和生命周期上独立；`yuheng-runtime-core` 只是库，不运行
...
Admin 发布内容和 Tianshu key 不变。Plane Strategy 只决定“从同一 Snapshot 编译哪些运行时索引”：
...
```mermaid
flowchart TD
    Publish(["Admin publishes existing Release"]) --> Ddc["Tianshu writes chunks + yuheng.rules.active"]
...
| Latency | MCP Operation 不增加 Yuheng hop | shared direct Operation Invoker/outbound adapters | Provider failures unchanged | shared library API must stay stable | call-count/trace tests | `REQ-005`,`REQ-008` |
...
| Capacity | scale MCP and API_RPC independently | independent deployment replica counts | local circuit/bulkhead state differs by role | duplicate Tianshu subscriptions/provider caches | load/config/metrics validation | `REQ-009` |
| Operability | Admin must expose role completeness and stale state | fixed metadata + Role Strategy + existing APIs/UI | Tianshu unavailable uses existing cached stale/503 behavior | one metadata convention | API/UI/script tests | `REQ-007`,`REQ-012` |
...
| --- | --- | --- | --- | --- | --- | --- |
| `1` | Admin -> Tianshu | existing Release publication | `GatewayRuleSnapshot` -> chunks + `GatewayRuleActivation` | existing Release Target/Change | existing retry/partial target semantics | `REQ-006` |
| `2` | Tianshu -> each Engine | `DdcConfigApplier.apply(key,value,version)` | active/chunks -> verified Snapshot | per-process chunk store | invalid version/hash/schema rejects and old active remains | `REQ-006` |
...
| `5` | Generic Applier -> AtomicReference/metadata | active DTO + status | release/version/SHA/stage -> Tianshu metadata | plane-local active pointer/status | no partial memory state | `REQ-006`,`REQ-007` |
...
| `8` | each Engine -> Tianshu | `INTERNAL-002 metadata()` | fixed role + active status -> bounded Map | config-client lease metadata | missing/invalid role causes Admin inconsistency | `REQ-007`,`REQ-009` |
| `9` | Admin/API/Web -> Projection Service | API-001/API-002 | Tianshu nodes + Release expectation -> role/node consistency | read cache only | stale fallback or existing 503 | `REQ-007`,`REQ-012` |
...
sequenceDiagram
    actor Operator
    participant Admin as Yuheng Admin
    participant Tianshu
    ...
    Admin->>Tianshu: one snapshot/chunks + yuheng.rules.active
    par independent local activation
        Tianshu->>Api: apply key/version
        Api->>Api: verify -> API_RPC compile -> LKG -> atomic switch
    and
        Tianshu->>Mcp: apply key/version
        Mcp->>Mcp: verify -> MCP compile -> LKG -> atomic switch
    end
    alt both succeed
        Api-->>Tianshu: API_RPC + ACK_SUCCESS + release/version/SHA
        Mcp-->>Tianshu: MCP + ACK_SUCCESS + release/version/SHA
        Operator->>View: GET API-002
        View->>Tianshu: read config clients
        View-->>Operator: consistent=true
    else one role fails or is missing
        Api-->>Tianshu: success or failure metadata
        Mcp-->>Tianshu: success/failure/no lease
        Operator->>View: GET API-001/API-002
        View->>Tianshu: read roles and ACK
...
| Plane rule activation | each Engine process | synchronized `apply` + monotonically increasing Tianshu version + `AtomicReference` | duplicate/old version ignored; one apply at a time per process | LKG persisted then active reference/status switched | old complete active retained | `REQ-006 / TEST-011,012` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Plane compile fails | Strategy exception + apply stage | reject this role apply; do not persist/switch | old active/LKG intact | Tianshu/Admin existing retry with same Release content | that role old behavior; API-002 false | Operator fixes/retries/rolls back | `TEST-012`,`TEST-013` |
...
| one role missing | Admin Role Strategy | no mutation | existing Release may still be SUCCESS for snapshotted targets | role starts later and initial Tianshu refresh applies latest | API-002 false; UI missing role | Deployment operator | `TEST-015`,`TEST-017` |
...
| Tianshu management timeout | existing projection `load` | return cached envelope with `stale=true`; without cache throw existing unavailable | no write | existing bounded refresh/poll | warning or 503 error wrapper | Tianshu/operator | `TEST-016` |
...
| API_RPC Engine down | health/Tianshu lease | API/RPC role missing/unready | MCP state unaffected | LB/Tianshu selects other replica | API/RPC unavailable if no replica; MCP remains | API_RPC operator | `TEST-020` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Tianshu config-client metadata | each Engine after apply/heartbeat | `yuheng.engine.role`,`activeReleaseId`,
`activeRuleVersion`,`activeRuleChecksum`,`lastApplyStatus`,`lastAckAt` | no Token/Secret/Body/path arguments | Role key
mandatory; ACK fields complete for consistent node | Admin API/Web; restart/fix/retry | source/contract tests; live Tianshu
later |
...
| Runtime consistency | Yuheng Admin API-002 | target Release、node counts、role metadata、stale/source | no secrets |
false if role incomplete or node mismatch | block rollout; retry/fix/rollback | API/unit/static; live Tianshu later |
...
│       ├── features/yuheng-groups/GatewayGroupDetailPage.tsx MODIFY role display/missing state
...
└── .github/workflows/tianquan-jianshen.yml and Yuheng-relevant workflows MODIFY focused module selectors
```
...
| --- | --- | --- | --- | --- | --- |
| Create | `yuheng-contract/.../runtime/GatewayEngineRoleEnum.java` | `API_RPC`,`MCP`,`fromWire` | fixed Tianshu/Admin role vocabulary | JDK/Jackson default only | `REQ-007`,`REQ-009` |
| Create | `yuheng-runtime-core/pom.xml` | Artifact | shared non-executable runtime ownership | contract/core/Tianshu/RPC/Reactor/Micrometer/managed deps | `REQ-004`–`REQ-006` |
...
| Modify | `yuheng-biz-gateway/.../GatewayEngineConfiguration.java` | named API/RPC Beans | API/RPC-only wiring; explicit import Runtime configuration | runtime-core + API/RPC feature | `REQ-002`,`REQ-004` |
| Modify | `yuheng-biz-gateway/.../GatewayEngineRuntime.java` | API/RPC SmartLifecycle | HTTP/RPC/Slot start/drain/readiness only | API/RPC servers/activation | `REQ-002`,`REQ-009` |
| Create | `yuheng-mcp-gateway/.../McpGatewayEngineConfiguration.java` | named MCP Beans | MCP-only wiring/stores/security/remote/handler/runtime | runtime-core + mcp-core | `REQ-003`,`REQ-010`,`REQ-011` |
| Create | `yuheng-mcp-gateway/.../McpGatewayEngineRuntime.java` | MCP SmartLifecycle | rule restore, MCP listener/task worker readiness/drain | MCP handler/activation/stores | `REQ-003`,`REQ-009`,`REQ-010` |
...
| Delete | `yuheng-biz-gateway/.../GatewayCompositeHttpDataPlaneHandler.java` | old Composite | remove same-listener MCP dispatch | None | `REQ-002`,`REQ-003` |
| Create | `admin/runtime/service/GatewayEngineRoleConsistencyStrategy.java` | `missingRoles(...)` | validate metadata and calculate required role set | contract Role Enum + Tianshu model | `REQ-007` |
| Modify | `admin/runtime/service/GatewayProjectionService.java` | `runtimeConsistency` | combine existing node consistency with role completeness | Role Strategy + existing release/Tianshu | `REQ-007`,`REQ-012` |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `API-001` | Existing/Modify; keep because the page independently displays live nodes | List Yuheng Engine nodes and their fixed roles | HTTP | Admin Web Yuheng group detail, MCP runtime status, operator tooling | `yuheng-admin` | `GET /api/v1/yuheng/admin/yuheng-groups/{gatewayGroupId}/engine-nodes` | Path `gatewayGroupId` | Existing `GatewayProjectionEnvelopeVO<List<DdcManagementConfigClientInstance>>`; `metadata.yuheng.engine.role` becomes required for split Engines | Existing authenticated Admin request and `CAP_yuheng:read` or `CAP_*`; group tenant scope unchanged | Existing Admin error wrapper; stale cached data remains HTTP 200 | Read-only; Tianshu observation timestamp is the response version | `REQ-007`,`REQ-009`,`REQ-012` |
| `API-002` | Existing/Modify; keep because release readiness is an independent operator decision | Evaluate role-complete runtime consistency | HTTP | Admin Web Yuheng group detail, MCP runtime status, release/operator tooling | `yuheng-admin` | `GET /api/v1/yuheng/admin/yuheng-groups/{gatewayGroupId}/runtime-consistency` | Path `gatewayGroupId` | Existing `GatewayRuntimeConsistencyVO`; JSON shape unchanged, `consistent` requires both roles | Existing authenticated Admin request and `CAP_yuheng:read` or `CAP_*`; group tenant scope unchanged | Existing Admin error wrapper; stale cached data remains visible | Read-only; evaluated against latest release expectation and current Tianshu observation | `REQ-006`,`REQ-007`,`REQ-012` |
| `INTERNAL-001` | Existing concept/Modify; add one interface to avoid compiler mode conditionals | Compile the shared snapshot into one Engine role's immutable runtime view | Internal Java Service SPI | Generic activation pipeline in both executable Engines | `yuheng-runtime-core` | `GatewayRuleCompilerStrategy<T extends GatewayCompiledRulesDTO>#compile(GatewayRuleSnapshot)` | Verified immutable snapshot | `ApiRpcGatewayCompiledRulesDTO` or `McpGatewayCompiledRulesDTO` | In-process only; no request identity | Typed exception/failure result consumed by existing apply failure path | Pure for a given snapshot; compilation version is snapshot version/SHA | `REQ-004`–`REQ-006` |
| `INTERNAL-002` | Existing/Modify; keep Tianshu metadata rather than add a discovery API | Publish fixed Engine role plus existing activation ACK metadata | Internal Java SPI/Tianshu lease metadata | Tianshu registry, Admin projection and consistency strategy | Existing Tianshu contract with role-specific contributors | `DdcInstanceMetadataContributor#metadata()` | No request; reads role constant and local activation status | Bounded `Map<String,String>` attached to config-client lease | Trusted in-process contributor; Tianshu registration credentials unchanged | Registration/heartbeat failure follows existing Tianshu availability path | Latest local status replaces prior metadata on heartbeat; no historical log | `REQ-006`,`REQ-007`,`REQ-009` |

### 9.2 Per-interface Detailed Contracts

#### 9.2.1 API-001 — List Yuheng Engine Nodes and Roles
...
| Parameter ownership and derivation | `gatewayGroupId` selects the existing Admin projection context; Tianshu owns lease identity/status, and each executable owns its fixed `yuheng.engine.role`. Admin must not infer role from host, port, artifact name, or process label. |
...
| Protocol and endpoint | HTTP `GET /api/v1/yuheng/admin/yuheng-groups/{gatewayGroupId}/engine-nodes` |
| Content type/version | `application/json`; existing `/api/v1` contract |
| Auth/permission/tenant | Existing authenticated Admin principal; `hasAnyAuthority('CAP_yuheng:read','CAP_*')`; existing group/tenant resolution remains authoritative. |
| Timeout/retry/rate limit | Existing Admin/Tianshu client timeout and cache fallback; UI retries only by its existing query policy or explicit refresh. No new rate limiter. |
| Idempotency/concurrency | Safe/idempotent GET. A response is a point-in-time Tianshu projection identified by `observedAt`; concurrent heartbeats may affect the next read only. |

##### Request parameters

| Name | Location | Type/format | Required/null | Default | Validation/range/enum | Meaning | Example | Source |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `gatewayGroupId` | Path | JSON/Java integer-compatible `long` | Required; not null | None | Existing positive identifier validation and not-found behavior | Yuheng group whose configured Engine clients are observed | `1001` | Route/user navigation |
...
```jsonc
{
  "value": [ // Required list. May be empty when Tianshu has no observed online/config-cached clients.
    {
      "bizCode": "identity", // Required. Existing Tianshu business namespace selected by the Yuheng group.
      "env": "local", // Required. Existing Tianshu environment namespace.
      "appCode": "yuheng-biz-gateway-default", // Required. Shared logical Yuheng appCode; both Engine roles use the same value.
      "instanceId": "yuheng-api-rpc-local-1", // Required. Unique process instance identity used to correlate consistency nodes.
      "leaseId": "lease-api-rpc-01", // Required. Current Tianshu lease identity; changes after a new registration.
      ...
      "leaseRole": "RUNTIME", // Required. Existing Tianshu lease role; it is not the Yuheng Engine role.
      "status": "ONLINE", // Required. Existing Tianshu lease status; UI uses the repository's existing label mapping.
      ...
      "metadata": { // Required for split Engine registrations; existing legacy nodes may temporarily omit the new role key during cutover.
        "yuheng.engine.role": "API_RPC", // Required for new binaries. Enum is exactly API_RPC or MCP and is serialized with Enum.name().
    ...
    {
      "bizCode": "identity", // Same logical Tianshu business namespace as the API/RPC Engine.
      "env": "local", // Same logical environment as the API/RPC Engine.
      "appCode": "yuheng-biz-gateway-default", // Same logical appCode so one Admin release targets both roles.
      "instanceId": "yuheng-mcp-local-1", // Unique MCP Engine process instance identity.
      ...
      "leaseRole": "RUNTIME", // Existing Tianshu role, preserved unchanged.
      "status": "ONLINE", // Existing Tianshu lease status.
      ...
      "metadata": { // MCP Engine contribution using the same ACK key vocabulary.
        "yuheng.engine.role": "MCP", // Fixed MCP Engine role; never supplied by deployment configuration.
  ...
  "source": "TIANSHU_CONFIG_CLIENT", // Required existing projection-source vocabulary.
  "stale": false, // Required. True means value came from the last usable Admin cache after Tianshu refresh failed.
...
| --- | --- | --- | --- | --- | --- |
| `value[]` | array of existing Tianshu instance records | Required; empty allowed | No duplicate live `leaseId`; repository Tianshu model governs remaining fields | Tianshu management/config-client projection | Node table and role grouping |
| `value[].instanceId` | string | Required | Existing Tianshu identifier rules | Process identity | Correlate with `API-002.nodes[].instanceId` |
| `value[].metadata.yuheng.engine.role` | string enum | Required for new Engine binaries; absent is a transition/error state | `API_RPC` or `MCP`, case-sensitive | Fixed executable constant | Role label/filter and missing/unknown warning |
...
| Yuheng group absent or outside visible scope | `404` | `YUHENG_ADMIN_NOT_FOUND` | Existing Admin error wrapper | No | Existing not-found/error result |
| Tianshu unavailable and no usable cache | `503` | `YUHENG_ADMIN_TIANSHU_UNAVAILABLE` | Existing Admin error wrapper | Yes | Keep prior client state if present and expose retry |
| Tianshu unavailable with usable cache | `200` | None | Success wrapper with `stale=true` and non-null `refreshError` | Yes via refresh | Render data plus stale warning; do not treat as consistent freshness |

```jsonc
{
  "code": "YUHENG_ADMIN_TIANSHU_UNAVAILABLE", // Stable Admin business code when no Tianshu projection or cache can be served.
  "message": "Yuheng runtime projection is temporarily unavailable", // Sanitized operator-readable message; no endpoint secret or stack trace.
  ...
  "errors": [ // Required/empty according to the existing wrapper; structured causes must remain sanitized.
    {
      ...
      "code": "TIANSHU_UNAVAILABLE", // Stable structured reason code.
...
1. Resolve authenticated identity, capability, Yuheng group, and existing Tianshu namespace in the current
   controller/service order.
2. Query existing Tianshu config-client instances; preserve the repository's fresh/cache/503 projection policy.
...
4. Treat `leaseRole` and `yuheng.engine.role` as distinct fields; only metadata supplies the Yuheng execution role.
...
Verification covers controller golden JSON, stale/503 behavior, both enum values, missing/unknown roles, Tianshu key bounds,
...
| Parameter ownership and derivation | Admin owns the expected release and compares it with Tianshu node ACK metadata. Required roles are the fixed contract set `{API_RPC, MCP}`, not caller input. |
| Direct/no-new-interface alternative | Extend the existing calculation. A second role-consistency endpoint would duplicate Tianshu reads and create conflicting readiness states. |
...
| Protocol and endpoint | HTTP `GET /api/v1/yuheng/admin/yuheng-groups/{gatewayGroupId}/runtime-consistency` |
| Content type/version | `application/json`; existing `/api/v1` contract |
...
| Idempotency/concurrency | Safe/idempotent GET. Results may change at each Tianshu heartbeat or release-state observation. |

##### Request parameters

| Name | Location | Type/format | Required/null | Default | Validation/range/enum | Meaning | Example | Source |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `gatewayGroupId` | Path | `long` | Required; not null | None | Existing positive identifier/not-found behavior | Yuheng group whose latest release and Engines are evaluated | `1001` | Route/user navigation |
...
```jsonc
{
  ...
  "observedAt": "2026-09-02T20:00:02Z", // Required ISO-8601 Instant for the Tianshu/projection observation.
  "source": "TIANSHU_CONFIG_CLIENT", // Required existing source vocabulary.
  ...
  "nodes": [ // Required list of existing per-node consistency projections; list order is not contractual.
    {
      "instanceId": "yuheng-api-rpc-local-1", // Required process identity; role is obtained by correlating API-001 metadata.
      "leaseId": "lease-api-rpc-01", // Required current Tianshu lease identity.
      "leaseStatus": "ONLINE", // Required existing Tianshu status used by readiness evaluation.
    ...
    {
      "instanceId": "yuheng-mcp-local-1", // Required MCP process identity; API-001 supplies its MCP role label.
      "leaseId": "lease-mcp-01", // Required current MCP Tianshu lease identity.
...
| Field path | Type/format | Required/null/default | Validation/enum/precision | Meaning and source | Frontend use |
| --- | --- | --- | --- | --- | --- |
| `engineNodeCount` | integer | Required; `>=0` | Count of evaluated nodes | Tianshu instance projection | Summary |
...
| `nodes[].activeRuleVersion` | JSON integer/Java `Long` | Nullable | Exact signed `long`; malformed metadata produces inconsistent node | Parsed Tianshu metadata | Diagnostic display |
...
| Yuheng group/release scope not found | `404` | `YUHENG_ADMIN_NOT_FOUND` | Existing Admin error wrapper | No | Not-found/error state |
| Tianshu unavailable without cache | `503` | `YUHENG_ADMIN_TIANSHU_UNAVAILABLE` | Same complete error shape documented by `API-001` | Yes | Retry control; do not synthesize `consistent=false` as a successful fresh evaluation |
| Tianshu unavailable with cache | `200` | None | Success shape with `stale=true` | Yes via refresh | Preserve values and show stale warning |

```jsonc
{
  "code": "YUHENG_ADMIN_NOT_FOUND", // Stable code when the requested Yuheng group or required scoped aggregate does not exist.
  "message": "Yuheng group was not found", // Sanitized user-facing reason.
  ...
  "errors": [ // Existing structured error list.
    {
      ...
      "message": "No visible yuheng group exists for id 1001" // Sanitized scoped explanation.
    }
  ],
...
2. Read the same Tianshu/config-client projection semantics as `API-001`; cache fallback remains authoritative for `stale`.
3. Parse each node's fixed role. Missing, blank, or unknown `yuheng.engine.role` is not silently assigned; that node is
...
| Parameter ownership and derivation | Tianshu release artifact owns the complete `GatewayRuleSnapshot`; each compiler implementation owns projection into its role DTO. |
...
| Content type/version | Java types at the same reactor version; snapshot releaseId/artifact checksum identify compilation; Tianshu numeric version is held by activation status |
...
| Timeout/retry/rate limit | Synchronous bounded CPU/memory compilation; no internal retry or rate limiter; delivery retry remains the existing Tianshu/apply policy |
...
| Output | Non-null immutable `ApiRpcGatewayCompiledRulesDTO` or `McpGatewayCompiledRulesDTO`; releaseId and artifact checksum must equal the input snapshot identity; numeric Tianshu version belongs to activation status, not the compiled DTO. |
| Side effects | None. No Provider reference mutation, listener mutation, persistence, Tianshu ACK, or network call occurs inside `compile`. |
...
| Unexpected compiler defect/resource failure | Runtime exception captured at apply boundary | Sanitized `COMPILE_FAILED` | Tianshu may redeliver; operator investigates | Failure metadata/metric/log; never return partial DTO |
...
#### 9.2.4 INTERNAL-002 — Tianshu Engine Role and Activation Metadata
...
| Direct/no-new-interface alternative | Reuse `DdcInstanceMetadataContributor#metadata()` and existing Tianshu heartbeat. Host/port/process-name inference is ambiguous; a new endpoint/topic is unnecessary. |
| Caller use of result | Tianshu attaches values to each config-client lease; Admin projects and evaluates them; UI displays diagnostics. |
| Round trips and failure points | No added round trip. Values ride the existing registration/heartbeat; Tianshu failure follows current cache/lease semantics. |
...
| Concern | Definition |
| --- | --- |
| Purpose/owner/consumer | Publish one process's fixed execution role and latest local activation evidence on its existing Tianshu config-client lease; produced by each Engine and consumed by Tianshu/Admin/UI. |
| Protocol and endpoint | In-process `DdcInstanceMetadataContributor#metadata()` whose map is attached to existing Tianshu registration/heartbeat messages |
| Content type/version | Bounded UTF-8 string key/value metadata under the existing Tianshu protocol; keys in this Spec are stable version-1 vocabulary |
| Auth/permission/tenant | Existing Engine Tianshu client credential and `biz/env/appCode` registration scope; no end-user input |
| Timeout/retry/rate limit | Contributor is local/non-blocking; registration/heartbeat timing, timeout and retry remain existing Tianshu client behavior |
...
```jsonc
{
  "yuheng.engine.role": "MCP", // Required fixed executable role; exact API_RPC or MCP Enum.name().
...
| --- | --- | --- | --- | --- |
| `yuheng.engine.role` | Enum text | Required and non-blank for new binaries; no default | Role-specific contributor using `GatewayEngineRoleEnum.name()` | Admin requires exactly `API_RPC` or `MCP`; unknown/missing is inconsistent. |
...
or rule content. It remains within current Tianshu limits (at most 32 entries, key length at most 64, value length at most
...
| Value exceeds Tianshu bound or contains invalid/null content | Fail fast/sanitize according to existing contributor aggregation policy; never truncate identity into a misleading match | No until producer fixed | Registration/heartbeat fails visibly; local active view is unchanged |
| Tianshu registration/heartbeat unavailable | Existing Tianshu client failure/cache/lease-expiry behavior | Yes by existing retry | Admin may serve stale cache, then fresh inventory loses expired lease; no local rule rewrite |
...
3. It emits role first and only bounded non-null activation fields that exist; enum/time/version formatting is
   deterministic.
4. Existing Tianshu client merges contributor output under current entry/key/value limits and sends it on
...
6. Tianshu transport failure does not mutate local active/LKG state; lease/cache semantics expose freshness separately.
7. Admin Web displays role/status through existing read APIs; it never sends metadata back as a command.

##### Compatibility and verification

Existing metadata keys and Tianshu registration protocol remain unchanged; one new namespaced key is backward-compatible for
...
| RPC | Existing gRPC service/full method name request path and Tianshu RPC slot/provider semantics owned by current `engine.rpc` path | Full method names, protobuf/grpc status behavior, and provider routing remain unchanged; API/RPC Engine retains the listener and slot. | Existing RPC integration and slot registration tests. |
| MCP | Existing `/mcp/{serverCode}`, `/legacy/mcp/**`, and `/.well-known/oauth-protected-resource/mcp/**` paths and MCP protocol/security/capability payloads | Stable external Host/Path is routed to the new MCP Engine. The accepted MCP contract remains effective; no Yuheng-to-Yuheng HTTP self-call is introduced. | Existing MCP initialize/tools/resources/prompts/tasks/security contract suites run against MCP Engine. |

No CLI, scheduled job, event topic, database query interface, or new public discovery operation is added by this Spec.

## 10. POJO and Data Model Design

Chapter 10 is `Affected` only for role vocabulary and compiled in-memory views. Existing public Admin VO and Tianshu
...
| --- | --- | --- | --- | --- | --- |
| `GatewayEngineRoleEnum` / contract `runtime` package | Enum contract | Both Engines, Tianshu metadata, Admin role consistency, Admin Web wire value | One shared vocabulary prevents string drift and forbids deployment-selected modes. | None; Jackson/default text uses `name()` | `REQ-007`,`REQ-009` |
...
Numeric Tianshu version is not a `GatewayRuleSnapshot` field and is not added to the compiled DTO. `GatewayRuleActivationApplier.apply(key,value,version)` supplies `GatewayRuleRuntimeStatus.activeDdcVersion`; LKG-only restoration retains existing 0/degraded semantics until Tianshu application. Both roles report releaseId/version/artifact checksum/ACK through the existing status metadata. `GatewayCompiledRulesDTO.ruleChecksum` equals `snapshot.artifactSha256()`; providerServices reuses `Set<ProviderServiceKey>`. The compiler stays `compile(GatewayRuleSnapshot)` with no new parameter, external field, or LKG schema. This correction was explicitly accepted after the [execution preflight](../review/2026-09-05-06-34-yuheng-execution-preflight.md).
...
role-local compiled DTO
  └─ GatewayRuleActivationApplier<T>
       ...
       └─ existing activation status -> Tianshu metadata Map<String,String>
```
...
unchanged. The implementation exposes the backend's existing `metadata` map in `EngineNode`, renders the fixed Yuheng
role instead of conflating it with Tianshu `leaseRole`, and makes missing/unknown role states visible.
...
| --- | --- | --- | --- | --- | --- | --- |
| Yuheng Group Detail / Overview and Engine Nodes tabs | `/yuheng-groups/:groupId/overview` | Existing Yuheng Group table `Overview` action | `GatewayGroupDetailPage.tsx` | Existing root `yuheng:read` guard; group scope from `groupId` and authenticated Admin context | Existing RequireAuth/RequireCapability/not-found/query failure behavior | `REQ-007`,`REQ-012` |
| MCP Server Workbench / Runtime Status tab | `/mcp/servers/:serverId` | Existing MCP Servers navigation and workbench tab | `McpServerWorkbenchPage.tsx` + `McpRuntimeStatus.tsx` | Existing `yuheng:mcp:read` nested guard plus owning server's `gatewayGroupId` | Existing denied/not-found/query failure behavior | `REQ-003`,`REQ-007`,`REQ-012` |
...
group page must show two separate columns/copy labels: `Yuheng Role` from `metadata['yuheng.engine.role']` and
`Tianshu Lease Role` from `leaseRole`. Missing/unknown nodes remain in the all-role table with a warning tag; they must not
disappear.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Inspect all Engine roles | Authenticated, `yuheng:read`, valid group | Open Overview -> wait for existing node/consistency queries -> select Engine Nodes | `groupId` must be non-empty before query; metadata enum parsed as display-only known/unknown | Returns node envelope and consistency | Display API_RPC/MCP rows separately, counts, freshness and role warnings | Existing query failure component; retry/refetch retains no fabricated role | `API-001`,`API-002` |
| Inspect MCP runtime | Authenticated, `yuheng:mcp:read`, valid MCP server/group | Open workbench -> Runtime Status -> load both queries | Filter only exact `MCP`; missing/unknown is counted for warning, not coerced | Same existing endpoints | Display known MCP nodes, overall release status, and role-completeness warning | If either query fails, existing QueryFailure; stale success remains visible with warning | `API-001`,`API-002` |
...
| --- | --- | --- | --- | --- | --- | --- |
| `GatewayGroupDetailPage` consistency card | `/yuheng-groups/:groupId/overview` | View/poll | `API-002` | Existing loading/error; fresh/stale; consistent/inconsistent; add role-complete explanation using `API-001` | `yuheng:read` | `REQ-007`,`REQ-012` |
| `GatewayGroupDetailPage` nodes table | Same, Engine Nodes tab | View/poll | `API-001` | Empty, populated, partial metadata, stale, dependency error | `yuheng:read` | `REQ-007`,`REQ-012` |
| `McpRuntimeStatus` | `/mcp/servers/:serverId`, Runtime Status tab | View | `API-001`,`API-002` | Loading only while either required query loads; error if either fails; stale warning; known-MCP empty/populated; missing/unknown warning | `yuheng:mcp:read` | `REQ-003`,`REQ-007`,`REQ-012` |
...
| Empty all-role | `API-001.value=[]` | Empty table plus “No Yuheng Engine nodes observed”; consistency is false/0 of 0 | Existing refresh/poll | Fresh or stale empty projection retained by query | New heartbeat/poll | `TEST-015`,`TEST-018` |
...
| Stale success | Either response says stale | Persistent warning “Tianshu runtime projection is stale; showing last known state”; consistency tag remains visible but not described as fresh | Retry/normal polling | Last usable cache | Fresh response | `TEST-016`,`TEST-018` |
...
| --- | --- | --- | --- |
| Yuheng Role | `API-001.value[].metadata['yuheng.engine.role']` | Exact `API_RPC`/`MCP` tag; absent/other -> `UNKNOWN` warning | MCP filter accepts exact `MCP`; no fuzzy/case-fold match; row order remains server/table behavior |
| Tianshu Lease Role | `API-001.value[].leaseRole` | Existing text/tag behavior; never labeled Yuheng Role | Display only |
...
normalizes null to an immutable empty map; the `yuheng.engine.role` key itself may be absent during cutover.
...
| --- | --- | --- |
| One binary plus `yuheng.engine.mode` State/Factory | Rejected | It permits accidental mixed or wrong-role startup, retains all dependencies in one artifact, and is not process-level isolation. Use two code-fixed executable bean graphs. |
| MCP Engine calls API/RPC Engine through HTTP/RPC Facade | Rejected | Adds a hop, availability coupling, timeout/retry ambiguity, and violates the accepted direct Provider operation invocation. Both roles reuse runtime-core transports and Tianshu Provider discovery directly. |
...
| Event/Observer synchronization between Engines | Rejected | The shared Tianshu release artifact already supplies the event/state source. Cross-process events would add ordering and reconciliation failure modes. |
...
- **Traditional profile direction:** controller/UI -> existing Admin service/projection -> Tianshu/release capabilities;
  executable bootstrap -> role feature services -> runtime-core/Egon infrastructure -> Tianshu/Provider/RPC/Redis/JDBC. No
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `TEST-001` | Static/Maven | Yuheng parent/module graph | Inspect parent modules and dependency trees | runtime-core non-executable; two executable artifacts; no executable-to-executable dependency; API Engine excludes mcp-core/JDBC | POM fixtures | Maven Enforcer/dependency tree plus `rg` | `REQ-001`–`REQ-004` |
...
| `TEST-004` | Component | API/RPC Engine context | Start test context without external services using existing fakes | API HTTP/RPC beans exist; MCP handler/store/task beans and MCP datasource binding do not | Existing fake Tianshu/provider/clock | Spring Boot context test | `REQ-002`,`REQ-009`,`REQ-010` |
| `TEST-005` | Integration | API HTTP/WebSocket | Existing route and upgrade fixtures | Method/path/header/body/status/WebSocket behavior unchanged; MCP reserved paths are not handled by API Engine | Existing route snapshots | Current Yuheng HTTP suite | `REQ-002`,`REQ-005`,`REQ-008` |
| `TEST-006` | Integration | API/RPC Engine RPC listener/slot | Existing full method/provider fixtures | gRPC status, fullMethodName forwarding, slot registration/drain unchanged | Existing RPC/Tianshu fakes | Current Yuheng RPC suite | `REQ-002`,`REQ-005`,`REQ-008` |
| `TEST-007` | Component | MCP Engine context | Start test context with local fakes | MCP handler/task/store beans exist; API route compiler, WebSocket composite, RPC server, and RPC slot beans do not | Existing MCP stores/fake Tianshu | Spring Boot context test | `REQ-003`,`REQ-009`–`REQ-011` |
...
| `TEST-010` | Integration/static | MCP direct Provider invocation | tools/resource/prompt/task operation | MCP selects/invokes Provider through shared transport/Tianshu; no HTTP/RPC request targets API/RPC Engine | Transport spy/dependency scan | JUnit + `rg` | `REQ-005` |
...
| `TEST-013` | Unit/contract | Admin cross-role consistency | Both roles online: V42/V42 then V42/V43, multi-replica case | True only when every node matches one target; mismatch false with deterministic reason/count | Tianshu/release fixtures | JUnit + MockMvc | `REQ-006`,`REQ-007` |
...
| `TEST-015` | Unit/UI | Missing role/zero nodes | only API_RPC, only MCP, or no nodes | Overall false; missing role named; no false positive from all-present nodes subset | Tianshu fixtures | JUnit + Vitest/RTL | `REQ-007`,`REQ-012` |
| `TEST-016` | Contract/UI | Tianshu freshness/failure | fresh, cached stale, no cache | Fresh response; 200 stale with warning; 503 with existing error wrapper; no fabricated consistency | Tianshu client fake/cache fixture | MockMvc + Vitest/RTL | `REQ-007`,`REQ-012` |
...
| --- | --- | --- |
| Authentication/authorization | External API/RPC/MCP auth, OAuth protected-resource behavior, RBAC permissions, tenant/group resolution, and secret formats remain existing. Each executable receives only credentials needed for its owned transports/stores; no secrets enter Tianshu metadata/logs. | Existing security suites by role, secret/config scan, Admin 401/403 tests, MCP OAuth contract. |
...
| Latency | MCP continues direct Provider invocation through shared transports; split adds no Yuheng-to-Yuheng hop. API/RPC hot paths keep one listener/operation/transport sequence. | Transport-spy `TEST-010`, existing latency metrics and implementation-stage benchmark comparison. |
| Capacity/scaling | Each role scales independently with `1..N` replicas. Shared release payload and Tianshu namespace do not imply equal replica counts. Duplicate JVM/common caches increase baseline memory relative to one combined process; this cost buys fault/resource isolation. | Role-specific resource limits/metrics, multi-replica consistency tests, capacity review before rollout. |
| Availability/fault containment | MCP task/store/remote/JDBC failure cannot stop API/RPC listeners; API/RPC listener/RPC-slot failure cannot stop MCP. Shared Tianshu/Provider/Redis dependencies remain common external failure domains where already used. | Separate context/lifecycle/fault tests and per-process health checks. |
...
| Observability | Every role emits `yuheng.engine.role`, `instanceId`, release/version/checksum/apply status and existing trace/metric context. Metric dimensions must be bounded; never use release ID, route, operation, or exception text as an unbounded label unless current instrumentation already bounds it. | Metric/log snapshot tests; Admin role display and Tianshu bounds. |
...
| Configuration | Existing moved MCP keys and `YUHENG_MCP_*` environment variables remain compatible. New bootstrap keys use the MCP Engine prefix and appear in every relevant base/operations/deployment profile. Role is code-fixed, not a property. | All-profile parity/binder test and environment inventory. |
...
| External RPC | Compatible; full method names, protobuf/grpc status and Tianshu provider semantics unchanged | Keep current gRPC listener/slot in API/RPC Engine. |
...
| Tianshu release/config key | Compatible; same `biz/env/appCode`, `yuheng.rules.active`, artifact/chunk schema, version and SHA | Both role clients subscribe independently using unique instance IDs. |
...
3. Start MCP Engine dark (not yet receiving stable external MCP traffic). It subscribes to the already active release
   key, restores/applies the current artifact, registers Tianshu metadata, and must pass role-local health plus current MCP
   protocol probes through an internal route.
4. Do **not** publish a new Yuheng release while old combined and new split binaries coexist. Admin consistency may be
...
1. Freeze new releases and capture both role health, Tianshu metadata, target release/version/SHA, proxy state, and
   sanitized logs.
...
| Tianshu identity | Same `biz/env/appCode`, unique per-process `instanceId`, fixed code role | Role missing is transitional only; no inferred default in Admin. |
| Active release | Same `yuheng.rules.active` and chunk/artifact contract | No second key, mirroring job, or data migration. |
| MCP configuration | Retain existing `egon.cola.component.yuheng.engine.mcp` and `YUHENG_MCP_*` binding names initially; executable bootstrap/listener settings use documented MCP Engine prefix | Avoid broad config rename; future deprecation requires a separate Spec. |
...
alter the selected topology. A demand for distinct Tianshu appCodes, zero-window distributed activation, mandatory release
...
- The initial request to separate mixed MCP Yuheng from API/RPC Yuheng is represented by `REQ-001`–`REQ-005` and the
  explicit executable/module boundaries in §7–§8.
- The confirmed decision “one admin server, two yuheng engine” is represented exactly as one logical Admin plus fixed
...
- Paths and symbols were traced from the current `main@085c20048e35` dirty snapshot: Yuheng parent/Engine/MCP
  Core/Admin/Admin Web, current composite handler, rule compiler/applier, Tianshu metadata/projection types, properties,
  Compose, scripts, tests, and predecessor Specs.
- The design uses the repository's Java/Spring Boot/Maven/Reactor Netty/gRPC/Tianshu/Jackson/Micrometer/Lombok/Jakarta
...
2. One Admin/release fact is required -> both roles consume one verified artifact -> Strategies compile role views ->
   local switches remain atomic -> Tianshu metadata/Admin projection proves eventual convergence without 2PC.
...
| --- | --- | --- |
| [Yuheng Engine 与 MCP Core 功能域分包设计](2026-08-19-13-51-yuheng-biz-yuheng-mcp-package-refactor.md) | `Accepted`; its functional package ownership and contract-preservation intent remain evidence | Amends §3.2 and §5.1 single-module constraint, §5.3 `DEC-004` rejection of Maven split, §7–§9 target tree/configuration, §12 deployment, §14–§18 tests/risks/rollout, and §20 effective conclusion. Its package semantics not contradicted here remain effective. |
| [Egon Yuheng 全能力 MCP 网关需求与技术设计](../../superpowers/specs/2026-08-02-yuheng-complete-mcp-design.md) | Existing MCP capability/security/state/direct-Provider design remains authoritative | Amends only §5.1–§5.2, §6–§7, §13.1–§13.2, §21.3 “same process” topology. Depends on unchanged §3–§4, §8–§12, §14–§20, §21.1–§21.2 contracts and the no-self-call/direct-Provider constraint. |
| [GWS-09 Yuheng Admin 后端 Spec](../../superpowers/specs/2026-07-25-yuheng-admin-backend-design.md) | Implemented Admin control-plane/release/projection contract | Depends on existing §3.5–§3.6, §6.4, §8, §14–§16. This Spec narrows runtime-consistency semantics and role metadata only; it does not supersede Admin ownership/write/release behavior. |
| [Yuheng HTTP OpenAPI 3.1 单一事实源改造](2026-08-25-19-01-yuheng-openapi31-source-refactor.md) and amendment | Accepted/related API rule source design | Related only; API/RPC compiler preserves effective route/OpenAPI semantics. No section is amended. |
| [Yuheng BIZ/APP 与直连 RPC](2026-08-15-16-57-yuheng-biz-app-scope-direct-rpc-design.md) | `Review`; related direct-RPC work | Related only and not used as accepted dependency. Any future conflict must be resolved by that document's approval/amendment, not inferred here. |
...
| `MC-REUSE-001` | Applicable | PASS | §6 reuse ledger; current Tianshu contributor/active key, activation/LKG, operation invoker/transports, MCP Core, Admin APIs, UI components | Existing Spring/Egon/repository capabilities are reused before new elements; new elements only form the confirmed process boundary | None |
