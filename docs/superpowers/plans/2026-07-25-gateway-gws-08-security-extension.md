...
**Commit:** `feat(yuheng): define security extension contracts`
...
**Commit:** `feat(yuheng): validate yuheng security capabilities`
...
**Commit:** `feat(yuheng): execute fail closed security chain`

## Task 4: 身份清洗与可信透传

- 固定保留 Header/Metadata 不允许被规则放宽。
- Identity Mapper 只写 `X-Egon-Yuheng-*` 或 RPC 白名单。
- 不透传 Authorization、Cookie、Token 或 Access Zone 声明。

**Commit:** `feat(yuheng): sanitize and map trusted identity`
...
**Commit:** `feat(yuheng): enforce listener trust boundaries`
