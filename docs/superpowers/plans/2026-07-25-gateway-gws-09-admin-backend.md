# Yuheng GWS-09 Admin 后端实现计划

状态：已执行

**Goal:** 建立可独立启动、可持久化、可恢复发布的 Yuheng Admin 控制面，承载三级
...
Infrastructure 负责 JPA、Tianshu Management Client、时钟与投影。网络发布严格发生在
本地事务提交之后。
...
- Application Service：编排事务、审计、幂等和 Tianshu 调用。
- Adapter：Tianshu Management Client 是唯一规则发布出口。
- 不引入通用工作流引擎；状态数量固定，显式状态机更容易审计和恢复。

## Task 1: 数据库与持久化骨架

- 为 Yuheng Admin 新增唯一 `V1__create_gateway_admin_schema.sql`。
...
**Commit:** `feat(yuheng): persist admin control plane model`
...
**Commit:** `feat(yuheng): manage interface catalog`
...
**Commit:** `feat(yuheng): manage revisioned yuheng drafts`

## Task 4: Release 状态机和 Tianshu 编排
...
**Commit:** `feat(yuheng): orchestrate recoverable releases`

## Task 5: 管理 API、错误与投影

- 实现 `/api/v1/yuheng/admin` 下的 Group、Application、Catalog、Draft、Release API。
- 统一 409/422/503 错误结构，不暴露 JPA/Tianshu 异常。
- Engine/Provider 投影响应带 `observedAt/source/stale`，不作为 Engine 寻址源。
- 重要操作落审计，快照只记录 SHA/大小/Release 引用。

**Commit:** `feat(yuheng): expose admin management api`

## Task 6: 验收

```bash
./mvnw -B -ntp -f egon-cola-components/pom.xml \
  -pl :egon-cola-component-yuheng-admin -am clean test
```

检查只有一个新 Yuheng Admin Migration；Admin 不依赖 Tianshu Redis/Repository；发布网络
