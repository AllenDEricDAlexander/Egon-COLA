# Yuheng GWS-02B Tianshu Management Extension Implementation Plan
...
**Goal:** 为 Yuheng Admin 提供不依赖 Tianshu Starter/JPA/Redisson 的稳定 Management
Client，并在 Tianshu Admin 中实现 HMAC 管理 OpenAPI、幂等删除和 UTF-8 配置容量保护。

**Architecture:** 新增独立 `dynamic-config-center-management-client` 公共模块，承载
稳定 DTO、HMAC canonical request、`RestClient` Adapter 和错误映射。Tianshu Admin 通过
...
- 不让 Management Client 依赖 Tianshu Starter、Redisson、JPA 或 Admin。
- 不实现 Yuheng Release 引用判断；GWS-09 在调用删除前证明未被 Active/Retryable
  Release 引用，Tianshu 只负责 expectedVersion、精确 Key、幂等与审计。
- Management OpenAPI 位于 `/api/v1/tianshu/openapi/management`，继续受现有 HMAC
  Filter 保护。
...
5. BOM 同时导出 Tianshu Starter 和 Management Client。
...
**Commit:** `build(tianshu): add management client contracts`
...
**Commit:** `feat(tianshu): add signed management client adapter`
...
**Commit:** `feat(tianshu): expose management machine openapi`
...
**Commit:** `feat(tianshu): enforce config value capacity`
...
- GWS-09 才实现 Yuheng Release 引用证明与 Active Key 删除编排。
