...
- 默认 Surefire 不启动外部拓扑；真实进程仅在 `yuheng-live` 显式 profile。
- Tianshu Redis 与 Rate Redis 必须物理分离并有行为断言。
...
**Files:**
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayLiveEnvironment.java`
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayAdminTestClient.java`
- Modify: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayProcessSpec.java`
- Modify: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayProcessHarness.java`
- Modify: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayTestInfrastructure.java`
- Test: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayProcessHarnessTest.java`

**Interfaces:**
- Produces one closeable environment owning containers and JVMs.
- Produces unique names `yuheng-biz-gateway-1`, `yuheng-biz-gateway-2` and per-instance LKG/log/manifest paths.
...
- [ ] **Step 4: Run all default Yuheng tests**

Expected: PASS; `yuheng-live` remains opt-in.

- [ ] **Step 5: Commit**

```bash
git add egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-test
git commit -m "test: isolate yuheng live topology fixture"
```

### Task 2: 验证 MVC/WebFlux Provider 与租约恢复

**Files:**
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayHttpProvidersLiveIT.java`
...
./mvnw -B -ntp \
  -pl egon-cola-components/egon-cola-component-yuheng/\
egon-cola-component-yuheng-test/egon-cola-component-yuheng-test-suite \
  -am -Pyuheng-live -Dit.test=GatewayHttpProvidersLiveIT verify
```
...
Pass Testcontainers Tianshu Redis host/port, unique advertised ports, matching version and unique zone/provider type.
...
```bash
git add egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-test
git commit -m "test: verify mvc and webflux yuheng providers"
```

### Task 3: 验证双 Engine RPC 与规则/基础设施生命周期

**Files:**
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayRpcDualEngineLiveIT.java`
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayRuleLifecycleLiveIT.java`
- Create: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/live/GatewayInfrastructureLiveIT.java`
- Reduce/replace: existing `GatewayLiveTopologyIT.java` after all assertions move.

**Interfaces:**
- Produces HTTP→RPC and RPC Consumer→Yuheng→Provider evidence for both Engine IDs.
...
non-idempotent CreateOrder with injected Yuheng failure and assert no second Provider call.
...
restart Admin/Tianshu -> PostgreSQL facts retained
```
...
```bash
git add egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-test
git commit -m "test: verify yuheng rpc and rule lifecycle"
```

### Task 4: 增加 Compose Demo 与可重复脚本

**Files:**
- Create: `.../yuheng/deployment/compose.demo.yml`
- Create: `.../yuheng/deployment/Dockerfile.test-app`
- Create: `.../yuheng/deployment/demo/applications.json`
- Create: `.../yuheng/deployment/demo/routes.json`
- Create: `.../yuheng/deployment/demo/policies.json`
- Create: `.../yuheng/deployment/scripts/demo.sh`
- Create: `.../yuheng/deployment/scripts/demo-token.sh`
- Create: `.../yuheng/deployment/scripts/wait-ready.sh`
- Modify: `.../yuheng/deployment/.env.example`
- Test: `.../yuheng-test-suite/src/test/java/top/egon/cola/component/yuheng/test/deployment/GatewayDemoScriptContractTest.java`
...
docker compose --env-file \
  ...
  -f egon-cola-components/egon-cola-component-yuheng/deployment/compose.demo.yml \
  config --quiet
```
...
```bash
cd egon-cola-components/egon-cola-component-yuheng/deployment
...
```bash
git add egon-cola-components/egon-cola-component-yuheng/deployment \
        egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-test
git commit -m "feat: add yuheng integration demo"
```

### Task 5: 编写并验证中英文联调 Runbook

**Files:**
- Create: `.../yuheng/docs/developer-integration.zh-CN.md`
- Create: `.../yuheng/docs/developer-integration.md`
- Modify: Yuheng/Tianshu/RPC root `README.md` and `README.zh-CN.md` files.
- Modify: `.../yuheng/deployment/README.md`
- Modify: `.../yuheng/deployment/README.zh-CN.md`
- Modify: Admin Web README files to remove nonexistent `VITE_YUHENG_ADMIN_ACTOR_ID`.
...
./mvnw -B -ntp \
  -pl egon-cola-components/egon-cola-component-yuheng/\
egon-cola-component-yuheng-test/egon-cola-component-yuheng-test-suite -am test
```
...
`demo.sh verify`, a relative-link resolver, `rg 'VITE_YUHENG_ADMIN_ACTOR_ID'` expecting no matches, and
...
git commit -m "docs: add yuheng tianshu rpc integration runbook"
```

### Task 6: 最终完成审计

**Files:**
- Create: `docs/superpowers/specs/2026-07-26-yuheng-tianshu-rpc-integration-acceptance.md`
...
- [ ] **Step 2: Run `yuheng-live` from a clean build**
...
```bash
git add docs/superpowers/specs/2026-07-26-yuheng-tianshu-rpc-integration-acceptance.md
