# GWS-13 Yuheng Test and Deployment Implementation Plan
...
production Yuheng Starter, Provider Runtime, RPC Starter, Admin, and Engine
...
`yuheng-live-test` Maven profile gates infrastructure-dependent Failsafe tests;
...
Yuheng is allowed only for unit seams and is forbidden in live scenarios.

---

## Task 1: Real HTTP provider

Create a Spring Boot MVC application with Yuheng Starter and Provider Runtime,
...
the existing Egon RPC annotations/runtime and Yuheng Starter reporting. Create
...
Starter report visibility, Tianshu registration, HTTP and RPC forwarding, rule
...
and the `yuheng-live-test` profile. Add documented commands for fast reactor,
...
image that does not manage Nginx yuheng nodes, Compose examples for local
...
Run the full Yuheng Maven reactor without live profile, frontend lint,
