...
> 实施方式：在 `codex/yuheng-wave-0-foundation` 隔离 Worktree 内按 TDD 小步提交。

## Task 1：Admin 身份与 Capability

文件：

- `egon-cola-component-yuheng-admin/pom.xml`
...
5. 提交 `fix(yuheng-admin): enforce authenticated management actors`。

## Task 2：流式 HTTP 与大小策略

文件：

- `...yuheng-biz-gateway/.../http/ReactorNettyHttpUpstreamAdapter.java`
- `...yuheng-biz-gateway/.../http/DefaultGatewayHttpDataPlaneHandler.java`
- `...yuheng-biz-gateway/.../traffic/**`
- 对应 HTTP/Traffic 测试
...
5. 提交 `fix(yuheng-biz-gateway): stream upstream responses and enforce size policies`。
...
5. 提交 `refactor(yuheng-biz-gateway): execute http traffic through staged filters`。

## Task 4：主动健康探测

1. 增加阈值、抖动、超时、HTTP/RPC 探测测试；
2. 实现 Probe SPI、Scheduler 和状态仓库；
3. Tianshu Adapter 合并主动/被动状态；
4. 增加配置、指标和关闭；
5. 提交 `feat(yuheng-biz-gateway): add active provider health probes`。
...
6. 提交 `feat(yuheng-admin): reconcile runtime and definition lifecycles`。

## Task 6：真实百分位与 OpenTelemetry

1. 增加非对称样本的 p50/p95/p99 测试；
2. PostgreSQL 使用 `percentile_cont`，测试数据库使用确定性降级；
3. 增加 Request/Attempt/Tianshu/Kafka Observation；
4. 验证 Span 传播、错误和低基数 Tag；
5. 提交 `feat(yuheng): add accurate percentiles and otel observations`。
...
4. 增加定义变化触发与单飞；
5. 提交 `fix(yuheng-starter): reconcile definition reports until acknowledged`。
...
4. 增加指标；
5. 提交 `fix(yuheng-admin): supervise call event consumption`。
...
6. 提交 `feat(yuheng-admin-web): complete authenticated management workflows`。
...
4. 增加 Yuheng 快速、Live、E2E、Nightly 工作流；
5. 校验 YAML/Compose/脚本语法，不自动启动产品；
6. 提交 `test(yuheng): add live topology e2e and ci gates`。
...
3. 增加阈值和归档说明；
4. 脚本静态校验；
5. 提交 `perf(yuheng): add capacity soak and fault scenarios`。

## Task 12：Tianshu HA、RPC 多活与 TLS/mTLS

1. 先增加 Tianshu 多实例任务竞争、RPC 多 Yuheng 选择/TLS 配置测试；
2. Tianshu 发布任务改为共享存储 CAS 领取；
3. RPC Consumer 管理多个 INTERNAL_GATEWAY Channel；
4. HTTP/RPC/Admin/Tianshu 增加 TLS/mTLS 配置与 Netty/Spring SSL；
5. 增加证书轮换和到期指标；
6. 提交 `feat(yuheng): support ha gateways and mutual tls`。

## Task 13：最终验证与审查

1. 运行所有新增定向测试；
2. 运行 Yuheng 27 模块 `clean verify`；
3. 运行 Admin Web typecheck/test/lint/build；
4. 校验 Compose、Workflow、Node 和 Shell；
5. 检查 `git diff --check`、迁移数量、依赖边界、未跟踪文件；
6. 更新验收文档为真实结果；
7. 提交 `docs(yuheng): record 17 gap remediation evidence`。
