...
Yuheng Admin 是管理平台后端，负责接口目录、Yuheng Group、路由与策略编辑、发布
...
更直接。Kafka 投影使用 Spring Kafka；Tianshu 访问只经过 GWS-02 Management Client。
...
│   ├── projection       Engine/Provider/Tianshu 投影
│   └── audit            审计编排
├── domain
│   ├── catalog
│   ├── yuheng
...
Repository；Infrastructure 通过 Port 接入 PostgreSQL、Tianshu 和 Kafka。
...
- 一个 Engine 实例首期只属于一个 Yuheng Group；
- 禁用 Group 阻止新发布，不直接杀死 Engine 或清空 LKG；
- 删除只允许逻辑删除，且不存在活动 Engine、Route 和可回滚 Release；
- Tianshu Config `appCode` 按 GWS-06 由 Group Code 派生。
...
- 目录是接口定义事实，不等同于 Yuheng Group；
- 同一 Operation 可以被不同 Yuheng Group 的 Route 引用。
...
每个 Yuheng Group 有一个当前可编辑 Draft：
...
- Engine Config Client：来自 Tianshu Config Client；
- Provider Service/Instance：来自 Tianshu Service Registry；
...
Engine 运行详情按 GWS-06 从 Tianshu Config Client 的管理地址查询。Admin 必须校验
...
| 表 | 核心用途 |
|---|---|
| `gateway_group` | Yuheng Group 和作用域 |
...
| `gateway_release_attempt` | 每次发布/重试 Attempt 与 Tianshu Task 状态 |
| `gateway_release_target` | Tianshu Target ACK 投影 |
...
- 数据库事务只覆盖本地持久化，不跨 Tianshu HTTP 调用持锁。

## 5. Redis 与 Tianshu 边界

用户确认的“DB + Redis 双写并通过 Redis 消息下发”由 Tianshu Component 统一实现：

```text
Yuheng Admin PostgreSQL
→ Tianshu Management OpenAPI
→ Tianshu PostgreSQL + Redis
...
- Yuheng Admin 不直连 Tianshu Redis；
- Yuheng Admin 不写 Tianshu 表；
- Yuheng Admin 只使用 GWS-02 `DdcManagementClient`；
- Yuheng PostgreSQL 是管理模型和 Release 审计事实；
- Tianshu PostgreSQL/Redis 是配置版本、发布任务和下发事实；
- 本地事务提交后才允许调用 Tianshu；
- Tianshu 调用失败时 Release 保持可恢复状态，由显式重试/校准处理；
- 不实现分布式事务假象。
...
### 6.1 Yuheng Group 与目录

| Method | Path | 用途 |
|---|---|---|
| GET/POST | `/yuheng-groups` | 查询/创建 Group |
| GET/PUT | `/yuheng-groups/{id}` | 详情/乐观锁更新 |
| POST | `/yuheng-groups/{id}/enable` | 启用 |
| POST | `/yuheng-groups/{id}/disable` | 禁用 |
...
| Method | Path | 用途 |
|---|---|---|
| GET | `/yuheng-groups/{id}/draft` | 当前 Draft |
| PUT | `/yuheng-groups/{id}/draft/routes/{routeId}` | 新增/更新 Route |
| DELETE | `/yuheng-groups/{id}/draft/routes/{routeId}` | 删除 Draft Route |
| PUT | `/yuheng-groups/{id}/draft/policies/{policyId}` | 新增/更新 Policy |
| DELETE | `/yuheng-groups/{id}/draft/policies/{policyId}` | 删除 Draft Policy |
| POST | `/yuheng-groups/{id}/draft/validate` | 完整校验/预编译 |
| GET | `/yuheng-groups/{id}/draft/diff` | 与基线 Release 比较 |
...
Revision 冲突返回 `409 YUHENG_ADMIN_REVISION_CONFLICT` 和当前 Revision，不自动重放
用户修改。
...
| Method | Path | 用途 |
|---|---|---|
| POST | `/yuheng-groups/{id}/releases` | 基于指定 Revision 创建并发布 |
...
| POST | `/yuheng-groups/{id}/rollback` | 用历史成功内容创建新 Release |
| GET | `/yuheng-groups/{id}/releases` | 发布历史 |
...
| Method | Path | 用途 |
|---|---|---|
| GET | `/yuheng-groups/{id}/engine-nodes` | Engine 节点、租约、版本、ACK |
| GET | `/providers/services` | Provider Service 目录 |
| GET | `/providers/instances` | Provider Instance、Metadata、健康投影 |
| GET | `/yuheng-groups/{id}/runtime-consistency` | 目标 Release 一致性 |
...
→ 提交事务
→ 调用 Tianshu Upsert/Publish
→ 持续投影 Tianshu Task
→ 更新 Release 终态
```

本地事务中不进行网络调用。发布编排器使用状态机模式，因为外部 Tianshu 调用存在超时、
...
- 有 `changeId`：查询 Tianshu Task 并更新投影；
- Tianshu 返回 UNKNOWN：保留 UNKNOWN，允许人工核对/重试；
- 无 `changeId` 且已保存内容：在幂等保护下重新发起；
- 已成功：不重复发布；
- 长时间失败：保持诊断，不自动回滚；
- 重试沿用原 Release 内容和原固化 Target，不能重新编译当前 Draft。
- Tianshu Active Version 或 Group Release 已被更新版本推进时，拒绝旧 Release Retry，
  用户只能创建新回滚 Release。

### 8.3 回滚

回滚不是修改 Tianshu 版本或让 Engine 接受低版本，而是：
...
- Tianshu Upsert 使用确定 Config Coordinate 和 Expected Version；
- 发布恢复任务采用数据库租约/CAS，避免多 Admin 进程重复执行；
- 首期 Tianshu 仍是单 Admin/单 Redis，但 Yuheng Admin 自身不得依赖 JVM 单例锁保证
  正确性；
- 所有后台任务可重入。
...
- Secret 不存入 Yuheng 业务表和审计 Payload；
- 日志对 HMAC、Cookie、Authorization 和 Tianshu Secret 脱敏；
...
| Code | HTTP | 含义 |
|---|---:|---|
| `YUHENG_ADMIN_VALIDATION_FAILED` | 422 | Draft/接口/策略校验失败 |
| `YUHENG_ADMIN_REVISION_CONFLICT` | 409 | 乐观锁冲突 |
| `YUHENG_ADMIN_IDEMPOTENCY_CONFLICT` | 409 | 幂等 Key Payload 不一致 |
| `YUHENG_ADMIN_RESOURCE_IN_USE` | 409 | 资源仍被 Route/Release 引用 |
| `YUHENG_ADMIN_RELEASE_IN_PROGRESS` | 409 | 同 Group 正在发布 |
| `YUHENG_ADMIN_TIANSHU_UNAVAILABLE` | 503 | Tianshu 调用不可用 |
| `YUHENG_ADMIN_RELEASE_UNKNOWN` | 202 | 外部发布结果未知，需查询 |
| `YUHENG_ADMIN_REPORT_REJECTED` | 422 | Definition Report 非法 |
| `YUHENG_ADMIN_SCOPE_DENIED` | 403 | Actor 无作用域权限 |

校验错误返回稳定 `field/path/code/message` 列表，不返回 JPA、SQL 或 Tianshu 内部异常。
...
### 15.3 Tianshu Integration

1. Upsert→Publish→Target ACK→Release 成功；
2. 部分失败、超时、UNKNOWN；
3. Admin 重启后恢复非终态 Release；
4. Chunk/Activation 顺序和清理；
5. Tianshu 不可用时本地 Release 可恢复；
6. Yuheng Admin 未直接访问 Tianshu Redis/Repository。
...
5. DB/Redis 双写与 Redis 消息由 Tianshu 承接，Admin 不直连 Tianshu 存储；
...
2. 认可 Yuheng Admin PostgreSQL 与 Tianshu PostgreSQL/Redis 的职责划分；
