# Yuheng、Tianshu、RPC 与 HTTP 联调闭环修复设计
...
- `2026-07-24-tianshu-standalone-rpc-framework-design.md`
- `2026-07-24-yuheng-component-design.md`
- `2026-07-25-yuheng-rule-publication-runtime-design.md`
- `2026-07-25-yuheng-17-gap-remediation-design.md`
- `2026-07-25-yuheng-test-deployment-design.md`

## 1. 背景与结论

当前 Yuheng、Tianshu、RPC 三个 Component 各自拥有较完整的单元能力，但真实组合链路仍有
确定性合同冲突：Yuheng 生成的 Tianshu 发布请求不能通过 Tianshu 校验，Tianshu 写出的服务状态会被
Yuheng 和 RPC 判断为不可用，HTTP Provider Runtime 没有业务可直接使用的自动装配，真实
测试也只覆盖 Spring MVC 的局部 happy path。

本轮采用“持久化分阶段发布 + 稳定公共契约 + 可执行联调环境”方案，修复已经审计确认的
P0、P1、P2 问题，并以真实 Redis、PostgreSQL、Kafka、独立 JVM 和双 Yuheng Engine
证明组合能力。
...
2. 给 Tianshu 增加 Yuheng 专用 `publishBundle`。它会让通用配置中心理解 Yuheng 的
...
1. Yuheng Admin 可以通过真实 Tianshu 合同创建、发布并恢复 inline/chunked 规则。
2. Tianshu Config、Service Registry、Yuheng Provider Directory 和 RPC Yuheng Directory
   使用一致的服务状态与租约语义。
3. HTTP Provider 仅添加依赖和配置即可在 Spring MVC 或 Spring WebFlux 应用中完成接口
   上报、服务注册、心跳和注销。
4. 打通三条真实数据路径：
   - HTTP Client → Yuheng → Spring MVC/WebFlux HTTP Provider；
   - HTTP Client → Yuheng → RPC Provider；
   - RPC Consumer → Yuheng → RPC Provider。
5. RPC 能准确区分 Yuheng 阶段和 Provider 阶段失败，只对显式幂等方法执行安全的
   Yuheng 实例故障转移。
...
3. Tianshu 发布失败时，周期 pull 不得读取未发布 draft。
...
- Yuheng Rule inline/chunked 发布、恢复、回滚、LKG 和 GC；
- Tianshu SINGLE、SENTINEL 与 CLUSTER 的配置合同和测试入口；
- Tianshu 管理面认证、HMAC scope 与共享 nonce 防重放；
- RPC unary 调用、多 Yuheng Slot、幂等故障转移和错误分类；
...
#### P0-01 Yuheng → Tianshu 发布请求无效

`GatewayDdcRulePublisher` 传递空 `expectedVersion`，并从普通字符串拼接 chunk changeId；
`GatewayReleaseService` 使用 `yuheng-release-*` 作为 changeId。Tianshu 要求配置已存在、
`expectedVersion` 精确匹配且 changeId 为 UUIDv7，因此真实发布必然失败。

#### P0-02 服务状态不一致

Tianshu 生产 `ONLINE`；Yuheng 只接受 `REGISTERED`；RPC Consumer 只接受 `UP` 或空值。
真实 Provider 和 Yuheng Slot 会被错误摘除。

#### P0-03 Tianshu 容器不可执行

Tianshu Admin 主产物是 thin JAR，可执行产物使用 `exec` classifier；Dockerfile 当前复制 thin
JAR，Compose 会在 `java -jar` 阶段失败。

#### P0-04 联调进程连接错误 Redis

Compose Engine 与现有 live Harness 没有把 Tianshu Redis 的服务地址或 Testcontainers 随机
端口传给所有 Tianshu Client，进程会退回 `127.0.0.1:6379`。

#### P0-05 HTTP Provider 不能直接消费

`yuheng-provider-runtime` 没有 Boot AutoConfiguration、配置属性或生命周期绑定。现有
...
#### P1-01 Yuheng 只记录 activation changeId

chunk 发布没有持久化 phase；响应丢失或任意 chunk 后崩溃时无法确定完成位置，也不能安全
决定是否进入 activation。

#### P1-02 Tianshu draft 与 published 指针混用
...
Tianshu Config、Registry 和 Yuheng Rate Limit 按类型判断缺省 Bean，却又按固定 qualifier
...
Tianshu 中旧 release chunks 也没有保护规则与延迟清理策略。

#### P1-08 RPC 重试与错误分类不安全

Consumer 会对缺乏明确 Provider failure trailer 的 `UNAVAILABLE` 重试，但方法合同没有
幂等标记；部分生产错误没有一致写入 failure-stage；Yuheng Slot 心跳失败后不能重新注册。

### 4.3 P2：安全、可测试性和文档不足

1. Tianshu HMAC 只保护 openapi，其他管理接口默认开放；nonce 是单进程内存态。
...
6. Tianshu manifest 默认版本仍为 `5.2.1`，与工程 `5.2.3` 漂移。
7. Yuheng Admin 本地联调缺少 JWT 生成工具；Admin Web 文档包含不存在的环境变量。
...
flowchart LR
    ...
    GatewayAdmin["Yuheng Admin"]
    DdcAdmin["Tianshu Admin"]
    DdcDb["Tianshu PostgreSQL / SQLite"]
    GatewayDb["Yuheng PostgreSQL"]
...
- Yuheng/RPC 选择候选时同时要求 `ONLINE` 且 `leaseExpireAt > now`。

状态解析位于 Tianshu management-client 公共模型，不允许 Yuheng、RPC 各自继续比较字符串。

## 7. Yuheng 持久化分阶段发布

### 7.1 publication journal

Yuheng Admin 新增 `gateway_release_publication`：
...
- `change_id` 唯一；
- 外部 Tianshu 调用前必须先持久化完整 operation；
...
第一阶段不增加客户端持久化 ACK outbox。客户端崩溃后的遗漏由 Tianshu 重发和周期 reconcile
收敛。

### 8.4 pull 应用顺序

为 `DdcConfigApplier` 增加向后兼容的默认 priority；启动 pull 与批量 reconcile 按
priority、configKey 排序。Yuheng chunk applier 优先于 activation applier，避免首次启动
...
- `/api/v1/tianshu/openapi/**`：HMAC 服务身份；
...
closed。JWT claim/capability 语义与 Yuheng Admin 保持一致，但 Tianshu 不依赖 Yuheng 模块。

开发环境允许通过显式 `local-dev` 配置启用受限简化认证；默认配置不得把生产管理接口开放。

## 11. HTTP Provider Runtime

`yuheng-provider-runtime` 新增：
...
4. env、namespace、serviceName、version 默认复用 Yuheng Reporting/Tianshu 配置；
...
Provider 阶段失败、非幂等方法、Deadline exceeded、contract 错误均不跨 Yuheng 重试。
...
生产 Yuheng 的路由/目录/连接失败写 GATEWAY 或 PROVIDER；Provider 返回的业务 gRPC
Status 保留 Provider 阶段。`UNIMPLEMENTED` 映射为稳定 method-not-found 错误，不伪装成
Yuheng unavailable。

### 12.3 Yuheng Slot 生命周期

Yuheng RPC registrar 使用：
...
禁止重注册。Consumer Directory 接受多个 ONLINE Yuheng Slot，按现有负载策略选择；实例
移除时排空并关闭 Channel。

默认 Yuheng service name 统一为 `egon-yuheng-rpc`；env、namespace、group、version 和
Tianshu appCode 的来源在 Yuheng Engine、RPC Consumer、Compose 与测试中保持一致。

## 13. 启动、部署与版本修复

1. Tianshu Dockerfile 复制 `*-exec.jar`，保留 thin JAR 为 Maven 主产物。
2. Compose 和 live Harness 给每一个 Tianshu Client 传递真实 Redis host/port。
3. 容器内 advertised host 使用 Compose service DNS，禁止使用 `127.0.0.1`。
4. Tianshu manifest 版本从 Maven/build metadata 获取，删除 `5.2.1` 硬编码。
5. Compose 为 Tianshu、Yuheng Admin、Engine 添加 healthcheck，依赖 readiness 而非单纯
   `service_started`。
...
- 新增且仅新增
  `egon-cola-component-yuheng-admin/src/main/resources/db/migration/`
  `V4__add_release_publication_journal.sql`。

Tianshu 对两个受支持数据库分别执行一次相同逻辑变更：
...
- Tianshu 未认证、错误 capability、越权 scope 和跨节点 nonce replay；
- PostgreSQL/SQLite V1→V4 migration。

### 15.2 L1 `yuheng-live` 真实进程测试

使用 Testcontainers 启动真实 PostgreSQL、Tianshu Redis、Rate Redis、Kafka，并启动独立 JVM：
...
10. Admin/Tianshu 重启后 PostgreSQL 数据、发布任务和 Flyway history 保留。
...
- 双 Tianshu Admin 验证 Redis nonce 防重放；
...
- Adapter：Tianshu publisher、HTTP/RPC upstream 和 Registry 保持外部系统适配边界；
- Application Coordinator：`GatewayReleasePublicationCoordinator` 负责编排固定阶段；
- Persistent Command Journal：保存外部调用身份与恢复进度；
- 显式 State Enum：发布、ACK、Provider Runtime、Yuheng Slot 状态；
...
1. 公共状态语义、Tianshu Docker/版本/Redis 地址与 Redisson Bean；
2. Tianshu exact GET、published pointer、v2 key、Lua 原子发布和迁移；
3. Yuheng publication journal、阶段 Coordinator、reconcile 和迁移；
...
7. Tianshu HMAC/JWT/Redis nonce；
...
2. Tianshu、Yuheng、RPC 各自模块测试通过；
3. Yuheng/Tianshu/RPC 相关 Reactor 构建通过；
4. `yuheng-live` 真实闭环覆盖 MVC、WebFlux、HTTP→RPC、RPC 双 Engine、发布恢复、
   rollback、LKG、分布式限流和 Provider 新 lease；
5. PostgreSQL、Tianshu Redis、Rate Redis、Kafka 均有行为断言，不只是容器成功启动；
