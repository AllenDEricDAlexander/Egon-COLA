# Tianshu 模块边界与 Starter 运行闭环调整设计
...
1. 业务系统、Yuheng 和 RPC 只依赖一个 Tianshu SDK 制品；
2. Yuheng Admin 不会依赖 Tianshu Admin 的 JPA、Flyway、数据库、安全和 Actuator 运行时；
3. Tianshu Admin 继续通过 Starter 复用协议契约，但不反向暴露服务端实现；
4. 删除 BOM 中容易被业务方误认为需要单独引入的 management-client 条目。

Tianshu test 模块恢复为纯 Starter 消费端样例和黑盒验证模块。所有直接使用 Tianshu Admin
...
- Tianshu Starter：HTTP 传输安全、HMAC 签名、实例元数据契约；
- Tianshu Admin：管理 OpenAPI DTO、错误码和签名验证；
- Yuheng Admin：配置写入、发布、任务查询和服务目录查询；
- Yuheng Starter：HMAC 请求签名；
- RPC Starter：实例元数据编码。

因此不能把整个模块直接放入 Tianshu Admin 后再让 Yuheng 依赖 Admin。这样会把
`spring-data-jpa`、Flyway、PostgreSQL、SQLite、Spring Security、Actuator 和 Admin
应用类传递到 Yuheng Admin，形成控制面服务之间的错误编译依赖。
...
- 不重构 Yuheng、RPC 或 Tianshu 的业务逻辑；
- 不启动 Tianshu Admin、Redis、PostgreSQL、Yuheng 或 RPC 进程。
...
Starter 是唯一面向业务应用的 Tianshu SDK，包含：

- `@DdcValue`、字段绑定和本地配置应用；
- 配置客户端注册、心跳、下线、默认值上报、快照拉取和 ACK；
- Redis 配置发布订阅与服务目录订阅；
- RPC Provider、HTTP Provider、Internal Yuheng 使用的注册契约；
- 原 management-client 的管理 OpenAPI Client 和 DTO；
- HMAC 规范请求、签名器、TLS HTTP Client 工厂；
- `ServiceInstanceMeta` 等 Yuheng/RPC 共享元数据契约。

原 management-client 的 Java 包名保持不变，例如：

```text
top.egon.cola.component.tianshu.management.*
top.egon.cola.component.tianshu.security.*
```
...
Admin 依赖 Starter 取得协议契约，不再依赖 management-client。Admin 不向 Yuheng/RPC
...
Test 不再依赖 Tianshu Admin，也不再持有 Testcontainers 依赖或 Admin Redis 拓扑 Profile。

### 4.4 依赖关系

```text
业务应用 ───────────────> Tianshu Starter
RPC Starter ─────────────> Tianshu Starter
Yuheng Starter ─────────> Tianshu Starter
Yuheng Admin ───────────> Tianshu Starter
Tianshu Admin ───────────────> Tianshu Starter
Tianshu Test ────────────────> Tianshu Starter

不存在任何模块 ─────────> Tianshu Admin（作为库依赖）
```
...
1. 将 management-client 的 `src/main` 源码移动到 Tianshu Starter；
2. 将 management-client 的 `src/test` 测试移动到 Tianshu Starter；
3. 删除 management-client 子模块目录；
4. 从 Tianshu 聚合 POM 的 `modules` 和 `dependencyManagement` 删除该坐标；
5. 从 Tianshu Starter、Tianshu Admin POM 删除该依赖；
6. Yuheng Admin、Yuheng Starter 改为依赖 Tianshu Starter；
7. 从组件 BOM POM 和中英文 BOM README 删除 management-client 条目；
8. 更新 Tianshu 中英文 README 的模块列表、依赖示例和构建命令；
9. 更新仍引用旧模块名的当前有效设计/运行文档。历史 Spec 保留历史事实，不批量改写。

### 5.2 Starter 自动装配必须显式启用

management Client 合并进 Starter 后，Yuheng Admin 等调用方可能只需要管理 Client，
...
- Yuheng Engine 等真实配置客户端 Runtime 应用显式补上 `enabled: true`；
- Tianshu Admin、Yuheng Admin 和仅使用管理 Client 的应用不启动 Tianshu Runtime；
- 现有已显式配置 `enabled` 的测试和样例保持原语义。

`DdcRegistryAutoConfig` 仍由 `egon.cola.component.tianshu.registry.enabled` 独立控制。
RPC/Yuheng 只使用服务注册能力时不必同时启动配置客户端 Runtime，但必须显式配置
Admin Endpoint、HMAC 凭据和 Redis 拓扑。

这是本次唯一有意的自动装配行为变化，目的是让“依赖 Starter”与“启动 Tianshu Runtime”解耦。
...
`egon.cola.component.tianshu.admin.endpoint`，不能以 NPE 或连接 localhost 的形式失败。
...
不订阅，也不能作为配置中心联调证据；启动日志必须明确提示当前没有远端 Tianshu 生命周期。

### 6.4 配置客户端注册与服务注册的区别

- `DdcRuntimeCoordinator` 注册的是 `CONFIG_CLIENT`，用于配置发布目标、心跳和 ACK；
- `DdcServiceRegistryClient` 注册的是 `RPC_PROVIDER`、`HTTP_PROVIDER` 或
  `INTERNAL_GATEWAY`，由 RPC/Yuheng 对应 Runtime 发起；
- `registry.enabled=true` 只装配服务注册 Client，不会猜测普通业务 Bean 并自动注册服务；
- Tianshu test 的消费端样例验证 `CONFIG_CLIENT` 闭环；RPC/Yuheng 服务注册继续由各自集成测试验证。

---

## 7. 测试归位

### 7.1 保留在 Tianshu Test
...
5. 不加载任何 `top.egon.cola.component.tianshu.admin` 类型。

该测试验证 Starter 编排，不伪装成真实外部 Redis/Admin 集成证明。

### 7.2 迁入 Tianshu Admin Test
...
Testcontainers 版本、依赖以及 `tianshu-redis-sentinel`、`tianshu-redis-cluster` Maven Profile
随这些测试移动到 Admin POM，现有 Profile 名称和执行语义保持不变。

### 7.3 Runnable Sample 配置

Tianshu test 主资源中的示例配置改为环境变量驱动，并显式出现所有关键地址：
...
默认 `TIANSHU_ENABLED=false`，避免构建或误运行时连接外部服务。进行本地联调时必须显式启用并
...
2. BOM 只暴露 Tianshu Starter；
3. Tianshu test 的依赖树不包含 Tianshu Admin；
4. Yuheng Admin、Yuheng Starter、RPC Starter 编译通过；
...
6. Yuheng Admin、Yuheng Starter、Yuheng Engine 和 RPC Starter 相关测试通过；
7. 显式启用配置客户端或服务注册但未配置 Endpoint 时，有确定、可读的启动失败；
8. 仅把 Starter 作为管理 Client 契约依赖时，不自动启动 Tianshu Runtime。
...
./mvnw -B -ntp \
  -pl egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-admin,egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-starter,egon-cola-components/egon-cola-component-yuheng/egon-cola-component-yuheng-biz-gateway,egon-cola-components/egon-cola-component-rpc/egon-cola-component-rpc-starter \
  -am test
