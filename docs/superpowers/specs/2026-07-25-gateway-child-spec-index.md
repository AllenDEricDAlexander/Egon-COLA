...
5. 约束后续流程为“子 Spec 审核 → 实施计划 → 代码实现”。

本轮只创建子 Spec，不创建 Gateway Maven 模块、不修改现有 RPC/DDC 代码、不启动
任何服务。
...
依赖只表示设计契约先后，不表示所有实现必须严格串行。依赖相同且写入范围不重叠的
实施任务可以在对应 Spec 审核后并行。
