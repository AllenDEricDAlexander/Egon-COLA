...
> 日期：2026-07-30
>
> 修订：R2 详细评审稿（逐项补全数据、状态、算法、接口、页面、异常与验收契约）
>
...
以下两条是本次确认中最容易产生歧义、也最需要显式锁定的结论：

> **RBAC3 完全不实现审批。** 角色分配和轮岗没有提交、审批、通过、驳回流程；有权管理者的
> IMMEDIATE 命令通过全部规则后直接在当前事务生效。
>
...
|---|---|---|---|
| D-01～D-03 平台形态 | 权限事实、运行执行和业务接入是不同职责，且平台需要独立运维 | 建立中心 Admin、轻量 Starter/Adapter、前端与测试模块；目录只保存授权快照 | 把所有逻辑塞进一个 Starter；复制完整 HR |
...
| D-11～D-12 任职 | 用户的权限来自全部有效任职，轮岗包含岗位切换和交接 | 登录无角色选择；普通分配和轮岗使用不同命令与状态 | 用户自行切角色；把轮岗实现成两次无关 CRUD |
| D-13～D-14 无审批 | 用户明确要求不审批 | 删除所有审批状态、字段、API、页面和错误码；立即命令直接执行 | 预留 `approved_by`；以“审核确认”换名保留审批 |
| D-15～D-19 Gateway/DDC | 服务注册、接口目录和路由发布是三个不同事实 | 复用现有 Gateway Starter/Provider Runtime/安全 SPI；三态独立验收 | RBAC3 自写注册器；DDC 有实例就自动公开接口；静态地址兜底 |
| D-20 一致性 | 权限变更成功后旧权限不能继续使用 | 数据库版本、Fence、同步投影、Outbox、会话撤销形成闭环 | 仅删本机缓存；只发内存事件；Redis 失败仍返回成功 |
| D-21 高风险管理 | 无审批时必须用硬边界防止直接提权 | 禁止自我授权；高风险角色隔离；强认证、原因和审计 | 前端隐藏按钮；普通委托策略包含平台管理员角色 |
| D-22～D-25 交付与 UI | 完整终态需要分阶段证明，前端只能改善体验 | 每阶段独立验收；React 技术栈对齐；后端始终最终拒绝 | 以页面隐藏替代后端授权；以聚合构建成功替代行为证明 |

### 2.2 配置项与业务规则的边界

以下内容允许配置：Token 时长、会话时长、租约周期、密码失败阈值、角色容量值、管理策略有效期、
轮岗时间、紧急角色最长时长以内的实际时长、限流阈值和审计保留期。

以下内容不允许通过配置关闭：租户隔离、禁止自我授权、无审批语义、Gateway/DDC 三事实分离、
...
|---|---|---|---|
| 角色分类、激活策略 | 13、20、21 | 保留并收紧 | 用户不自选角色，全部有效任职自动参与；TEMPORARY/EMERGENCY 由管理者分配 |
| 普通分配与轮岗 | 20、21 | 保留 | 分成两个命令模型；轮岗额外处理来源岗位、交接、调度和恢复 |
| 委托管理 | 19 | 保留并补强 | 五元组必须由同一条有效策略完整满足，禁止把多条策略碎片拼成一次授权 |
| 轮岗审批 | 5、19、21、26～29、32 | 删除 | 不建表、不建状态、不建接口、不建页面、不建错误码 |
| 登录与 Landing Route | 12、13、28 | 保留并补强 | 登录不选角色；Route 必须可访问且 `componentKey` 存在于本地白名单 |
| 实时生效 | 12、22、23 | 重构 | 三类版本 + Fence + 同步投影 + Outbox；不是只删缓存 |
| SSD/DSD/前置/容量 | 14、25、27 | 保留并补强 | 明确校验时点、变化后闭包、时间重叠容量和并发锁 |
| 同对象职责分离 | 18 | 保留并补强 | 使用不可因轮岗消失的 Participation 事实；写入与业务动作可靠绑定 |
...
| Rotation | 带来源岗位处理、交接和时态的岗位变化命令 | 普通角色新增 |
| Snapshot | 某用户在特定 `authVersion/policyVersion` 下的授权运行投影 | PostgreSQL 权威事实 |
...
| 委托管理者 | 在一条有效策略完整授权的范围内查看/分配/撤销/轮岗 | 不能拼接多条策略绕过角色或用户范围，不能管理高风险角色 |
| 普通用户 | 登录、刷新、退出、获取自己的 Bootstrap 和使用已授权业务 | 不能选择角色、不能查看他人任职、不能自行新增角色 |
| 低风险自助撤销用户 | 在策略显式允许时撤销自己的低风险辅助角色 | 不能撤销岗位来源角色、高风险角色或绕过业务交接 |
...
| 可追踪性 | 管理命令、事件、投影、Gateway 请求和业务请求可通过 `traceId/requestId` 关联 | 日志、审计、事件头和执行日志联合查询 |
| 幂等性 | Manifest、目录同步、管理命令、轮岗调度、事件消费可安全重试 | 相同 Key 同内容返回既有结果，不同内容返回冲突 |
...
以“部门负责人将张三从出纳岗立即轮换到付款复核岗”为例：

1. 负责人携带有效 Token 调用影响分析；系统返回会结束的任职、将获得/失去的权限、SSD/容量、
   交接阻断项及分析所基于的 `authVersion/policyVersion`，但不写任何事实；
2. 负责人完成可处理的交接项后，以新的 `Idempotency-Key` 提交 IMMEDIATE 轮岗；
3. Gateway 校验 Token 与 `system:role-rotation:create` API Permission；
4. Admin 再校验功能 Permission、同一条管理策略的主体/目标用户/角色/操作、非本人、强认证时效；
5. Admin 建立 Fence，锁用户和冲突轮岗，再以最新事实重做全部约束与交接检查；
6. 同一 PostgreSQL 事务暂停/结束原任职、创建新任职、写轮岗快照/执行日志/审计/Outbox，并递增
   张三 `authVersion`；
7. 提交后同步更新 Redis 版本和快照、撤销张三旧会话，再清除 Fence；
8. 投影成功才向负责人返回 `EFFECTIVE`；若投影失败则返回 `AUTH_PROPAGATION_PENDING`，Fence
   保留且旧权限继续被拒绝；
9. 张三旧 Token 下一次请求得到版本错误，重新认证后 Bootstrap 进入付款复核 Landing Route；
10. 张三过去以出纳身份参与过的付款对象仍保留历史 Participation，不能因轮岗获得复核权。
...
|---|---|---|---|---|
| 授权控制面 | RBAC3 Admin + PostgreSQL | 身份快照、资源、角色、约束、任职、管理策略、轮岗、审计 | 当前授权事实和执行历史 | 低频写、高一致性、完整审计 |
...
-> 物化 Snapshot -> 返回 Access/Refresh/Bootstrap 最小入口
```
...
SDK 不公开“给当前用户切角色”或“前端直接修改 Permission Store”的 API。
...
| admin | PostgreSQL/Redis 集成、事务/锁/Outbox/调度测试 | 只有 Controller CRUD |
| admin-web | Typecheck/Lint/组件测试/关键流程 E2E | 页面能打开 |
| react-sdk | 类型声明、Guard、版本失效和宿主集成测试 | 仅导出 UI 组件 |
| test | 双 Admin + Gateway/DDC + 业务 Provider 真实闭环 | 单 JVM 或 Testcontainers 冒充本机拓扑 |
...
| admin | 允许 | 允许 | 仅测试 |  | 允许 | 允许 | 允许 |  |
| test | 允许 | 允许 | 允许 | 允许 | 允许 | 允许 | 允许 | 允许 |

Admin 正常运行时不通过 Starter HTTP 调用自己；表中 `admin -> starter` 仅用于接入契约测试，
若实施能通过独立测试夹具完成，则应移除该依赖。Gateway Adapter 可依赖 Gateway Core SPI，但不能
依赖 Gateway Engine 的具体路由实现。
...
5. 新快照缺失的目录项先转 INACTIVE，不级联删除 Assignment、Scope、Rotation 或审计；
...
4. 计算有效任职、继承闭包、DSD、权限和 Landing Route；无权限用户允许认证但 Bootstrap 进入
   无权限页，除非 Tenant 策略配置为禁止无任何基础角色登录；
5. 创建 ACTIVE Session、Refresh Family 和首个 Refresh Token Hash；
6. 物化 Redis 版本与 Snapshot；
7. 签发 Access Token 和一次可见的 Refresh Token；
8. 写登录审计，响应不回显失败次数、密码状态内部字段或其他 Tenant 信息。
...
| 用户状态/目录归属/任职/轮岗 | 受影响 Session 撤销 | 目标 User +1 | 通常不变 |
...
2. APP/MENU/ROUTE/ACTION 只返回 ACTIVE 且 Permission Allow 的资源，并按父子关系裁剪孤儿节点；
...
6. Field Policy 只返回当前可见应用资源相关字段；
...
角色状态为 `ACTIVE | DISABLED | ARCHIVED`。DISABLED 角色不参与新快照，也不能新建任职；禁用
前必须影响分析，提交后递增 `policyVersion`。ARCHIVED 为终态，只有没有未来有效任职且不再被其他
ACTIVE 角色继承时才能归档；历史 Assignment/Audit 仍引用原 ID 和 Code 快照。

角色关键属性语义：

| 属性 | 规则 |
|---|---|
| `roleCode` | 租户内稳定唯一，匹配 `[A-Z][A-Z0-9_]{2,63}`，推荐 `ROLE_` 前缀；创建后不可改，归档后不立即复用 |
| `roleName` | 可修改展示名，变更需审计但不影响权限版本，除非 Bootstrap 展示要求刷新 |
| `roleType` | 创建后不可变，避免 POSITION 变成 PUBLIC 绕过轮岗语义 |
| `riskLevel` | LOW/MEDIUM/HIGH/CRITICAL；决定强认证、最长有效期和可否委托 |
| `privileged` | 是否属于平台/租户特权角色；创建后不可改，普通租户 Role 创建接口不能设 true |
| `landingRouteCode` | 仅 POSITION/PUBLIC 可配置，且必须引用 ACTIVE ROUTE |
| `landingPriority` | 整数，值越小优先级越高；默认 1000 |
| `maxAssignmentDays` | 可空表示无额外上限；不得突破 TEMPORARY/EMERGENCY 全局上限 |
| `handoverRequired` | POSITION 角色是否要求至少一个已注册 Contributor 完成交接检查 |
...
使用同一值，避免长命令前后漂移；Scheduler 领取到期任务使用 `clock_timestamp()` 观察真实当前时间，
领取后的单任务事务再使用自己的 `transaction_timestamp()`。应用 JVM 时钟只用于非安全日志展示。
...
| Role Type | 允许来源 | 特殊限制 |
|---|---|---|
| PUBLIC | AUTO、DIRECT | AUTO 由租户规则产生；DIRECT 可按管理策略撤销 |
| POSITION | DIRECT、ROTATION | 一个时间点可有多个 POSITION 仅在 SSD/岗位策略允许时；推荐通过 ROTATION 变更主岗位 |
| MANAGEMENT | DIRECT、TEMPORARY | 高风险管理角色不能由普通委托策略分配 |
| TEMPORARY | TEMPORARY | `valid_to` 必填，默认最长 30 天，可配置更短 |
| EMERGENCY | EMERGENCY | `valid_to` 必填且最长 4 小时，强认证、reason、ticketNo 必填 |

AUTO Assignment 只由版本化 Tenant 自动任职规则产生。首期规则类型固定为：

- `ALL_ACTIVE_USERS`：所有 ACTIVE User 获得指定 PUBLIC Role；
- `POSITION`：目录快照中持有指定 Position 的 User 获得对应 POSITION Role。
...
EXPIRED/REVOKED 是终态。`PENDING` 用于已排期但未到 `valid_from` 的事实，不表示待审批。到时即使
调度器尚未更新为 ACTIVE，授权算法可按时间认定其应生效，但在需要同时处理来源岗位的 Rotation
场景，只有 Rotation 原子执行成功后目标 Assignment 才能成为 ACTIVE。

每条 Assignment 保存创建来源：`sourceType/sourceId/sourceRotationId/createdBy/reason/ticketNo`。
修改时间窗不覆盖原历史；高风险变更用撤销旧 Assignment + 新建新 Assignment 表达。
...
```text
assignments = load tenant/user assignments
direct = assignments.filter(status/time/user/tenant valid)
roleIds = direct.roleIds
closure = load active descendants for roleIds where depth <= 10
candidates = distinct(roleIds + closure.descendantRoleIds)
candidates = candidates.filter(role ACTIVE)
assertDsd(candidates, databaseNow)
permissions = stableSortedUnion(rolePermissions(candidates))
dataScopes = normalizeByPermission(roleDataRules(candidates))
fieldPolicies = maxLevelByField(roleFieldRules(candidates))
return Snapshot(candidates, permissions, dataScopes, fieldPolicies, versions)
```

排序只用于生成稳定 Checksum 和响应，不能改变 Allow 结果。继承只向 `senior -> junior` 获取 Junior
的 Permission/Scope/Field，不继承 Junior 的管理主体身份、Assignment 有效期、Landing Priority
或高风险豁免。
...
1. 选择当前有效 POSITION 角色中 `landing_priority` 数值最小（优先级最高）且可访问的 Route；
...
当最高优先级 POSITION Route 因 Manifest 版本变化不可用时，按同一算法选择下一个候选；不把旧
URL 当作兜底。如果用户当前正在访问已失权 Route，React SDK 在下一次版本刷新时关闭该页面并
导航到新 Landing Route；后端在刷新前已经拒绝对应 API。
...
6. 以变化后闭包执行 SSD/DSD/权限影响分析；
7. 同事务写边、闭包、`policyVersion`、审计和 Outbox。
...
Role 经多条继承路径只计一次。校验对象是变化后的有效时间窗口：对未来 Assignment/Rotation，
...
DSD Set 也使用 `memberRoleIds[]/maxActiveRoles`，但校验对象是当前要物化到 Session 的全部有效
角色。由于首期不能用户选角色，冲突时不能提示用户“少选一个角色”；登录/Refresh/Bootstrap
直接失败并返回结构化冲突 evidence，由管理员调整 Assignment 或规则。
...
通过唯一插入/冲突重读取得锁，避免“空集合无法加锁”。统计 ROTATION 中已排期但尚未执行且会创建
目标 Assignment 的时间窗。取消 Rotation 后释放未来容量；FAILED 不占用，EFFECTIVE 由真实
Assignment 占用。

### 14.6 统一约束执行顺序

为了错误稳定和避免先泄露高敏配置，角色变化按以下顺序返回第一个主要拒绝，同时可在影响分析中
收集全部非敏感冲突：
...
执行命令必须在取得必要锁后用最新事实重跑，不得直接信任几分钟前的影响分析结果。
...
```text
Result = TenantBoundary AND Union(normalize(each active role rule))
```
...
历史判断不看当前 Role、Position、Department 或 Session，因此用户轮岗、角色被撤销、用户名改变
...
Role 白名单按稳定 Role ID 存储，Role Code 仅展示。策略还可通过 allowedRoleTypes、maxRiskLevel 和
maxAssignmentDays 进一步收紧，但不能扩大显式 Role 列表。Operation 精确匹配命令：VIEW 不隐含
ASSIGN，ASSIGN 不隐含 REVOKE，ROTATE 不隐含 SCHEDULE_ROTATION、TEMPORARY_ASSIGN 或
CANCEL_ROTATION。
...
matches = candidates.filter(policy ->
    ...
    AND policy.roles.contains(targetRole)
...
这些内置 Role 固定 `privileged=true` 且 riskLevel=CRITICAL。普通业务 Role 即使 riskLevel=HIGH，仍可
...
未来 `validFrom` 返回 PENDING；到时由调度器激活并递增 `authVersion`。若 POSITION 命令会结束或
暂停现有主岗位，必须拒绝并提示使用 Rotation API。

### 20.2 撤销、暂停和恢复

- revoke：ACTIVE/PENDING/SUSPENDED -> REVOKED，终态且不可恢复；
- suspend：ACTIVE -> SUSPENDED，保留有效期；
- resume：SUSPENDED -> ACTIVE，但重新检查当前时间、Role、Policy、前置、SSD/DSD 和容量；
- 对 EXPIRED/REVOKED 重复 revoke 幂等返回当前状态，不物理删除；
- 管理者撤销他人 Role 重新检查 `REVOKE_ROLE`；本人自助撤销仅在策略显式允许、LOW 风险、非
  POSITION/ROTATION 来源且不破坏前置依赖时允许；
...
| 模式 | `effectiveAt` | `expireAt` | 创建成功状态 |
|---|---|---|---|
| IMMEDIATE | 为空或不晚于数据库当前时间 | 仅目标 Role 本身为临时类型时允许 | EFFECTIVE |
| SCHEDULED | 必须晚于数据库当前时间 | 可空；非空表示目标 Assignment 结束时间 | SCHEDULED |
| TEMPORARY | 可为当前或未来 | 必填且大于 effectiveAt；默认最长 30 天，EMERGENCY 最长 4 小时 | 当前直接 EFFECTIVE，否则 SCHEDULED |

客户端时间只作为输入，最终以数据库 UTC 比较。`effectiveAt` 落在允许时钟偏差内且已过去的
IMMEDIATE 可以执行；SCHEDULED 已过期不自动改成 IMMEDIATE，返回配置错误让调用方明确重提。
...
| 处理模式 | 生效时 | 临时到期时 | 适用限制 |
|---|---|---|---|
| EXPIRE_SOURCE | 原 Assignment 转为 REVOKED/终止 | 不恢复 | 永久轮岗默认；保留历史行 |
| SUSPEND_SOURCE | 原 Assignment -> SUSPENDED，并记录 suspendedByRotationId | 仍合法则恢复 ACTIVE | 临时代岗默认 |
| KEEP_SOURCE | 原 Assignment 不变 | 无需恢复 | 只有岗位允许并存且全部约束通过 |

由 Rotation 暂停的 Assignment 只能由同一个 Rotation 的恢复流程自动恢复，或由安全管理员使用专用
修复命令；普通 resume 不能抢先恢复。临时到期时若原 Role 已禁用、有效期已结束或恢复会产生冲突，
Rotation 保持最小权限：目标临时任职先失效，恢复结果记为 FAILED 并告警，不能为了达到 RESTORED
绕过约束。
...
| 当前状态 | 命令/触发器 | 前置 | 下一状态 | 副作用 |
|---|---|---|---|---|
| 新建 IMMEDIATE | create | 全部校验通过 | EXECUTING -> EFFECTIVE | 原子切换任职、版本、审计、Outbox |
| 新建未来模式 | create | 预校验通过且容量已计入 | SCHEDULED | 保存不可变执行参数和影响摘要 |
| SCHEDULED | scheduler claim | `effectiveAt <= dbNow` 且条件更新成功 | EXECUTING | 锁定任务并记录 attempt |
| SCHEDULED | cancel | 有 CANCEL_ROTATION 且尚未领取 | CANCELLED | 释放未来容量并审计 |
| EXECUTING | execute success | 事务全部提交 | EFFECTIVE | 投影完成后才对外成功 |
| EXECUTING | domain/business failure | 业务事务回滚 | FAILED | 保存安全失败码，不改变任职 |
| EFFECTIVE(TEMPORARY) | expiry claim | `expireAt <= dbNow` | EXPIRED | 撤销目标临时任职 |
| EXPIRED | restore success | 来源仍可恢复且约束通过 | RESTORED | 恢复来源任职、版本和投影 |
| EXPIRED | restore impossible | 确定性冲突或重试耗尽 | EXPIRED + restoreResult=FAILED | 保持最小权限，告警和人工处置 |

`EXECUTING` 是短暂持久态，Worker 崩溃后由 `lockedAt` 超时回收。回收前读取 execution log 和目标
Assignment 的 `sourceRotationId` 判断是否已经提交，防止重复创建。临时依赖错误可重试；SSD、
范围、自我授权等确定性错误不可自动重试。
...
创建 SCHEDULED/TEMPORARY 时做一次预校验，执行时必须全部重做最终校验。预校验成功不冻结 Role、
Policy、目录或交接事实；只预占定义明确的容量窗口。执行时若 Policy 已禁用、用户离职、Role 禁用
或出现新约束，状态转 FAILED 且不改变来源 Assignment。

#### 21.5.1 影响分析请求与响应

请求与创建命令使用相同业务字段，但无 Idempotency Key 且不写 Rotation：

```json
{
  "targetUserId": "10001",
  "sourceAssignmentId": "60001",
  "targetRoleId": "50002",
  "mode": "IMMEDIATE",
  "sourceHandling": "EXPIRE_SOURCE",
  "effectiveAt": null,
  "expireAt": null
}
```
...
blockingReasons[]
warnings[]
matchedManagementPolicyIds[]
sourceAssignmentBefore
targetAssignmentPlanned
permissionsAdded[] / permissionsRemoved[]
dataScopeChanges[] / fieldPolicyChanges[]
landingRouteBefore / landingRouteAfter
ssd/dsd/prerequisite/cardinality results
handoverItems[]
basedOnAuthVersion / basedOnPolicyVersion / directorySnapshotVersion
analyzedAt / expiresAt
```

影响分析最多用于展示 5 分钟（默认），不是执行授权。创建请求携带
`expectedUserAuthVersion/expectedPolicyVersion` 只能尽早发现变化，最终仍以锁内最新事实判断。
...
displaySummary（已脱敏）
handlingType
allowedTargetUsers/queues（可选且受 Scope 过滤）
status = OPEN | RESOLVED | BLOCKED | FAILED
version
```

交接项更新需要 item version；操作者同时拥有 Rotation 管理权和 Contributor 要求的业务 Permission。
`TRANSFER_USER` 接收人必须在业务 Scope 和 Contributor 候选中，不能由客户端任意指定。任何
BLOCK/FAILED 或版本冲突都阻止岗位切换。

跨业务库无法与 RBAC3 使用一个本地事务时，采用可补偿 Prepare/Commit：Contributor 先 Prepare
并返回不可重复 token，RBAC3 锁内校验全部 Prepare，提交 Rotation 后通过 Outbox Commit；Commit
未收敛时目标高风险权限由 Fence 保持不可用。不得把多库交接描述成不存在的单数据库原子事务。
...
```text
Tenant Policy Version（需要时）
-> Target User
-> Rotation
-> Source Assignment
-> Role/Closure 只读版本
-> Cardinality Scope Row
-> Target Assignment
-> Handover Rows
```

同一用户同时只能有一个 EXECUTING Rotation。冲突的未来 Rotation 定义为：目标用户相同且有效
窗口重叠，并且至少一个会改变 POSITION 来源 Assignment；以用户锁 + 范围查询保证，不能只依赖
普通唯一键。
...
```json
{
  ...
  "expireAt": null,
  "reason": "岗位调整",
  "ticketNo": "HR-2026-1024",
  "expectedUserAuthVersion": 43,
  "expectedPolicyVersion": 18
}
```

Header `Idempotency-Key` 必填。IMMEDIATE 成功响应为 `201 + EFFECTIVE + targetAssignmentId +
authVersion + propagationStatus=APPLIED`。未来生效返回 `202 + SCHEDULED + effectiveAt`。相同 Key
重试返回同一 Rotation ID 和原业务结果，不能重新执行。

### 21.9 调度、取消与失败恢复

```text
SELECT due rows FOR UPDATE SKIP LOCKED LIMIT N
-> 条件更新 SCHEDULED->EXECUTING, attempt+1, locked_by/locked_at
-> 每条独立事务执行
-> 成功 EFFECTIVE；确定性失败 FAILED；临时失败记录 next_retry_at
```

取消只允许 SCHEDULED 且尚未开始执行；与 Worker 竞争时由状态条件更新决定唯一结果。EFFECTIVE
不能“取消”，需要新的逆向 Rotation/Assignment 命令并重新授权。FAILED 的人工重试使用专用运维
权限、保留 Rotation ID 并创建新 execution attempt；不能直接把数据库状态改回 SCHEDULED。

到期 Worker 与生效 Worker 使用不同领取条件但相同幂等/锁原则。重试使用指数退避和上限，超过
上限进入告警处置；不存在审批队列。
...
| `rbac3:v1:{t:20001}:session:40001` | Hash/序列化值 | userId、状态、sv、av、pv、idle/absolute expiry | 到 absolute expiry + 2m |
| `rbac3:v1:{t:20001}:snapshot:u:10001:av:43:pv:18` | 版本化值 | Permission、Scope、Field、Route 摘要 | 默认 30m，可重建 |
...
读取者必须先校验 Schema Version、Tenant/User、版本、Checksum 和 Fence，再使用内容。未知 Schema
Version 不能尝试宽松反序列化放行。Snapshot 只保存当前有效结果，不替代 Assignment/Audit 历史。
...
3. Login、Refresh、授权变更和 Policy 变更主动物化受影响 Snapshot，避免把正常请求依赖于冷加载；
...
1. 小事务插入 PREPARING Journal，保存 mutationId、tenant/user scope、expected versions、commandId；
...
#### 22.2.2 User Fence 与 Tenant Fence

- Assignment/User/Rotation 变化使用 User Fence，只阻断目标用户；
...
Starter 原子验证：无 User/Tenant Fence、Session ACTIVE、三版本一致、Snapshot Checksum 一致、
Permission 仍在。返回的 Fence Decision 带极短 `verifiedAt`，只对当前业务事务/请求有效，不能缓存
...
Event Type；添加可选字段仍需兼容测试。Partition Key 按事件语义选择 `tenantId:userId` 或 tenantId，
保证同一用户/租户 Policy 事件有序，不承诺所有 Tenant 全局有序。
...
| `rbac3.rotation.changed.v1` | 创建/执行/到期/恢复 | rotationId、state、attempt、userId | Projector、通知、审计 |
...
rbac3_rotation_total{mode,state}
...
Strategy 的选择由受控枚举/配置和 Spring Bean 注册表完成；未知 strategyCode 启动或激活配置失败，
不动态加载类名。不会为每条 Data Rule、每个 Role 或每个 Tenant 创建独立 Strategy Bean。
...
Specification 只表达纯规则，不做数据库写入、HTTP 调用或日志副作用。Application Facade 先加载
完整事实，再组合 Specification；影响分析可收集多条结果，执行命令按第 14.6 节稳定顺序返回主错误。
...
State 转换由 `(currentState, command/event) -> nextState` 表驱动并带 expected version。禁止 Controller
直接 `setStatus`，也不使用可绕过转换表的通用 PATCH。
...
Facade 负责用例顺序和事务边界；Domain Service 负责不自然属于单一 Aggregate 的继承、约束、轮岗
和管理策略判断；Repository 只保存/加载。简单 User 展示查询不强行套 Command Bus 或多层 Handler。
...
Outbox 解决数据库提交后的可靠传播，不宣称提供跨 PostgreSQL、Redis 和业务库的 ACID 事务。安全
窗口由 Fence + Mutation Journal 关闭，跨业务交接由 Prepare/Commit + 补偿收敛。
...
| 表 | 关键字段 | 主约束 | 主要索引 |
|---|---|---|---|
| `rbac3_application` | `tenant_id`, `application_code`, `application_name`, `status`, `current_manifest_id`, `current_manifest_version` | UQ `(tenant_id,application_code)` | `(tenant_id,status)` |
...
| `rbac3_role` | `tenant_id`, `role_code`, `role_name`, `role_type`, `risk_level`, `privileged`, `status`, `landing_route_id`, `landing_priority`, `max_assignment_days`, `handover_required` | UQ `(tenant_id,role_code)`；role_type/privileged 创建后不可改；priority/max days 合法 | `(tenant_id,role_type,status)`；`(tenant_id,privileged,status)` |
| `rbac3_role_inheritance` | `tenant_id`, `senior_role_id`, `junior_role_id` | UQ 三元组；senior != junior；双方同租户 | `(tenant_id,junior_role_id)` |
| `rbac3_role_closure` | `tenant_id`, `ancestor_role_id`, `descendant_role_id`, `depth` | UQ 三元组；depth 0..10；自身行 depth=0 | `(tenant_id,descendant_role_id,depth)` |
| `rbac3_role_permission` | `tenant_id`, `role_id`, `permission_id`, `valid_from`, `valid_to`, `status` | UQ `(tenant_id,role_id,permission_id,valid_from)`；时间合法 | `(tenant_id,role_id,status)`；`(tenant_id,permission_id,status)` |
| `rbac3_data_rule` | `tenant_id`, `role_id`, `permission_id`, `scope_type`, `directory_snapshot_version`, `status`, `valid_from`, `valid_to` | 每个 Role/Permission 可多规则；时间合法 | `(tenant_id,role_id,permission_id,status)` |
...
| `rbac3_field_rule` | `tenant_id`, `role_id`, `permission_id`, `field_definition_id`, `access_level`, `valid_from`, `valid_to`, `status` | UQ `(tenant_id,role_id,permission_id,field_definition_id,valid_from)` | `(tenant_id,role_id,permission_id,status)` |

#### 25.2.4 任职与约束表

| 表 | 关键字段 | 主约束 | 主要索引 |
|---|---|---|---|
| `rbac3_user_role_assignment` | `tenant_id`, `user_id`, `role_id`, `assignment_type`, `status`, `valid_from`, `valid_to`, `source_type`, `source_id`, `source_rotation_id`, `suspended_by_rotation_id`, `reason`, `ticket_no` | 时间合法；TEMPORARY/EMERGENCY valid_to 非空；source FK 同租户 | `(tenant_id,user_id,status,valid_from,valid_to)`；`(tenant_id,role_id,status,valid_from,valid_to)` |
...
| `rbac3_sod_set` | `tenant_id`, `set_code`, `constraint_type`, `max_active_roles`, `status`, `valid_from`, `valid_to` | UQ `(tenant_id,set_code)`；type SSD/DSD；max >= 1 | `(tenant_id,constraint_type,status)` |
...
| `rbac3_management_role` | `tenant_id`, `policy_id`, `role_id` | UQ `(tenant_id,policy_id,role_id)`；高风险角色硬拒绝普通 Policy | `(tenant_id,role_id)` |
...
| 表 | 关键字段 | 主约束 | 主要索引 |
|---|---|---|---|
| `rbac3_role_rotation` | `tenant_id`, `target_user_id`, `source_assignment_id`, `target_role_id`, `mode`, `source_handling`, `effective_at`, `expire_at`, `status`, `restore_result`, `idempotency_key_hash`, `request_hash`, `expected_auth_version`, `expected_policy_version`, `reason`, `ticket_no`, `attempt`, `next_retry_at`, `locked_by`, `locked_at`, `executed_at` | UQ `(tenant_id,created_by,idempotency_key_hash)`；时间/模式 Check；无审批列 | 到期领取 partial index `(effective_at,id) WHERE status='SCHEDULED'`；到期恢复 `(expire_at,id) WHERE status='EFFECTIVE'`；`(tenant_id,target_user_id,status)` |
| `rbac3_rotation_handover` | `tenant_id`, `rotation_id`, `contributor_id`, `business_resource`, `business_item_id`, `handling_type`, `status`, `target_user_id`, `target_queue`, `prepare_token_hash`, `item_version`, `detail jsonb` | UQ `(tenant_id,rotation_id,contributor_id,business_resource,business_item_id)` | `(tenant_id,rotation_id,status)` |
| `rbac3_rotation_execution_log` | `tenant_id`, `rotation_id`, `attempt_no`, `trigger_type`, `from_status`, `to_status`, `outcome`, `reason_code`, `worker_id`, `started_at`, `finished_at`, `snapshot jsonb` | UQ `(tenant_id,rotation_id,attempt_no)`；追加只读 | `(tenant_id,rotation_id,started_at)`；outcome |

Rotation 表明确禁止出现 `DRAFT/PENDING_APPROVAL/APPROVED/REJECTED` 状态以及
`approved_by/approved_at/rejected_by/approval_comment` 字段。

#### 25.2.7 会话、幂等、Mutation 与审计表

| 表 | 关键字段 | 主约束 | 主要索引 |
|---|---|---|---|
| `rbac3_session` | `tenant_id`, `user_id`, `session_id`, `status`, `session_version`, `auth_version_at_issue`, `policy_version_at_issue`, `token_family_id`, `device_id_hash`, `auth_strength`, `authenticated_at`, `last_seen_at`, `idle_expires_at`, `absolute_expires_at`, `revoked_at`, `revoke_reason` | UQ session_id；UQ token_family_id；expiry 合法 | `(tenant_id,user_id,status)`；`(status,absolute_expires_at)` |
...
| `rbac3_authorization_mutation` | `mutation_id`, `tenant_id`, `user_id nullable`, `scope_type`, `command_id`, `status`, `old_auth_version`, `new_auth_version`, `old_policy_version`, `new_policy_version`, `fence_created_at`, `committed_at`, `projected_at`, `completed_at`, `last_error_code`, `attempt` | UQ mutation_id；UQ command_id；状态转换受控 | `(status,updated_at)`；`(tenant_id,user_id,status)` |
...
- User、Role、Permission、Resource、Assignment、Rotation、Policy、Participation、Audit 不提供普通
...
| Rotation 执行 | Source/Target Assignment、Rotation/Execution Log、可本地提交的 Handover、User authVersion、Audit、Outbox、Mutation COMMITTED |
...
Controller 不开启跨请求长事务。影响分析是只读快照；执行命令取得锁后重算。外部 HTTP、DDC、
Gateway Admin 和业务 Handover Commit 不放在持有数据库锁的本地事务中，采用先 Prepare 或事务后
Outbox，避免网络超时长期占锁。
...
- `FOR UPDATE SKIP LOCKED` 领取 Rotation/Outbox/恢复任务；
...
满足现有唯一约束的分区。Execution Log 与 Outbox 先使用组件既有清理策略，压测后再决定分区。
...
| `Idempotency-Key` | Assignment/Rotation/Policy/Manifest 激活等命令 | 1～128 ASCII；服务端只存 Hash；相同 Key 不同请求冲突 |
...
创建返回 201；异步排期接受返回 202；更新/命令返回 200；无响应体的幂等删除式动作可返回 204，
但撤销/轮岗通常返回最新状态和版本，因此使用 200。`meta.version` 是该资源乐观版本，不等同
...
| Assignment/Rotation | REQUIRED +管理功能 Permission | Management Policy + 约束 + Fence | 自我拒绝、强认证、幂等 |
...
批量绑定为一个事务；任一 Permission 跨租户、非 ACTIVE 或违反影响规则时整批失败。继承变更请求
带 expectedRoleVersion，并返回 closureAdded/closureRemoved、permissionsAdded 和冲突列表。
...
```json
{
  ...
  "roleIds": ["50010", "50011"],
  "operations": ["VIEW_ASSIGNMENT", "ASSIGN_ROLE", "REVOKE_ROLE", "ROTATE_ROLE"],
  "restrictions": {
...
响应返回 `policyId/version/policyVersion` 和四类子项摘要。更新不允许只追加一个 roleId 的无版本
...
请求/响应、影响分析、状态转换和幂等语义见第 21 节。`cancel` 请求必须包含 reason、
expectedRotationVersion 和 expectedUserAuthVersion；若 Scheduler 已抢到 EXECUTING，返回 409
`ROTATION_STATE_CONFLICT`，不能强制把执行中的任务改为 CANCELLED。
...
产生 Assignment/Rotation；模拟操作本身写 READ/SIMULATION Audit。
...
| Rotation | `system:role-rotation:create`、`system:role-rotation:cancel`、`system:role-rotation:read` | Management Policy + mode 对应 operation |
...
| 409 | ROTATION_RESTORE_FAILED | 人工处置后重试 | 临时到期但来源任职无法安全恢复 |
...
| `/rotations` | 轮岗列表 | `system:role-rotation:read` | create/cancel 对应 Permission + Policy |
| `/rotations/:id` | 影响、交接、执行记录 | `system:role-rotation:read` | handover/cancel 专用 Permission |
...
风险 Role 在普通策略选择器中不返回；手工篡改请求后端仍拒绝。
...
选择目标用户
-> 选择来源任职和目标岗位
-> 选择 IMMEDIATE/SCHEDULED/TEMPORARY 与来源处理
-> 输入时间、原因、工单
-> 查看影响与交接
-> 强认证（需要时）
-> 确认执行/创建排期
...
IMMEDIATE 主按钮文案为“确认并立即生效”；未来任务为“创建定时轮岗”。页面不存在“提交审批”。
若有 BLOCK 交接项，按钮禁用并显示可安全披露的原因；直接调用 API 仍由后端拒绝。

**运行状态：**

Definition、DDC Lease、Gateway Release 三张独立状态卡；下面分别展示 Mutation/Fence、Snapshot、
Outbox、Scheduler 指标。Recovery 按钮只能触发指定 mutation/outbox 的幂等重试，不能输入 Redis Key
或目标状态。
...
- Role/Policy/Rotation 使用独立 DTO，不复用一个万能 Form Model；
- DISABLE、REVOKE、IMMEDIATE ROTATION、高风险 Role 等动作展示具体影响和不可撤销语义；
...
2. 同一时刻只允许一个 Refresh/Bootstrap 请求，其余请求等待同一 Promise，避免 Token Rotation 重放；
...
| 威胁 | 典型攻击 | 强制控制 | 验收 |
|---|---|---|---|
| 账号接管 | 暴力破解、撞库、Refresh 重放 | BCrypt、失败锁定、多维限流、Family Rotation、Step-up | 并发刷新和重放测试 |
...
| Roles | PUBLIC_REPORT、CASHIER、PAYMENT_APPROVER、DEPT_MANAGER、TENANT_ADMIN、EMERGENCY_ADMIN |
| Constraints | CASHIER 与 PAYMENT_APPROVER SSD max=1；CREATE->APPROVE 同对象冲突；审批岗容量 D1=2 |
...
| Runtime | authVersion=43、policyVersion=18、SessionVersion=2；Gateway Definition/Release version=5.3.2 |
...
| Closure | 自继承、两节点/多节点环、深度 10 允许、11 拒绝、替代路径删除 |
| SSD/DSD | 直接 + 继承成员去重；未来窗口重叠；配置变化后 Session 冲突 |
| Cardinality | 0->1、达到上限、并发最后一个名额、取消未来 Rotation 释放 |
...
| Rotation | 所有合法/非法状态边；IMMEDIATE/SCHEDULED/TEMPORARY；三种来源处理 |
...
- Consumer Fixture 只依赖 BOM 暴露的 `contract/starter/gateway-adapter` 即可编译；
...
2. 同一 User 两个 IMMEDIATE Rotation，只允许一个进入 EXECUTING/EFFECTIVE；
3. Scheduler 两 Worker 用 SKIP LOCKED 领取，同一 Rotation 只有一个 execution attempt 生效；
...
7. 响应返回前连接断开，客户端同 Key 重试得到同一 Assignment/Rotation，不产生重复事实；
...
3. 创建 IMMEDIATE Rotation，页面没有审批步骤，成功后 Zhang 旧页面关闭并进入新 Route；
4. 定时 Rotation 只显示 SCHEDULED/执行记录，不出现 APPROVED；
...
- Audit 分页和 Rotation Scheduler 吞吐；
...
| 5 | 2+3+4；Mutation/Fence 设计可用 | 普通 Assignment、同 Policy 委托、无审批 Rotation、调度/恢复/交接测试通过 | 业务对象职责/Data/Field 已全接入 |
...
4. 未完成阶段 5 时，Assignment/Rotation 写 API 和页面不对用户开放；
...
- **Then** Contract/Core/Starter/Adapter/Admin/Test 和 React SDK/Admin Web 依赖方向符合第 9 节；
- **And** Gateway Adapter 中不存在调用 RBAC3 Admin 的每请求 HTTP Client；生产模块不依赖 Test。
...
- **When** 分别通过 path、query、body、Header 尝试查询、分配、继承、轮岗、Manifest 和 Scope；
...
- **Given** Zhang 同时有两个有效兼容任职和一个未来任职；
- **When** Zhang 登录；
- **Then** 请求/页面没有角色选择，Snapshot 自动包含两个当前有效任职，不包含未来任职；
- **And** Landing Route 按稳定优先级选择；没有 Route 时进入无权限页而不是越权默认页。

### AC-07 Refresh Rotation 与重放
...
- **Given** 标准夹具约束；
- **When** 创建使 CASHIER 与 PAYMENT_APPROVER 时间重叠的 Assignment；
- **Then** SSD 拒绝；
- **When** 配置变化使当前 Session 组合违反 DSD；
- **Then** Refresh/Bootstrap 拒绝并给结构化 evidence；
- **When** 两并发请求竞争最后一个容量；
- **Then** 只有一个提交；
...
- **Then** 只新增 Assignment，不结束/暂停 CASHIER；authVersion 和 Snapshot 更新；
...
- **When** Bob 给自己 Assignment、Rotation 或高风险提升；
...
- **When** 搜索审批状态、字段、API、菜单和按钮；
- **Then** 不存在 Rotation/Assignment 的 draft/submit/approve/reject/approver/approvalPolicy 能力；
- **And** IMMEDIATE 的唯一前置是同步权限、策略、约束、交接和强认证，不会产生待审批状态。

### AC-16 IMMEDIATE Rotation

- **Given** Bob 对 Zhang 和 PAYMENT_APPROVER 有完整 ROTATE_ROLE 权限，全部约束/交接通过；
- **When** 提交 IMMEDIATE；
- **Then** Source 与 Target Assignment、Rotation、Execution Log、authVersion、Audit、Outbox 在本地事务
  同成败；投影成功才返回 EFFECTIVE；
- **When** 任一步失败；
- **Then** 任职不出现半切换。

### AC-17 SCHEDULED Rotation

- **Given** effectiveAt 在未来；
- **When** 创建成功但尚未到时；
- **Then** 状态 SCHEDULED，Zhang 权限不变化；
- **When** 两 Worker 到时竞争；
- **Then** 只有一个领取和生效；执行时 Role/Policy/User 已变化则 FAILED 且 Source 不变。

### AC-18 TEMPORARY Rotation 与恢复

- **Given** TEMPORARY + SUSPEND_SOURCE；
- **When** 生效；
- **Then** Source SUSPENDED、Target ACTIVE；
- **When** expireAt 到达且 Source 仍合法；
- **Then** Target 失效、Source 恢复、Rotation RESTORED；
- **When** Source 已禁用或恢复冲突；
- **Then** Target 仍先失效、恢复标 FAILED 并告警，不绕过约束扩大权限。

### AC-19 Rotation 取消与幂等

- **Given** SCHEDULED 尚未领取；
- **When** 有权者 cancel；
- **Then** CANCELLED 并释放未来容量；
- **Given** Worker 已将其置 EXECUTING/EFFECTIVE；
- **When** cancel；
- **Then** 返回状态冲突，不能倒改状态；
- **And** 创建响应丢失后同 Key 重试不重复执行。

### AC-20 交接

- **Given** 有 OPEN/BLOCK Handover Item；
- **When** 执行 Rotation；
- **Then** 不进入 EFFECTIVE；
- **When** 跨库 Contributor Prepare 成功但 Commit 暂未收敛；
- **Then** 高风险 Target 权限受 Fence 阻断；
- **When** Commit 最终成功；
- **Then** 投影完成并可追踪；失败不能伪装成已交接。

### AC-21 同对象职责历史

- **Given** Zhang 已对 PAYMENT-1 执行 SUBMIT Participation；
- **When** Zhang 轮岗为 APPROVER 后尝试 APPROVE PAYMENT-1；
...
- **Given** Assignment/Rotation 已提交但响应连接断开；
...
### AC-36 Admin Web 无审批流程

- **Given** Rotation 创建页面、列表、详情、导航和 API Client；
- **When** 执行静态与 E2E 检查；
- **Then** 只有影响分析、交接、确认立即执行/创建排期、取消未执行任务和执行记录；不存在待审批、
  我审批、approve/reject/approver 控件或请求。

### AC-37 Audit 与敏感数据

- **Given** 一次成功高风险变更和一次自我提权拒绝；
- **When** 用 traceId 查询；
...
| R-11 | 跨库 Handover 部分成功 | 中/高 | Prepare/Commit token、Fence、补偿和人工处置 | Prepare 未 Commit、Rotation EXECUTING age | Contributor + RBAC3 |
| R-12 | Closure/容量在大 Tenant 变慢 | 中/中高 | 深度上限、显式索引/SQL、影响分析、基准与分批投影 | p95、锁等待、任务积压 | RBAC3 Platform/DBA |
...
这些细节不能被用来改变“无审批、DDC/Gateway 强制接入、租户隔离、双层 PEP、Fence/Outbox、
类型化 Scope/Field/Participation”等已确认边界。本稿没有需要再向用户零散确认的阻塞问题。
...
4. 本地认证、JWT、Refresh Rotation、Session 和三类版本；
5. 角色继承、SSD/DSD/前置/容量、Data/Field/Participation；
6. 普通 Assignment 与 Rotation 分离，登录不选 Role；
7. Management Policy 同一条完整命中，禁止自我提权和普通委托高风险 Role；
8. IMMEDIATE 直接原子生效，SCHEDULED/TEMPORARY 到时执行，完全没有审批；
...
14. 按第 31 节分阶段、逐任务测试和提交，最终按第 30/32 节分级验证；
