...
Tianshu Admin 当前直接把 `configValue` 原文放进列表和文本框。普通 JSON 因为是单行文本而难读，
`yuheng.rules.active` 更包含 Yuheng 发布所需的 activation、snapshot、checksum 等运行元数据，
...
- 保存 Yuheng 内联规则时自动把内容合并回原外壳，并重算内容校验和、产物校验和和字节长度。
...
文件类型通过配置键扩展名、`valueType` 和内容特征自动识别，不改变 Tianshu 运行时契约。

优点是改动集中、无需数据库迁移、旧配置立即生效，并能针对 Yuheng 外壳安全地往返转换。
缺点是新增的系统外壳类型需要显式增加适配规则。

### 方案二：给 Tianshu 表增加 `file_type` 字段

文件类型可以持久化，但需要 PostgreSQL 和 SQLite 迁移、管理 API 与客户端契约扩展；仍不能解决
Yuheng 发布外壳的内容提取和校验和重建，因此本次不采用。

### 方案三：仅把 JSON 进行格式化

实现最小，但 Yuheng 元数据仍会占满编辑器，也无法区分 YAML、TOML 和 TXT，因此不满足需求。
...
- `detectConfigFormat(config)`：优先识别 `.yaml/.yml/.toml/.json/.txt` 等扩展名，随后识别
  Yuheng 内联规则、`valueType=JSON` 和内容特征，最终回退到 TXT。
- `prepareConfigEditor(config)`：普通 JSON 返回缩进文本；识别到 Yuheng INLINE activation 时，
  解析 `inlineSnapshot` 并只返回 `snapshot.content`。
- `serializeConfigEditor(editor)`：普通内容按编辑结果保存；Yuheng 内容按现有 canonical JSON 规则
...
- `valueType` 继续表示 Tianshu 的运行时值转换类型，不与文件格式混用。
- 不新增依赖，不修改数据库，不修改 Tianshu Starter 或 Yuheng 运行契约。
- 仅 INLINE Yuheng activation 支持内容级编辑；其他 JSON 完整展示，避免猜测性丢字段。
- SHA-256 使用浏览器原生 Web Crypto；canonical JSON 对对象键排序、忽略 null，并保持数组顺序，
  同时兼容 Yuheng 对空 `parameters` 的省略规则。

## 测试

- Node 内置测试验证 JSON/YAML/TOML/TXT 识别和普通业务字段不被误删。
- 以固定 Yuheng activation fixture 验证只展示 `content`，保存后元数据保留、校验和与长度为固定值。
- Java classpath 资源测试验证新模块随 Admin JAR 一同打包。
- 运行 JavaScript 测试、Tianshu Admin Maven 测试、打包和 `git diff --check`。

## 设计模式判断

采用 Adapter：它隔离后端持久化值与编辑器展示值的差异，并把 Yuheng 特有的往返规则封装在单一
