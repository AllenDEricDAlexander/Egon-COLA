...
本 Spec 固定 Yuheng Component 的工程边界、分层方式、模块依赖、公共身份、版本、
...
Yuheng 作为 `egon-cola-components` 下的独立大型 Component：
...
| 模块 | 类型 | 职责 |
|---|---|---|
| `yuheng-contract` | Java Library | 跨进程 DTO、枚举、事件 Schema 和公共扩展契约 |
| `yuheng-core` | Java Library | 不依赖 Spring/基础设施的请求模型、路由模型、Filter、执行器和错误模型 |
| `yuheng-biz-gateway` | Executable | HTTP/RPC Listener、规则运行时、Provider 调用、治理、Kafka 和生命周期 |
| `yuheng-admin` | Executable | 接口目录、规则配置、发布编排、节点投影、管理 API 和审计 |
| `yuheng-starter` | Starter | Provider 接口定义扫描、编译和向 Admin 上报 |
| `yuheng-provider-runtime` | Starter | 仅为 HTTP Provider 维护 Tianshu `HTTP_PROVIDER` 租约 |
| `yuheng-admin-web` | Node/Vite App | React 管理平台，独立构建和部署 |
| `yuheng-test` | Test Aggregator | 真实 HTTP/RPC 应用、基础设施编排和端到端验证 |

`yuheng-starter` 与 `yuheng-provider-runtime` 必须是两个独立 Artifact。安装
`yuheng-starter` 不能隐式注册 Provider 实例，安装 `yuheng-provider-runtime`
不能隐式上报接口定义。

### 2.2 Maven 与 BOM

- Yuheng 根模块使用 `pom` packaging；
- Java 模块继承 `egon-cola-components-parent`；
- `egon-cola-components/pom.xml` 只聚合 Yuheng 根模块；
- BOM 只导出下游业务应用直接使用的：
  - `egon-cola-component-yuheng-starter`；
  - `egon-cola-component-yuheng-provider-runtime`；
- Engine、Admin、Core、Contract 和 Test 不进入公共 BOM；
- `yuheng-starter` 可以传递依赖 `yuheng-contract`，业务方不直接依赖内部 Core；
- Admin 与 Engine 是可执行应用，不作为下游 Library 使用。

### 2.3 Admin Web

`yuheng-admin-web` 是前后端分离的 Vite 工程，不强制进入 Maven 编译生命周期。
...
| Infrastructure | PostgreSQL、Redis、Tianshu、Kafka、Netty、文件和外部 Client | Application Port、Contract |

依赖从外向内。Domain 不依赖 Spring、JPA、Redis、Tianshu、Kafka、Reactor Netty 或
gRPC。

### 3.2 Core 与 Engine 的关系

- `yuheng-core` 承载数据面领域模型和稳定 SPI；
- `yuheng-biz-gateway` 承载网络、组件接入和运行编排；
- 网络请求进入 Engine 后必须先转换成 `GatewayExchange`；
- Core 不直接读取 Spring `ApplicationContext`；
- Core 不直接发送 Kafka、访问 Redis 或调用 Tianshu；
...
```text
top.egon.cola.component.yuheng.contract
top.egon.cola.component.yuheng.core
top.egon.cola.component.yuheng.engine.interfaces
top.egon.cola.component.yuheng.engine.application
top.egon.cola.component.yuheng.engine.infrastructure
top.egon.cola.component.yuheng.admin.interfaces
top.egon.cola.component.yuheng.admin.application
top.egon.cola.component.yuheng.admin.domain
top.egon.cola.component.yuheng.admin.infrastructure
top.egon.cola.component.yuheng.starter
top.egon.cola.component.yuheng.provider
```
...
| 资源 | 标识 |
|---|---|
| Yuheng Group | `gatewayGroupId`，UUIDv7；另有唯一稳定 `gatewayGroupCode` |
| Engine Node | `engineNodeId`，UUIDv7；运行身份包含稳定 `instanceId` |
| RPC Yuheng Slot | Tianshu `serviceName + group + version` |
...
| Release | `releaseId`，UUIDv7；Tianshu 发布使用独立 UUIDv7 `changeId` |
| Provider Instance | Tianshu `instanceId + leaseId` |
...
所有 Yuheng 自有跨进程请求必须携带：

```text
contractVersion = "v1"
```

HTTP 管理/上报 API 使用 `/api/v1/yuheng/...`。Kafka 事件使用独立
...
- 对外错误不包含堆栈、类名、SQL、Redis Key、Tianshu Secret 或 Provider 内部地址；
- 内部日志通过 `traceId + requestId + errorCode` 关联详细异常；
- HTTP 和 gRPC 使用同一个 Yuheng Error Category，再映射为各自状态；
- 未分类异常统一映射 `INTERNAL_ERROR`，不返回原始异常文本。

## 8. 跨进程契约清单

`yuheng-contract` 只包含以下稳定边界：

1. Yuheng Starter → Yuheng Admin 的定义上报 DTO；
2. Yuheng Engine Runtime Status DTO，由 Yuheng Admin 通过内部管理接口查询；
3. Yuheng Admin 管理 API 的公共 DTO/枚举；
...
- JPA Entity；
- Tianshu Admin Entity/Repository；
...
```mermaid
flowchart TD
    Contract["yuheng-contract"]
    Core["yuheng-core"]
    Starter["yuheng-starter"]
    ProviderRuntime["yuheng-provider-runtime"]
    Engine["yuheng-biz-gateway"]
    Admin["yuheng-admin"]
    Test["yuheng-test"]
...
- Starter 依赖 Engine；
- Admin 依赖 Engine 实现包；
- RPC/Tianshu Component 反向依赖 Yuheng；
- 业务 Provider 依赖 Yuheng Core 或 Engine。

外部 Component 依赖：

| Yuheng 模块 | 允许依赖 |
|---|---|
| Engine | Tianshu Starter、RPC Starter 公共契约、Common Trace/Mask、Kafka/Redis Client |
| Admin | Tianshu Management Client、Common Trace/Mask、Spring Kafka、PostgreSQL/JPA |
| Starter | RPC Starter（可选）、Common Crypto/Trace |
| Provider Runtime | Tianshu Starter、Yuheng Contract |

Yuheng Admin 不依赖 Tianshu Starter；使用 GWS-02 新增的轻量 Management Client，避免
...
- `READY` 必须表示 Listener、有效 Rule Snapshot、Provider Directory 和必要资源均可
  执行；正常模式还要求 Tianshu Config Client 租约在线；
- 已运行节点在 Tianshu 短时中断时，可以在已有 Rule 和未过期 Provider Lease 范围内进入
  `DEGRADED` 子状态继续服务；
- 冷启动时只有 Rule LKG、但无法从 Tianshu 建立 Provider Directory，不能进入 READY；
- Tianshu 配置客户端租约在线不等于 Engine Ready；
...
- FAILED 节点不能注册或继续持有 RPC Yuheng Slot。
...
RPC 和 Tianshu 继续使用各自已有前缀，Yuheng 不复制或重命名其配置。

Secret 只能通过环境变量、Secret Manager 或加密配置注入，不能进入 Rule Snapshot、
Starter Report、Kafka Event 或 Tianshu 服务 Metadata。

## 12. 设计模式选择

采用：

- Ports and Adapters：隔离 Tianshu、Kafka、Redis、HTTP、gRPC；
