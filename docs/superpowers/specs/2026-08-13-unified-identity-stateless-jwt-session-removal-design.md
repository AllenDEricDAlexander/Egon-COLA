# 统一身份无 Session JWT 与 Yuheng 自动刷新改造规格

> 状态：已确认，核心无状态 JWT/Tianquan-Jianshen 代码改造、真实应用 Catalog 上报与 Yuheng route 编译/发布实现已完成；运行态联调待用户启动本地栈后执行
> 编写日期：2026-08-13
> 复审日期：2026-08-14
> 用户确认日期：2026-08-14
> 代码基线：`main@b90f0c95`
...
- Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 等现有 Admin Web
- `scripts/unified-xingyuan` 与统一身份本地启动、验证脚本

本文固化 2026-08-13 已确认的统一身份改造方向：人员登录链只使用一个平台级
Access Token 和一个 Refresh Token；彻底移除 Tianquan-Shoubing、Tianquan-Jianshen、Yuheng 和业务服务中的
人员 Session 语义；Yuheng 成为唯一外部入口，并在人员 Access Token 缺失或过期时
使用 Refresh Token 向 Tianquan-Shoubing 自动换取新的 Access Token；Tianquan-Shoubing 同时保留开放身份协议面和
受 Tianquan-Jianshen 保护的管理业务面。Tianquan-Jianshen 继续保留角色激活能力，但激活集合从“登录 Session
状态”改为“租户内 RBAC 用户的应用级授权状态”；未激活角色及其权限不得进入权限上下文。
无论 USER 还是 SERVICE，Access Token、签名密钥和认证凭据的权威都统一在 Tianquan-Shoubing；Tianquan-Jianshen
只保存最小授权主体与权限之间的授权事实，不再签发 Token、保存密码或复制用户核心资料。
Tianquan-Shoubing 作为人员身份与凭据权威，当前只完成本改造所需的既有身份能力收口，不在本期建设完整
用户中台。JWT 的身份协议、签发和验签能力归 Tianquan-Shoubing 模块体系，Yuheng Adapter 复用 Starter
...
- Yuheng 外部入口、Access Token 校验和自动刷新；
- Tianquan-Shoubing 管理业务域的 Access Token 与 Tianquan-Jianshen 双重保护；
- Tianquan-Shoubing、Tianquan-Jianshen 的用户数据职责及 Tianquan-Jianshen 最小授权用户模型；
- JWT 签发、验签、Claim 契约和各 Tianquan-Shoubing 模块的代码归属；
- Tianquan-Jianshen 人员授权快照与 Session 解耦；
- 业务服务绕过 Yuheng 时的本地 Access Token 与权限校验；
- 相关前端、Cookie、Redis Key、数据库 Session 表和运行配置清理。

### 1.2 被本文覆盖的旧决策

本文覆盖
`docs/superpowers/specs/2026-08-01-unified-identity-xingyuan-design.md`
中与下列内容有关的旧决策：

- SSO Session、SSO Cookie 和 `sid`；
- `(tenantId, sessionId)` Tianquan-Jianshen Authorization Context；
...
Client Credentials、Admission Ticket、Tianshu Provider 准入和机器身份边界不由本文删除；
它们不进入人员登录、人员 SSO、人员 Refresh Token 或人员 Session 模型。其中 SERVICE
认证凭据和 Access Token 的权威实现固定在 Tianquan-Shoubing；这句话不授权保留 Tianquan-Jianshen 自己的
`ServiceCredential`、JWT签名密钥或 Token签发链。

如果其他旧 Spec 或 Plan 把 Tianquan-Shoubing/Tianquan-Jianshen Session、`sid`、USER Resource Audience、
前端 Refresh 或即时 `tokenVersion` 撤销声明为冻结契约，以本文为准。实施计划必须显式
标记这些旧计划中的冲突步骤，不得机械续跑旧计划。

同日的 Tianquan-Jianshen Admin 领域分包已经完成并合并到当前 `main`：生产代码已按
...
| 术语 | 本文含义 |
|---|---|
| USER Access Token | Tianquan-Shoubing 为人员身份签发的、固定 5 分钟、全平台通用的 JWT |
| Refresh Token | Tianquan-Shoubing 为人员身份签发并在 Tianquan-Shoubing Redis 保存有效摘要的长期 JWT |
| SERVICE Access Token | Tianquan-Shoubing 通过 Client Credentials 签发的 `typ=at+jwt`；仍属于 Access Token 类型，不是第三种 Token 类型，不配套 Refresh Token |
| 开放协议接口 | 不要求调用方已经持有 USER Access Token 或 Tianquan-Jianshen 权限，但仍严格校验该接口自己的凭据和请求完整性 |
| 保护身份接口 | 必须持有有效 USER Access Token，可触发 Yuheng 自动 Refresh，但不执行业务域 Tianquan-Jianshen 权限判断 |
| 保护业务接口 | 必须持有有效 USER Access Token，并通过 Yuheng 与目标服务的 Tianquan-Jianshen 权限判断 |
| 无 Session | 不存在服务端登录 Session、SSO Session、Tianquan-Jianshen Session、Session Fence、Session Version 或按登录 Session 保存的 Active Role；不表示删除 Tianquan-Jianshen 的用户角色激活能力 |
| 无状态 Access Token | Yuheng、Tianquan-Shoubing、Tianquan-Jianshen 和业务服务仅通过签名与声明验证 USER Access Token，不查询用户登录 Session 或 Token Version |
| 用户激活角色集合 | Tianquan-Jianshen 按 `(tenantId, rbac3UserId, applicationId)` 保存的授权状态；它决定哪些有效角色族进入权限上下文，与 AT、RT 和登录 Session 无关 |
| Tianquan-Shoubing 用户 | 人员身份主数据与认证安全状态的唯一权威；当前承载已有登录标识、展示名、账号状态、密码凭据和登录安全状态，不表示本期建设完整用户中台 |
| Tianquan-Jianshen 用户 | 某个 Tianquan-Shoubing `identitySub` 在一个 Tianquan-Jianshen Tenant 中的最小授权主体，只承载身份绑定、租户授权状态、授权版本和审计元数据，不承载密码或用户核心资料 |
| JWT 签名/验签 | 本文口语中的“JWT 加解密”实际指当前 RS256 JWS 的私钥签名与公钥验签；Token Payload 未加密，本期不引入 JWE |

Refresh Token 有效记录、JWK 缓存、Tianquan-Jianshen 权限快照缓存、审计日志和 Outbox 都不是登录
Session。Tianquan-Shoubing 必须保存 Refresh Token 的服务端有效状态，才能执行退出、强制退出和过期
...
| 编号 | 决策 |
|---|---|
| SJ-01 | 所有浏览器和外部 USER HTTP 调用只能通过 Yuheng 访问 Tianquan-Shoubing、Tianquan-Jianshen、Tianshu、Yuheng Admin 和普通业务系统；受信机器内部调用继续遵守各自 SERVICE/Admission 边界 |
| SJ-02 | Tianquan-Shoubing 内部监听地址只作为 Yuheng 上游、JWK 内部读取或受信服务调用地址，不作为浏览器 Issuer/API 地址 |
| SJ-03 | Tianquan-Shoubing 同时包含开放协议面和受保护管理面；Tianquan-Shoubing Admin 不是认证旁路 |
| SJ-04 | `/oauth2/login`、USER Refresh、撤销、退出、JWKS 和元数据是开放协议路由，不要求已有 USER Access Token 或 Tianquan-Jianshen 权限 |
| SJ-05 | “开放”只表示可匿名发起；登录必须校验账号密码，刷新必须校验 Refresh Token，Client Credentials 必须校验 Client Assertion |
| SJ-06 | Tianquan-Shoubing 的 `/api/**` 管理接口必须像普通业务系统一样再次校验 USER Access Token，并使用 `systemCode=tianquan-shoubing-admin` 执行 Tianquan-Jianshen 权限判断 |
| SJ-07 | Tianquan-Shoubing 管理员与其他用户使用同一个登录入口和同一种 USER Access Token/Refresh Token，不签发管理员专用 Token |
| SJ-08 | 身份认证成功与业务域授权分离；用户可以登录成功并获得 AT/RT，但无 `tianquan-shoubing-admin` 权限时访问 Tianquan-Shoubing Admin 返回 403 |
...
| SJ-13 | USER Access Token 使用固定的平台 Audience；业务域权限由 Tianquan-Jianshen 决定，不由 Audience 隔离 |
| SJ-14 | USER Access Token 不在 Tianquan-Shoubing、Yuheng、Tianquan-Jianshen 或业务服务持久化；Tianquan-Shoubing 只负责签发和密钥权威 |
...
| SJ-17 | Tianquan-Shoubing Redis 只保存 Refresh Token 摘要、主体/租户索引、过期时间和有效状态，不保存明文 Refresh Token |
| SJ-18 | Yuheng 不保存 AT、RT 或人员 Session；Yuheng 节点扩缩容不改变登录状态 |
| SJ-19 | Yuheng 只在 USER Access Token 缺失或确认为过期且存在 RT 时自动调用 Tianquan-Shoubing 刷新 |
| SJ-20 | USER Access Token 签名错误、Issuer 错误、Audience 错误、类型错误或格式错误时直接 401，不尝试刷新 |
| SJ-21 | Tianquan-Jianshen 拒绝产生 403；403 不触发刷新，也不伪装为登录失败 |
| SJ-22 | Yuheng 刷新成功后必须在同一个原始请求中使用新 AT 继续权限校验和路由，并把新 AT Cookie 写回浏览器 |
| SJ-23 | Refresh 路由本身不得触发 Yuheng 自动刷新，Yuheng 调 Tianquan-Shoubing 刷新使用内部上游地址，避免递归进入公开 Yuheng 路由 |
| SJ-24 | Yuheng 仅在保护业务路由执行外围业务域/Route/API 权限判断，目标服务再次执行服务侧权威权限判断；任一层拒绝即拒绝 |
| SJ-25 | Yuheng 只向目标服务转发 USER Access Token；Refresh Token 和浏览器认证 Cookie不得转发给普通业务服务 |
| SJ-26 | Tianquan-Shoubing、Tianquan-Jianshen 和所有业务服务都必须在本地再次校验同一个 USER Access Token；目标服务永不使用 RT 自动刷新 |
| SJ-27 | 绕过 Yuheng 调用目标服务时，有效 AT 仍需通过服务侧 Tianquan-Jianshen；过期 AT 直接 401，服务端不读取 RT |
| SJ-28 | Tianquan-Jianshen 授权快照身份键改为 `(systemCode, tenantId, identitySub)`，不再包含 Session ID 或 Session Version |
| SJ-29 | Tianquan-Jianshen 从已验证 USER Access Token 派生 `identitySub` 和 `tenantId`，不信任调用方传入的身份查询参数 |
| SJ-30 | Tianquan-Jianshen Session、Session Fence、Session Version 和 Tianquan-Jianshen Refresh Token 运行链全部移除；`rbac3_session_active_role` 及其 Session 外键删除，但角色激活能力不得删除 |
| SJ-31 | Assignment 只产生可激活候选；Tianquan-Jianshen 按 `(tenantId, rbac3UserId, applicationId)` 保存当前激活根角色集合，只有仍满足 Role/Assignment/层级/DSD/策略规则的激活角色族才能进入权限上下文，未激活角色权限不得回查或填充 |
| SJ-32 | 权限字符、菜单、按钮、路由、API、数据和字段策略继续归 Tianquan-Jianshen；本期不重新设计细粒度权限模型 |
...
| SJ-35 | 权限或角色被撤销时，Tianquan-Jianshen 仍可在 AT 到期前立即拒绝请求；“AT 最多继续五分钟”只表示身份 JWT 仍有效，不保证业务权限仍允许 |
| SJ-36 | 人员浏览器主链不再使用 SSO Cookie、Authorization Code、PKCE 回调或前端 Token Store；JWT Cookie 本身提供跨客户端免登录 |
| SJ-37 | 现有 SERVICE Access Token、Client Credentials 和 Admission Ticket 保持在机器身份边界，不得混入 USER Refresh、USER SSO 或 USER Tianquan-Jianshen Session |
| SJ-38 | 不新增登录 JWT、业务域 JWT、客户端 JWT、Session JWT 或其他人员认证 Token |
| SJ-39 | Cookie 模式下必须保留登录 CSRF 防护，并在 Yuheng 对变更类请求执行可信 Origin/Referer 或等价请求完整性校验；CSRF 随机值不是身份 Token |
| SJ-40 | 该改造是开发期破坏式直接切换：旧 SSO Cookie、旧 USER AT、旧 RT、旧授权码、旧 Tianquan-Jianshen Session 和旧 Session Active Role 数据全部丢弃，不做迁移、回填、双读或兼容，用户重新登录并按需重新选择激活角色 |
| SJ-41 | 登录、Refresh、Access Token 过期、Yuheng 节点切换和跨客户端访问都不得重置用户激活角色集合；激活变更推进 Tianquan-Jianshen 用户授权版本并使对应系统快照失效，不要求重签 USER AT |
| SJ-42 | 现有高风险角色激活的二次认证语义保留，但不再写入 Tianquan-Jianshen Session；Tianquan-Shoubing 在同一种 USER AT 中签名携带认证强度和认证时间，二次认证只签发新的 5 分钟 AT，不创建第三种 Token 或服务端 Session |
| SJ-43 | Yuheng Route 必须显式且互斥地分为“开放协议”、“保护身份”和“保护业务”三类；前两类不得因缺少业务权限阻断登录、Refresh、Step-up 或 UserInfo |
| SJ-44 | USER/SERVICE Access Token、Refresh Token、签名密钥和人员/机器认证凭据的唯一权威都是 Tianquan-Shoubing；Tianquan-Jianshen 不得签发或刷新任何 Token，不得保存 `ServiceCredential` 或自有签名私钥 |
| SJ-45 | Tianquan-Jianshen 可保留 `ServicePrincipal + ServicePermission` 作为机器主体授权事实，但机器 `client_id`、凭据校验、SERVICE AT签发和 Scope/Resource认证仍归 Tianquan-Shoubing；Admission Ticket 继续属于现有 DDC启动准入协议，不得复用为 RBAC登录或第三种平台访问 Token |
| SJ-46 | Tianquan-Jianshen 不再保存人员密码、密码散列、失败次数、登录锁定时间、用户名、规范化用户名、展示名或其他用户核心资料；`rbac3_user` 只保留租户内最小授权主体 |
| SJ-47 | Tianquan-Shoubing 是人员身份、登录标识、密码凭据、账号状态和登录安全状态的唯一权威；本期只收口现有能力和本方案必需字段，不开发完整用户中台、人员档案或组织人事能力 |
| SJ-48 | `rbac3_user` 固定以 `(tenant_id, identity_sub)` 唯一绑定 Tianquan-Shoubing 用户，保留 RBAC内部 `id`、本地授权状态、`auth_version`、行版本和审计字段；组织、岗位、角色、权限继续作为 RBAC授权关系或独立事实存在，不塞回用户核心资料 |
| SJ-49 | Tianquan-Shoubing Admin 持有平台 JWT 私钥并负责 AT/RT 签发与 JWKS；`tianquan-shoubing-starter` 负责共享公钥验签和严格 Claim验证；`tianquan-shoubing-gateway-adapter` 只把 Starter能力适配到 Yuheng，不复制密码学和 Claim规则 |
...
| SJ-51 | 本期不新增 `component-jwt`，也不把 Tianquan-Shoubing Claim/Issuer/Audience/主体规则下沉到 Common Crypto；只有未来出现多个非 IdP协议的真实复用者时，才可另行评审只含无状态 JOSE/JWK原语的组件 |
...
### 4.1 当前 Tianquan-Shoubing 已具备的正确边界

以下能力已经存在，实施时应复用而不是推倒重建：

| 当前能力 | 代码位置 |
|---|---|
| Spring Security 已配置 `SessionCreationPolicy.STATELESS` | `tianquan-shoubing-admin/.../support/security/IdpSecurityConfig.java` |
| Tianquan-Shoubing 保护接口按 Tianquan-Shoubing Bearer Filter、Tianquan-Jianshen Filter 顺序执行 | `IdpSecurityConfig.java` |
| Tianquan-Shoubing Admin 已依赖 `egon-cola-tianquan-jianshen-starter` | `egon-cola-tianquan-shoubing-admin/pom.xml` |
| Tianquan-Shoubing Tianquan-Jianshen `system-code` 已是 `tianquan-shoubing-admin` | `tianquan-shoubing-admin/src/main/resources/application.yml` |
| Tianquan-Shoubing 管理 Controller 已使用 `@AuthenticationPrincipal IdentityPrincipal` | `identity/controller/IdentityUserController.java` 等 |
| Tianquan-Shoubing 管理业务已显式检查 `tianquan-shoubing:*` 权限 | `IdpAdminAuthorizationPort` 及各 Controller |
| Tianquan-Shoubing 已签发 RS256 `typ=at+jwt` 并提供 JWKS | `Rs256TokenService.java`、`OAuthJwksController.java` |
| Refresh Token 已使用签名 JWT 和 Redis 摘要状态 | `RefreshTokenClaims.java`、`RedisRefreshTokenStore.java` |
| 现有账号已具有 `ACTIVE/DISABLED/LOCKED` 状态与登录限制 | `IdentityFacade.java`、Tianquan-Shoubing V1 migration |
| Tianquan-Shoubing 已有用户核心与登录安全模型 | `IdentityUser.java`、`IdentityUserEntity.java`、Tianquan-Shoubing Password Credential模型 |
| Tianquan-Shoubing Starter 已持有 Spring Security JOSE/JWT 验证依赖 | `tianquan-shoubing-starter/pom.xml`、`IdpJwtVerifier.java`、`RetryingJwtDecoder.java` |
| Yuheng Adapter 已依赖 Starter并委托共享验证器 | `tianquan-shoubing-gateway-adapter/pom.xml`、`IdpGatewayJwtVerifier.java` |
| Tianquan-Shoubing RPC Contract 当前只有稳定 Admission Proto | `tianquan-shoubing-rpc-contract/pom.xml`、`resource_server_admission.proto` |

`@AuthenticationPrincipal` 只注入安全链已经认证的人员主体。它不自行解析 JWT、不访问
Tianquan-Jianshen，也不等价于权限注解。改造后该用法继续保留，但 `IdentityPrincipal` 不再携带
...
| `TokenFacade.refresh` | 每次 Refresh 轮换并在并发时执行重放撤销 | Yuheng 并发自动刷新会出现误撤销风险 |
...
| `IdpBearerCredentialExtractor` | 只读取 Authorization Bearer | Yuheng 看不到浏览器 RT，不能自动刷新 |
| `IdpIdentityAuthenticationProvider` | 验证异常统一为 DENY | 无法区分 EXPIRED 与 INVALID |
| Admin Web Shared | Access Token 存内存，401 后由前端调用 Tianquan-Shoubing 刷新 | 刷新责任不在 Yuheng |
| Tianquan-Jianshen React SDK | `InMemoryAccessTokenStore`、`Rbac3ApiClient.refresh()`、`Rbac3Provider` 启动刷新与 `useRbac3Session` 仍保存 Token/Session 语义 | 即使后端删表，前端仍会直接刷新、保存 Bearer 并依赖 Session Contract |
| 本地脚本 | `VITE_TIANQUAN_SHOUBING_ISSUER` 与各 Admin API 指向独立后端端口 | Yuheng 不是唯一外部入口 |
| `HttpRbac3AuthorizationClient` | 按 tenant/session/identity 查询快照 | Tianquan-Jianshen 权限上下文依赖 Session |
| `SingleFlightSnapshotLoader` | 缓存和一致性绑定 `principal.sessionId()` | 无法在无 Session 主体下工作 |
| `Rbac3JwtSessionAuthenticationProvider` / `Rbac3GatewayJwtVerifier` | Yuheng 再验证 Tianquan-Jianshen 自签 `Rbac3TokenClaims`，再以 `SessionVerifier` 查 Session/Version | USER AT 只能由 Tianquan-Shoubing 签发和验证；Tianquan-Jianshen Yuheng Adapter 不应再是第二个人员认证器 |
| `Rbac3GatewayRuntimeSnapshotReader` | 从 `tianquan-jianshen.session-id/auth-version/session-version/policy-version` 属性读 Redis Session、Fence 和 Session Snapshot | 应只从已认证 Tianquan-Shoubing `GatewayPrincipal(sub, tid)` 读取用户级授权快照 |
| `Rbac3TrustedIdentityMapper` | 向下游写 `x-egon-yuheng-session-id/session-version` 等 Session Header | 该 USER 映射职责删除；下游以原始 Tianquan-Shoubing USER AT 为身份权威 |
...
| `RoleActivationFacade` | 高风险根角色依赖 Session 中的认证强度和 `strongAuthenticatedAt` | Session 删除后必须改为校验 Tianquan-Shoubing 签名 AT中的认证上下文 |
| `AuthController` / `SessionController` | Tianquan-Jianshen 还提供 Session Logout、Session Bootstrap、会话列表与撤销 API | 全部 Session API 应删除；Bootstrap 改为用户级授权快照，全局 Logout 唯一调用 Tianquan-Shoubing |
| Tianquan-Jianshen `admin.auth` | 仍存在人员 `AuthenticationFacade`、`PasswordIdentityAuthenticator`、`JwtTokenService`、`RefreshFacade`、`StepUpFacade`、`JwtKeyRingService` 以及 Session 强化适配器 | USER/SERVICE认证、AT/RT签发、签名密钥和二次认证统一归 Tianquan-Shoubing；RBAC3只保留资源服务验证与授权能力 |
| `rbac3_user_credential` | RBAC3仍保存人员密码凭据 | IdP已经是人员凭据权威；该表及人员密码读写链在破坏式切换中删除 |
| `rbac3_user` | 同时保存 `username/normalized_username/display_name/locked_until` 等身份/登录字段，以及组织岗位快照字段 | Tianquan-Jianshen 用户必须缩为以 `identitySub` 绑定的最小授权主体；身份核心字段和登录安全状态只能由 IdP持有 |
...
| `rbac3_service_credential` / `ServiceCredentialPO` | RBAC3仍可保存机器 Secret/Public Key，且与 `ServicePrincipalPO`同处授权模型 | Tianquan-Shoubing 已有 Client Credential、SERVICE AT和Resource/Scope权威；RBAC3机器凭据表与读写模型删除，只保留机器主体和权限关系 |
| `JpaPlatformAdminBootstrapRepository` / `Rbac3PlatformAdminBootstrapCli` | Tianquan-Jianshen 首管引导仍接收密码、使用 `PasswordEncoder`并写 `UserCredentialPO` | 引导不能成为 Tianquan-Jianshen 本地密码旁路；身份先由 Tianquan-Shoubing 建立，Tianquan-Jianshen 只绑定 `identitySub`并创建授权事实 |
| Tianquan-Jianshen V1 表 | 存在 `rbac3_session`、`rbac3_session_active_role`、`rbac3_refresh_token` | 认证 Session/Refresh 表应删除；Session Active Role 表应被用户级 Active Role 表替代，而不是退化为全部 Assignment 生效 |

Tianquan-Jianshen 包迁移后的当前关键路径为：
...
`ServiceCredentialPO`、`JwtTokenService`、`JwtKeyRingService` 及 Tianquan-Jianshen JWT 私钥配置属于
认证/签发职责，必须删除。真实机器调用者继续从 Tianquan-Shoubing 获取 SERVICE AT，再由 RBAC3决定其
...
当前本地 Admin Web 的 Issuer 和 API Proxy 直接指向 Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Admin
端口；Yuheng 当前安全适配器只处理 Bearer，现有本地发布脚本主要验证 Mock/MCP Route。
因此不能把当前运行形态描述为“所有人员请求已经经过 Yuheng”。

```mermaid
flowchart LR
    U["Admin Web"] -->|"登录、授权码、Token、Refresh"| I["Tianquan-Shoubing 直连端口"]
    U -->|"各自 Vite Proxy"| B["各 Admin Backend"]
    I --> S["Redis SSO Session"]
    I --> T["Client/Resource 绑定的 USER AT 与轮换 RT"]
    U -->|"Authorization Bearer"| B
    B -->|"AT + Redis 实时状态校验"| IS["Tianquan-Shoubing 状态投影"]
    B -->|"system + tenant + session"| R["Tianquan-Jianshen"]
    G["Yuheng"] -->|"当前 Bearer 保护的已发布 Route"| B
```
...
2. 完全移除人员 Session、SSO Session、Tianquan-Jianshen Session 和 Session 级 Active Role 绑定，同时保留用户级角色激活能力；
3. 让一个 5 分钟 USER Access Token 在同一租户内跨客户端、跨业务域使用；
4. 让 Yuheng 成为所有外部请求的唯一入口；
5. 让 Yuheng 在 AT 缺失或过期时使用 RT 向 Tianquan-Shoubing 自动刷新；
6. 让 Tianquan-Shoubing 的开放协议接口和受保护管理接口具有明确、不同的安全策略；
7. 让 Tianquan-Shoubing 管理员和普通平台用户使用同一登录与 Token 模型；
8. 让 Yuheng 与每个保护业务目标服务都校验同一个 USER AT并执行各自 Tianquan-Jianshen 权限判断；
   保护身份端点只做 Yuheng + IdP两次 USER AT校验，不要求业务域权限；
9. 让 Tianquan-Jianshen 权限快照以业务系统、租户和人员身份为键，不再以 Session 为键；
10. 让强制退出通过撤销 RT 生效，已签发 AT 最多保留五分钟身份有效期；
11. 让跨客户端免登录完全由 Yuheng 域的 JWT Cookie 完成，不建立服务器 SSO 状态；
12. 保持 Tianquan-Shoubing 的 SERVICE Access Token与现有 Admission能力，但删除 RBAC3重复的机器凭据、签名和发 Token能力；
13. 保证只有用户明确激活且当前仍有效的角色族进入权限上下文，未激活角色权限不生效；
14. 把 Tianquan-Jianshen 用户收缩为租户内最小授权主体，彻底删除 RBAC3人员密码和重复用户核心资料；
15. 固定 JWT模块职责：Tianquan-Shoubing Admin签发，Tianquan-Shoubing Starter统一验签，Yuheng Adapter复用适配，
...
- 不在本期建设完整用户中台，不新增人员档案、联系方式、实名、画像、偏好、生命周期、
  组织人事主数据或对应管理页面；
- 不删除 Tianquan-Shoubing Client Credentials、Tianquan-Shoubing SERVICE Access Token、Admission Ticket 或 MCP 异步任务机器身份；
- 不把角色、权限、数据范围或字段策略写进 USER Access Token；
- 不让 Yuheng 或业务服务保存人员 AT/RT/Session；
- 不让普通业务服务接收、读取、代理或刷新人员 Refresh Token；
- 不保留旧 Session、旧授权码或旧 USER Token 的双栈兼容；
- 不迁移旧 Session Active Role、旧 Refresh Family 或 Tianquan-Shoubing `tokenVersion` 数据；
...
```mermaid
flowchart TB
    U["浏览器 / 所有 Admin Web"] --> G["Yuheng<br/>唯一外部入口"]

    subgraph Tianquan-Shoubing["Tianquan-Shoubing"]
    ...
    B["普通业务系统"]
    RA["Tianquan-Jianshen Admin"]
    R["Tianquan-Jianshen 最小授权用户、权限事实<br/>用户激活角色与快照"]
    ...
    IP --> IU
    A --> IU
    P -->|"登录/Refresh：受信 Tianquan-Shoubing SERVICE调用校验租户成员<br/>不查业务权限"| R
    G -->|"保护身份 Route：USER AT，不查业务权限"| IP
    IP --> K

    G -->|"保护业务 Route：USER AT + Yuheng Tianquan-Jianshen"| A
    G -->|"保护业务 Route：USER AT + Yuheng Tianquan-Jianshen"| B
    G -->|"保护业务 Route：USER AT + Yuheng Tianquan-Jianshen"| RA

    G -->|"USER AT + system/route/API"| R
    A -->|"再次校验 USER AT + tianquan-shoubing-admin 权限"| R
    B -->|"再次校验 USER AT + 本系统权限"| R
    RA -->|"再次校验 USER AT + tianquan-jianshen-admin 权限"| R

    R -->|"通过 Tianquan-Shoubing JWK本地校验同一 USER AT；不创建 Session"| K
```

### 6.1 Tianquan-Shoubing 的双重角色

Tianquan-Shoubing 必须同时满足两种角色，不能只实现其中之一：
...
2. **受保护业务系统**：用户、OAuth Client、Resource Server、签名密钥、审计和管理配置。
   这些接口必须通过 Yuheng USER AT 校验、Yuheng Tianquan-Jianshen、Tianquan-Shoubing 本地 USER AT 校验和
   Tianquan-Shoubing Tianquan-Jianshen 权限校验。

Tianquan-Shoubing 管理页面的登录不建立特殊“管理员会话”。认证成功后是否能进入 Tianquan-Shoubing Admin，由
`tianquan-shoubing:bootstrap:read` 及后续 `tianquan-shoubing:*` 权限决定。

### 6.2 两层 PEP

Yuheng 与目标服务都是 Policy Enforcement Point：

- Yuheng 使用 Route、Operation、business domain 和 required permission 做外围拒绝；
- 目标服务使用自己的接口/方法权限做最终权威拒绝；
- 两层共享 Tianquan-Jianshen 权限事实，但可以各自缓存快照；
- Yuheng 放行不替代服务鉴权，服务放行也不能绕过 Yuheng 外围策略；
- Tianquan-Jianshen 不可用时两层均 fail closed，不能降级为匿名或默认允许。
...
| Claim | 要求 |
|---|---|
| `iss` | 稳定的对外 Tianquan-Shoubing Issuer；该 URL 由 Yuheng 路由，不暴露 Tianquan-Shoubing 内部端口 |
...
| `acr` | 可选；允许值按现有等级为 `PASSWORD/MFA/STRONG`，只表达 Tianquan-Shoubing 已验证的认证强度，不得表达角色或权限。本期密码 Step-up 成功签发 `STRONG`；缺失时规范化为 `PASSWORD` |
| `auth_time` | Epoch Second；`acr=MFA/STRONG` 时必须存在，表示该强度最近一次由 Tianquan-Shoubing 验证的时间；`PASSWORD` 可省略 |
...
`iat/nbf/exp`；存在 `acr/auth_time` 时还必须校验允许值、类型和时间关系。不得读取 Tianquan-Shoubing 用户
状态 Redis、OAuth Client 状态、Resource Server 状态、Refresh Store 或 Tianquan-Jianshen Session。
...
Step-up -> STRONG”语义迁到 Tianquan-Shoubing。高风险角色激活需要二次认证时，Tianquan-Shoubing 校验当前
...
`username/identitySub/tenantId`。Tianquan-Shoubing 必须从已验签 USER AT 获得 `sub/tid`，在
`IdentityFacade` 增加按 `identitySub` 绑定的当前密码验证用例，复用现有账号
状态、PasswordHashPort、失败计数、密码升级和审计能力。不能把用户名+密码
再次交给 Tianquan-Jianshen，也不能允许用 A 的 AT 与 B 的凭据完成 Step-up。

### 7.2 Refresh Token

人员 Refresh Token 继续使用 Tianquan-Shoubing 签名 JWT，Header 保持 `alg=RS256`、`typ=JWT` 和
...
| Claim | 要求 |
|---|---|
| `iss` | 与 USER AT 相同的 Tianquan-Shoubing Issuer |
| `sub` | 人员 `identitySub` |
| `tid` | Refresh 所属唯一租户 |
...
Refresh TTL 继续由现有 Tianquan-Shoubing Runtime Policy 管理并保持当前 1 至 30 天安全边界；本文不
改变其默认值。无论配置多少天，`exp` 都在首次登录签发时固定，后续 Refresh 不滑动续期。

Tianquan-Shoubing 校验 Refresh Token 时必须同时满足：
...
Tianquan-Shoubing SERVICE身份调用 Tianquan-Jianshen 做租户成员校验，不执行业务域/API 权限判断；该内部调用
复用 IdP现有 SERVICE认证能力，不把刚签发或待刷新的 USER AT当作调用方凭据。Tianquan-Jianshen 或依赖暂时
不可用时返回暂时不可用，不得伪装成密码错误或 Token 伪造。

每次成功输入凭据建立一个独立 RT 记录；同一浏览器通过 Yuheng Host Cookie在多个
...
- 只能由 Tianquan-Shoubing 通过现有 Client Credentials/机器身份流程签发；
- 必须携带固定 `principal_type=SERVICE`，不得被 USER 入口接受；
- 不附带人员 Refresh Token；
- 不参与 JWT SSO、Yuheng 人员自动刷新或人员 Cookie；
- 可继续使用精确 Resource Audience、Service Scope 和 Admission 契约；
- 不得被转换为 `IdentityPrincipal` 或用于模拟人员访问；
- 不得因此保留任何人员 Session 结构；
- 由 Yuheng、Tianquan-Jianshen 或业务服务通过 Tianquan-Shoubing JWK和 SERVICE Contract验证；RBAC3不得二次
  签发、刷新或用自己的 `JwtKeyRingService` 验证另一套 Tianquan-Jianshen Token。

Tianquan-Shoubing 保存 Client Credential、Client/Resource Grant、Scope和签名密钥；RBAC3最多保存
`ServicePrincipal + ServicePermission` 授权事实，并以验签后的 Tianquan-Shoubing SERVICE Principal
作为主体输入。当前 `rbac3_service_credential`、`ServiceCredentialPO`、Tianquan-Jianshen
`JwtTokenService/JwtKeyRingService` 和 Tianquan-Jianshen JWT 私钥配置必须删除。Admission Ticket
继续是现有 Tianquan-Shoubing/Tianshu 的窄用途启动准入 JWT，不是第三种平台访问 Token，不得用于用户登录、
...
当前 Tianquan-Shoubing 模块依赖方向已经适合直接收口：

```text
tianquan-shoubing-gateway-adapter -> tianquan-shoubing-starter -> tianquan-shoubing-core + tianquan-shoubing-rpc-contract
tianquan-shoubing-admin -> tianquan-shoubing-core + tianquan-shoubing-rpc-contract + tianquan-jianshen-starter
```
...
| 模块 | 本期职责 | 明确禁止 |
|---|---|---|
| `egon-cola-tianquan-shoubing-core` | 保存不依赖 Spring/Nimbus 的 USER/SERVICE/Refresh Claim语义、Token Policy和签发/验证端口 | 私钥文件读取、JWK HTTP客户端、Spring Security Filter、Yuheng SPI |
| `egon-cola-tianquan-shoubing-admin` | 唯一 Token Issuer；持有 IdP平台私钥，签发 USER AT、RT和 SERVICE AT，提供 JWKS并校验 RT；改造现有 `Rs256TokenService`而不是建立第二套签发器 | 把 IdP签名私钥下发给 Yuheng、RBAC3或业务服务 |
...
`tianquan-shoubing-starter` 中现有 `PrivateKeyJwtAssertionFactory` 是调用方用自身 Client私钥签名
`private_key_jwt` Client Assertion 的窄用途工具，不是 IdP平台 AT/RT签发器。它可以留在
Starter的机器调用能力内，但不得接触 IdP平台签名私钥，也不得被复用为 USER Token签发入口。

`tianquan-shoubing-gateway-adapter` 当前已经依赖 `tianquan-shoubing-starter`，因此 Gateway直接适配共享验证结果即可；
...
时钟工具的无状态组件；Tianquan-Shoubing Claim规则、密钥权威和 Token Policy仍不得下沉。

```mermaid
flowchart LR
    IA["Tianquan-Shoubing Admin<br/>私钥签名 / JWKS / RT校验"] -->|"签发 JWS"| T["USER AT / RT / SERVICE AT"]
    T -->|"JWK公钥验签"| IS["Tianquan-Shoubing Starter<br/>统一 Decoder + Claim Policy"]
    IS -->|"适配验证结果"| GA["Tianquan-Shoubing Yuheng Adapter"]
    IS -->|"嵌入资源服务器"| RS["Tianquan-Shoubing / Tianquan-Jianshen / 业务服务"]
    RC["Tianquan-Shoubing RPC Contract<br/>仅 Proto / RPC契约"] -. "不承载 JWT实现" .-> IS
```
...
浏览器人员认证使用两个 Yuheng 对外 Host 的 Cookie：
...
| `SameSite` | `Lax` | `Lax` |
| `Path` | `/` | `/` |
| `Domain` | 不显式设置，使用 Yuheng 对外 Host-only | 不显式设置，使用 Yuheng 对外 Host-only |
...
RT 的 Path 不能继续使用当前 `/oauth2`，因为 Yuheng 必须在任意保护请求到达时判断是否
存在 RT。Yuheng 读取后必须剥离 Refresh Cookie，普通业务上游永远看不到 RT。

Tianquan-Shoubing 对经 Yuheng 代理的登录/刷新响应写 `Set-Cookie` 时不设置内部 Tianquan-Shoubing Domain；浏览器
以实际响应的 Yuheng Host 保存 Cookie。Yuheng 内部自动刷新调用必须读取 Tianquan-Shoubing 响应的
...
- Yuheng 对使用认证 Cookie 的非安全 HTTP Method 校验可信 Origin/Referer 或等价机制；
- Yuheng CORS 只允许已登记 Admin Web Origin，且显式允许 Credentials；
- CSRF 随机值只证明请求来源完整性，不代表身份、不进入 JWT、不计作第三种身份 Token；
- 不能因为 Spring Security 设置为 `STATELESS` 就继续无条件忽略全部 `/api/**` CSRF 风险。

---

## 9. Tianquan-Shoubing 端点与安全策略

### 9.1 对外端点矩阵

| Tianquan-Shoubing 端点 | Yuheng 策略 | Tianquan-Shoubing 校验 | Tianquan-Jianshen |
|---|---|---|---|
...
| `GET /oauth2/userinfo` | 保护身份 Route；可触发 Yuheng 自动 Refresh | Tianquan-Shoubing 校验 USER AT并只返回当前主体信息 | 不要求 Tianquan-Shoubing Admin 权限 |
| `/api/v1/auth/bootstrap` | 保护业务 Route | Tianquan-Shoubing 再次校验 USER AT | `tianquan-shoubing:bootstrap:read` |
| `/api/**` 其他 Tianquan-Shoubing Admin API | 保护业务 Route | Tianquan-Shoubing 再次校验 USER AT | 对应 `tianquan-shoubing:*` 权限 |

公开 Route 必须使用精确 Method + Path，不允许用宽泛 `/oauth2/**` 匿名白名单覆盖未来新增
接口。`/oauth2/logout` 必须能在 AT 过期后依靠 RT 完成，不能继续因缺少有效 AT 被
Spring Security 拦截。保护身份 Route 先按第 10 章恢复/校验 AT，然后直达 Tianquan-Shoubing；
它不调用 Yuheng Tianquan-Jianshen。Step-up 成功后以 `Set-Cookie` 替换 AT，响应 Body 不包含原始 Token。

### 9.2 人员登录

内部 Admin Web 人员登录不再经过 `/oauth2/authorize` 和 Authorization Code：

1. 浏览器从 Yuheng 获取登录 CSRF；
2. 浏览器向 Yuheng `/oauth2/login` 提交 `tenantId + username + password`；
3. Yuheng 按公开 Route 转发 Tianquan-Shoubing；
4. Tianquan-Shoubing 校验 CSRF、账号密码、现有账号状态和目标租户成员关系；
5. Tianquan-Shoubing 签发 5 分钟平台 USER AT 和长期 RT；
6. 响应只写 HttpOnly AT/RT Cookie及非敏感登录结果；
7. 前端跳转目标 Admin Web，并调用其 Bootstrap；
8. Bootstrap 的 Yuheng 和服务侧 Tianquan-Jianshen 决定用户能否进入该业务域。
...
- Tianquan-Shoubing 不把并发使用同一有效 RT 判定为重放；
- RT 到期、撤销或摘要不存在时统一 `invalid_grant`；
- 安全审计记录成功/失败原因，但不记录明文 Token 或摘要。

该选择避免为 Yuheng 集群增加分布式 Refresh Single Flight 或 Rotation Grace Window。
以后如重新引入 Rotation，必须先设计 Tianquan-Shoubing 端幂等并发语义，不能恢复当前“并发失败即撤销
整个 Family”的行为。
...
Yuheng 在路由前完成 USER AT 判断：

| 输入状态 | Yuheng 行为 |
|---|---|
| AT 有效 | 继续执行该 Route 类型的策略；仅保护业务 Route进入 Yuheng Tianquan-Jianshen，然后转发同一 AT |
| AT 缺失，RT 有效 | 调 Tianquan-Shoubing Refresh；校验新 AT；写 AT Cookie；继续原请求 |
| AT 已过期，RT 有效 | 同上 |
...
| AT 有效但保护业务 Route的 Yuheng Tianquan-Jianshen 拒绝 | 返回 403，不 Refresh |
| 保护业务 Route的 Tianquan-Jianshen 暂时不可用 | Fail closed，返回 503，不 Refresh |

Yuheng 不应先把过期 AT 发给目标服务再根据任意上游 401 猜测是否刷新。自动刷新只由
路由前的 USER AT 验证结果触发。目标服务在 Yuheng 已验证 AT 后仍返回 401 时，Yuheng
...
sequenceDiagram
    ...
    participant R as "Tianquan-Jianshen"
    participant S as "Tianquan-Shoubing Admin 或业务系统"

    U->>G: "POST /oauth2/login，tenantId + 用户名 + 密码"
    G->>I: "公开登录 Route；不要求已有 AT，不查业务权限"
    I->>I: "校验 CSRF、凭据和现有账号状态"
    I->>R: "受信 Tianquan-Shoubing SERVICE调用：校验 tid/sub 租户成员；不查业务权限"
    ...
    alt "AT 有效"
        ...
        I->>R: "受信 Tianquan-Shoubing SERVICE调用：重新校验 tid/sub 租户成员"
        R-->>I: "MEMBER_ACTIVE / DENY / UNAVAILABLE"
        alt "RT与租户成员均有效"
...
精确开放协议/保护身份/保护业务 Route 策略
  ...
  -> Missing/Expired Recovery through Tianquan-Shoubing
  -> [仅保护业务] Yuheng Tianquan-Jianshen Authorization
...
当前 Yuheng 存在 Tianquan-Shoubing 和 Tianquan-Jianshen 两套 USER Bearer 认证链。目标职责分配固定为：
...
- `Rbac3PermissionAuthorizationProvider` 保留为 `BUSINESS_PROTECTED` 的授权提供者；
  `Rbac3GatewayRuntimeSnapshotReader` 按已认证 Tianquan-Shoubing `GatewayPrincipal` 的 `tenantId + subject`与
  Route `systemCode/operation` 读用户级快照，不读 Session/Fence；
- `Rbac3BearerCredentialExtractor`、`Rbac3TrustedIdentityMapper` 及 Tianquan-Jianshen Session 保留头集在
  USER 链删除；`IdpTrustedIdentityMapper` 的 USER 分支也删除。Yuheng 的通用
  `TrustedIdentitySanitizer`/入口清理规则仍先移除外部 `X-Egon-*` 伪造头，但对 USER
  不重建任何可信身份头。下游只重新验证 Yuheng Relay 的原始 USER AT，
  不信任 `x-egon-yuheng-session-*`、`X-Egon-Identity-*` 或权限 Header。SERVICE
  专用 Route 如仍需 Tianquan-Shoubing Trusted Identity Mapping，保留在显式 SERVICE Policy 中，不进入
  USER 流程。

这里的“物理删除”包括 Tianquan-Jianshen 人员自签 JWT/Session 验证链，也包括 RBAC3自己的
SERVICE签发/验签链。如该模块仍有独立 SERVICE Token消费者，必须使用 Tianquan-Shoubing SERVICE
Contract 和明确的机器身份 Adapter承载；不得通过保留
`Rbac3TokenClaims/sid/sessionVersion/JwtKeyRingService` 来顺带兼容。

### 10.4 Credential 冲突规则

- 浏览器主路径从 Yuheng Host-only Cookie读取 USER AT；
- 受信非浏览器调用可继续使用 `Authorization: Bearer`；
- 同一请求同时携带 Cookie AT 和 Authorization AT 时，两者必须完全相同，否则返回 401；
- 外部请求不得自带受信 `X-Egon-*` 身份头，Yuheng 继续先清理再重建；
- 对 USER Route，Yuheng 清理后不重建 `X-Egon-*` 身份头，身份仅由下游验证原始
  USER AT 获得；SERVICE Route 是独立显式策略；
- RT 只允许从 HttpOnly Cookie进入 Tianquan-Shoubing Protocol/Refresh 适配器，不允许 Query、普通 Header
  或业务 Request Body 传递。
...
JWT、`GatewayPrincipal` 的人员认证字段、Tianquan-Jianshen 授权快照身份键或普通业务身份 Header。

MCP 的 `clientId` 只用于协议会话、任务、远程调用和审批的隔离：SERVICE 请求使用经 Tianquan-Shoubing
验证的 SERVICE `client_id`，USER 请求使用经 Tianquan-Shoubing 验证的固定平台 Audience（内部属性
`tianquan-shoubing.audience`）作为协议隔离值。USER JWT 仍然禁止 `client_id` Claim，MCP 也不得从请求头或
调用方自报值补造该字段。

---

## 11. Tianquan-Shoubing Starter、Tianquan-Shoubing Admin 与业务服务
...
- `IdpAuthenticationToken` 不保存明文 AT；如 Tianquan-Jianshen Starter 需要 Relay 当前 AT，使用
  请求级受控 Credential Carrier，禁止写入日志、缓存、Principal 属性或跨请求状态。

### 11.2 Tianquan-Shoubing Admin Security Chain
...
2. 仅精确公开第 9.1 节协议端点；
3. 其他请求必须 USER AT 认证；
4. Tianquan-Shoubing Bearer Filter 在 Tianquan-Jianshen Filter 前执行；
...
9. 原 Tianquan-Jianshen `/api/tianquan-jianshen/v1/auth/step-up` 的 Session 强化职责删除；二次认证统一由 Tianquan-Shoubing
   `/oauth2/step-up` 完成并返回同一种 USER AT。

### 11.3 目标服务二次校验

每个业务服务和 Tianquan-Shoubing Admin 必须：

- 接收 Yuheng 转发的原始 USER AT；
- 使用相同 Issuer、平台 Audience 和 JWK进行本地验证；
- 从 `sub/tid` 构造 IdentityPrincipal；
- 如存在 `acr/auth_time`，把经过签名校验的认证上下文作为请求级只读信息提供给 Tianquan-Jianshen，
  不在目标服务持久化；
- 使用本系统 `systemCode` 获取 Tianquan-Jianshen 授权快照；
- 在 URL、方法或现有 `AuthorizationService` 调用处执行权限；
- 401 只表示身份 Token 无效，403 只表示身份有效但权限不足；
- 不读取 Refresh Cookie，不调用 Tianquan-Shoubing Refresh，不依赖 Yuheng 受信头替代 AT 验证。

---

## 12. Tianquan-Jianshen 无 Session 授权模型

### 12.1 最小授权用户与 Tianquan-Shoubing 用户权威

Tianquan-Jianshen 的 `UserPO/rbac3_user` 不再是账号或用户资料主表，只是“一个 IdP人员在一个租户中
...
Tianquan-Shoubing 继续作为用户核心信息与认证安全状态权威。本期基于现有 `IdentityUser/IdentityUserEntity`
...
本期 Tianquan-Jianshen Contract和管理页面只展示 `identitySub`与本地授权状态，不为展示资料新增
Tianquan-Jianshen -> IdP同步或同步 RPC依赖。将来用户中台提供统一查询/聚合能力后，页面可以按独立权限
...
另立规格扩展 Tianquan-Shoubing，不得因此阻塞本次 JWT、密码归属和无 Session改造。

身份与授权状态必须分别判断：IdP账号可登录，不代表拥有某租户/业务权限；IdP状态通过后，
Tianquan-Jianshen `status=DISABLED/ARCHIVED` 仍会拒绝该租户授权。反之，RBAC3不能用本地状态替代 Tianquan-Shoubing
...
Tianquan-Jianshen Snapshot 至少包含：

- `systemCode`
- `tenantId`
- `identitySub`
- Tianquan-Jianshen Tenant User 映射标识
...
### 12.3 Tianquan-Jianshen Snapshot 获取

目标保留现有两种消费形态，但统一使用同一份用户级授权投影：

- Yuheng 已由 `IdpIdentityAuthenticationProvider` 验证当前 USER AT，继续通过改造后的
  `Rbac3GatewayRuntimeSnapshotReader` 直读 Tianquan-Jianshen Redis投影；Reader只用已认证
  `GatewayPrincipal` 的 `tenantId + subject`、Route `systemCode/operation`和当前授权版本，
  不再读取 Session/Fence，也不接受外部身份 Header；
- Tianquan-Shoubing Admin和普通业务服务的 Tianquan-Jianshen Starter在本地验证当前 USER AT后，缓存未命中时把
  同一个 USER AT Relay给 RBAC3内部 Snapshot API；同时继续使用现有 Tianquan-Shoubing Client
...
1. 按当前 `@RequiresServiceScope("service:authorization:snapshot")`边界验证 Tianquan-Shoubing SERVICE AT；
...
`IdentityResolveRequestDTO`和 Tianquan-Shoubing `TenantMembershipPort` 的 USER 分支按
`(tenantId, identitySub)` 解析 Tianquan-Jianshen 用户和租户成员关系。SERVICE Client/Resource 的
`clientId` 契约不因此删除。
...
- 现有 `rbac3_user.auth_version` 保留为 Tianquan-Jianshen 用户授权事实版本；角色激活成功也推进该版本，
  但它不进入 USER AT，也不参与 Tianquan-Shoubing 的 JWT身份有效性判断；
- 逻辑快照身份键固定为 `(systemCode, tenantId, identitySub)`；实体缓存可以在该键后追加
  `authVersion + policyVersion`，但不得追加 Session/Token/Client；
- 每次授权决策在使用快照前必须校验 Tianquan-Jianshen 发布的当前用户 `authVersion`、
...
- 授权变更开始时按 `(tenantId, rbac3UserId)`（租户级变更使用 tenant scope）
  ...
  Yuheng 直读 Redis 时使用原子脚本或前后双检避免竞态；通过 Tianquan-Jianshen HTTP 取快照时由
  Tianquan-Jianshen 服务端执行同一检查；
- 上述保护是 Tianquan-Jianshen 授权投影一致性状态，不含 AT/RT、不表示登录、不按客户端或
...
| 端点 | 目标行为 |
|---|---|
| `GET /api/tianquan-jianshen/v1/auth/role-activation-candidates` | 从当前 USER AT派生 `tid/sub` 并映射 RBAC3用户；只返回当前有效 Assignment 可形成的规范根角色候选 |
| `GET /api/tianquan-jianshen/v1/auth/role-activations` | 返回该 RBAC3用户按 Application保存的当前激活根角色集合、`authVersion` 和 `activationRequired`；响应不再有 `sessionId/sessionVersion/contextVersion` |
| `PUT /api/tianquan-jianshen/v1/auth/role-activations` | 原子替换该 RBAC3用户跨 Application 的完整激活根角色集合；请求使用 `roleIds + expectedAuthVersion`，不接受 `sessionId` |
...
Tianquan-Shoubing 登录、Refresh 或 Step-up 通过 HttpOnly Cookie 建立。
...
sequenceDiagram
    ...
    participant G as "Yuheng"
    participant I as "Tianquan-Shoubing"
    participant R as "Tianquan-Jianshen Activation"
    participant D as "Tianquan-Jianshen PostgreSQL"
...
### 12.7 Tianquan-Jianshen 自身的管理接口

Tianquan-Jianshen Admin 也是普通保护业务域：

- 浏览器从 Gateway进入；
- Gateway校验 USER AT并执行 `tianquan-jianshen-admin` 外围权限；
- Tianquan-Jianshen Admin 再次校验 USER AT；
- Tianquan-Jianshen Admin 使用自身权限服务执行具体管理权限；
- 获取当前人员快照的内部入口只做 USER AT身份和快照计算，不递归调用自己。

Tianquan-Jianshen 人员 Auth/Session API 的处置固定为：

- 删除 `AuthController.logout()`；人员全局退出唯一调用 Yuheng
  `/oauth2/logout`，由 Tianquan-Shoubing 撤销 RT；
- 删除 `SessionController`、`admin.session/**`、`/sessions/**`、
  `/users/{userId}/sessions/revoke-all` 和 `system:session:*` 权限/资源种子；
- 删除 Tianquan-Jianshen 人员 `/login`、`/refresh`、`/step-up` 及其 Contract，不做路由转发兼容；
- 保留 `/api/tianquan-jianshen/v1/auth/bootstrap` 路由作为 Tianquan-Jianshen Admin 启动视图，但
...
当前 `JpaPlatformAdminBootstrapRepository` 和 `Rbac3PlatformAdminBootstrapCli` 会在 Tianquan-Jianshen
接收密码并写入 `UserCredentialPO`，这是 Tianquan-Shoubing 凭据权威的旁路，必须一起删除。
目标引导顺序固定为：

1. Tianquan-Shoubing `IdpBootstrapService` 先创建首个人员身份和唯一密码凭据，得到
   `identitySub`；不记录密码或 Token；
2. Tianquan-Jianshen Bootstrap 只接收 `tenantCode + identitySub`，在一个事务中创建租户、最小
   Tianquan-Jianshen User、平台角色和 Assignment；不创建 External Identity Mapping，不复制
   `username/displayName`；
3. Tianquan-Jianshen Bootstrap 不注入 `PasswordEncoder`、不创建 Credential、不签发 AT/RT；
4. 生产引导不隐式激活高风险平台角色。引导完成后，用户经 Yuheng 登录，
   以最小激活引导权限读候选，必要时在 Tianquan-Shoubing Step-up，再显式激活；
5. 现有 `Rbac3DevelopmentBootstrap` 通过 `TIANQUAN_JIANSHEN_DEVELOPMENT_IDENTITY_SUB`绑定 Tianquan-Shoubing
...
1. 浏览器经 Yuheng 调 `/oauth2/logout`；
2. 即使 AT 已过期，Gateway仍按公开协议 Route 转发 RT；
3. Tianquan-Shoubing 校验 RT 后删除/失效当前 RT摘要；
4. Tianquan-Shoubing 返回过期 AT/RT Cookie；
...
```mermaid
sequenceDiagram
    participant A as "Tianquan-Shoubing 管理员"
    participant G as "Yuheng"
    participant R as "Tianquan-Jianshen"
    participant I as "Tianquan-Shoubing Admin"
    participant T as "Tianquan-Shoubing Refresh Store"
    participant U as "被强制退出用户"

    A->>G: "POST /api/v1/tianquan-shoubing/users/{sub}/revoke-all"
    G->>R: "校验管理员外围权限"
    R-->>G: "ALLOW"
    G->>I: "转发管理员 USER AT"
    I->>I: "再次校验 AT"
    I->>R: "tianquan-shoubing:identity-user:revoke-all"
    R-->>I: "ALLOW"
    I->>T: "删除该 sub 的全部有效 RT 摘要"
    I-->>A: "成功"

    U->>G: "继续使用尚未过期的 AT"
    G-->>U: "身份 AT 在原 exp 前仍可能通过；Tianquan-Jianshen 仍独立决定权限"
    U->>G: "AT 过期后再次请求"
    G->>I: "用 RT 尝试 Refresh"
    I->>T: "查询 RT 摘要"
    T-->>I: "不存在"
...
Principal、Verifier、Yuheng Attribute和撤销比较链全部删除，不保留兼容列或占位字段。
物理删除必须通过一个新的 Tianquan-Shoubing Flyway migration完成，不得修改现有 V1/V2/V3。

密码修改、密码重置、现有账号禁用和管理员 revoke-all 都至少撤销相关 RT。已签发 AT
仍按原 `exp` 最多保留五分钟身份有效期；若同时撤销 Tianquan-Jianshen 权限，业务访问可以更早 403。
...
- 所有 Yuheng API请求 `credentials: include`；
- 统一登录页面调用 Yuheng `/oauth2/login/csrf` 和 `/oauth2/login`；
- 401 清理本地非敏感 UI 状态并跳转登录；
- 403 展示“已登录但无当前系统权限”，不能跳成密码登录失败；
- Admin Web 启动时调用各自 Bootstrap；
- Bootstrap 成功后根据 Tianquan-Jianshen 资源/权限投影加载现有菜单和能力。

Tianquan-Jianshen React SDK 是本次破坏性契约的明确改造面：

- 删除 `InMemoryAccessTokenStore`、`Rbac3ApiClient.refresh()`、Tianquan-Jianshen
  `/auth/refresh` 和 `/auth/logout` 调用，HTTP Client 只用 `credentials: include`请求 Yuheng；
...
- 由于不需要旧 API 兼容，`Rbac3SessionContext/useRbac3Session` 改名为
  ...
  不表达服务端 Session；
- `STEP_UP_REQUIRED` 时，前端收集当前用户凭据并调用 Yuheng
  `/oauth2/step-up`；成功后用原 `expectedAuthVersion` 重试一次角色激活，不接触 AT；
- 删除 Tianquan-Jianshen Admin Web `features/session/**`、会话路由/菜单和
  `system:session:*` UI 能力；全局退出按钮调用 Yuheng `/oauth2/logout`。

### 14.2 跨客户端免登录

所有 Admin Web 访问同一个 Yuheng API Host：

1. 用户在任一 Admin Web 登录，Yuheng Host保存 AT/RT Cookie；
2. 打开第二个 Admin Web 时，它直接请求该系统 Bootstrap；
3. AT 有效时 Yuheng 直接校验和授权；
4. 只有 RT、AT 已过期时 Yuheng 自动 Refresh；
5. 该用户无第二个业务域权限时 Bootstrap 返回 403；
6. AT/RT 都无效时返回 401并要求重新登录。

不存在 Tianquan-Shoubing SSO Cookie、跨应用 SSO Session 查询或“已登录 Tianquan-Shoubing 再静默走授权码”的过程。
...
- USER Token `iss` 使用稳定的对外 Identity URL；该 URL 的 DNS/Route 进入 Yuheng；
- 浏览器只看到 Yuheng 对外 URL，不使用 `http://127.0.0.1:18120` 等 Tianquan-Shoubing 内部端口；
- `/.well-known/oauth-authorization-server` 和 `/oauth2/jwks` 由 Gateway公开代理 Tianquan-Shoubing；
- Yuheng、Tianquan-Shoubing、Tianquan-Jianshen 和业务服务可通过内部地址拉取 JWK，但必须校验相同外部 Issuer；
- JWK 缓存和密钥轮换保留，不能因短 AT 而禁用 `kid` 或旧公钥验证窗口。

### 15.2 Yuheng Route 分类

Yuheng Release 必须为每个 Method + Path 标记且只标记一种策略：

| Route 策略 | USER AT/Refresh | Yuheng Tianquan-Jianshen | 典型端点 |
|---|---|---|---|
...
| `BUSINESS_PROTECTED` | 要求 USER AT；缺失/过期可使用 RT 自动 Refresh | 必须调用 | Tianquan-Shoubing/Tianquan-Jianshen/Yuheng/Tianshu Admin 和其他业务 API |

三种 Route 使用不同安全 Policy，不能依赖 Controller 内部补救宽泛 Yuheng 白名单。
Yuheng 自动 Refresh 的内部 Tianquan-Shoubing Upstream 不经过对外 Route发布，避免路由循环。

### 15.3 外部与内部端口

- 浏览器、CLI 人员调用和外部调用只允许 Yuheng 对外端口；
- Tianquan-Shoubing/Tianquan-Jianshen/各 Admin Backend 内部端口由网络策略限制；
- 即使内部端口被误访问，保护身份接口仍本地校验 USER AT，保护业务接口仍本地校验
  USER AT和 Tianquan-Jianshen；
- Tianquan-Shoubing 开放协议接口被内部直接调用时仍校验密码、RT、Client Assertion或请求完整性；
- Yuheng 不成为下游唯一安全边界。
...
```text
PUBLIC_YUHENG_URL
TIANQUAN_SHOUBING_INTERNAL_BASE_URL
TIANQUAN_JIANSHEN_INTERNAL_BASE_URL
各 Admin Backend 内部地址
```

`VITE_TIANQUAN_SHOUBING_ISSUER`、Admin Web API Base URL和登录 URL使用对外 Yuheng URL；内部服务
配置继续使用内部地址。验证脚本不能再通过浏览器路径直接调用 `18120/18130/...` 证明
外部拓扑成功，内部健康检查除外。

现有 `scripts/unified-identity-local.sh` 已经具备“先 Tianquan-Shoubing Bootstrap，再读取
`identitySub` 写入 `TIANQUAN_JIANSHEN_DEVELOPMENT_IDENTITY_SUB`”的正确先后关系，这个边界保留。
但人员登录和验证流必须破坏性重写：

- 删除“establishing Tianquan-Shoubing SSO session”、Authorization Code、按 Admin Client的
  `oauth_token`、`fresh_oauth_token` 和人员 Refresh Replay 流程；
- 浏览器和外部人员主链不再按客户端生成、读取或传递 `*.access.jwt`文件；人员
  契约测试使用同一 Yuheng Cookie Jar 保存 AT/RT，不从 JSON 提取 Token。脚本仍可为
  明确的 CLI/直连负向验证生成短时 USER AT 文件，这些文件只供命令行测试读取，不进入
  浏览器、Yuheng route publisher 或业务服务配置，也不代表新增 Token 类型；
- 角色候选、激活、Bootstrap、全局退出和所有 Admin 访问都通过 Yuheng；
- SERVICE Client Credentials、Admission 及 MCP 等确有机器消费者的私钥/公钥凭据文件可保留，
  但其凭据注册与 Access Token签发端必须是 Tianquan-Shoubing；命名和测试必须明确它们是 SERVICE，
  不能被人员 Admin Web 使用，也不能指向 Tianquan-Jianshen 自签 Token端点；现有
  `*_TIANQUAN_JIANSHEN_SERVICE_CREDENTIAL_FILE` 静态 Bearer配置删除，统一启用 Starter
  `service-token` 的 Tianquan-Shoubing Client Assertion换取流程。机器私钥文件可以保留作为 Tianquan-Shoubing
...
`identity:v1:refresh-index:user:*`、`identity:v1:user:*`，以及 Tianquan-Jianshen 的
`tianquan-jianshen:*:session:*`、`tianquan-jianshen:*:snapshot:*`、`tianquan-jianshen:*:fence:session:*`、
`tianquan-jianshen:*:key-ring`。不得执行针对 Redis DB 的
宽泛 flush。旧身份 Key不迁移；清理前后都要确认目标 Prefix，避免删除 Tianshu、Yuheng、
RBAC3或其他业务数据。“不保留旧数据”只授权丢弃本次身份链自己的旧数据，不授权清空共享
Redis中的无关业务状态。

### 16.2 Tianquan-Jianshen PostgreSQL

现有 Flyway 文件保持不可变。以当前序列 V1 至 V4 为基线，实施时新增一个下一版本
Tianquan-Jianshen migration，破坏式重建人员授权相关结构：
...
租户主组织索引以及 `INVITED/LOCKED`身份状态语义。实施后由 Tianquan-Shoubing Bootstrap/受保护管理链按
...
Tianquan-Jianshen Refresh、RBAC3用户资料、人员凭据和机器凭据数据都不迁移、不回填。Tianquan-Jianshen
...
### 16.3 Tianquan-Shoubing PostgreSQL
...
| Refresh 时租户成员不存在/无效 | Tianquan-Shoubing `invalid_grant`；原业务请求映射 401 | 撤销该 RT并清理人员 Cookie |
| 登录依赖/Tianquan-Jianshen 暂时不可用 | 503 | 不返回“用户名密码错误” |
| USER Refresh Token 无效/撤销/过期 | Tianquan-Shoubing `invalid_grant`；原业务请求映射 401 | 清理人员 Cookie |
...
| 激活高风险角色但 AT无有效 `acr=STRONG/auth_time` | 401 `STEP_UP_REQUIRED` | 前端经 Gateway调用 Tianquan-Shoubing Step-up；成功后携新 AT重试原激活集合，不创建 Session |
...
| Yuheng Tianquan-Jianshen 拒绝 | 403 | 不调用 Refresh |
| 服务侧 Tianquan-Jianshen 拒绝 | 403 | 不调用 Refresh |
| Tianquan-Jianshen 暂时不可用 | 503 | Fail closed |
| Yuheng 已验证但目标服务仍返回 401 | 401 | 不二次 Refresh、不循环重试 |
...
这是身份协议、Cookie、Yuheng 路由、Starter Principal、Tianquan-Jianshen Snapshot 和角色激活持久化
契约的联合破坏性迁移，不允许长期混跑新旧 USER Token，也不迁移旧身份/Session数据。

推荐发布波次：

1. 冻结最新 USER Token、Principal、Cookie、Yuheng 三类 Route、错误、Tianquan-Jianshen Snapshot
   和用户激活角色 Contract；
2. 改造 Tianquan-Shoubing USER Claims、稳定 RT Store、直接登录/Refresh/Logout；
3. 改造 USER Tianquan-Shoubing Starter 和无 Session IdentityPrincipal；
4. 改造 Tianquan-Jianshen Snapshot、缓存 Key、人员映射、用户级角色激活、授权版本和数据库 Session清理；
5. 扩展 Yuheng USER Credential Recovery、RBAC3外围授权和 IdP公开/保护 Route；
6. 改造 Tianquan-Shoubing Admin、Tianquan-Jianshen Admin及其他目标服务的二次校验；
7. 改造 Admin Web Shared、Tianquan-Jianshen React SDK、所有 Admin Web、IdP先行/RBAC3授权引导和本地脚本；
...
- Yuheng Route/Policy、Admin Web、Token Contract、IdP和RBAC3必须作为同一协议版本恢复，
  不能让新旧组件交叉接受 `sid` AT或旧 OAuth Code Cookie；
- 共享 Redis只清理本方案明确拥有的身份 Prefix；Tianshu、Gateway和业务数据不在破坏范围内；
...
- 同一 AT能被 Yuheng、Tianquan-Shoubing Admin、RBAC3和普通业务服务验证；
...
- Tianquan-Shoubing Admin签发的同一 Token样本可由 Starter验证，Yuheng Adapter的结果与 Starter完全一致；
- `tianquan-shoubing-rpc-contract` 的 POM不直接声明 JOSE/Nimbus/Spring Security，生产源码无 JWT实现
  Import，Proto不含原始 AT/RT字段；
- 仓库不存在新 `component-jwt`，Tianquan-Shoubing Claim Policy未下沉到 Common Crypto。

### 19.2 Tianquan-Shoubing

- 登录接口无需已有 AT，但错误密码不能签发 Cookie；
- 登录成功同时建立 AT/RT Cookie，不创建 SSO Key；
- Refresh无需有效 AT，只依赖有效 RT；
- Logout在 AT过期时仍能撤销 RT；
- Tianquan-Shoubing Admin接口无 AT返回 401；
- Tianquan-Shoubing Admin接口有 AT但无 `tianquan-shoubing:*` 权限返回 403；
- `@AuthenticationPrincipal` 能注入无 Session IdentityPrincipal；
- Yuheng USER 身份只由 `IdpIdentityAuthenticationProvider` 认证；Tianquan-Jianshen Adapter 不再验
  Tianquan-Jianshen 自签 USER JWT、Session 或 Session Version；
- Tianquan-Shoubing Step-up校验当前用户凭据后只重签同一种 5 分钟 AT，不创建 Session、不轮换 RT；
- 普通 Refresh回到 `PASSWORD`强度，不继承已过期 AT的 `MFA/STRONG`认证上下文；
- revoke-all只撤销 RT，不使当前 AT在 `exp` 前因 Token Version失效。
- 首个人员凭据只在 Tianquan-Shoubing Bootstrap创建；Tianquan-Jianshen Bootstrap只绑定 `identitySub`，
...
- Refresh Route不递归；
- 多个并发过期请求不会撤销稳定 RT；
- Yuheng Tianquan-Jianshen 403不刷新；
- Yuheng Tianquan-Jianshen 授权直接使用 Tianquan-Shoubing 已认证 `sub/tid` 读用户级快照，不需要
  `tianquan-jianshen.session-id/session-version` Principal Attribute；
- 新 AT写回 Cookie；
- 普通业务上游看不到 RT Cookie；
- Gateway重启或切换节点不影响有效 AT/RT。
- `PUBLIC_PROTOCOL`、`IDENTITY_PROTECTED`、`BUSINESS_PROTECTED` 互斥；
  Step-up/UserInfo 校验/恢复 AT 但不调用 Yuheng Tianquan-Jianshen。

### 19.4 Tianquan-Jianshen 与业务服务
...
- RBAC3内部 Snapshot同时要求调用方 Tianquan-Shoubing SERVICE AT和原始 USER Subject Token；SERVICE
...
- 重校验在事务中删除无效激活行、推进 `authVersion`并发布新快照，
  不只修改一个内存“需重选”标志；
- 高风险角色只接受有效 AT的 IdP签名 `acr=STRONG/auth_time`，不读取 Tianquan-Jianshen Session强度；
- 权限撤销可使仍有效 AT立即得到 403；
- 业务服务绕过 Gateway时有效 AT + 权限可访问；
- 绕过 Gateway时过期 AT返回 401且不使用 RT；
- Tianquan-Shoubing Admin作为目标服务执行第二次 AT和 RBAC3校验。
...
tianquan-jianshen yuheng SessionVerifier/verifySession
x-egon-yuheng-session-id
x-egon-yuheng-session-version
rbac3_session runtime repository
rbac3_session_active_role runtime repository
rbac3_refresh_token runtime repository
Tianquan-Jianshen USER AuthenticationFacade
Tianquan-Jianshen USER PasswordIdentityAuthenticator
Tianquan-Jianshen USER JwtTokenService
Tianquan-Jianshen USER RefreshFacade
Tianquan-Jianshen USER StepUpFacade
Tianquan-Jianshen admin.auth package after bootstrap migration
Tianquan-Jianshen SessionStrengthRepository
Tianquan-Jianshen ServiceCredentialPO/ServiceCredential repository
Tianquan-Jianshen JwtKeyRingService/Tianquan-Jianshen JWT private-key configuration
Rbac3JwtConfiguration/TIANQUAN_JIANSHEN_JWT_PRIVATE_KEY_FILE/TIANQUAN_JIANSHEN_JWT_PUBLIC_KEY_FILE
Tianquan-Jianshen USER or SERVICE token issuance endpoint
Tianquan-Jianshen Starter authorization.serviceCredentialFile/static Bearer fallback
*_TIANQUAN_JIANSHEN_SERVICE_CREDENTIAL_FILE/*.service.jwt pre-signed token flow
...
Tianquan-Jianshen xingyuan bootstrap password/PasswordEncoder/UserCredentialPO
Tianquan-Jianshen user username/normalizedUsername/displayName/lockedUntil fields
...
Tianquan-Jianshen user authorization publication guard
Rbac3AuthorizationContext
useRbac3Authorization
AuthenticationContext(acr, authTime)
Yuheng IDENTITY_PROTECTED route policy
...
Tianquan-Jianshen bootstrap identitySub binding
minimal rbac3_user tenantId + identitySub authorization subject
Tianquan-Shoubing IdentityUser and Password Credential as sole user core/credential authority
Tianquan-Shoubing Admin TokenSigner + Tianquan-Shoubing Starter verifier + Yuheng Adapter delegation
```

`ServicePrincipal/ServicePermission` 可以作为 RBAC3机器授权事实保留，但
`ServiceCredential`、RBAC3自有签名密钥和任何 USER/SERVICE Token签发链必须删除；真实
SERVICE调用者统一改为 Tianquan-Shoubing Client Credentials。静态扫描必须区分这两个边界，不能把
...
- Tianquan-Shoubing Contract/Core/Starter/Yuheng Adapter/Admin 模块测试与编译；
- Maven依赖树与源码扫描验证 Yuheng Adapter只经 Starter复用 JWT验证、RPC Contract不直接
  声明或实现 JOSE安全能力；
- Tianquan-Jianshen Contract/Core/Starter/Admin 模块测试与编译；
- RBAC3内部 Snapshot双凭据负向测试：缺少/错误 SERVICE Scope、跨租户、跨系统绑定、
  伪造 USER Subject Token均必须拒绝，日志不得出现原始 Token；
- Yuheng Security Chain、Route Policy和 Engine测试；
...
3. USER AT、RT、IdentityPrincipal、Yuheng Principal、Tianquan-Jianshen Snapshot、Cache Key中都没有 Session；
4. Tianquan-Shoubing SSO Session、Tianquan-Jianshen Session、Session级 Active Role、Session Fence和 Session Version均从生产运行链删除，但 RBAC3用户级角色激活保留；
5. Gateway是所有外部请求的唯一入口，USER AT 仅由 Tianquan-Shoubing Yuheng Adapter
   认证，Tianquan-Jianshen Yuheng Adapter 只做业务权限决策；
6. 登录、Refresh、撤销、退出为精确公开 Route，但各自严格校验密码、RT或 Client Assertion；
7. Tianquan-Shoubing Admin使用同一 USER AT/RT登录并受 Yuheng + IdP两层 RBAC3保护；
8. Gateway只对 AT缺失或过期自动刷新；非法 AT、403和上游 401不触发刷新；
9. 保护业务 Route由 Gateway和目标服务都校验同一个 USER AT并执行各自权限；保护身份
   Route由 Gateway和 IdP都校验同一个 USER AT但不调用 Tianquan-Jianshen；
...
12. RBAC3快照键固定为 `(systemCode, tenantId, identitySub)`；
    服务侧内部快照读取同时验证 Tianquan-Shoubing SERVICE AT、原始 USER Subject Token、同租户和
    SERVICE到系统绑定；Gateway直读投影执行等价身份键和版本保护；
13. 跨 Admin Web依靠 Yuheng JWT Cookie免登录，不依赖 Tianquan-Shoubing SSO Cookie或授权码；
14. 无业务域权限返回 403，不伪装成认证失败；
15. 绕过 Gateway时，目标服务仍校验 AT和 Tianquan-Jianshen，过期 AT直接 401；
16. Tianquan-Shoubing SERVICE Access Token/Admission能力不进入人员 SSO且未被误删；RBAC3只保留
    `ServicePrincipal/ServicePermission`授权事实，机器 Credential、签名私钥和发 Token能力已删除；
17. 未实现新的账号冻结功能，现有账号状态行为得到保留；
18. 所有旧 USER Token、RT、SSO Cookie、Authorization Code和 Tianquan-Jianshen Session在切换时失效；
...
25. Tianquan-Shoubing `identity_user.token_version`、RBAC3人员/机器凭据、Tianquan-Jianshen USER/SERVICE发 Token、
    自有签名密钥和Refresh链均已物理删除，不保留旧数据或兼容分支；IdP签名密钥继续保留；
26. Yuheng 三类 Route 互斥；Step-up/UserInfo 要求身份但不要求任何业务域权限；
27. Tianquan-Jianshen React SDK 不保存/返回/解析人员 Token，不做前端 Refresh，但保留无 Session
    的角色激活和细粒度权限 Guard。
28. Tianquan-Jianshen Session 页面、API、权限字符和资源种子均删除；所有人员退出均调用 Tianquan-Shoubing；
29. 首管密码只存 Tianquan-Shoubing；Tianquan-Jianshen Bootstrap 以 Tianquan-Shoubing `identitySub` 创建最小授权用户和授权事实，
    不自动激活生产高风险角色。
30. RBAC3用户是 `(tenantId, identitySub)` 唯一的最小授权主体，只保留内部 ID、授权成员
    状态、`authVersion`、行版本和审计字段；密码、登录安全状态、用户名、展示名及其他核心
    用户资料只存在 Tianquan-Shoubing。
31. `rbac3_external_identity`删除；未来外部身份统一由 IdP归一为稳定 `identitySub`，Tianquan-Jianshen
    不建设第二套身份目录。
32. IdP当前保留既有基础用户与凭据能力作为未来用户中台承载点，但本期没有新增完整用户中台
    字段、接口、页面或业务流程。
33. Tianquan-Shoubing Admin是平台 AT/RT唯一签发者；Tianquan-Shoubing Starter是共享验签/Claim Policy实现；Yuheng
...
- **Adapter**：继续由 `tianquan-shoubing-gateway-adapter` 和 Tianquan-Jianshen Starter把统一身份/RBAC3契约适配到
  Yuheng SPI与 Spring Security，不让 Yuheng Core依赖 Tianquan-Shoubing/RBAC3领域实现。
...
与端口边界，不引入 Factory/Strategy或 `component-jwt`：当前唯一变化点是 Tianquan-Shoubing Claim Policy，
...
- Session相关能力全部移除；
- Tianquan-Jianshen 角色激活能力保留，但改为用户/应用级授权状态，未激活角色不进入权限上下文；
...
- 账号冻结仅保留扩展意图，本期不实现。
- RBAC3不再保存密码或用户核心资料，只保存以 Tianquan-Shoubing `identitySub`绑定的最小授权主体；
- IdP是用户核心信息与凭据权威，未来可承载用户中台，但本期不开发完整用户中台；
- JWT职责固定为 Tianquan-Shoubing Admin签发、Tianquan-Shoubing Starter统一验签、Yuheng Adapter适配复用，RPC
...
Tianquan-Shoubing/Yuheng/Tianquan-Jianshen/Tianshu 的真实运行态、干净数据库/Redis 切换和强制退出后的五分钟窗口仍由用户
