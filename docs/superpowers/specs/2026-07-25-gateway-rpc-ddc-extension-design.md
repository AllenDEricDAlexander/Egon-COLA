# GWS-02 Yuheng 所需 RPC/Tianshu 扩展 Spec
...
本 Spec 只补齐 Yuheng 依赖 RPC/Tianshu 时缺少的公共契约。路由、限流、负载均衡、
生产 Yuheng Handler 和 Admin 业务模型仍属于 Yuheng，不下沉到 RPC/Tianshu。

当前事实：

- Tianshu 只有 `RPC_PROVIDER`、`INTERNAL_GATEWAY`；
- Tianshu 配置运行时固定使用字段绑定 Apply；
- Tianshu 配置只在启动/租约恢复时全量拉取，运行期依赖 full-value Pub/Sub；
- Tianshu HMAC OpenAPI 没有稳定的配置管理机器接口；
- RPC Provider 已校验 Contract，但没有公开只读 Contract Catalog；
- RPC Provider 注册 Metadata 只有框架字段；
- RPC Test 中的 Mock Yuheng 不是生产 API。
...
4. 固化 Yuheng 可依赖的 Metadata/Error 契约；
5. 不提供生产 Yuheng 实现。

## 3. 明确不做

- 不实现 Nacos、Dubbo 或兼容迁移；
- 不把 HTTP Provider 生命周期放入 Yuheng Starter；
- 不把 Provider Directory、负载均衡或限流放入 Tianshu；
- 不把动态 gRPC Handler、Channel Cache 或 Forwarder 放入 RPC Starter；
- 不改变 RPC Consumer“只发现唯一 `INTERNAL_GATEWAY`”的规则；
- 不实现 Tianshu 多 Admin、Redis Sentinel/Cluster、选主或分布式锁；
- 不增加 RPC Streaming；
- 不直接让 Yuheng Admin 调用 Tianshu Repository/JPA Entity。
...
- 继续复用 Tianshu 当前服务注册 Redis Key 结构；
...
`DdcFieldBindingService::apply`。Yuheng Rule Snapshot 只有完成 Schema 校验、
...
4. 重复注册同一精确 Key 或同一前缀时应用启动失败；允许前缀嵌套，并始终选择最长
   前缀；
5. 前缀必须以业务命名空间和分隔符结尾，例如 `yuheng.rules.chunk.`，不能注册空前缀
   或直接接管全部 Config Key；
6. 自定义 Applier 抛出异常时：
   - 本地版本和 checksum 不更新；
   - ACK 为 `FAILED`；
...
`DdcRefreshService` 继续负责 Target、Version、Checksum、锁和 ACK；Yuheng 只实现
...
- 间隔必须大于 0；
- Yuheng Engine 生产环境默认开启；
- 测试可以缩短间隔；
- 校准线程不能阻塞 Tianshu 心跳线程。
...
Yuheng Admin 是机器调用方，需要配置 Upsert、发布、任务查询和实例投影。当前
`/api/v1/tianshu/configs` 与 `/api/v1/tianshu/publish-tasks` 是管理端接口，部分响应暴露
...
Host/Port 是 Config Client 自报的管理地址，只能作为 Yuheng Admin 受控查询节点状态
...
Redisson、Tianshu Runtime AutoConfiguration、JPA 或 Admin 实现。Yuheng Admin 不应为了
机器管理调用引入完整 Tianshu Starter。
...
Yuheng Admin 只能依赖该 Client 或等价公共契约。

删除约束：

- 只有 Yuheng 已证明 Config 不再被当前 Active 或仍允许 Retry 的 Release 引用时才能
  删除；
- 删除必须幂等，不存在的 Config 返回成功；
- 请求携带 `expectedVersion`，版本已变化时拒绝；
- 删除操作进入 Tianshu 审计日志并执行与 Upsert 相同的 HMAC、作用域和权限校验；
- 删除 Draft 不能隐式发布新版本，Yuheng 必须先完成新 Activation 发布和保留期检查；
- 删除不向普通 Config Client 下发“空值”或清空字段；Yuheng Engine 由自身 Staging
  Retention 清理已无引用 Chunk；
- 被删除的不可变 Chunk Key 永不复用；
- 禁止通过前缀或通配符批量删除，禁止删除 Yuheng 当前 Active Key。

## 8. Tianshu 配置容量保护

Tianshu Pub/Sub 当前携带完整 Config Value。新增：

```yaml
egon.cola.component.tianshu.admin.config.max-value-bytes: 1048576
```
...
3. 超限在写 DB/Redis 前失败；
4. 默认 1 MiB 是安全上限，不代表 Yuheng Rule Snapshot 目标容量；
5. Yuheng 子 Spec 可以设置更低发布上限；
6. Tianshu 不在本轮实现透明压缩或分片。

若 Yuheng 需要分片，由 Yuheng Rule Activation 定义，Tianshu 仍只管理普通版本化
配置。
...
- `tianshu.*` 与 `egon.internal.*` 保留；
- `egon.rpc.*` 只有框架白名单。

Yuheng Provider 公共 Metadata 使用：
...
- Yuheng Starter 只能读取，不能修改 Provider Method Registry。

RPC Provider Lifecycle、Server Definition 和 Yuheng Starter 使用同一份校验结果，
避免重复扫描产生漂移。
...
4. 合并结果进入 Tianshu 原有 Metadata 校验；
5. Yuheng 可提供独立 Contributor 写入 `yuheng.*`，RPC Component 不依赖
   Yuheng；
6. Contributor 抛异常时 Provider 不启动、不注册半成品租约。

## 12. Yuheng 可依赖的 RPC 协议
...
Yuheng Engine 自己实现上述生产能力，并用 RPC Test Contract 做兼容验证。
...
- Yuheng Admin 对 Tianshu 的 Access Key/Secret 只来自安全配置。
...
- 旧管理 UI/API 可以保留，但 Yuheng 只使用新 Management OpenAPI；
...
1. Yuheng 可以注册和发现 `HTTP_PROVIDER`；
2. Yuheng Rule Applier 真正生效后才产生成功 ACK；
3. Pub/Sub 丢失可在租约不变的情况下周期收敛；
4. Yuheng Admin 不依赖 Tianshu Entity/Repository；
5. Tianshu Management 调用全程 HMAC；
6. RPC Starter 提供不可变 Contract Catalog；
7. Contract Snapshot 不含反射对象且可重建 Descriptor；
8. RPC Provider 可以安全贡献 `yuheng.*` Metadata；
9. RPC/Tianshu 均不包含生产 Yuheng 路由、负载均衡或治理实现；
10. Tianshu 单 Admin/单 Redis 和 RPC 单 Yuheng 约束保持不变。

## 17. 本轮审核项

1. 认可 `HTTP_PROVIDER` 作为 Tianshu 第三个 Service Kind；
...
7. 认可所有 Yuheng 生产数据面能力仍留在 Yuheng Component。
