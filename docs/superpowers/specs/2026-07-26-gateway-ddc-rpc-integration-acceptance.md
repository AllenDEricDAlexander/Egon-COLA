# Yuheng、Tianshu、RPC 联调闭环验收记录

状态：代码实现与默认自动化验证已完成；真实外部拓扑待人工启动验收

执行日期：2026-07-26

实施分支：`codex/yuheng-tianshu-rpc-integration`

关联设计：

- `2026-07-26-yuheng-tianshu-rpc-integration-remediation-design.md`
- `2026-07-26-yuheng-tianshu-rpc-integration-plan-index.md`
- `2026-07-26-integration-01-foundation-contracts.md` ～
  `2026-07-26-integration-07-live-demo-runbook.md`

## 1. 验收结论

本轮已经完成设计中 P0、P1、P2 的代码闭环、组件级测试、默认 Yuheng 集成测试、
可选真实拓扑测试入口、Compose Demo 与中英文 Runbook。Yuheng、Tianshu、RPC 当前在公共状态、
租约、规则发布、HTTP Provider、RPC 故障语义和 Tianshu 管理安全合同上可以正确组合。

本结论严格限定在已经执行的自动化证据内。以下两项没有执行，因此不能标记为真实环境通过：

1. `yuheng-live` Testcontainers/独立 JVM 测试；
...
│   ├── Yuheng-stage and Provider-stage failure classification
│   ├── cross-Yuheng retry only for idempotent methods
│   ├── availability snapshot consistency
│   └── failed Yuheng slot re-registration and lease recovery
├── Tianshu management security
...
|---|---|---|---|
| P0-01 Yuheng→Tianshu 请求无效 | 已闭环 | Yuheng/Tianshu Reactor、发布协调测试 | 真实 PostgreSQL/Redis 发布链路 |
| P0-02 服务状态不一致 | 已闭环 | Tianshu、RPC、Yuheng 状态与选择测试 | 真实进程 TTL 时序 |
| P0-03 Tianshu 容器不可执行 | 已闭环 | thin/exec 构件打包测试 | 实际镜像启动 |
| P0-04 联调 Redis 地址错误 | 已闭环配置与 Fixture | Yuheng 默认套件、Compose render | Testcontainers 随机端口实连 |
| P0-05 HTTP Provider 不可直接消费 | 已闭环 | MVC 5 项、WebFlux 1 项、Provider Runtime 13 项 | 独立 Provider 进程租约循环 |
| P1-01 Yuheng 无发布恢复日志 | 已闭环 | Admin publication journal/recovery 测试 | 真实进程中途强杀 |
| P1-02 draft/published 混用 | 已闭环 | Tianshu publish/pull 一致性测试 | 真实 Redis 投递中断 |
...
| P2 Tianshu 管理安全 | 已闭环 | Tianshu Reactor、HMAC scope/nonce 测试 | 多 Admin 真实共享 nonce |
| P2 Compose/真实测试 | 入口已交付 | Yuheng suite 16/16、Compose 静态渲染 | 完整 Demo 和 live profile |
...
Tianshu test suite；没有连接外部 Redis/PostgreSQL。
...
直接以 RPC 子聚合 POM 执行 `clean verify` 时，本地仓库中同版本 Tianshu Starter 旧构件会先于
当前源码被解析，不能构成正确的跨组件 Reactor。最终证据使用 Components 根 POM 和具体叶子模块，
确保 Tianshu、RPC 当前源码同时进入 Reactor。

### 4.3 Yuheng

```bash
./mvnw -B -ntp -f egon-cola-components/pom.xml \
  -pl egon-cola-component-yuheng/egon-cola-component-yuheng-test/\
egon-cola-component-yuheng-test-suite \
  -am clean verify
```

结果：PASS，27 个 Reactor 模块全部成功，Yuheng suite 16/16。

干净构建额外发现 Yuheng Engine 的主构件曾被 Boot 重打包覆盖，导致下游测试无法编译引用。
现已与 Tianshu Admin 的既有模式对齐：Admin/Engine 主构件为 thin JAR，可执行构件附加
...
同时确认 Yuheng 文档中不存在已经废弃的 `VITE_YUHENG_ADMIN_ACTOR_ID`。

## 5. 验收期间发现并修复的问题

1. RPC Mock Yuheng 的默认服务名仍为旧的 `egon-internal-rpc-yuheng`，与生产 Consumer 的
   `egon-yuheng-rpc` 不一致。修复后 RPC 全量 suite 10/10。
2. Yuheng Admin/Engine 可执行 Boot JAR 覆盖主构件，干净跨模块编译无法消费 Engine 类型。
   修复为 thin 主构件加 `exec` 可执行构件，并同步 Dockerfile；Yuheng 27 模块全量通过。
...
- 真实 PostgreSQL、Tianshu Redis、Rate Redis、Kafka 的组合行为；
...
3. 执行 `yuheng-live`，重点核对双 Engine、Provider 新 lease、rollback/LKG 和分布式限流；
