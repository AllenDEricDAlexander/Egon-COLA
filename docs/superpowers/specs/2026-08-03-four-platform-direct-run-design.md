...
> 状态：基于已批准的统一身份平台架构实施
> 日期：2026-08-03
> 范围：Tianquan-Shoubing、Tianquan-Jianshen、Yuheng、Tianshu 的本机直接打包、启动和全链路验收

## 1. 目标

在保留真实 PostgreSQL、Redis、统一 SSO、多租户授权、Tianshu 注册发现和 Yuheng 路由的前提下，使开发者完成一次安全准备后，可以：
...
- 五个后端进程（Yuheng 包含 Admin 与 Engine）都能产出 `*-exec.jar`，但 Jar 只会读取进程环境；`target/local-unified-xingyuan/env/*.env` 不会被 Spring Boot 自动加载。
- 当前 `.env` 使用 Shell 转义格式，不能作为稳定的 Java Properties 契约直接复用。
- Tianquan-Jianshen、Yuheng、Tianshu Admin Web 的 Vite 默认代理仍指向旧端口 `8080`、`8080`、`18080`，只有统一启动脚本覆盖环境变量时才正确。
- 现有深度验收覆盖 SSO、撤销、授权快照、Tianshu LKG、Yuheng 发布和 MCP，但没有明确证明“脱离启动脚本、仅用 `java -jar`/`npm run dev`”的启动方式。

## 3. 方案

### 3.1 一次准备，随后直接运行

保留 `target/local-unified-xingyuan` 作为唯一开发运行目录。准备流程继续负责：
...
- `.properties` 只改变配置加载方式，不放宽 OAuth、CORS、Tianquan-Jianshen、Tianshu 签名或 Yuheng 身份校验。
...
| Web | 端口 | 默认 `/api` 代理 |
|---|---:|---|
| Tianquan-Shoubing Admin Web | 18121 | `http://127.0.0.1:18120` |
| Tianquan-Jianshen Admin Web | 18131 | `http://127.0.0.1:18130` |
| Yuheng Admin Web | 18141 | `http://127.0.0.1:18140` |
| Tianshu Admin Web | 18152 | `http://127.0.0.1:18150` |
...
后端直接启动顺序固定为 Tianshu、Tianquan-Shoubing、Tianquan-Jianshen、Yuheng Admin、Yuheng Engine。Yuheng 是一个平台但包含控制面与数据面两个 JVM：
...
- Tianquan-Shoubing：登录、用户状态、OAuth Client、JWK、Token/Refresh/Replay/撤销、审计读取；
- Tianquan-Jianshen：租户映射、角色候选与激活、授权快照、撤权与原 Token 恢复、运行状态；
- Tianshu：业务域、环境、应用、命名空间、环境应用绑定、配置发布、服务注册、LKG；
- Yuheng：应用目录、凭据、Yuheng Group、草稿、校验、发布、非法发布保护、Provider 路由、HTTP 与 MCP；
- 跨平台：同一 SSO 多租户、Yuheng 只做身份校验、下游独立 Tianquan-Jianshen 授权、双 Engine 会话/任务、Remote MCP 熔断恢复。
...
- 不把五个后端合并成一个进程；
- 不改变 Yuheng 只做基础身份校验、下游自行授权的既定边界；
