...
存在 SLF4J 时会启用 MDC 传播，`MdcContextCarrier` 使用 `common-trace` 的一个完整 `TraceContext` 捕获和恢复上下文。其他 carrier 实现仅依赖 JDK 的 `ContextCarrier` API。带有 `EgonInstrumentedTask` 标记的任务不会被再次包装；既不会修改动态线程池注册表，也不会使用它进行发现。
