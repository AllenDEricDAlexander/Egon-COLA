...
生命周期，并通过动态配置中心（Tianshu）接入服务租约与发现。

本组件不是 Yuheng 数据面。生产 Yuheng、路由规则、Provider 健康策略、流量治理以及
HTTP/RPC 转发由独立的 [Yuheng 平台](../../egon-cola-xingyuan/egon-cola-yuheng/README.zh-CN.md)
负责。
...
- 通过 `@EgonRpcProvider` 扫描 Provider Bean，管理 unary gRPC Server、可用性门控、
  Tianshu 租约注册、心跳、恢复和精确租约注销。
- Consumer CGLIB Proxy 统一使用 `@EgonRpcReference`：默认通过 Tianshu 发现业务
...
- 可选 Tianshu RPC Adapter，提供 ConfigData、服务注册中心和管理客户端，并支持显式 mTLS
  或开发明文模式，以及按能力隔离的 HMAC 凭据。
- 生成包含依赖文件的 Protobuf Descriptor Snapshot，供 Yuheng Reporting Starter 使用，
...
业务 Consumer
  ...
  └─ @EgonRpcReference(mode=DIRECT, bizCode/appCode) ──> Tianshu RPC 注册中心 ──> RPC_PROVIDER ──> Provider

Provider ── 注册 / 心跳 / 注销 ─┐
Yuheng  ── 注册 / 心跳 / 注销 ─┼─> Tianshu 直连 gRPC 目标 ─> Tianshu Admin ─> Redis
Consumer ── 发现 / 订阅 Yuheng 或 Provider ───────────────────────────────┘
```

Tianshu 目标是本地引导配置，Tianshu 不通过自身被发现。Provider 和 Yuheng 注册信息是 Redis
...
| Provider    | gRPC Server、Bean 分发、可用性、租约生命周期                                     | Yuheng 路由或 Provider 健康探测       |
| Consumer    | Proxy、Channel 生命周期、Deadline、Metadata、Yuheng/Provider Directory 接口、Consumer 候选节点负载均衡 | Yuheng 规则、Yuheng 侧 Provider 路由、业务重试策略 |
| Tianshu Adapter | 配置、注册、管理端口的直连 gRPC Client                                          | Tianshu Admin 持久化和 Redis 实现         |
| Yuheng 集成  | 传输无关的 Yuheng/Provider Directory Port 和 Contract Catalog           | 生产 Yuheng 数据面和控制面              |

Yuheng 业务链路中，Consumer 只发现 `INTERNAL_GATEWAY`，不会查询 `RPC_PROVIDER`，也不会
创建 Provider Channel。`@EgonRpcReference` 的默认模式是 `DIRECT`；需要 Yuheng
...
- 启用 Provider 租约、Yuheng/Provider 发现或 Tianshu ConfigData 时，需要可访问的 Tianshu
  直连 RPC 端点和 Redis。
- 为启用的每项能力配置匹配的最小权限 Tianshu HMAC 凭据；本地开发可显式使用明文，生产应
  配置 mTLS。

## Quick Start

最短的 Spring Boot 使用方式是引入 Tianshu Adapter，准备一个 Protobuf Contract、一个
Provider 应用和一个 Consumer 应用；调用点可以使用直连默认值，也可以显式选择 Yuheng
代理模式。
...
4. 在 Consumer 中使用 `@EgonRpcReference` 注入该接口；Yuheng 代理调用设置
   `mode = RpcReferenceMode.GATEWAY`，直连调用提供 `bizCode` 和 `appCode`。
5. 开启 RPC、对应角色和 Tianshu Registry。若本地使用明文，必须同时显式打开 Tianshu 和业务
   RPC 的明文开关。
...
Consumer 使用同一 Tianshu Registry 作用域，将角色配置替换为：
...
请参阅 [Yuheng、Tianshu 与 RPC 联调 Runbook](../../egon-cola-xingyuan/egon-cola-yuheng/docs/developer-integration.zh-CN.md)。

## Maven Dependency

导入 Components BOM，统一 Starter 和可选 Tianshu Adapter 的版本：
...
使用 Tianshu ConfigData、基于 Registry 的 Provider/Yuheng 发现或 Tianshu 管理 RPC 时依赖
Tianshu Adapter；它会传递引入 Starter 和 Tianshu SDK：
...
`consumer.enabled` 独立创建。Tianshu Adapter 导入 `DdcRpcAutoConfiguration`，仅在对应 Tianshu
能力开启时暴露 Tianshu Client。
...
| `provider.advertised-host`              |    本机 Host | 发布到 Tianshu 的可路由 Host。                       |
| `provider.advertised-port`              |     实际绑定端口 | 发布到 Tianshu 的可路由端口。                          |
...
公共字段。字段注入时就固定模式，默认是 `DIRECT`：Yuheng 不可用时不会改走 Direct，Direct
失败时也不会进入 Yuheng。Direct 模式必须配置 `bizCode`、`appCode`，可选 `env`；Yuheng
模式必须保持这些直连字段为空。只有获取失败和 Provider/Yuheng 阶段 `UNAVAILABLE` 等可用性故障，
...
`appCode` 和可选 `env`。既有 Yuheng 调用点必须增加
...
| 配置项                                     | 说明                                 |
|-----------------------------------------|------------------------------------|
| `identity.env`                          | 未由 Tianshu Adapter 提供身份时使用的进程环境。       |
...
是否安全。Tianshu 传输使用独立的 `egon.cola.component.tianshu.rpc` 配置命名空间，并按
`runtime`、`registry`、`management` 能力使用不同 HMAC 凭据。完整 Tianshu 配置请参阅
[Tianshu README](../../egon-cola-xingyuan/egon-cola-tianshu/README.zh-CN.md)。

### Tianshu RPC 配置

Tianshu Adapter 使用本地配置的直连 gRPC 目标：

| `egon.cola.component.tianshu.rpc` 配置项 |           默认值 | 说明                                |
|-----------------------------------|--------------:|-----------------------------------|
| `target`                          |             无 | 必填目标，例如 `dns:///127.0.0.1:19080`。 |
| `connect-timeout`                 |          `3s` | Tianshu Channel 连接超时。                 |
| `default-timeout`                 |         `10s` | Tianshu 调用默认超时。                       |
| `load-balancing-policy`           | `round_robin` | gRPC Target 负载均衡策略。               |
| `max-inbound-message-size`        |     `4194304` | 最大入站消息字节数。                        |
| `shutdown-timeout`                |          `5s` | Tianshu Channel 关闭等待时间。               |
| `tls.development-plaintext`       |        `true` | Tianshu RPC 本地开发明文默认值。                |
| `auth.enabled`                    |        `true` | 启用 Tianshu RPC HMAC Metadata。         |

Tianshu Registry 路径还需要 `egon.cola.component.tianshu.registry.enabled=true`、基于
Redisson 的 Tianshu Redis Client 和 Registry 凭据。Tianshu ConfigData 生命周期相互独立：
`tianshu.enabled=true` 启用配置客户端；不启用远程配置时也可以单独启用 Registry 租约。
...
`@EgonRpcService` 接口。启用 Tianshu Registry 集成后，Provider 会和业务元数据一起上报
...
该路径需要 `RpcGatewayDirectory` 实现，通常由 Tianshu Adapter 提供。Consumer 按精确的
Yuheng Service/Group/Version 订阅快照，为每个有效 Yuheng 保持一个 gRPC Channel，并在
这些 Channel 之间选择调用目标。

### 5. 显式调用发现到的 Provider

适用于明确绕过 Yuheng 规则的受信任内部调用：
...
该路径需要 `RpcProviderDirectory` 实现，通常由 Tianshu Adapter 提供。它发现 `RPC_PROVIDER`
条目并管理有效 Provider 的 Channel；它不执行 Yuheng 路由、鉴权或流量治理。

### 6. 创建基础设施直连 Client

`RpcDirectClientFactory` 为一个显式配置的 gRPC 目标创建类型化 Proxy，例如 Tianshu 端口。
返回的 `RpcDirectClientHandle` 持有该 Channel，调用方必须负责关闭。它和
`@EgonRpcReference` 不同：后者按 `mode` 从 Tianshu 发现业务 Provider 或 Yuheng。
...
生成包含依赖文件的 `FileDescriptorSet` 和 SHA-256 摘要，供 Yuheng Reporting 和兼容性
检查使用。
...
|-------------|---------------------------|--------------------|--------------|--------------------------------------------|
| Yuheng     | `@EgonRpcReference(mode=GATEWAY)` | `INTERNAL_GATEWAY` | RPC Consumer | 仅同模式获取失败/`UNAVAILABLE` 换候选节点 |
...
普通 Consumer 链路不会直接发现 Provider。Yuheng 的 Provider 选择、健康探测、路由规则和
Provider 负载均衡都属于 Yuheng 平台。

Consumer 会为每个精确发现查询维护一份不可变本地 Snapshot。Tianshu Adapter 负责事件监听和
周期性全量对账；调用热路径只读本地 Snapshot，不在每次调用时拉 Tianshu。`RpcLoadBalancers`
提供 `RANDOM`、`WEIGHTED_RANDOM`、`ROUND_ROBIN`、`SMOOTH_WEIGHTED_ROUND_ROBIN`、
`CONSISTENT_HASH` 和 `LEAST_IN_FLIGHT`。权重来自 Provider 的 `yuheng.weight` Metadata。
...
Metadata、Endpoint 地址、Tianshu 凭据、Map/Object 序列化、Streaming 和第二套泛化 wire Service
...
侧 fixed-delay 调度；Tianshu 只负责租约校验、续期、过期和变更事件，不主动探测 Provider。Consumer
...
| 未携带 Provider 标记的 `UNAVAILABLE`                          | `RPC_YUHENG_UNAVAILABLE`  |
...
Provider Exception Mapper 和 Yuheng 转发下游失败时，必须保留 failure-stage Trailer。

### Yuheng 与 Tianshu 边界

Tianshu Adapter 负责三个能力域的直连 Tianshu RPC Client：ConfigData Runtime、Service Registry 和
Management。Tianshu Registry 提供临时 Provider/Yuheng 租约及实时快照。Yuheng 平台通过自身
Starter 消费 RPC Contract Catalog/Snapshot，并负责生产数据面。路由、规则、健康、安全和
流量治理请参阅 [Yuheng README](../../egon-cola-xingyuan/egon-cola-yuheng/README.zh-CN.md)。

## Extension Points

Starter 将发现和注册封装为 Port，业务应用不需要依赖 Tianshu 实现类型：

| 扩展点                              | 用途                                             |
|----------------------------------|------------------------------------------------|
| `RpcProviderRegistry`            | 提供 Provider 租约注册、心跳和注销。                        |
| `RpcGatewayDirectory`            | 为 Yuheng 模式 Consumer 提供实时 Yuheng Snapshot。   |
...
生产 Yuheng 的路由、鉴权、负载均衡、熔断和限流扩展点不属于本模块，应在 Yuheng 平台实现。
...
│       ├── autoconfigure/      # Tianshu RPC 配置与 Spring 装配
│       ├── client/             # Config、Registry 和 Management Client
│       ├── contract/            # 面向 Tianshu Protobuf 的 Java Contract
│       ├── mapping/             # Protobuf/领域 Mapper 与 Status 映射
│       ├── registry/            # Tianshu RPC Directory 和 Registry Port
...
└── egon-cola-component-rpc-test/
    ...
    └── ...-test-consumer/      # Consumer 进程 Fixture 与 Mock Yuheng 测试
```
...
- **Schema 演进：** 保持生成的 Service/Method Name 与 Protobuf 字段兼容；Descriptor
  SHA-256 变化会影响 Yuheng 接口上报和兼容性检查。
- **安全：** 明文必须显式处于开发模式；生产 Endpoint 应使用 mTLS，并为 Tianshu 各能力使用
  独立 HMAC 凭据。
...
- Streaming RPC 及其 Yuheng Descriptor/Reporting 模型。
- 生产级 Metrics 面板和故障演练自动化；运行时已有有界诊断及 Status/Trailer 合同。
- 依赖具体环境的 Tianshu/Redis 拓扑与 mTLS 校验；它们属于部署侧证据，不由模块单测替代。
...
普通测试使用真实 loopback TCP、测试 Mock Yuheng 和 Direct Provider Fixture，不能证明生产 Tianshu/Redis/Yuheng 拓扑。
可选进程测试需要外部管理的 Redis：
