...
import top.egon.cola.component.tianshu.format.DdcConfigFormatStrategy;
...
/**
 * 将 Tianshu ConfigData 资源加载为动态 Spring 属性源。 Loads a Tianshu ConfigData resource as a dynamic Spring property source.
 */
public final class DdcConfigDataLoader
    ...
    /**
     * 从引导上下文取得 Tianshu 客户端并加载配置数据。 Obtains the Tianshu client from the bootstrap context and loads configuration data.
     *
     * @param context  ConfigData 加载上下文。 ConfigData loading context
     * @param resource 待加载的 Tianshu 资源。 Tianshu resource to load
     ...
     * @throws ConfigDataResourceNotFoundException 必选资源在 Tianshu 中不存在时抛出。 thrown when a required resource is absent from Tianshu
     */
    @Override
     ...
     *
     * @param propertySource Tianshu 动态属性源。 Tianshu dynamic property source
