...
| Engine PUBLIC | 18081 | 外部 HTTP 数据面 |
| Engine INTERNAL | 18082 | 内部 HTTP 数据面 |
| Engine Management | 18083 | Actuator |
| Engine RPC Slot | 19090 | Egon RPC 内部网关 |
| Admin Web | 18090 | React 管理页面 |

`gateway-engine-lkg` 必须持久化；DDC Redis 与分布式限流 Redis 使用不同实例和数据卷。
...
- RPC 发现遵循 `INTERNAL_GATEWAY` 单 Slot，0 个或 2 个均快速失败；
- Provider 只通过 DDC Registry 发现，规则只通过 DDC DB/Redis/PubSub 下发；
- Nacos、Dubbo 与 Nginx 管理不属于该部署；
- Gateway Engine 可以多 HTTP 节点，但 RPC Gateway HA 需后续单独设计；
- TLS、入口负载均衡、Secret Manager、NetworkPolicy 和外部可观测平台由部署平台负责。
