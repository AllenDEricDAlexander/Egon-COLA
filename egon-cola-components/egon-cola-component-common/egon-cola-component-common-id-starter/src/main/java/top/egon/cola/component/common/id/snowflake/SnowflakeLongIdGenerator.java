...
/**
 * Stateful, thread-safe engine for the fixed 41/10/12-bit Snowflake layout.
 ...
 *
 * <p>{@link SnowflakeIdGenerator} binds exactly one engine for production use;
 * a separate instance exists per test fixture or benchmark scenario.</p>
 */
public final class SnowflakeLongIdGenerator implements LongIdGenerator {
    ...
    /**
     * Creates an engine using the system clock and the default rollback tolerance.
     *
     * @param machineId explicitly assigned machine identifier in the range 0 to 1023
     */
    public SnowflakeLongIdGenerator(long machineId) {
        this(machineId, SnowflakeIdGenerator.DEFAULT_MAX_CLOCK_BACKWARD, System::currentTimeMillis);
    }

    /**
     * Creates an engine using the system clock.
     *
     * @param machineId explicitly assigned machine identifier in the range 0 to 1023
     * @param maxClockBackward maximum clock rollback that may be waited out
     */
    public SnowflakeLongIdGenerator(long machineId, Duration maxClockBackward) {
        this(machineId, maxClockBackward, System::currentTimeMillis);
    }

    /**
     * Creates an engine with an injectable wall-clock source.
     *
     * @param machineId explicitly assigned machine identifier in the range 0 to 1023
     * @param maxClockBackward maximum clock rollback that may be waited out
     * @param timeSource wall-clock source used for generation
     */
    public SnowflakeLongIdGenerator(long machineId, Duration maxClockBackward, TimeSource timeSource) {
    ...
    /**
     * Returns the decimal representation of the next positive ID.
     *
