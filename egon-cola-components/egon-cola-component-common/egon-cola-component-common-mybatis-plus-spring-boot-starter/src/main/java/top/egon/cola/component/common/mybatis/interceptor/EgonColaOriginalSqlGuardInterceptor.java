...
public final class EgonColaOriginalSqlGuardInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (statement.getSqlCommandType() == SqlCommandType.UPDATE || statement.getSqlCommandType() == SqlCommandType.DELETE) {
            ...
            if (!allIgnored(shape, properties)) {
                ...
                Set<Object> ids = values(target.predicate(), target, root == null ? "id" : root,
                        shape, sql, statement.getConfiguration(), true, new HashSet<>());
                if (ids == null || ids.isEmpty() || ids.size() > properties.getBatch().getMaxCollectionSize()
                        || ids.stream().anyMatch(value -> !(value instanceof Long id) || id <= 0)) {
                ...
                if (root == null && entity instanceof EgonModel<?> model
                        && (model.getId() == null || ids.size() != 1 || !ids.contains(model.getId()))) {
    ...
    static boolean allIgnored(SqlShapeBO shape, EgonColaMybatisPlusProperties properties) {
        return !shape.scopes().isEmpty() && shape.scopes().stream()
                .allMatch(scope -> properties.getTenantId().ignores(identifier(scope.table().getName())));
    }
    ...
    static SqlShapeBO shape(String sql) {
        try {
            ...
            if (statement instanceof Update update) {
                if (update.getFromItem() != null || update.getJoins() != null && !update.getJoins().isEmpty()
                        || update.getWithItemsList() != null && !update.getWithItemsList().isEmpty()) {
                ...
                if (!(insert.getSelect() instanceof Values)) { throw new IllegalStateException("SQL_SHAPE_UNSUPPORTED"); }
            ...
            if (scopes.isEmpty()) { throw new IllegalStateException("SQL_SHAPE_UNSUPPORTED"); }
    ...
    private static void collect(Select select, List<TableScopeBO> scopes, Set<String> inheritedCtes) {
        ...
        if (select.getWithItemsList() != null) {
            for (WithItem<?> item : select.getWithItemsList()) {
                ...
                if (item.getSelect() == null) { throw new IllegalStateException("SQL_SHAPE_UNSUPPORTED"); }
        ...
        if (select instanceof ParenthesedSelect nested) {
            ...
            if (plain.getJoins() != null) {
                for (Join join : plain.getJoins()) {
                    ...
                    if (!join.isLeft()) {
                        for (Expression on : join.getOnExpressions()) { mandatory = and(mandatory, on); }
                    }
                }
            }
            ...
            if (plain.getJoins() != null) {
                for (Join join : plain.getJoins()) {
                    Expression on = null;
                    for (Expression expression : join.getOnExpressions()) { on = and(on, expression); }
                    from(join.getRightItem(), and(mandatory, on), join.isLeft() ? on : and(mandatory, on),
                            scopeId, tableCount, scopes, ctes);
                    nested(on, scopes, ctes);
                }
            }
            Set<String> aliases = new HashSet<>();
            for (TableScopeBO scope : scopes) {
                if (scope.scopeId() == scopeId && !aliases.add(scope.alias())) { throw new IllegalStateException("SQL_ALIAS_AMBIGUOUS"); }
            }
    ...
    private static void from(FromItem from, Expression predicate, Expression isolation, Statement scopeId, int count,
                             List<TableScopeBO> scopes, Set<String> ctes) {
        if (from instanceof Table table) {
            if (!ctes.contains(identifier(table.getName()))) { scopes.add(new TableScopeBO(table, predicate, isolation, scopeId, count)); }
    ...
    static Set<Object> values(Expression expression, TableScopeBO scope, String column, SqlShapeBO shape,
                               BoundSql sql, Configuration configuration, boolean boundOnly, Set<String> visited) {
        ...
        if (checked instanceof EqualsTo equal) {
            Expression other = matches(equal.getLeftExpression(), scope, column) ? equal.getRightExpression()
                    : matches(equal.getRightExpression(), scope, column) ? equal.getLeftExpression() : null;
            if (other == null) { return null; }
            if (other instanceof Column reference && !boundOnly) {
                String referenceKey = reference.toString();
                if (visited.contains(referenceKey)) { return null; }
            ...
            if (boundOnly && !(unwrap(other) instanceof JdbcParameter)) { return null; }
            Object value = value(other, sql, configuration);
            return value == null ? null : Set.of(value);
        }
        if (checked instanceof InExpression in && !in.isNot() && matches(in.getLeftExpression(), scope, column)
                && in.getRightExpression() instanceof ExpressionList<?> expressions) {
            Set<Object> result = new LinkedHashSet<>();
            for (Object item : expressions) {
                if (!(item instanceof Expression member) || boundOnly && !(unwrap(member) instanceof JdbcParameter)) { return null; }
    ...
    static boolean active(Expression expression, TableScopeBO scope) {
        Expression checked = unwrap(expression);
        if (checked instanceof AndExpression and) { return active(and.getLeftExpression(), scope) || active(and.getRightExpression(), scope); }
        if (checked instanceof OrExpression or) { return active(or.getLeftExpression(), scope) && active(or.getRightExpression(), scope); }
    ...
    static boolean matches(Expression expression, TableScopeBO scope, String column) {
        if (!(unwrap(expression) instanceof Column candidate) || !identifier(candidate.getColumnName()).equals(column)) { return false; }
    ...
    static Object value(Expression expression, BoundSql sql, Configuration configuration) {
        ...
        if (checked instanceof JdbcParameter parameter) {
            ...
            if (index < 0 || index >= sql.getParameterMappings().size()) { throw new IllegalStateException("SQL_BINDING_UNRESOLVED"); }
            String property = sql.getParameterMappings().get(index).getProperty();
            if (sql.hasAdditionalParameter(property)) { return sql.getAdditionalParameter(property); }
            Object argument = sql.getParameterObject();
            if (argument == null) { return null; }
            if (configuration.getTypeHandlerRegistry().hasTypeHandler(argument.getClass())) { return argument; }
            if (argument instanceof Map<?, ?> map && map.containsKey(property)) { return map.get(property); }
            try { return configuration.newMetaObject(argument).getValue(property); }
            catch (RuntimeException failure) { throw new IllegalStateException("SQL_BINDING_UNRESOLVED", failure); }
        }
        ...
        if (checked instanceof LongValue number) { return number.getBigIntegerValue().longValueExact(); }
        if (checked instanceof StringValue string) { return string.getValue(); }
        if (checked == null || checked instanceof NullValue) { return null; }
        return null;
    }

    static String property(Expression expression, BoundSql sql) {
        if (unwrap(expression) instanceof JdbcParameter parameter && parameter.getIndex() != null
                && parameter.getIndex() > 0 && parameter.getIndex() <= sql.getParameterMappings().size()) {
    ...
    static String identifier(String name) {
        if (name == null || name.isBlank() || name.indexOf('`') >= 0) { throw new IllegalStateException("SQL_IDENTIFIER_UNSUPPORTED"); }
        String value = name;
        if (name.startsWith("\"") && name.endsWith("\"")) {
            value = name.substring(1, name.length() - 1);
            if (!value.equals(value.toLowerCase(Locale.ROOT))) { throw new IllegalStateException("SQL_IDENTIFIER_UNSUPPORTED"); }
        }
        if (!value.matches("[a-zA-Z_][a-zA-Z0-9_]{0,62}")) { throw new IllegalStateException("SQL_IDENTIFIER_UNSUPPORTED"); }
    ...
    record TableScopeBO(Table table, Expression predicate, Expression isolation, Statement scopeId, int tableCount) {
        String alias() { return identifier(table.getAlias() == null ? table.getName() : table.getAlias().getName()); }
    }
}
