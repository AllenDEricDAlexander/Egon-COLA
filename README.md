...
# cd .\egon-cola-archetypes 目录 这一步会把包打到本地仓库
cd .\egon-cola-archetypes\
...
[![Java support](https://img.shields.io/badge/Java-21+-339933?logo=OpenJDK&logoColor=white)](https://openjdk.java.net/)
[![Maven Central](https://img.shields.io/maven-central/v/top.egon/egon-cola-component-dto.svg?logo=apache-maven&label=maven%20central)](https://central.sonatype.com/namespace/top.egon)
...
为了能够快速创建满足COLA架构的应用，我们提供了两个`archetype`，位于[`egon-cola-archetypes`目录](egon-cola-archetypes)下：

1. `egon-cola-archetype-service`：用来创建纯后端服务的`archetype`。
2. `egon-cola-archetype-web`：用来创建`adapter`和后端服务一体的`web`应用`archetype`。

# 二、COLA组件

此外，我们还提供了一些非常有用的通用组件，这些组件可以帮助我们提升研发效率。

这些功能组件被收拢在[`egon-cola-components`目录](egon-cola-components)下面。到目前为止，我们已经沉淀了以下组件：

组件名称 | 功能 | 依赖
------ | ---- | ----
`egon-cola-component-dto` | 定义了`DTO`格式，包括分页 |无
`egon-cola-component-exception` | 定义了异常格式，<br>主要有`BizException`和`SysException` |无
`egon-cola-component-statemachine` | 状态机组件 | 无
`egon-cola-component-domain-starter` | `Spring`托管的领域实体组件 | 无
`egon-cola-component-catchlog-starter` | 异常处理和日志组件 | `exception`、`dto`组件
`egon-cola-component-extension-starter` | 扩展点组件 | 无
`egon-cola-component-test-container` | 测试容器组件 | 无
...
## 5.0.0 版本
1. 支持JDK21和SpringBoot 3.5.x
2. 增加egon-cola-archetype-light，支持新的基于package轻量级分层架构
3. 增加egon-cola-component-unittest组件，支持[新的单元测试](https://blog.csdn.net/significantfrank/article/details/137495244)
4. 增强egon-cola-component-test-container组件，支持Junit5的Extension
...
4. 将`egon-cola-archetype-web`中的`controller`改名为`adapter`，为了呼应六边形架构的命名。
