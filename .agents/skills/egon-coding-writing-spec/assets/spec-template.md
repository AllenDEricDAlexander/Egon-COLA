...
Read `references/backend-code-generation.md`. For affected DDL/CRUD scaffolding record the verified generator, profile, DDL/manifest version, logical tables, output paths and exact artifacts; identify generated versus custom files and update/hash conflicts. On native light/web/service, catalog-owned types are generator output, not handwritten classes. A new module is an input from `egon-coding-create-new-module`, not a tree this Spec invents. After a skill-driven generator apply, or a plan that is the last authorized generator action, append the classpath SQL journal defined there. Record current dependency evidence and explicit approval for each introduction. Absent tooling or unapproved missing dependencies blocks the dependent work.
