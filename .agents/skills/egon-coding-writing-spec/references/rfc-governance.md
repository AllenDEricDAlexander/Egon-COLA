...
- New Specs use `Template Version: 5` plus `Complexity`, `Complexity Drivers`, `Change Surface`, `Affected Chapters`, and the complete blocking Manual Check catalog. Version 2, Version 3, and Version 4 Specs remain valid under their original contracts; older historical Specs remain usable when their design is authoritative. Do not rewrite a predecessor only to upgrade its template.
