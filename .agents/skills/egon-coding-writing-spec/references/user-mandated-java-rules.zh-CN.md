...
原文拼写保持不动。执行时，`@qualify` 对应 Spring `@Qualifier`，`convertor` 对应 Egon 已有 `BaseConverter<S,T>` / `BaseForwardConverter<S,T>` 契约，`SpringBoot-JackSon` 对应 Spring Boot 管理的 Jackson 体系。这些是准确落地原文，不是放宽规则。
...
优先 Jakarta 原生注解。电话号码必须写 libphonenumber 解析、默认/允许地区、E.164 规范形式、有效性、非法错误和幂等规范化。业务约束使用自定义注解与 ConstraintValidator；ValidationUtils 仅作通用手工触发。
...
Spec 必须核对构造目标、父类 Builder 及实际生成签名；使用约定的无父类/零字段适配，不向 POJO 添加依赖注入用的 @RequiredArgsConstructor。

所有跨层转换使用 MapStruct/MapStructPlus。每个新增受影响 Converter 必须继承/实现 `egon-cola-component-common-core` 的 `BaseConverter<S,T>` / `BaseForwardConverter<S,T>` 体系，写清泛型源/目标、Mapper 注解、生成实现、Bean 名、空值/默认/枚举/时间/敏感字段和测试。不可逆投影使用现有 Common BaseForwardConverter，两种共享契约都不能表达时才报告缺口；禁止用 Setter、`BeanUtils.copyProperties`、反射、JSON 往返或另造 Converter 抽象绕过。
