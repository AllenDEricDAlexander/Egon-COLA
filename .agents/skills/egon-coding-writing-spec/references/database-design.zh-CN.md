...
存在关系型持久化但模型/关系不变时，写 `Relational model change: No` 和仓库证据。没有关系型持久化时写有证据的 `N/A`。文档、图、键值或事件存储只有在确实增加信息时才使用准确 Mermaid 模型，不能伪装成关系 ER 图。
...
- 关系型模型/关系变化但缺少 ER 图、ER 图遗漏受影响清单表、虚构关系，或与详细设计中的 Key/基数/可选性矛盾；
