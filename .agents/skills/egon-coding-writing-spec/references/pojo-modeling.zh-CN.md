...
> 本文件是 `pojo-modeling.md` 的全中文审核镜像。编写 Spec 第 10 章和第 13 章的 Service 结构时使用。以下名称表示语义职责，不是要求每一行都创建一个类的清单。
...
| PO | Persistent Object | 与表、行或持久化记录对应的持久化表达 | Repository/Mapper 持久化边界 |
| DO | Data Object / Domain Object | 含义依团队而异，存在歧义 | 只有仓库已经明确其含义时才使用 |
| DTO | Data Transfer Object | 在层、模块、进程或服务之间传输的数据 | 不拥有业务规则的边界传输 |
| VO | View Object | 为前端或展示层组织的数据 | API/页面展示输出；仓库用 `VO` 表示 View Object 时，不得再用它缩写 DDD Value Object |
| BO | Business Object | Service 内部计算或编排使用的业务对象 | 只有中间业务计算具有独立语义时使用 |
| Entity | Entity | 具有稳定身份的 ORM 或 DDD 实体 | 承担状态和生命周期；必须说明是持久化实体、领域实体还是二者兼具 |
...
DDD Value Object 是由值相等性和不变量定义的领域概念，应使用 `Money`、`EmailAddress` 等领域名称；不要和表示 View Object 的 `VO` 后缀混淆。

## 仓库优先的分类规则

1. 提议名称前，检查现有后缀、包位置、框架注解、序列化器、Mapper、持久化类型和公开契约。
...
5. DAO、Repository、Gateway、Mapper 是带行为的访问组件，不能混入数据对象清单。
...
实体允许继承，但不强制继承。只有仓库惯例和语义支持真正可替换的子类型，或支持稳定的通用实体基类时才采用。

通用实体基类可以集中仓库实体已经共享的身份、审计时间、租户归属、乐观锁版本或其他生命周期机制。Spec 必须说明：
...
如果继承只是为了少写一些无关字段或工具方法，应拒绝继承。不存在可替换实体关系时，优先使用内嵌值对象或组合协作者。

## 业务 Service 组合原则

具体 Application Service、Domain Service 和 Business Service 默认采用组合与委托。Repository、Policy、Strategy、Validator、Calculator、Gateway 和领域协作者应被显式注入或组装。
