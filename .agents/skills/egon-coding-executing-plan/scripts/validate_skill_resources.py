#!/usr/bin/env python3
"""Validate that bundled Execute Plan skill resources are resolvable."""
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            for match in ROOT_RESOURCE_RE.finditer(line):
                ...
                if not inside_root(root, target):
                    errors.append(
                        f"{document.relative_to(root)}:{line_number}: resource escapes skill root: {relative}"
                    )
                elif not target.is_file():
                    errors.append(
                        f"{document.relative_to(root)}:{line_number}: missing resource: {relative}"
                    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate bundled EGON Execute Plan resources")
    parser.add_argument(
        "--root",
        type=Path,
