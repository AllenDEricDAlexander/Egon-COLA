...
expected Session version
  ...
  -> immutable snapshot
  -> Session version CAS
  -> Redis projection + token reissue
```

The same normalized root set is idempotent. A different request using a stale
Session version fails with a conflict. No partial root activation is committed.
...
Reference JWTs contain identity and version claims, not an embedded permission
list. The PEP validates:

1. one syntactically valid Bearer credential;
2. trusted RS256 issuer, audience, `kid`, signature and time claims;
3. exact Tenant, User, Session, authentication and policy versions;
...
- Sessions: maximum active roots and absolute/idle timeouts.
