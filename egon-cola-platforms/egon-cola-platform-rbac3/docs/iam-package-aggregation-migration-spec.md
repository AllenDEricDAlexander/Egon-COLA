# RBAC3 Admin IAM 领域重命名、API 与管理前端迁移 Spec

## 1. 文档信息与本次决策

| 项目 | 内容 |
| --- | --- |
| 适用模块 | egon-cola-platform-rbac3-admin、egon-cola-platform-rbac3-admin-web |
| 变更类型 | IAM 业务域重命名、Java package 迁移、管理 API 迁移与补全、管理前端同步迁移 |
...
1. 现有分散的 RBAC 管理能力迁入统一的 iam 根包，并按 user、application、resource、permission、organization、position、policy、role 等语义重新命名；
2. 管理 API 统一迁移到 /api/rbac3/v1/iam，补齐主要实体 CRUD 与业务语义接口；
3. RBAC3 Admin Web 同步迁移页面、前端路由、API Client、表单与测试，支持基础 CRUD 和增强的授权配置操作。
...
| resource 中 ApplicationPO 及其应用服务 | admin.iam.application | Application 是 IAM 一级管理对象，不再隐含在 resource 中 |
...
| /api/rbac3/v1/applications | /api/rbac3/v1/iam/applications | 应用管理 |
...
| 追加用户角色 | POST /iam/users/{userId}/roles | AssignUserRolesCommand；支持单个/批量 role 项，保留有效期、原因、工单等字段 |
| 替换人工角色 | PUT /iam/users/{userId}/roles | ReplaceUserRolesCommand；仅以本次管理范围内的人工授权集为 replace 对象，不删除自动/继承来源 |
...
| 用户访问组合视图 | GET /iam/users/{userId}/access-profile | 单次返回用户、组织、岗位、直接角色、自动角色、继承角色、当前激活角色、有效权限；它是 Query View，不新增聚合实体 |
...
实现时只新增一个 Flyway V6 migration：同时完成 5.2 的来源字段/约束调整，并创建这两张表及必要索引。两个关系模型均遵循 TenantScopedPO 的 tenant、版本、审计和状态规范，包含 userId、orgUnitId、有效期与状态；岗位关系额外包含 positionId、primary。服务层负责保证同租户、岗位归属组织一致、幂等重复提交和有效期合法，不以 positionId=null 或虚构 DirectorySnapshot 规避模型。

DirectorySnapshotPO 与 UserPositionSnapshotPO 保留为外部目录投影，只读展示/同步，不与人工管理关系混写。用户 access-profile 可合并展示“目录快照来源”和“人工来源”，并保留来源标记。

### 5.4 Manifest 与人工资源的所有权
...
| ApplicationController | Application 基础 CRUD、状态 |
...
已有类不是机械改名为一个巨型 IamController。原 Application/Domain Service、Repository、Assembler 保持单一职责并按新包归属移动；跨域编排只由业务动作所属的 Application Service 负责。
...
| 页面/详情区域 | 必须支持的能力 |
| --- | --- |
| Tenant、Application、Resource、Permission、Role、Organization、Position、DataRule、FieldRule | 列表/分页、筛选、详情、新增、编辑、归档/状态调整 |
| User | RBAC 成员 CRUD、identitySub 展示、状态；不展示或编辑虚构的 IdP profile 字段 |
| User detail / access profile | 组织、岗位、直接/自动/继承/有效角色、当前激活角色、有效权限的组合视图 |
| User roles | 追加、批量追加、replace、解除、暂停/恢复；页面清楚标出来源和有效期 |
...
所有选择类配置页面使用角色/权限/资源/组织/岗位的受限选择器，不允许继续让管理员通过逗号分隔 id 文本框提交关系。replace 保存调用单个 PUT 接口并在成功后使相关 React Query 缓存失效。

### 7.3 API Client、导航与测试同步

- application.api、role.api、assignment.api、directory.api、constraint.api、tenant.api、roleActivation.api 及其测试改到新的 iam API Client/路径；
...
- 每个 CRUD 页面至少有 API Client 单测和组件交互测试；关键组合页、replace、组织移动、岗位自动角色和 MANIFEST 只读限制补充端到端/集成覆盖。
...
1. 冻结当前 Controller URI、GatewayOperation/资源声明、实体注解、前端调用与测试清单；
2. 新增唯一的 Flyway V6，建立用户组织和人工用户岗位关系；
3. 先完成 Java package/类职责迁移及全仓库 import 更新，再实现/迁移 Controller 与 Command/Query；
4. 实现基础 CRUD、语义接口、所有权校验、授权版本/缓存失效和自动角色重算；
5. 同步更新 GatewayOperation、资源 Manifest、权限声明和前端 IAM feature；
6. 删除旧 URI、旧 Java package、旧 Web route/API 调用，执行全仓库旧路径搜索；
7. 进行后端、前端、迁移和契约回归验证。
...
- identity 改名为 user，constraint 改名为 policy，directory 拆为 organization/position，resource 拆为 application/resource/permission，assignment/activation 并入 role 的子包；
- admin.shared 仍是跨域技术公共包；没有为了目录形式错误搬迁非 IAM 依赖；
- 所有公开 IAM 管理 URI 使用 /api/rbac3/v1/iam；服务间成员解析使用 /internal/v1/iam，Manifest/目录快照的内嵌内部提交使用 /api/rbac3/v1/iam/internal；旧 URI 不可再被 Controller、前端或 Gateway 清单引用；
- API 不暴露 RoleClosure、PermissionResource、RolePermission、DataRuleReference、UserPositionSnapshot 等裸关联表 CRUD。

### 9.2 业务行为

- 十一个主要对象均具备约定的 CRUD、列表/分页和状态操作；
...
- 后端：编译、架构测试、Controller/Service/Repository 测试、Flyway migration 校验、关键授权事务和跨租户/跨应用拒绝测试；
- 前端：TypeScript typecheck、单元/组件测试、构建、关键 CRUD 与增强操作的端到端/集成测试；
...
本 Spec 的重点是把已经存在的 RBAC 管理模型按 IAM 业务语言重新组织，并补齐实际管理页面所需的 CRUD 与语义配置能力；不借此创造另一套账户体系、资源模型或通用规则框架。
