...
public class AuthController {
    ...
    /**
     * 类型 `LogoutView` 位于 `AuthController` 内，是记录类型，用于承载 `Logout View` 相关的职责、状态或契约；调用方通常通过其公开 API、Spring 装配或实现关系使用。
     * Type `LogoutView` is a record inside `AuthController` and carries the responsibility, state, or contract for `Logout View`; callers normally use it through its public API, Spring assembly, or implementation relationship.
     *
     * 语义与用法：将 `LogoutView` 作为 `AuthController` 的职责边界使用，优先依赖其已有构造、接口或 Spring 装配方式。
     * Semantics and usage: use `LogoutView` as the responsibility boundary of `AuthController`, following its existing construction, interface, or Spring-assembly mechanism.
     *
     ...
     */
    public record LogoutView(/**
 * 字段 `success` 表示 `LogoutView` 中与 `success` 相关的状态、依赖、配置或结果（声明类型 `boolean`）；其生命周期和取值含义由声明类型及所属对象共同确定。
 * Field `success` stores the `success`-related state, dependency, configuration, or result of `LogoutView` (declared type `boolean`); its lifecycle and value semantics are defined by its declared type and owning object.
 *
 * 含义与用法：读取、传递或更新 `success` 时应保持 `LogoutView` 的生命周期、不可变性和线程安全约束。
 * Meaning and usage: when reading, passing, or updating `success`, preserve `LogoutView`'s lifecycle, immutability, and thread-safety constraints.
 */ boolean success, /**
 * 字段 `stateChanged` 表示 `LogoutView` 中与 `state Changed` 相关的状态、依赖、配置或结果（声明类型 `boolean`）；其生命周期和取值含义由声明类型及所属对象共同确定。
 * Field `stateChanged` stores the `state Changed`-related state, dependency, configuration, or result of `LogoutView` (declared type `boolean`); its lifecycle and value semantics are defined by its declared type and owning object.
 *
 * 含义与用法：读取、传递或更新 `stateChanged` 时应保持 `LogoutView` 的生命周期、不可变性和线程安全约束。
 * Meaning and usage: when reading, passing, or updating `stateChanged`, preserve `LogoutView`'s lifecycle, immutability, and thread-safety constraints.
