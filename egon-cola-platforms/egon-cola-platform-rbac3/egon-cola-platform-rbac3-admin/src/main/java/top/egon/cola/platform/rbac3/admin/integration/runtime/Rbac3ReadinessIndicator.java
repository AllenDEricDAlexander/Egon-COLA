...
public final class Rbac3ReadinessIndicator implements HealthIndicator {
    ...
    /**
     * 字段 `applicationChecks` 表示 `Rbac3ReadinessIndicator` 中与 `application Checks` 相关的状态、依赖、配置或结果（声明类型 `List&lt;ReadinessCheck&gt;`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `applicationChecks` stores the `application Checks`-related state, dependency, configuration, or result of `Rbac3ReadinessIndicator` (declared type `List&lt;ReadinessCheck&gt;`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
    ...
    /**
     * 类型 `ReadinessCheck` 位于 `Rbac3ReadinessIndicator` 内，是记录类型，用于承载 `Readiness Check` 相关的职责、状态或契约；调用方通常通过其公开 API、Spring 装配或实现关系使用。
     * Type `ReadinessCheck` is a record inside `Rbac3ReadinessIndicator` and carries the responsibility, state, or contract for `Readiness Check`; callers normally use it through its public API, Spring assembly, or implementation relationship.
     *
     * 语义与用法：将 `ReadinessCheck` 作为 `Rbac3ReadinessIndicator` 的职责边界使用，优先依赖其已有构造、接口或 Spring 装配方式。
     * Semantics and usage: use `ReadinessCheck` as the responsibility boundary of `Rbac3ReadinessIndicator`, following its existing construction, interface, or Spring-assembly mechanism.
     *
 ...
 * 字段 `name` 表示 `ReadinessCheck` 中与 `name` 相关的状态、依赖、配置或结果（声明类型 `String`）；其生命周期和取值含义由声明类型及所属对象共同确定。
 * Field `name` stores the `name`-related state, dependency, configuration, or result of `ReadinessCheck` (declared type `String`); its lifecycle and value semantics are defined by its declared type and owning object.
 *
 * 含义与用法：读取、传递或更新 `name` 时应保持 `ReadinessCheck` 的生命周期、不可变性和线程安全约束。
 * Meaning and usage: when reading, passing, or updating `name`, preserve `ReadinessCheck`'s lifecycle, immutability, and thread-safety constraints.
 */ String name, /**
 * 字段 `ready` 表示 `ReadinessCheck` 中与 `ready` 相关的状态、依赖、配置或结果（声明类型 `BooleanSupplier`）；其生命周期和取值含义由声明类型及所属对象共同确定。
 * Field `ready` stores the `ready`-related state, dependency, configuration, or result of `ReadinessCheck` (declared type `BooleanSupplier`); its lifecycle and value semantics are defined by its declared type and owning object.
 *
 * 含义与用法：读取、传递或更新 `ready` 时应保持 `ReadinessCheck` 的生命周期、不可变性和线程安全约束。
 * Meaning and usage: when reading, passing, or updating `ready`, preserve `ReadinessCheck`'s lifecycle, immutability, and thread-safety constraints.
 */ BooleanSupplier ready) {

        /**
         * 构造器 `ReadinessCheck` 用于创建并初始化 `ReadinessCheck` 实例，建立该类型后续方法所依赖的状态和不变量。
         * Constructor `ReadinessCheck` creates and initializes `ReadinessCheck`, establishing the state and invariants required by subsequent operations.
         *
         * 用法：通过 `ReadinessCheck` 的构造入口创建实例，不绕过构造器建立的校验和初始化约束。
         * Usage: create the instance through `ReadinessCheck`'s constructor entry point and do not bypass the validation and initialization constraints established there.
         *
