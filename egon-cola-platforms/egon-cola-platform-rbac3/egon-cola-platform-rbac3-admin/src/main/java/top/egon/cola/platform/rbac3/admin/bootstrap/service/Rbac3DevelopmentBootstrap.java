...
public class Rbac3DevelopmentBootstrap implements ApplicationRunner {
    ...
    /**
     * 字段 `bootstrap` 表示 `Rbac3DevelopmentBootstrap` 中与 `bootstrap` 相关的状态、依赖、配置或结果（声明类型 `DevelopmentBootstrapPort`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `bootstrap` stores the `bootstrap`-related state, dependency, configuration, or result of `Rbac3DevelopmentBootstrap` (declared type `DevelopmentBootstrapPort`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
