...
public class Rbac3RuntimeProjectionRecovery implements
    ...
    /**
     * 字段 `runtimeStore` 表示 `Rbac3RuntimeProjectionRecovery` 中与 `runtime Store` 相关的状态、依赖、配置或结果（声明类型 `RedisAuthorizationRuntimeRepository`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `runtimeStore` stores the `runtime Store`-related state, dependency, configuration, or result of `Rbac3RuntimeProjectionRecovery` (declared type `RedisAuthorizationRuntimeRepository`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
    ...
    private void publishMinimum(SessionPO session, Instant generatedAt) {
        ...
        runtimeStore.publish(new PublishCommandDTO(
