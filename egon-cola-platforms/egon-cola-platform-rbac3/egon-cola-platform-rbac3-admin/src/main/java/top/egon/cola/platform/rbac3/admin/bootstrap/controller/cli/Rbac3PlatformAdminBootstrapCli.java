...
public final class Rbac3PlatformAdminBootstrapCli {
    ...
    /**
     * 字段 `bootstrapPort` 表示 `Rbac3PlatformAdminBootstrapCli` 中与 `bootstrap Port` 相关的状态、依赖、配置或结果（声明类型 `PlatformAdminBootstrapService`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `bootstrapPort` stores the `bootstrap Port`-related state, dependency, configuration, or result of `Rbac3PlatformAdminBootstrapCli` (declared type `PlatformAdminBootstrapService`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
