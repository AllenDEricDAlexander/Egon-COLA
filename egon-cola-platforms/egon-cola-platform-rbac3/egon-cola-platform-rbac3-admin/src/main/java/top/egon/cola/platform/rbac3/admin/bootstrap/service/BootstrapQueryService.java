...
public final class BootstrapQueryService {
    ...
    /**
     * 字段 `snapshotSource` 表示 `BootstrapQueryService` 中与 `snapshot Source` 相关的状态、依赖、配置或结果（声明类型 `BootstrapSnapshotRepository`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `snapshotSource` stores the `snapshot Source`-related state, dependency, configuration, or result of `BootstrapQueryService` (declared type `BootstrapSnapshotRepository`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
     ...
     */
    private final BootstrapSnapshotRepository snapshotSource;
     ...
     */
    public BootstrapQueryService(BootstrapSnapshotRepository snapshotSource) {
