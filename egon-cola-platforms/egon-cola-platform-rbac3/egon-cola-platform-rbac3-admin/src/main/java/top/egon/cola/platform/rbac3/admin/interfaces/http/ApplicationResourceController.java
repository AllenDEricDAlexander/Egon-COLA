...
public class ApplicationResourceController {
    ...
    /**
     * 类型 `ArchiveResourceRequest` 位于 `ApplicationResourceController` 内，是记录类型，用于承载 `Archive Resource Request` 相关的职责、状态或契约；调用方通常通过其公开 API、Spring 装配或实现关系使用。
     * Type `ArchiveResourceRequest` is a record inside `ApplicationResourceController` and carries the responsibility, state, or contract for `Archive Resource Request`; callers normally use it through its public API, Spring assembly, or implementation relationship.
     *
     * 语义与用法：将 `ArchiveResourceRequest` 作为 `ApplicationResourceController` 的职责边界使用，优先依赖其已有构造、接口或 Spring 装配方式。
     * Semantics and usage: use `ArchiveResourceRequest` as the responsibility boundary of `ApplicationResourceController`, following its existing construction, interface, or Spring-assembly mechanism.
     *
 ...
 * 字段 `expectedVersion` 表示 `ArchiveResourceRequest` 中与 `expected Version` 相关的状态、依赖、配置或结果（声明类型 `long`）；其生命周期和取值含义由声明类型及所属对象共同确定。
 * Field `expectedVersion` stores the `expected Version`-related state, dependency, configuration, or result of `ArchiveResourceRequest` (declared type `long`); its lifecycle and value semantics are defined by its declared type and owning object.
 *
 * 含义与用法：读取、传递或更新 `expectedVersion` 时应保持 `ArchiveResourceRequest` 的生命周期、不可变性和线程安全约束。
 * Meaning and usage: when reading, passing, or updating `expectedVersion`, preserve `ArchiveResourceRequest`'s lifecycle, immutability, and thread-safety constraints.
