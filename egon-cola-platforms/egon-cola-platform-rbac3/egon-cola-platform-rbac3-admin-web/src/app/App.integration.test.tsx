import {type BootstrapView, type Rbac3Client, Rbac3Provider} from '@egon-cola/rbac3-react-sdk'
...
const bootstrap = (permissions: readonly string[]): BootstrapView => ({
    user: {id: '7', tenantId: '9', identitySub: 'mario', status: 'ACTIVE'}, permissions,
  fieldPolicies: {}, activeRoleContexts: [], apps: [], menus: [], routes: [], actions: [],
    defaultApplicationCode: null, defaultRoute: null, authVersion: 1, policyVersion: 3,
})

const wrapper = (permissions: readonly string[], path: string) => ({ children }: { readonly children: React.ReactNode }) => {
    const sdk = {getBootstrap: async () => bootstrap(permissions)} as unknown as Rbac3Client
