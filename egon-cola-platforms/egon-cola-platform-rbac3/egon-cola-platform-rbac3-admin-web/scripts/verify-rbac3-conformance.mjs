...
const assertSharedVersion = async () => {
  const consumerRoots = [
    packageRoot,
    join(platformsRoot, 'egon-cola-platform-dynamic-config-center/egon-cola-platform-dynamic-config-center-admin-web'),
    join(platformsRoot, 'egon-cola-platform-idp/egon-cola-platform-idp-admin-web'),
    join(platformsRoot, 'egon-cola-platform-gateway/egon-cola-platform-gateway-admin-web'),
  ]
  for (const root of consumerRoots) {
    const manifest = JSON.parse(await readFile(join(root, 'package.json'), 'utf8'))
    if (manifest.dependencies?.['@egon-cola/admin-web-shared'] !== '^0.2.0') {
      throw new Error(`shared dependency is not ^0.2.0 (${root})`)
    }
    const lockPath = root === packageRoot
      ? join(platformsRoot, 'egon-cola-platform-rbac3/package-lock.json')
