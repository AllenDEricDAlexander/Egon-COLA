...
ESLint 与生产构建。真实拓扑门禁由进程 Harness 启动真实 DDC、Admin、两个 API_RPC 和两个 MCP 副本、
...
- DDC 的 `biz/env/appCode/namespace` 相同，appCode 为 `ge`；每个副本使用唯一 Config Client/Registry/Node ID。角色由制品固定，不增加 mode 开关。
- `ddc-rpc-credentials.yml` 显式保留 Runtime、Registry、Management 三类现有凭据，并增加 MCP Runtime/Registry。Spring 的高优先级 list 会整体替换，因此不能仅配置下标3/4。示例 scope 保留原通配行为；生产按实际 Provider 访问需求收紧。
- API/MCP 各自填写 IdP Resource ID/URI，不能复用进程身份。共享业务 MCP Server Resource、原有 Token/audience/权限契约不变。示例占位符必须替换为实际已登记身份；本目录不部署 IdP。
- MCP Session/Subscription 使用共享 Redis；Task/Approval 使用现有 gateway_admin 表，Flyway 仍由 Admin 负责。API 没有 MCP 数据库配置。两个 MCP 副本共享 `GATEWAY_MCP_ARTIFACT_DIRECTORY`，启动前由操作者准备 UID/GID10001 可访问的现有目录；不自动 chown 或清除用户文件。
- 四个 Engine 使用四个独立 LKG 卷。先启动 DDC/Admin/Provider，再启动 Engine，发布已有合法规则后执行 `./scripts/wait-ready.sh --engines`；Admin 必须看到两个角色全部在线实例的相同 Release/Version/Checksum 和 ACK_SUCCESS。
...
- 当前静态渲染/单元测试不等于 IdP 联调、mTLS 握手、浏览器或真实 HA 验收。生产上线前必须完成相应运行验收。
