...
const wrapper = ({ children }: PropsWithChildren) => {
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    ...
    getAbout: async () => ({
      ...
      permissions: ['system:application:read', 'system:resource:read', 'system:field-definition:read', 'system:permission:read', 'system:resource-permission:read', 'system:resource-permission:manage'],
  ...
  const api: FeatureApiClient = {
    request: async <T,>(path: string) => {
      ...
      if (path.includes('/resources')) {
        return [{ resourceId: '81', applicationId: '71', resourceType: 'ROUTE', resourceCode: 'orders.list', resourceName: '订单列表', status: 'STALE', version: 4, suggestedPermissionCode: 'orders:read' }] as T
      }
      ...
      if (path.includes('/permission-mapping')) {
        return { resourceId: '81', applicationId: '71', resourceCode: 'orders.list', resourceType: 'ROUTE', suggestedPermissionCode: 'orders:read', actualPermissionId: '91', actualPermissionCode: 'orders:read', mappingStatus: 'CONFIGURED', resourceVersion: 4, activeRoleCount: 0, changed: false, authVersion: 1, policyVersion: 1 } as T
      }
...
describe('application pages', () => {
  ...
  it('opens the resource mapping workflow from the catalog', async () => {
    render(<ResourceCatalogPage applicationId="71" />, { wrapper })
