package top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.repository.jdbc;
...
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.repository.ParticipationRepository;
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.domain.vo.ParticipationRecordVO;
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.domain.vo.ParticipationFactVO;
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.domain.vo.PriorActionRuleVO;
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.domain.vo.AppendResultVO;
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.policy.participation.domain.dto.ConflictQueryDTO;

/**
 * 类型 `PostgresqlParticipationRepository` 位于当前包内，是类型，用于承载 `Postgresql Participation Store` 相关的职责、状态或契约；调用方通常通过其公开 API、Spring 装配或实现关系使用。
 * Type `PostgresqlParticipationRepository` is a type in its package and carries the responsibility, state, or contract for `Postgresql Participation Store`; callers normally use it through its public API, Spring assembly, or implementation relationship.
 * Serializes same-object checks and append through a PostgreSQL transaction lock.
 */
@Repository
public class PostgresqlParticipationRepository
        implements ParticipationRepository {

    /**
     * 字段 `entityManager` 表示 `PostgresqlParticipationRepository` 中与 `entity Manager` 相关的状态、依赖、配置或结果（声明类型 `EntityManager`）；其生命周期和取值含义由声明类型及所属对象共同确定。
     * Field `entityManager` stores the `entity Manager`-related state, dependency, configuration, or result of `PostgresqlParticipationRepository` (declared type `EntityManager`); its lifecycle and value semantics are defined by its declared type and owning object.
     *
     * 含义与用法：读取、传递或更新 `entityManager` 时应保持 `PostgresqlParticipationRepository` 的生命周期、不可变性和线程安全约束。
     * Meaning and usage: when reading, passing, or updating `entityManager`, preserve `PostgresqlParticipationRepository`'s lifecycle, immutability, and thread-safety constraints.
     */
     ...
     *
     * 含义与用法：读取、传递或更新 `databaseClock` 时应保持 `PostgresqlParticipationRepository` 的生命周期、不可变性和线程安全约束。
     * Meaning and usage: when reading, passing, or updating `databaseClock`, preserve `PostgresqlParticipationRepository`'s lifecycle, immutability, and thread-safety constraints.
     */
    private final DatabaseClock databaseClock;

    /**
     * 构造器 `PostgresqlParticipationRepository` 用于创建并初始化 `PostgresqlParticipationRepository` 实例，建立该类型后续方法所依赖的状态和不变量。
     * Constructor `PostgresqlParticipationRepository` creates and initializes `PostgresqlParticipationRepository`, establishing the state and invariants required by subsequent operations.
     *
     * 用法：通过 `PostgresqlParticipationRepository` 的构造入口创建实例，不绕过构造器建立的校验和初始化约束。
    ...
    /**
     * 方法 `appendAtomically` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `append Atomically` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `appendAtomically` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `append Atomically` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
    ...
    @Override
    @Transactional
    public AppendResultVO appendAtomically(
            ParticipationRecordVO record,
            List<PriorActionRuleVO> rules) {
        lock(record);
        BusinessParticipationPO existing = existing(record);
        if (existing != null) {
        ...
        entityManager.flush();
        return new AppendResultVO(true, id.toString(), List.of());
    }

    /**
     * 方法 `find` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `find` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `find` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `find` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
    ...
    public List<ParticipationFactVO> find(
    ...
    /**
     * 方法 `lock` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `lock` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `lock` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `lock` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
     ...
     * @param record 输入参数 `record`，用于确定本次操作的范围或内容；input value used to determine the operation's scope or content.
     */
    private void lock(ParticipationRecordVO record) {
    ...
    /**
     * 方法 `existing` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `existing` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `existing` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `existing` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
     ...
     * @return 操作产生的结果，其具体语义由返回类型和所属 API 定义；the result of the operation, whose exact semantics are defined by the return type and owning API.
     */
    private BusinessParticipationPO existing(
            ParticipationRecordVO record) {
        return entityManager.createQuery("""
                        select p from BusinessParticipationEntity p
                         where p.tenantId = :tenantId
    ...
    /**
     * 方法 `objectFacts` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `object Facts` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `objectFacts` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `object Facts` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
     ...
     * @return 操作产生的结果，其具体语义由返回类型和所属 API 定义；the result of the operation, whose exact semantics are defined by the return type and owning API.
     */
    private List<BusinessParticipationPO> objectFacts(
            ParticipationRecordVO record) {
        return entityManager.createQuery("""
                        select p from BusinessParticipationEntity p
                         where p.tenantId = :tenantId
    ...
    /**
     * 方法 `toFact` 按照 `PostgresqlParticipationRepository` 的职责处理输入，完成 `to Fact` 操作并返回结果或产生声明的副作用；调用方应遵守参数和异常契约。
     * Method `toFact` processes its inputs according to `PostgresqlParticipationRepository`'s responsibility, performs the `to Fact` operation, and returns a result or declared side effect; callers must follow its parameter and exception contract.
     *
     ...
     * @return 操作产生的结果，其具体语义由返回类型和所属 API 定义；the result of the operation, whose exact semantics are defined by the return type and owning API.
     */
    private ParticipationFactVO toFact(
            BusinessParticipationPO entity) {
        return new ParticipationFactVO(
