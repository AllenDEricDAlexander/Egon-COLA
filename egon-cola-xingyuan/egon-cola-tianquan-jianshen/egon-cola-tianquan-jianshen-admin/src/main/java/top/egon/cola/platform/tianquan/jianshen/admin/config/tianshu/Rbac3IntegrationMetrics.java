...
import top.egon.cola.platform.tianquan.jianshen.admin.authorization.runtime.repository.ApplyObserver;
 ...
 * Registers the bounded Tianquan-Jianshen Tianshu and Gateway integration metrics.
 */
public final class Rbac3IntegrationMetrics
    ...
    public Rbac3IntegrationMetrics(
        ...
        Gauge.builder("tianquan_jianshen_tianshu_config_ready", this, Rbac3IntegrationMetrics::ready)
                .register(registry);
    ...
    public void recordApply(String key, String status) {
        if (!AtomicRbac3RuntimePolicy.CONFIG_KEYS.contains(key)) {
            throw new IllegalArgumentException("unknown Tianquan-Jianshen metric key");
        }
        if (!APPLY_STATUSES.contains(status)) {
            throw new IllegalArgumentException("unknown Tianquan-Jianshen metric status");
        }
        Counter.builder("tianquan_jianshen_tianshu_config_apply_total")
                .tag("key", key)
                .tag("status", status)
                .register(registry)
                .increment();
    ...
    private void registerVersionGauge(String key) {
        Gauge.builder("tianquan_jianshen_tianshu_config_snapshot_version", policy,
