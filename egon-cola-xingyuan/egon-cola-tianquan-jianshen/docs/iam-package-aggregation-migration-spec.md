# Tianquan-Jianshen Admin IAM 领域重命名、API 与管理前端迁移 Spec（含 Tianshu Business/Application 边界）

## 1. 文档信息与本次决策

| 项目 | 内容 |
| --- | --- |
| 适用模块 | egon-cola-tianquan-jianshen-admin、egon-cola-tianquan-jianshen-admin-web；egon-cola-tianshu-admin、egon-cola-tianshu-admin-web、egon-cola-tianshu-starter；egon-cola-component-rpc-tianshu-adapter |
| 变更类型 | IAM 业务域重命名、Java package 迁移、管理 API 迁移与补全、管理前端同步迁移；Tianshu Business/Application 主数据边界与只读 RPC 契约补齐 |
| 目标根包 | top.egon.cola.platform.tianquan.jianshen.admin.iam |
| 对外 API 根路径 | /api/tianquan-jianshen/v1/iam |
| 内部 API 根路径 | /internal/v1/iam（服务间成员解析）；/api/tianquan-jianshen/v1/iam/internal（现有管理服务内嵌内部提交） |
...
2. 明确 Tianshu 的 `Business -> Application` 两层目录为唯一主数据：Business/Application 的新增、修改、删除、启停继续由 Tianshu 管理；RBAC 不复制这两类主数据，也不提供或代理它们的 CRUD；
3. RBAC 通过 Tianshu RPC 查询和校验目录，并继续在 RBAC 内管理“用户可访问哪些 Business”和“用户通过哪些应用角色/权限可访问哪些 Application”的授权关系；
4. RBAC 管理 API 统一迁移到 /api/tianquan-jianshen/v1/iam，补齐 RBAC 自身实体 CRUD 与授权语义接口；Tianquan-Jianshen Admin Web 同步迁移页面、前端路由、API Client、表单与测试。

这不是 Tianquan-Shoubing、JWT、Session、Yuheng 鉴权链路或 Starter 的改造。RBAC 侧 User 仍只是租户内授权成员；账户密码、用户资料、Access Token、Refresh Token 继续属于 Tianquan-Shoubing 边界。
...
当前公开 Controller 的共同根路径是 /api/tianquan-jianshen/v1；内部 Controller 使用 /internal/v1。迁移后保留这两个系统级前缀，在其后显式加入 /iam，不另起一套 API 版本。

### 2.2 不能因重命名而破坏的模型事实

- UserPO 只保存 RBAC 侧用户成员和 Tianquan-Shoubing identitySub 等最小授权信息；它不是 Tianquan-Shoubing 账户，不包含密码、邮箱、手机号、头像或 Refresh Token。
...
当前 Tianshu 已经具备两层目录，且它是本次的唯一主数据来源：

| Tianshu 主数据 | 当前实体/关系 | 当前管理 API | 本次归属 |
| --- | --- | --- | --- |
| Business（Tianshu 现有命名为 Biz） | `DdcBizEntity`，`ddc_biz`；`id`、`bizCode`、`bizName`、`enabled` | `/api/v1/tianshu/bizs` 的 list/page/detail/create/update/delete/enabled | Tianshu Admin |
| Application | `DdcAppEntity`，`ddc_app`；`id`、`bizCode`、`appCode`、`appName`、`enabled` | `/api/v1/tianshu/apps` 的 list/page/detail/create/update/delete/enabled；列表可按 `bizCode` 过滤 | Tianshu Admin |

`DdcAppEntity.bizCode` 表示 Application 属于 Business。RBAC 不把这两个实体迁入自己的 JPA 模型，不直连 Tianshu 表，也不把 Tianshu 管理 REST API 包装成 RBAC 的写接口。Tianshu Admin Web 的 `BizsPage`、`AppsPage` 继续是这两类主数据的唯一 CRUD 页面。

当前 `DdcManagementService` / `DdcManagementRpc` 只覆盖配置、发布、实例和 scope binding 等能力，尚未提供 Biz/App 目录读取。因此本次必须在现有 Tianshu Management RPC 链路中补齐只读目录能力；不能假定 RBAC 已经能通过 RPC 读取 Business/Application。
...
本 Spec 中的 **Business** 专指 Tianshu `Biz` 这一层目录；它不等同于现有 `businessResource`、`BusinessParticipationPO` 或操作 SOD 规则中的业务对象。
...
| 新增的 Business 目录 RPC 适配、用户 Business 访问授权 | admin.iam.business | 只读查询 Tianshu Business 目录，维护 RBAC 的 UserBusinessAccess；不承载 Tianshu Business CRUD |
| resource 中 ApplicationPO 及其应用服务 | admin.iam.application | ApplicationPO 保留为租户内 RBAC 应用授权范围，引用 Tianshu Application；不再把它当作 Tianshu Application 主数据 |
| Tianshu 的 DdcBizEntity、DdcAppEntity 及其 Controller/Web 页面 | 保持 Tianshu 现有包 | 继续由 Tianshu 管理，绝不迁入 RBAC 的 iam 包 |
...
- 公开 IAM API：/api/tianquan-jianshen/v1/iam。
- 服务间身份成员解析 API：/internal/v1/iam。
- 现有挂在管理服务公开根路径下的内部提交 API：/api/tianquan-jianshen/v1/iam/internal。它们不强行改为 /internal/v1，以保持当前 Manifest/目录快照的服务路由方式。
- Tianshu Business/Application 的管理 API 继续是 `/api/v1/tianshu/bizs` 和 `/api/v1/tianshu/apps`，不属于 `/api/tianquan-jianshen/v1/iam`，也不迁移到 RBAC 服务。
- RBAC 对 Tianshu 目录只提供受权限保护的只读选择/查询视图，并通过 RPC 取数；它不以 RBAC API 转发 Tianshu 的 POST、PUT、DELETE 或 enabled 操作。
- 公开业务 API 的 tenantId 来自当前认证/租户上下文，不允许普通调用方通过 body/query 覆盖 tenantId。
- 租户平台管理接口仍按现有平台管理员模型使用目标租户头；前端仅对 /api/tianquan-jianshen/v1/iam/tenants 路径注入 X-Tianquan-Jianshen-Target-Tenant，不能误加到所有 IAM 请求。
...
| --- | --- | --- |
| /api/tianquan-jianshen/v1/xingyuan/tenants | /api/tianquan-jianshen/v1/iam/tenants | 租户管理 |
| /api/tianquan-jianshen/v1/users、/internal/v1/identity | /api/tianquan-jianshen/v1/iam/users、/internal/v1/iam/users | 用户成员管理和内部解析 |
| /api/tianquan-jianshen/v1/roles | /api/tianquan-jianshen/v1/iam/roles | 角色、权限、继承 |
| /api/tianquan-jianshen/v1/applications | /api/tianquan-jianshen/v1/iam/applications | 租户内 RBAC Application 授权范围；不是 Tianshu Application 主数据管理 |
| /api/v1/tianshu/bizs、/api/v1/tianshu/apps | 保持 Tianshu 原 URI | Tianshu Business/Application 主数据 CRUD 和状态管理，不创建 RBAC 映射 URI |
| /api/tianquan-jianshen/v1/applications/{applicationId}/resources、/api/tianquan-jianshen/v1/resources/{resourceId}/archive、/api/tianquan-jianshen/v1/resource-manifests | /api/tianquan-jianshen/v1/iam/applications/{applicationId}/resources、/api/tianquan-jianshen/v1/iam/resources/{resourceId}、/api/tianquan-jianshen/v1/iam/resource-manifests | 资源、字段、Manifest |
| /api/tianquan-jianshen/v1/org-units、/api/tianquan-jianshen/v1/positions | /api/tianquan-jianshen/v1/iam/organizations、/api/tianquan-jianshen/v1/iam/positions | 组织和岗位 |
| /api/tianquan-jianshen/v1/data-rules、/field-rules 及约束路由 | /api/tianquan-jianshen/v1/iam/policies/... | 数据/字段规则和高级约束 |
| /api/tianquan-jianshen/v1/users/{userId}/role-assignments | /api/tianquan-jianshen/v1/iam/users/{userId}/roles 及嵌套授权动作 | 不再把关联表作为主 API |
| /api/tianquan-jianshen/v1/auth/role-activation-candidates、/role-activations | /api/tianquan-jianshen/v1/iam/users/me/role-activation-candidates、/active-roles | 保持当前用户自服务边界 |
| /api/tianquan-jianshen/v1/internal/directory-snapshots、/api/tianquan-jianshen/v1/internal/resource-manifests | /api/tianquan-jianshen/v1/iam/internal/directory-snapshots、/api/tianquan-jianshen/v1/iam/internal/resource-manifests | 保持现有服务内嵌内部提交方式 |
...
下表只列 RBAC 自己拥有的实体或授权范围。GET 集合接口同时承担分页/筛选查询；详情、创建、更新、删除和状态调整均为独立、可授权的管理动作。Business/Application 的**目录主数据**不在本表中：其 CRUD 由 Tianshu 的既有 API 和 Tianshu Admin Web 完成。

| 管理对象 | 基础 URI | 特殊规则 |
| --- | --- | --- |
| Tenant | /iam/tenants | 平台管理员接口；DELETE 采用 Tenant 现有关闭/归档语义 |
| User | /iam/users | 仅创建/维护 RBAC 成员和 identitySub 绑定；不创建 Tianquan-Shoubing 账户、不管理密码或资料 |
| Application 授权范围 | /iam/applications | 以已存在的 Tianshu Application 为输入，在当前 tenant 建立/管理 RBAC 授权范围；只允许 admit、查询、RBAC 本地状态调整和无依赖时撤销，禁止修改 Tianshu 的编码、名称、Business 归属、启停或删除 |
...
| 业务能力 | URI 与动作 | 语义与限制 |
| --- | --- | --- |
| Tianshu Business 目录选择 | GET /iam/catalog/businesses | 只读；RBAC 通过 Tianshu RPC 返回可见的 Business 目录，不提供 create/update/delete/enabled |
| Tianshu Application 目录选择 | GET /iam/catalog/businesses/{ddcBusinessId}/applications | 只读；返回该 Business 下的 Application，供 RBAC 授权范围和用户授权选择，不能跨 Business 伪造关联 |
| 建立 RBAC Application 授权范围 | POST /iam/applications:admit | body 只接受 `ddcApplicationId` 和 RBAC 本地展示/排序类配置；服务经 Tianshu RPC 取得其父 Business、编码、名称和 enabled 状态后建立本地范围，不能由请求伪造目录字段 |
| RBAC Application 授权范围状态 | GET /iam/applications；GET /iam/applications/{applicationId}；PUT /iam/applications/{applicationId}/status；DELETE /iam/applications/{applicationId} | 操作的是本地 RBAC 范围。DELETE 只在没有 Role/Permission/Resource 等依赖时允许；否则采用本地停用或返回冲突。任何动作均不调用 Tianshu 写 API |
| 用户 Business 访问授权 | GET/PUT /iam/users/{userId}/business-accesses | RBAC 直接维护用户对 Tianshu Business 的访问授权；PUT 为人工授权集合的整体 replace，记录有效期、状态、原因/工单等证据，并在写入前经 Tianshu RPC 校验目标仍存在且 enabled |
...
`ApplicationPO` 的保留不表示 RBAC 继续拥有 Tianshu Application 主数据。迁移后它的语义是“当前 tenant 内的 RBAC Application 授权范围”；`id` 仍是 Role、Permission、Resource 等 RBAC 内部关系的主键，Tianshu Application 只以外部引用进入该范围。

### 5.2 Business/Application：Tianshu 目录与 RBAC 授权模型

#### 5.2.1 唯一主数据与本地授权范围

Tianshu 是 Business 和 Application 的唯一主数据写入者；RBAC 只保存下列两类授权事实，不复制或维护 Tianshu 主数据：

| RBAC 模型 | 建议表/现有表 | 归属 | 含义 |
| --- | --- | --- | --- |
| ApplicationPO（保留并扩展） | `rbac3_application` | iam.application | 当前 tenant 接纳的 Tianshu Application 的本地授权范围。它承载本地 `applicationId`、RBAC 本地状态及资源/角色/权限关系，不是 Tianshu App 的副本或 CRUD 对象 |
| UserBusinessAccessPO（新增） | `rbac3_user_business_access` | iam.business | 当前 tenant 内某用户可访问某个 Tianshu Business 的直接授权证据；它是 Business 层的 RBAC 权限，不是 Tianshu 目录成员关系 |

`rbac3_application` 至少增加 `ddc_application_id` 与 `ddc_business_id` 两个不可由 RBAC 管理请求伪造的引用字段；`bizCode`、`bizName`、`appCode`、`appName` 如保留在本地，只能是 Tianshu RPC 返回的只读展示快照，不能在 RBAC API 中独立修改。唯一约束改为 `(tenant_id, ddc_application_id)`，以允许不同 Business 下出现相同的 `appCode`。Tianshu Application 的 `id` 是绑定的稳定身份；`Business + Application` 的人可读定位使用 Tianshu 返回的 `(bizCode, appCode)`，不再将单独的 `applicationCode` 当作跨 Business 的全局身份。

所有当前仅以 `(tenant_id, application_code)` 关联应用的 RBAC 表、查询和契约，在同一迁移中改为使用本地 `application_id` 作为授权关系键；至少覆盖服务主体/服务权限、Operation SOD 和业务参与记录等现有代码值关联。`applicationCode` 如仍出现在审计或展示 VO 中，只是由 `applicationId` 反查出的展示值。这样既不要求把 Tianshu Business 主数据复制到 RBAC，也不会在 Tianshu 的两层目录下把同名 `appCode` 误判为同一个应用。
...
3. 一项 Application 范围内的角色只有同时满足“本地 Application 授权范围有效、它对应的 Tianshu Business/Application 仍有效、用户具有该 Business 的有效访问授权、角色分配自身有效”时，才进入用户的 effective role、active role 候选和权限上下文。
...
#### 5.2.3 Tianshu 只读 RPC 契约

在现有 `DdcManagementService`、`DdcManagementRpc`、`DdcManagementClient`、`RpcDdcManagementClient` 和 Tianshu provider 链路中，增加只读目录契约。最小方法集如下：
...
这些 RPC 方法只能读取 Tianshu 目录，不能携带 RBAC 的 User、Role、Permission 或授权写命令。RBAC 的 Application 范围接纳、Business 授权 replace、用户角色授予/replace、active-role 候选计算和权限上下文装配都必须使用该目录事实校验；缺失、跨 Business 或 disabled 的目录对象不得变成有效授权。允许使用缓存优化读取，但缓存只能是 Tianshu 目录的派生快照，不能成为独立写源，且失效时必须安全地拒绝把目标加入有效权限上下文。
...
实现时只新增一个 Flyway V6 migration：同时完成 5.2 的 Tianshu 引用/应用范围键迁移、`rbac3_user_business_access` 创建、5.3 的来源字段/约束调整，并创建本节的两个组织/岗位关系表及必要索引。两个组织/岗位关系模型均遵循 TenantScopedPO 的 tenant、版本、审计和状态规范，包含 userId、orgUnitId、有效期与状态；岗位关系额外包含 positionId、primary。服务层负责保证同租户、岗位归属组织一致、幂等重复提交和有效期合法，不以 positionId=null 或虚构 DirectorySnapshot 规避模型。
...
| BusinessCatalogController、UserBusinessAccessController | 通过 Tianshu RPC 提供只读 Business/Application 选择器；维护用户 Business 访问授权。前者不写 Tianshu，后者不写 Tianshu 主数据 |
| ApplicationController | 当前 tenant 的 RBAC Application 授权范围接纳、查询、本地状态和无依赖撤销；不提供 Tianshu Application CRUD |
...
已有类不是机械改名为一个巨型 IamController。原 Application/Domain Service、Repository、Assembler 保持单一职责并按新包归属移动；跨域编排只由业务动作所属的 Application Service 负责。Tianshu 目录访问收敛为 iam.business 中的单一 RPC adapter，不允许 Controller、Repository 或 JPA Entity 分散地访问 Tianshu REST/数据库。
...
- Business 目录对象不属于 RBAC tenant，但每一条 `UserBusinessAccess` 和 Application 授权范围必须属于当前 tenant，并且引用 Tianshu RPC 返回的存在、归属正确且 enabled 的 Business/Application；
...
- 用户角色写入、有效角色计算和 active-role 候选计算都必须同时校验该角色的 Application 授权范围、其父 Business 访问授权和 Tianshu 目录状态；不满足时不得进入权限上下文；
...
### 6.4 Tianshu RPC 扩展与依赖方向

本次不新增 RBAC 到 Tianshu 的 HTTP Client，也不让 RBAC 读取 `ddc_biz`、`ddc_app` 数据库表。实现必须沿用现有 Tianshu Management RPC 链路，并在同一契约变更中同步以下位置：

1. `egon-cola-component-rpc-tianshu-adapter` 的 `tianshu_management.proto`、生成契约和 `DdcManagementRpc`；
2. Tianshu starter 的 `DdcManagementClient`；
3. RPC adapter 的 `RpcDdcManagementClient`、mapper/异常映射和 client factory；
4. Tianshu Admin 的 `DdcManagementRpcProvider`，由 `DdcBizService`、`DdcAppService` 提供只读实现；
5. RBAC Admin 的 iam.business RPC adapter 与其 Controller/Application Service。

依赖方向固定为 `RBAC IAM -> Tianshu Management RPC -> Tianshu Admin service`。Tianshu 不依赖 RBAC 的 User、Role、Permission 或其数据库；新增 RPC 也不包含 Tianshu Business/Application 的写方法。目录查询失败、找不到或目录状态无效时，RBAC 的授权写操作必须失败，权限上下文不能把该范围标记为有效。

## 7. Tianquan-Jianshen Admin Web 迁移与功能范围
...
`/iam/businesses` 是 Tianshu Business/Application 的只读选择与授权入口，不是 RBAC 版本的 Business CRUD 页面；`/iam/applications` 是 RBAC Application 授权范围页面，不是 Tianshu Application CRUD 页面。Tianshu Admin Web 的 `BizsPage`、`AppsPage` 保持现有 Tianshu CRUD 职责，Tianquan-Jianshen Admin Web 不复制它们。
...
| Business catalog | 只读展示 Tianshu Business 及其下 Application；作为用户 Business 授权和 RBAC Application 授权范围接纳的受限选择器；没有新增、编辑、删除、启停控件 |
| Application authorization scope | 已接纳 Application 的列表/详情、按 Business 筛选、接纳、本地启停和无依赖撤销；展示 Tianshu 来源及链接/跳转信息，不编辑 Tianshu 编码、名称、Business 归属或 enabled |
| User | RBAC 成员 CRUD、identitySub 展示、状态；不展示或编辑虚构的 Tianquan-Shoubing profile 字段 |
...
所有选择类配置页面使用 Business/Application、角色/权限/资源/组织/岗位的受限选择器，不允许继续让管理员通过逗号分隔 id 文本框提交关系。Business/Application 选项来自 RBAC 后端的只读 Tianshu RPC 查询，不允许前端伪造 `bizCode`、`appCode` 或 Tianshu id。replace 保存调用单个 PUT 接口并在成功后使相关 React Query 缓存失效。

### 7.3 API Client、导航与测试同步

- business.api、application.api、role.api、assignment.api、directory.api、constraint.api、tenant.api、roleActivation.api 及其测试改到新的 iam API Client/路径；其中 business/application client 只调用 RBAC 的目录读取与授权范围/授权关系接口，绝不调用 Tianshu 主数据写接口；
- FeatureApi 中目标租户头的路径判断由 /api/tianquan-jianshen/v1/xingyuan/ 改为精确的 /api/tianquan-jianshen/v1/iam/tenants，防止向普通租户 IAM 请求错误注入平台目标租户头；
...
用户已确认不需要兼容旧数据/旧 API，因此本次不创建旧 URI alias、旧包 wrapper 或旧前端路由重定向。仍应保证一次发布中的后端、Yuheng/Manifest、Admin Web 同步完成。

实施按以下顺序进行：

1. 冻结当前 Controller URI、GatewayOperation/资源声明、实体注解、前端调用、Tianshu Biz/App API 和测试清单；
2. 先扩展 Tianshu Management 的只读 Biz/App RPC 契约、provider、client adapter 和契约测试；Tianshu `/api/v1/tianshu/bizs`、`/api/v1/tianshu/apps` 及 Tianshu Admin Web CRUD 保持其主数据职责；
3. 新增唯一的 Flyway V6：建立 Tianshu 引用的 RBAC Application 授权范围键、`rbac3_user_business_access`、用户组织和人工用户岗位关系，并把既有 code-only 应用关系切换为本地 `applicationId`；
...
6. 同步更新 GatewayOperation、资源 Manifest、权限声明和 Tianquan-Jianshen Admin Web IAM feature；Business/Application 的主数据写页面仍在 Tianshu Admin Web；
7. 删除旧 RBAC URI、旧 Java package、旧 Web route/API 调用，执行全仓库旧路径搜索；不删除 Tianshu 的 Biz/App URI；
8. 进行 Tianshu RPC、后端、前端、迁移和跨域授权契约回归验证。

## 9. 验收标准

### 9.1 结构与契约

- 目标领域模型均位于 top.egon.cola.platform.tianquan.jianshen.admin.iam 下的对应子域；旧顶层业务包不再保留实现类；
...
- 所有公开 **RBAC IAM** 管理 URI 使用 /api/tianquan-jianshen/v1/iam；服务间成员解析使用 /internal/v1/iam，Manifest/目录快照的内嵌内部提交使用 /api/tianquan-jianshen/v1/iam/internal；旧 RBAC URI 不可再被 Controller、前端或 Yuheng 清单引用。Tianshu Business/Application 继续使用 `/api/v1/tianshu/bizs`、`/api/v1/tianshu/apps`；
- Tianshu 是 Business/Application 的唯一主数据写入者；RBAC 只经 Tianshu Management RPC 读取/校验目录，不读 Tianshu 表、不代理 Tianshu 主数据写操作；
...
- RBAC 自有 Tenant、User、Resource、FieldDefinition、Permission、Role、Organization、Position、DataRule、FieldRule 等对象具备约定的 CRUD、列表/分页和状态操作；Application 授权范围具备接纳、查询、本地状态和无依赖撤销语义；Business/Application 主数据 CRUD 只在 Tianshu；
- 管理员可以在 RBAC 为用户 replace Business 访问授权，并通过 RBAC 的用户角色配置 Application 权限；二者均不调用 Tianshu 写 API；
- 用户的 Application 角色/权限只有在对应 Business 授权、本地 Application 授权范围和 Tianshu Business/Application 状态都有效时才进入 effective role、active role 候选和权限上下文；撤销 Business 访问不删除已有角色记录，但立即使其不生效并刷新授权版本；
...
- 后端：编译、架构测试、Controller/Service/Repository 测试、Flyway migration 校验、Tianshu Management RPC proto/provider/client 契约测试，以及关键授权事务、跨 tenant/Business/Application 拒绝测试；
- 前端：Tianquan-Jianshen Admin Web 与 Tianshu Admin Web 的 TypeScript typecheck、单元/组件测试、构建；覆盖 Tianshu 主数据 CRUD、RBAC Business 授权 replace、Application 授权范围接纳、用户角色的 Business 前置校验和关键增强操作；
...
- Tianquan-Shoubing 用户中台、密码、账户冻结、JWT 签发/验签、Refresh Token、Session、SSO、Yuheng 认证流程的再设计；
- 将 Tianshu Business/Application 主数据复制到 RBAC、让 RBAC 直连 Tianshu 数据库、让 RBAC 代理 Tianshu 的主数据写 API，或把用户/角色授权关系写入 Tianshu；
...
本 Spec 的重点是把已经存在的 RBAC 管理模型按 IAM 业务语言重新组织，在不夺取 Tianshu Business/Application 主数据职责的前提下补齐 RBAC 的用户范围授权、管理页面和语义配置能力；不借此创造另一套账户体系、资源模型或通用规则框架。
