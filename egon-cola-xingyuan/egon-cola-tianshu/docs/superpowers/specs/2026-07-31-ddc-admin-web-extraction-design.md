...
`egon-cola-tianshu-admin/src/main/resources/static/ddc-admin/`，
...
└── egon-cola-tianshu/
    ...
    ├── egon-cola-tianshu-admin     (移除 static/ddc-admin + src/test/js)
...
1. 脚手架：基于 gateway-admin-web 骨架复制改造（`@egon-cola/tianshu-admin-web`，
   环境变量前缀 `DDC_ADMIN_*`）。
