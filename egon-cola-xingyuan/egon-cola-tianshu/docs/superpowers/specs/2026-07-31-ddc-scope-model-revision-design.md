...
| 决策点 | 结论 |
|---|---|
| 注册身份 | `DdcServiceKey` 增加 `bizCode` + `appCode`（恒有值），Redis key 结构随之变化；SDK 消费者零改动（appCode 取自现有 `DdcProperties.appCode`，bizCode 新增配置 `egon.cola.component.tianshu.biz-code`，缺失时注册被拒并提示） |
...
| 删除语义 | 保护性删除：biz 下有 app、app 下有 ns、ns 下有配置、env 被引用 → 拒绝（`TIANSHU_*_IN_USE`）；无子数据才可删；禁用不删数据 |
| 禁用门控 | 注册/拉取路径校验四实体 enabled（短缓存 5s），任一禁用 → `TIANSHU_SCOPE_DISABLED`；心跳/续租不逐次校验，存量 lease 自然过期收敛 |
...
| 实体 | 端点 | 能力 |
|---|---|---|
| biz | `GET/POST /api/v1/tianshu/bizs`，`PUT/DELETE /api/v1/tianshu/bizs/{code}` | 列表（名称模糊）、新建、编辑、删除（保护性）、启用/禁用 |
| env | `GET/POST /api/v1/tianshu/envs`，`PUT/DELETE /api/v1/tianshu/envs/{code}` | 同上 + 排序 |
| app | `GET/POST /api/v1/tianshu/apps`，`PUT/DELETE /api/v1/tianshu/apps/{code}` | 列表（biz 过滤 + 名称模糊）、新建（必选 biz）、编辑、删除、禁用 |
| ns | `GET/POST /api/v1/tianshu/namespaces`，`PUT/DELETE /api/v1/tianshu/namespaces/{id}` | 列表（app 过滤 + ns 模糊，无 env）、新建（app + ns 名）、删除、禁用 |

- 删除保护错误码：`TIANSHU_BIZ_IN_USE` / `TIANSHU_APP_IN_USE` / `TIANSHU_NAMESPACE_IN_USE` / `TIANSHU_ENV_IN_USE`。
...
- 注册/拉取校验：查四实体 enabled（批量 + 5s 短缓存）→ 禁用返回 `TIANSHU_SCOPE_DISABLED`。
...
主视图：APP 列表（有实例的 APP：appCode、业务域、环境、ns、在线实例数）
  └─ 点击 APP 行 → Drawer 抽屉：按服务分组（kind/name/group/version → 实例表格）
      ...
      抽屉头部：app 信息 + 刷新 + 分组计数
概览卡：APP 数、在线实例数、HTTP/RPC/Yuheng 服务数
```
...
- 后端：V5 迁移（去重/回填/约束）、四实体 CRUD 与删除保护错误码、注册契约（key 结构/Lua/序列化）、
  门控（register/pull 被禁返回 `TIANSHU_SCOPE_DISABLED`、启用恢复、心跳不校验）、现有测试回归更新。
