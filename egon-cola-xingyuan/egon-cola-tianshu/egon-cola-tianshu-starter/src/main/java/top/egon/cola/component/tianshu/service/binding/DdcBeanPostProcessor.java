package top.egon.cola.component.tianshu.service.binding;
...
/**
 * Tianshu 配置字段绑定的 Spring Bean 后处理器。
 * Spring Bean post-processor for Tianshu configuration field binding.
 *
 * <p>每个 Spring Bean 初始化完成后，本处理器尽可能解析其用户定义类型，并委托
 * {@link DdcFieldBindingService} 扫描带有 Tianshu 配置注解的字段。初次字段注入由 Spring 原生
 ...
 * possible and delegates to {@link DdcFieldBindingService} to scan fields carrying Tianshu configuration
...
public class DdcBeanPostProcessor implements BeanPostProcessor, DestructionAwareBeanPostProcessor {
    ...
    /**
     * 负责扫描和登记可刷新 Tianshu 配置字段的服务。 Service that scans and registers refreshable Tianshu configuration fields.
     */
    private final DdcFieldBindingService fieldBindingService;

    /**
     * 创建 Tianshu Bean 后处理器。
     * Creates a Tianshu Bean post-processor.
     *
     * @param fieldBindingService Tianshu 配置字段绑定服务; Tianshu configuration field binding service
     */
    ...
    /**
     * 在 Bean 初始化完成后登记其中的 Tianshu 配置字段。
     * Registers Tianshu configuration fields after Bean initialization.
     *
