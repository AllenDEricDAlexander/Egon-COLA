...
import top.egon.cola.component.tianshu.error.DdcClientTransportException;
...
import top.egon.cola.component.rpc.tianshu.contract.proto.v1.DdcRpcErrorDetail;
...
/**
 * 将 Admin 领域失败映射为稳定且脱敏的 Tianshu RPC 状态。
 * / Maps Admin domain failures to stable, sanitized Tianshu RPC statuses.
 */
public final class DdcRpcProviderExceptionMapper
    ...
    public Optional<StatusRuntimeException> map(Throwable throwable) {
        ...
        if (throwable instanceof TransientDataAccessException
            ...
            return Optional.of(status(
                    ...
                    "Tianshu service is temporarily unavailable",
                    true
            ));
        }
    ...
    /** 构造携带类型化 Tianshu detail 的脱敏状态。 / Builds a sanitized status with typed Tianshu detail. */
    ...
    private String message(String code) {
        return Arrays.stream(DdcErrorStatus.values())
                ...
                .or(() -> Arrays.stream(DdcManagementErrorCode.values())
                        ...
                        .findFirst())
                .orElse("Tianshu operation failed");
    }
}
