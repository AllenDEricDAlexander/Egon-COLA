...
### 审计范围
- **模块：** `egon-cola-tianshu-admin`（管理端 Web 应用）
- **模块：** `egon-cola-tianshu-starter`（SDK Starter）
- **模块：** `egon-cola-tianshu-http-registration-starter`（HTTP 注册 Starter）
- **模块：** `egon-cola-tianshu-test`（测试模块）
...
- **starter 模块：** 所有 HTTP 客户端调用委托给外部 adapter 模块（`egon-cola-component-rpc-tianshu-adapter`），通过接口 `DdcConfigClient`、`DdcManagementClient`、`DdcServiceRegistryClient` 定义契约
...
本次安全审计对 `egon-cola-tianshu` 项目的全部 4 个模块（约 44,591 行 Java 代码）进行了全面的安全审查。审计覆盖了 HTTP 请求链路、认证鉴权链路、业务调用链路、数据库访问链路、文件操作链路和外部网络调用链路。
