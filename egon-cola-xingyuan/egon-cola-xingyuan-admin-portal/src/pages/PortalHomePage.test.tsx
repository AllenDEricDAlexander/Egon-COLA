...
describe('PortalHomePage', () => {
  it('renders the static platform menu and keeps partial summary cards visible', async () => {
    renderHome({
      idp: async () => ({ status: 'READY', summary: '12 个身份' }),
      gateway: async () => ({ status: 'TIMEOUT' }),
      ddc: async () => ({ status: 'READY', summary: '8 个配置' }),
    })
    ...
    expect(screen.getByRole('link', { name: /独立打开 API 网关/ })).toHaveAttribute('href', '/gateway/dashboard')
  })

  it('hides a platform without the host-level capability', async () => {
    renderHome({}, ['idp'])
