...
public record PreparedGatewayRelease(
        ...
        /**
         * 中文说明：保存 attemptNo 对应的状态、依赖或配置值；字段类型为 {@code int}，由 {@code top.egon.cola.component.yuheng.admin.release.service.PreparedGatewayRelease} 在其生命周期内读取或更新。
         * English summary: Holds the state, dependency, or configuration represented by attempt no; its type is {@code int}, and {@code top.egon.cola.component.yuheng.admin.release.service.PreparedGatewayRelease} reads or updates it during its lifecycle.
         *
         * 用法 / Usage: 该字段通过 {@code top.egon.cola.component.yuheng.admin.release.service.PreparedGatewayRelease} 的构造、初始化或业务方法使用；/ Access it through the construction, initialization, or business methods of {@code top.egon.cola.component.yuheng.admin.release.service.PreparedGatewayRelease}; do not couple callers to its representation when the owning type exposes an API.
         */
        int attemptNo
