...
DURATION_SECONDS="${YUHENG_SOAK_DURATION_SECONDS:-86400}"
OUTPUT_DIRECTORY="${YUHENG_PERF_OUTPUT_DIRECTORY:-${PERFORMANCE_DIRECTORY}/artifacts}"

if [[ ! "${DURATION_SECONDS}" =~ ^[1-9][0-9]*$ ]]; then
  echo "YUHENG_SOAK_DURATION_SECONDS must be a positive integer" >&2
  exit 2
fi
...
export YUHENG_PERF_PROFILE=soak
export YUHENG_PERF_DURATION="${DURATION_SECONDS}s"
