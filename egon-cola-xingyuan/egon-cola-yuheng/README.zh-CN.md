...
| `yuheng-starter` | Provider 接口定义上报，并向 DDC HTTP 注册贡献 Gateway 元数据 | 是 |
...
依赖 `yuheng-starter`，它会组装 DDC HTTP 注册 Starter；不需要
...
HTTP 注册 Java 类型已迁入 `top.egon.cola.component.tianshu.http.registration`，Gateway
