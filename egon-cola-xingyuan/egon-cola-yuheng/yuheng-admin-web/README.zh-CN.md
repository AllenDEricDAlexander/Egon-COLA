...
Yuheng Admin Web 是独立的 React 管理控制台。它只调用 Yuheng Admin，不直接调用
Yuheng Engine、Tianshu Admin 或 Provider 端点。

## 认证

Admin Web 不发送 actor identity header。默认将已校验的 IAM Bearer Token 存储在
`sessionStorage`，并从 `GET /api/v1/yuheng/admin/session` 获取 actor 与 capabilities。
...
只有在 Yuheng Admin 可访问且浏览器场景所需拓扑已经启动时，才运行 `npm run e2e`。
该命令不能替代 Yuheng 的 Maven Live Suite。

## 运行时配置

浏览器调用 Yuheng Admin。设置 `VITE_YUHENG_ADMIN_API_BASE_URL` 可使用不同的 API
