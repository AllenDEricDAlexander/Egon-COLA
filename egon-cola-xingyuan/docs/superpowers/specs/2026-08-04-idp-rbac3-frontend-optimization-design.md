...
**方案 C: 统一基础设施先行**。新建共享包 `@egon-cola/xingyuan-admin-web-shared`，将两个项目的通用能力（OAuth、HTTP 客户端、JWT 工具、通用组件、设计 token、i18n）下沉到此包。两个消费项目逐步替换为共享包实现，从根本上消除代码重复和不一致。
...
egon-cola-tianquan-shoubing-admin-web ──── depends on ────> @egon-cola/xingyuan-admin-web-shared

egon-cola-tianquan-jianshen-admin-web ── depends on ────> @egon-cola/xingyuan-admin-web-shared
