...
第 2 节那 4 个顶层工具各配一个。全部在 `mktemp` 目录里自建 fixture 仓库并注入**假 `mvnw`**，不污染工作树、不需要真实 Maven 构建，耗时 2 秒到 1 分钟。`test-egon-codegen.sh` 额外要求 `EGON_CODEGEN_CLASSPATH`，缺失时按契约返回 `BLOCKED_TOOLING`（退出码 7）。
