...
This project is generated by `egon-cola-archetype-web` as one independent, organization-only Project. Outbound cross-Project consumers are intentionally deferred; external Projects integrate through the published Facade artifact.
...
- Infrastructure owns JPA, Flyway, Redis adapters, outbound RabbitMQ events, local fallback adapters, and Application-method logging AOP.
...
`local` and `test` use H2 in PostgreSQL compatibility mode, in-memory cache/idempotency adapters, a local event publisher, disabled RabbitMQ and Nacos connections, and Dubbo `injvm` with no registry.

`dev` and `prod` read datasource, Redis, RabbitMQ, Nacos, and Dubbo settings from environment variables. Integration switches are `ORGANIZATION_REDIS_ENABLED` and `ORGANIZATION_RABBIT_ENABLED`; cache and idempotency TTLs use the `ORGANIZATION_*_TTL` variables documented in the profile YAML files.
