-- 变更内容：将 Evaluation 排课、考试、试卷和成绩分表迁移到 BIGINT、tenant_id 和 EgonModel 七项技术字段。
-- 影响范围：每个 shard 库的 course_schedule_0/1、exam_0/1、exam_paper_0/1、score_0/1。
-- 兼容性说明：旧表中的历史行必须先完成明确的 Long/tenant 离线映射；不可转换或未知数据会使本迁移失败。

-- 脚手架只接受空的旧分片表；未知 UUID/租户归属不得在 Flyway 内猜测。
