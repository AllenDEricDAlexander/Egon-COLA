...
`egon-cola-source-service` 是一个只面向 service 的 COLA 示例，覆盖 Course、Schedule、Exam、Paper 和 Score 流程。业务流量通过 COLA native unary RPC 或 RabbitMQ 进入；HTTP 仅用于 Spring Boot Actuator 管理端点。
