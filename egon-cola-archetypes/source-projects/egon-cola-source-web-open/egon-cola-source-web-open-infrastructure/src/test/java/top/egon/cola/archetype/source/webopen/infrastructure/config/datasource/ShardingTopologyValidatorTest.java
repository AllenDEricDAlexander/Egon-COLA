...
class ShardingTopologyValidatorTest {
    ...
    void rejectsReadOnlyPrimaryEvenWhenItIsNotAReplica() throws Exception {
        ...
        var manifest = new top.egon.cola.component.common.mybatis.ddl.EgonColaDdlManifestBO("web-open", List.of(
    ...
    void validatesTypedSingleAndLegacyRulesWithoutChangingLegacyAddresses() {
        ...
        assertThat(result.profiles().get("users").kind()).isEqualTo(EgonColaRoutingProfileBO.TableKindEnum.SINGLE);
        assertThat(result.profiles().get("school_classes").kind()).isEqualTo(EgonColaRoutingProfileBO.TableKindEnum.TENANT_LEGACY);
    ...
    void rejectsWildcardSingleNodesIncorrectSchemaAndUnmanagedGroups() {
        ...
        for (String invalid : List.of(valid.replace("master_data.public.users", "master_data.*.*"),
                valid.replace("master_data.public.users", "master_data.other.users"),
                valid.replace("master_data.public.users", "unknown.public.users"))) {
