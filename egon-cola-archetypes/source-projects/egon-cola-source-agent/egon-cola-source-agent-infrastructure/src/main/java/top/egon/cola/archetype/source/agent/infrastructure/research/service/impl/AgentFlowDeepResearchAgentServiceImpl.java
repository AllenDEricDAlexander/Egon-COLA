...
@Service("deepResearchAgentService")
@RequiredArgsConstructor
@Slf4j
public class AgentFlowDeepResearchAgentServiceImpl implements DeepResearchAgentService {
    ...
    public DeepResearchRunService start(DeepResearchTaskBO task, DeepResearchEventObserverService observer) {
        ...
        try {
            ...
            return new SessionRun(task.runId(), terminal, cancelled, subscription, cleanup);
    ...
    private void onFailure(DeepResearchTaskBO task, DeepResearchEventObserverService observer, Throwable failure,
        ...
        if (terminal.compareAndSet(false, true)) {
            ...
            try {
                ...
                log.warn("Deep Research service runId={} traceId={} stage=STREAM outcome=OBSERVER_ERROR errorType={}",
    ...
    private void cleanup(AgentFlowSessionResult session, String userId, AtomicBoolean cleaned,
        ...
        try {
            ...
            log.warn("Deep Research service stage=SESSION outcome=DELETE_ERROR errorType={}",
    ...
    private static final class SessionRun implements DeepResearchRunService {
        ...
        private SessionRun(String runId, AtomicBoolean terminal, AtomicBoolean cancelled,
