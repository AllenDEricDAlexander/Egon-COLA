...
class ShardingDataSourcePropertiesTest {
    ...
    @Test
    void binds_physical_data_sources_and_tenant_routing_without_schema_runner() {
        Map<String, Object> values = Map.ofEntries(
                ...
                Map.entry("app.sharding.routing.node-map",
                ...
                Map.entry("app.sharding.physical-data-sources[0].driver-class-name",
                        "org.h2.Driver"),
                Map.entry("app.sharding.physical-data-sources[0].jdbc-url",
        ...
        ShardingDataSourceProperties properties = new Binder(
                ...
                .bind("app.sharding", Bindable.of(ShardingDataSourceProperties.class)).get();

        assertThat(properties.config()).isEqualTo("classpath:sharding/rules.yml");
        assertThat(properties.routing().nodeCount()).isEqualTo(4);
        assertThat(properties.physicalDataSources()).singleElement().satisfies(dataSource -> {
