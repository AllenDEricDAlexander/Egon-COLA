...
CREATE UNIQUE INDEX pk_users_id ON light_users (id);
...
CREATE UNIQUE INDEX pk_roles_id ON light_roles (id);
...
CREATE UNIQUE INDEX pk_permissions_id ON light_permissions (id);
...
CREATE TABLE light_user_roles (
    ...
    CONSTRAINT fk_user_roles_user FOREIGN KEY (user_id) REFERENCES light_users (id),
    CONSTRAINT fk_user_roles_role FOREIGN KEY (role_code) REFERENCES light_roles (code)
);

CREATE UNIQUE INDEX pk_user_roles_id ON light_user_roles (id);
...
CREATE TABLE light_role_permissions (
    ...
    CONSTRAINT fk_role_permissions_role FOREIGN KEY (role_code) REFERENCES light_roles (code),
    CONSTRAINT fk_role_permissions_permission
        FOREIGN KEY (permission_code) REFERENCES light_permissions (code)
);

CREATE UNIQUE INDEX pk_role_permissions_id ON light_role_permissions (id);
...
CREATE UNIQUE INDEX pk_courses_id ON light_courses (id);

CREATE TABLE light_school_classes (
    id BIGINT PRIMARY KEY,
    name VARCHAR(120) NOT NULL,
...
CREATE INDEX idx_school_classes_tenant_active_0 ON light_school_classes (tenant_id, id);

CREATE TABLE light_class_course_schedules (
    id BIGINT PRIMARY KEY,
    ...
    CONSTRAINT fk_schedule_class_0
        FOREIGN KEY (school_class_id) REFERENCES light_school_classes (id)
);
