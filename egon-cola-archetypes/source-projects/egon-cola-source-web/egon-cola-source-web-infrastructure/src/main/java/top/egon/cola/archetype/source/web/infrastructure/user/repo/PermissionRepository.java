...
import top.egon.cola.archetype.source.web.infrastructure.user.dao.PermissionDAO;
...
public class PermissionRepository extends EgonColaRepository<PermissionDAO, PermissionPO> {
    ...
    public PermissionPO selectByCode(@NotBlank String code) {
        return getBaseMapper().selectByCode(code);
    }

    public List<PermissionPO> selectPermissionsByIds(@NotNull Collection<Long> ids) {
        return getBaseMapper().selectPermissionsByIds(ids);
    }
}
