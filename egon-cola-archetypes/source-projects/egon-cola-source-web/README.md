...
`egon-cola-source-web` is generated by `egon-cola-archetype-web` as one independent, organization-only Project. Its Adapter implements the Organization contract published by its own Facade module, and its Infrastructure consumes the Evaluation contract that the peer project publishes.
