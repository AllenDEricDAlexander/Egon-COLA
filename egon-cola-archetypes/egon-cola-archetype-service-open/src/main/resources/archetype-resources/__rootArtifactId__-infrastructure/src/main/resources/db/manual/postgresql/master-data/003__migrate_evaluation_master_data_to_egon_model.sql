-- Manual operator migration: 将 Evaluation 主数据课程表迁移到正数 BIGINT、tenant_id 和 EgonModel 七项技术字段。
-- Scope: master_data 库中的 course（迁移后逻辑表 evaluation_course）。
-- Compatibility: 旧表中的历史行必须先完成明确的 Long/tenant 离线映射；不可转换或未知数据会使本迁移失败。

-- 脚手架只接受空的旧主数据表；未知历史身份不得由自动迁移工具猜测。
