...
public class ScoreFacadeImpl implements ScoreFacade {
    ...
    public SingleResponse<ScoreResponse> getScore(GetScoreRequest request) { try { validator.require(request); return SingleResponse.of(converter.toResponse(scoreManage.get(new GetScoreQuery(parseId(request.examId()), parseId(request.scoreId()))))); } catch (RuntimeException e) { return handler.toFailure(e); } }
    public SingleResponse<PageResponse<ScoreResponse>> pageScores(PageScoreRequest request) { try { validator.require(request); var page = scoreManage.page(new PageScoreQuery(parseId(request.examId()), request.currentPage(), request.pageSize())); return SingleResponse.of(PageResponse.of(page.records().stream().map(converter::toResponse).toList(), page.currentPage(), page.totalPages(), page.pageSize(), page.totalCount())); } catch (RuntimeException e) { return handler.toFailure(e); } }
