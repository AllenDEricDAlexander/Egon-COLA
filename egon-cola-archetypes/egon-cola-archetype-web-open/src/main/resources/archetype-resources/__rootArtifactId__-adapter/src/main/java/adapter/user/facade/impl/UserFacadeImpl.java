...
public class UserFacadeImpl implements UserFacade {
    ...
    public UserDetailDTO createUser(CreateUserDTO request) {
        return support.invoke(() -> converter.toDTO(userManage.createUser(
            new CreateUserCommand(support.requestId(), request.name(), request.email()))));
    }

    @Override
    public UserDetailDTO getUser(String userId) {
        return support.invoke(
                () -> converter.toDTO(userManage.getUser(new UserDetailQuery(
